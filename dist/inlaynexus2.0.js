...
//@display-name Inlay Nexus 2.1.2
//@api 3.0
//@version 2.1.2
...
## {{user}} Info
${l(e.persona_description,8e3)}`});const a=l(e.assistant_text,2e4),i=Array.isArray(e.source_session_ids)?e.source_session_ids.map(_=>l(_,200)).filter(Boolean):[],o=t.lorebook||t.char_appearance!==!1?await wa(n,l(e.unified_session_id||"",200),l(e.character_id||"",200),i):[],s=_m(o);if(t.lorebook){const _=Array.isArray(e.lore_trigger_keys)?e.lore_trigger_keys:null,y=Bc(t.lore_extra),v=ym(e.lorebook||[],a,s,5,1200,_,y),x=[],N=[];for(const M of v){const j=Ot(M)||M.always,R=l(M.content||"",j?5e4:1200);if(!R)continue;const k=l(M.comment||"",200),S=k?`### ${k}
${R}`:R;j?x.push(S):N.push(S)}const C=[];x.length&&C.push(`## lb-xnai.lb.extra — OFFICIAL PACK (START)
...
[END OF lb-xnai.lb.extra] Pack finished — text below is not lb-xnai ground truth.`),N.length&&C.push(`## Reference Lorebook (trigger-matched only)
Naming/context only. Not lb-xnai. Do not copy lore prose into tags.

`+N.join(`

`)),w("job.lore.extra",{trigger_keys:Array.isArray(_)?_.length:0,injected:x.length,reference:N.length,sections:v.filter(M=>Ot(M)||M.always).map(M=>M.key||"").filter(Boolean).join(" | ")}),C.length&&r.push({role:"system",content:`${await ue("lore_inject")}
${C.join(`

`)}`})}if(t.char_appearance!==!1){const _=o,y=_.filter(N=>Se(N)),v=_.filter(N=>l(N.name,200)&&!Se(N)),x=Dl(a,_);if(y.length||v.length||x.length){const N=S=>{const O=l(S.name,200),U=l(S.appearance||"",200),Z=l(S.attire||"",160),me=l(S.accessories||"",120),J=[U?`appearance=${U}`:"",Z?`attire=${Z}`:"",me?`accessories=${me}`:""].filter(Boolean);return J.length?`${O} ← ${J.join(" | ")}`:O},C=x.filter(S=>Se(S)),M=x.filter(S=>!Se(S)),j=C.length?C.map(N).filter(Boolean).join(`
`):"(none)",R=M.length?M.map(S=>`- ${S.name} (aliases: ${cr(S).slice(0,8).join(", ")}) → empty appearance; full looks required in new_characters`).join(`
...
${j}`,r.push({role:"system",content:k}),w("job.roster.split",{filled:y.map(S=>S.name),incomplete:v.map(S=>S.name),matched:x.map(S=>S.name),matched_with_looks:C.map(S=>S.name),session_id:n})}}const c=ka(t.natural_base),u=nt(t),d=Math.max(1,Number(t.image_min??1)||1),f=Math.max(d,Number(t.image_max??3)||3);r.push({role:"system",content:["Every shot MUST include `y_percent` (0–100): reading position top→bottom. Spread across the full range in order (shot0 < shot1 < …); ~even gaps. E.g. 2→~25/~75; 3→~20/~50/~80; 4→~15/~40/~65/~90. Forbidden: all under 40, duplicates, or gaps under ~15 unless 1 shot.",d===f?`SHOT COUNT: produce exactly ${f} shot(s) in scenes[].shots (across all scenes).`:`SHOT COUNT: produce between ${d} and ${f} shots in scenes[].shots (across all scenes). Prefer the count that fits the message; never fewer than ${d} or more than ${f}.`,km(c),`CHARACTER CAP: at most ${u} characters per shot (char1..char${u}). If more are visible, keep the ${u} most important; fold extras into situation/place.`].join(`
`)});const p=await fc();p&&r.push({role:"system",content:p});const m=[];if(Number(t.include_max||0)>0)for(const _ of e.recent_messages||[]){const y=l(_?.role,40)||"char",v=l(_?.content||_?.data,12e3);v&&(a&&v===a&&["char","assistant","bot"].includes(y.toLowerCase())||m.push(`[${y}]
...
var Zt = "inlay-nexus-native", ea = "Inlay Nexus", He = "2.1.2", rn = class extends Error {
  segment;
  constructor($) {
