...
//@display-name Inlay Nexus 2.0.11
//@api 3.0
//@version 2.0.11
...
## {{user}} Info
${u(e.persona_description,8e3)}`});const r=u(e.assistant_text,2e4),s=Array.isArray(e.source_session_ids)?e.source_session_ids.map(f=>u(f,200)).filter(Boolean):[],i=t.lorebook||t.char_appearance!==!1?await Xn(n,u(e.unified_session_id||"",200),u(e.character_id||"",200),s):[],o=ll(i);if(t.lorebook){const f=Array.isArray(e.lore_trigger_keys)?e.lore_trigger_keys:null,h=Vr(t.lore_extra),m=ul(e.lorebook||[],r,o,5,1200,f,h),y=[],_=[];for(const M of m){const S=tt(M)||M.always,k=u(M.content||"",S?5e4:1200);if(!k)continue;const A=u(M.comment||"",200),b=A?`### ${A}
${k}`:k;S?y.push(b):_.push(b)}const x=[];y.length&&x.push(`## lb-xnai.lb.extra — OFFICIAL PACK (START)
...
`)}`})}if(t.char_appearance!==!1){const f=i,h=f.filter(_=>pe(_)),m=f.filter(_=>u(_.name,200)&&!pe(_)),y=Li(r,f);if(h.length||m.length||y.length){const _=h.length?h.map(k=>{const A=u(k.name,200),b=u(k.appearance||"",120);return b?`${A} ← ${b}`:A}).filter(Boolean).join(`
...
var Zt = "inlay-nexus-native", ea = "Inlay Nexus", He = "2.0.11", rn = class extends Error {
  segment;
  constructor($) {
