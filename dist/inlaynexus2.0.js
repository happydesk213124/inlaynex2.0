...
## {{user}} Info
${u(e.persona_description,8e3)}`});const a=u(e.assistant_text,2e4),i=Array.isArray(e.source_session_ids)?e.source_session_ids.map(_=>u(_,200)).filter(Boolean):[],o=t.lorebook||t.char_appearance!==!1||t.asset_nai_tags?await Ea(n,u(e.unified_session_id||"",200),u(e.character_id||"",200),i):[],s=zm(o);if(t.lorebook){const _=Array.isArray(e.lore_trigger_keys)?e.lore_trigger_keys:null,y=ml(t.lore_extra),x=Um(e.lorebook||[],a,s,5,1200,_,y),v=[],S=[];for(const M of x){const T=Bt(M)||M.always,B=u(M.content||"",T?5e4:1200);if(!B)continue;const k=u(M.comment||"",200),I=k?`### ${k}
${B}`:B;T?v.push(I):S.push(I)}const $=[];v.length&&$.push(`## lb-xnai.lb.extra — OFFICIAL PACK (START)
...
`)}`})}if(t.char_appearance!==!1){const _=o,y=_.filter(S=>be(S)),x=_.filter(S=>u(S.name,200)&&!be(S)),v=Do(a,_);if(y.length||x.length||v.length){const S=N=>{const R=u(N.name,200),H=u(N.appearance||"",200),me=u(N.attire||"",160),J=u(N.accessories||"",120),q=[H?`appearance=${H}`:"",me?`attire=${me}`:"",J?`accessories=${J}`:""].filter(Boolean);return q.length?`${R} ← ${q.join(" | ")}`:R},$=v.filter(N=>be(N)),M=v.filter(N=>!be(N)),T=$.length?$.map(S).filter(Boolean).join(`
`):"(none)",B=M.length?M.map(N=>`- ${N.name} (aliases: ${pr(N).slice(0,8).join(", ")}) → empty appearance; full looks required in new_characters`).join(`
`):"(none)";let k=await se("appearance_inject");k.includes("{registered_block}")&&(k=k.replace("{registered_block}",T)),k=k.includes("{incomplete_block}")?k.replace("{incomplete_block}",B):`${k}

## Incomplete
${B}`,k=k.includes("{detected_block}")?k.replace("{detected_block}",T):`${k}

${T}`,r.push({role:"system",content:k}),w("job.roster.split",{filled:y.map(N=>N.name),incomplete:x.map(N=>N.name),matched:v.map(N=>N.name),matched_with_looks:$.map(N=>N.name),session_id:n});const I=M.length>0||v.length===0;if(t.asset_nai_tags&&I){const N=[...Array.isArray(e.lore_trigger_keys)?e.lore_trigger_keys:[],...hi(e.lorebook||[],a)];try{const R=await js(N);if(R?.block){let H=await se("asset_tags_inject");H=H.includes("{asset_tags_block}")?H.replace("{asset_tags_block}",R.block):`${H}

${R.block}`,r.push({role:"system",content:H})}else $s(new Map)}catch(R){$s(new Map),w("asset-tags.inject.fail",{message:String(R?.message||R)},"warn")}}}}else if(t.asset_nai_tags){const _=Do(a,o);if(_.filter(y=>!be(y)).length>0||_.length===0){const y=[...Array.isArray(e.lore_trigger_keys)?e.lore_trigger_keys:[],...hi(e.lorebook||[],a)];try{const x=await js(y);if(x?.block){let v=await se("asset_tags_inject");v=v.includes("{asset_tags_block}")?v.replace("{asset_tags_block}",x.block):`${v}

${x.block}`,r.push({role:"system",content:v})}}catch(x){w("asset-tags.inject.fail",{message:String(x?.message||x)},"warn")}}}const c=Ta(t.natural_base),l=rt(t),f=Math.max(1,Number(t.image_min??1)||1),d=Math.max(f,Number(t.image_max??3)||3);r.push({role:"system",content:["Every shot MUST include `y_percent` (0–100): reading position top→bottom. Spread across the full range in order (shot0 < shot1 < …); ~even gaps. E.g. 2→~25/~75; 3→~20/~50/~80; 4→~15/~40/~65/~90. Forbidden: all under 40, duplicates, or gaps under ~15 unless 1 shot.",f===d?`SHOT COUNT: produce exactly ${d} shot(s) in scenes[].shots (across all scenes).`:`SHOT COUNT: produce between ${f} and ${d} shots in scenes[].shots (across all scenes). Prefer the count that fits the message; never fewer than ${f} or more than ${d}.`,qm(c),`CHARACTER CAP: at most ${l} characters per shot (char1..char${l}). If more are visible, keep the ${l} most important; fold extras into situation/place.`,t.auto_aspect?"ASPECT (required on every shot): set `aspect` to exactly one of `portrait` (832×1216 vertical), `square` (1024×1024), or `landscape` (1216×832 horizontal). Pick from the scene framing — tall full-body / standing → portrait; equal crop / face close-up square → square; wide group / side-by-side / scenic → landscape. Like Asset Maid size presets 1/5/2.":""].filter(Boolean).join(`
...
${h}
> These are instructions explicitly given by the user. If in conflict with previous instructions, this section MUST take precedence.`}),r}function Gm(e){const t=rt(C().card),n=e,r=[];for(const a of n.scenes||[]){const i=u(a.place);for(const o of a.shots||[]){const s={...o,place:i,characters:fa(o.characters||[],[],t).slice(0,t)};r.push(s)}}return r}function qm(e){return e==="off"?"Natural base mode OFF. Omit the `natural` field (or leave it empty). Do not invent natural-language base phrases.":e==="detailed"?["Natural base mode DETAILED. Every shot MUST include `natural`: a compact English phrase for NovelAI base caption only (NOT Danbooru comma tags, NOT characters[].action).","Include framing/vantage when useful (side view, close-up, viewed through, reflected in…), then for each visible person in left-to-right (or front-to-back) order: hair color + age band + gender, facial expression, clothing, and pose/action as separate details.","Multi-person: NEVER use character names — use position (left girl, right boy) or hair. Add shared action and lighting/atmosphere briefly.","Telegraphic and objective. Example: `side view upper body, left red hair adult woman tense smile in coat pulling right blue hair teen boy startled arms pinned, warm cafe light`. Keep ~12–28 words, English only."].join(" "):e==="supplement"?["Natural base mode SUPPLEMENT. Every shot MUST include `natural`: telegraphic English sentences describing what tags cannot express (composition, framing, actions, atmosphere, lighting) for NovelAI base caption only (NOT Danbooru comma tags, NOT characters[].action).","Keep tags and natural language clearly separated. Multi-person: NEVER use names — identify by position (left girl, bottom boy) or appearance. For each person include facial expression and clothing as distinct details; also hair and pose unless already obvious.","Unusual framing welcome (viewed through, reflected in shards of a broken mirror, behind…). Concise, minimal, objective — no subjective interpretation.","Example: `Side view, upper body. Left: adult woman with red hair, tense smile, coat, pulling. Right: teen boy with blue hair, startled face, arms pinned. Warm cafe lighting.` Prefer ~2–5 short sentences, English only."].join(" "):["Natural base mode SHORT. Every shot MUST include `natural`: a short English natural-language phrase for NovelAI base caption only (NOT Danbooru comma tags, NOT characters[].action).","Include hair color + age band + gender for each visible person, plus the shared action. Example: `red hair adult woman forced hug blue hair boy`. Keep ~6–20 words, English only."].join(" ")}te(),W(),F(),vt(),Ct(),ai(),at(),qt(),je(),_e(),Vn(),ut();var gl=5e3,Wm=2500,Vm=64;function Ym(e,t,n){const r=Rr(t,n),a=(En.get(r)?.epoch||0)+1;return En.set(r,{epoch:a,jobId:e}),xt.set(e,{key:r,epoch:a,cancelRequested:!1,publishedIds:[]}),{key:r,epoch:a}}function Xm(e){const t=xt.get(e);if(!t||t.cancelRequested)return!1;const n=En.get(t.key);return!!(n&&n.jobId===e&&n.epoch===t.epoch)}async function Zm(e){const t=xt.get(e),n=t?.publishedIds?[...t.publishedIds]:[];t&&(t.publishedIds=[]);for(const r of n)try{await pi(r)}catch{}return n.length}async function Ie(e,t="interrupted"){if(Xm(e))return!1;const n=await Zm(e);return await pe(e,"cancelled",{phase:"cancelled",progress:0,message:`${t}${n?` · discarded ${n}`:""}`,shot_count:0,shot_done:0},null),w("job.cancelled",{job_id:e,message:t,discarded:n,focus:!0}),!0}function Zn(e={}){return{shot_count:e.shot_count??0,shot_index:e.shot_index??0,shot_done:e.shot_done??0,progress:e.progress??0,phase:e.phase||"generating",message:e.message||"",cards_so_far:e.cards_so_far,debug_stage:gt()||kn(),debug_error:ar()?.message||""}}function Qm(e){return!e||typeof e!="object"?e:JSON.parse(JSON.stringify(e,(t,n)=>{if(t==="image_url"&&typeof n=="string"&&n.startsWith("data:"))return"";if(!(t==="tagged"||t==="appearance"||t==="debug_tail"))return n}))}async function pe(e,t,n=null,r=null){const a=await z("jobs",e);if(!a)return;const i=a.state,o={...a,state:t},s=Qm(n);o.result_json=s!=null?JSON.stringify(s):null,o.error=r,o.updated_at=Date.now()/1e3;const c=t!=="generating"||i!=="generating"||!!r;await D("jobs",o,{persist:c}),(t==="done"||t==="error"||t==="cancelled")&&await ss(),w("job.set",{message:`${t}${r?" ERR":""}${c?"":" (mem)"}`,job_id:e,state:t,persist:c,err:r?String(r).slice(0,160):"",background:t==="generating",focus:t!=="generating"})}async function hl(e){if(!(C().card||{}).power&&!e.force)throw new Error("Power가 OFF 상태입니다.");const t=u(e.session_id,200)||`sess_${bt().replace(/-/g,"").slice(0,12)}`,n={...e,session_id:t},r=await mi(n,t);if(r)return w("job.busy",{message:r.error.message||"busy",key:Rr(n,t),focus:!0},"warn"),r;const a=bt(),i=Date.now()/1e3;if(Ym(a,n,t),n.force)try{await fi(t,u(n.content_hash||""),n.message_index)}catch{}return await D("jobs",{id:a,session_id:t,state:"queued",request_json:JSON.stringify(n),result_json:null,error:null,created_at:i,updated_at:i}),ng(a).catch(async o=>{console.error("[Inlay Nexus] job crashed",a,o);try{await pe(a,"error",null,String(o?.message||o).slice(0,1500))}catch{}}),{ok:!0,accepted:!0,job_id:a,session_id:t,job_state:"queued"}}async function eg(e){const t=await z("jobs",e);if(!t)return{ok:!1,error:{code:"not_found",message:"job not found"}};const n=t.result_json?JSON.parse(t.result_json):null,r={};if(n&&typeof n=="object")for(const a of["shot_count","shot_index","shot_done","progress","phase","message","cards_so_far","debug_stage","debug_error"])a in n&&(r[a]=n[a]);return n&&await ot(n),{ok:!0,job_id:t.id,session_id:t.session_id,state:t.state,error:t.error,result:n,progress:r,debug:{last_stage:kn(),last_error:ar(),events:Xr(e,40)},created_at:t.created_at,updated_at:t.updated_at}}function tg(e){for(const t of["y_percent","anchor_percent","read_percent"]){const n=e[t];if(n!=null)try{return Math.max(0,Math.min(100,Number(n)))}catch{return null}}return null}async function ng(e){const t=await z("jobs",e);if(!t)return;const n=JSON.parse(t.request_json??"{}"),r=String(t.session_id??""),a=Fi();xn(e);const i=ue("job.run");w("job.start",{job_id:e,session_id:r,message_index:n.message_index,text_len:String(n.assistant_text||"").length});try{if(await Ie(e,"superseded before start")||(n.force&&await fi(r,u(n.content_hash||""),n.message_index),await Ie(e,"superseded before tagging")))return;await pe(e,"tagging",{phase:"tagging",progress:0,message:"장면 태깅 중…",shot_count:0,shot_done:0,debug_stage:"job.tagging"});const o=await Km(n),s=Hm(o);if(w("job.tagger.messages",{msgs:o.length}),C().card?.preprocessing){const k=sr(await se("preprocess"));if(k){const I=[{role:"system",content:k},o[o.length-1]],N=await on(C().llm,I);if(await Ie(e,"superseded during preprocess"))return;o.splice(o.length-1,0,{role:"system",content:`## Preprocess Summary
${N}`})}}const c=await on(C().llm,o);if(await Ie(e,"superseded after tagging"))return;const l=Qr(c);let f=Gm(l);if(w("job.tagger.done",{shots:f.length,raw_len:String(c||"").length}),!f.length)throw new Error("태거가 shot을 반환하지 않았습니다.");const d=C().card||{},m=Math.max(1,Number(d.image_min??1)),p=Math.max(m,Number(d.image_max??3));f=f.slice(0,p);const g=jr();if(g==="two_stage"){if(await pe(e,"tagging",{phase:"tagging",progress:.35,message:"큐레이션 2단 씬 태그 보강 중…",shot_count:f.length,shot_done:0,debug_stage:"job.curation_refine"}),await Ie(e,"superseded during curation refine"))return;await Hc(f,{chatContext:s})}else if(g==="embed_snap"){if(await pe(e,"tagging",{phase:"tagging",progress:.4,message:"씬 태그 임베딩 매칭 중…",shot_count:f.length,shot_done:0,debug_stage:"job.curation_snap"}),await Ie(e,"superseded during curation snap"))return;const k=await qc(f);k.ok||(w("job.curation_snap.fallback",{message:k.reason},"warn"),await pe(e,"tagging",{phase:"tagging",progress:.45,message:k.reason||"임베딩 없음 → 큐레이션 없이 생성",shot_count:f.length,shot_done:0,debug_stage:"job.curation_snap_fallback"}))}const h=f.flatMap(k=>k.characters||[]),_=u(n.unified_session_id||"",200),y=u(n.character_id||"",200),x=Array.isArray(n.source_session_ids)?n.source_session_ids.map(k=>u(k,200)).filter(Boolean):[],v=await cd({sessionId:r,tagged:l,shotChars:h,unifiedSessionId:_,characterId:y,sourceSessionIds:x});w("job.roster",{roster:v.length});const S=rt(d);for(const k of f)k.characters=fa(k.characters||[],v,S);if(await Ie(e,"superseded before generate"))return;await pe(e,"generating",Zn({shot_count:f.length,shot_index:0,shot_done:0,progress:0,phase:"generating",message:`이미지 1/${f.length} 생성 준비`}));const $=[],M=!!d.llm_anchor_percent;for(let k=0;k<f.length;k+=1){if(await Ie(e,`superseded before shot ${k+1}`))return;const I=f[k],{main:N,neg:R,captions:H,meta:me}=await Br({shot:I,roster:v}),J=bt(),q=Date.now()/1e3,G=u(n.content_hash||"");let qe=tg(I);qe==null&&M&&(qe=Math.round(k/Math.max(1,f.length)*1e4)/100),w("job.shot.prepare",{shot:k,card_id:J,prompt_len:String(N||"").length,captions:(H||[]).length}),await pe(e,"generating",Zn({shot_count:f.length,shot_index:k,shot_done:k,progress:Math.round(k/Math.max(1,f.length)*1e3)/10,phase:"generating",message:`NovelAI 요청 중 ${k+1}/${f.length}… [${gt()}]`}));let oe=0;const pt=setInterval(()=>{oe+=gl/1e3;const un=Gs();qs()&&Ba()>=Vm&&un&&Date.now()-un>=Wm&&bd("heartbeat-idle");const Vl=Ba()||Ks();pe(e,"generating",Zn({shot_count:f.length,shot_index:k,shot_done:k,progress:Math.round(k/Math.max(1,f.length)*1e3)/10,phase:"generating",message:`NovelAI 대기 ${k+1}/${f.length} (${oe}s) · ${gt()}${Vl?` ${Math.round(Vl/1024)}KB`:""}`})).catch(()=>{})},gl);let Y,Re;try{({bytes:Y,seed:Re}=await nl({main:N,neg:R,captions:H},I.aspect))}finally{clearInterval(pt)}if(w("job.shot.nai_done",{shot:k,bytes:Y?.byteLength||0,seed:Re}),await Ie(e,`superseded after shot ${k+1} nai`))return;await pe(e,"generating",Zn({shot_count:f.length,shot_index:k,shot_done:k,progress:Math.round((k+.5)/Math.max(1,f.length)*1e3)/10,phase:"generating",message:`이미지 저장 중 ${k+1}/${f.length}… [${gt()}]`}));const re=bm({imageId:J,sessionId:r,request:n,shotIndex:k,paragraph:I.paragraph,yPercent:qe,line:(()=>{const un=Math.floor(Number(I.line));return Number.isFinite(un)&&un>=1?un:null})(),contentHash:G});await ba(J,Y,re);const Lh={...si(me,re,Y?.byteLength||0),aspect:I.aspect||void 0};await D("cards",{id:J,job_id:e,session_id:r,shot_index:k,paragraph:Number(I.paragraph||0),main_prompt:N,negative_prompt:R,characters_json:JSON.stringify(me.characters||[]),seed:Re,meta_json:JSON.stringify(Lh),created_at:q});const Wl=xt.get(e);if(Wl&&Wl.publishedIds.push(J),await Ie(e,`superseded after shot ${k+1} save`))return;$.push({id:J,shot_index:k,paragraph:re.paragraph,y_percent:re.y_percent,line:re.line,message_index:re.message_index??-1,message_role:re.message_role||"",content_hash:re.content_hash||"",character_id:re.character_id||"",chat_id:re.chat_id||"",character_name:re.character_name||"",chat_name:re.chat_name||"",char_index:re.char_index??-1,chat_index:re.chat_index??-1,assistant_preview:u(n.assistant_text||"",Li),main_prompt:N,negative_prompt:R,characters:me.characters||[],image_url:Pe(J),seed:Re,storage:"indexeddb",png_bytes:Y?.byteLength||0}),w("job.shot.saved",{shot:k,card_id:J,has_url:!!Pe(J)}),await pe(e,"generating",Zn({shot_count:f.length,shot_index:k,shot_done:k+1,progress:Math.round((k+1)/Math.max(1,f.length)*1e3)/10,phase:"generating",message:`이미지 ${k+1}/${f.length} 완료`,cards_so_far:$.length}))}if(await Ie(e,"superseded before done"))return;const T={cards:$,message_index:n.message_index!=null?Number(n.message_index):-1,shot_count:f.length,shot_done:f.length,progress:100,phase:"done",message:`이미지 ${f.length}/${f.length} 완료`};await ot(T),await pe(e,"done",T);const B=xt.get(e);B&&(B.publishedIds=[]),i.end({message:"done",cards:$.length})}catch(o){i.fail(o);const s=o,c=`${s?.message||o}
${s?.stack||""}`.slice(-1500);await pe(e,"error",{phase:"error",message:String(s?.message||o).slice(0,240),debug_stage:kn(),debug_tail:Xr(e,12)},c)}finally{xn(a)}}F(),at(),Ht(),je(),_e();function Rt(e,t){try{return JSON.parse(e)}catch{return t}}async function rg(e,t={}){const n=u(e,80),r=await z("cards",n);if(!r)return{ok:!1,error:{code:"not_found",message:"card not found"}};const a=Rt(r.characters_json||"[]",[]),i=Array.isArray(a)?a:[];let o=r.main_prompt;"main_prompt"in t&&(o=u(t.main_prompt,8e3));let s=r.negative_prompt;"negative_prompt"in t&&(s=u(t.negative_prompt,8e3));let c=i;if("characters"in t){const d=t.characters||[];if(!Array.isArray(d))return{ok:!1,error:{code:"bad_request",message:"characters must be a list"}};c=[];const m=rt(C().card||{});for(let p=0;p<d.slice(0,m).length;p++){const g=d[p];if(typeof g!="object")continue;const h=g,_=p<i.length&&typeof i[p]=="object"?i[p]:{},y=u(h.name!=null?h.name:_.name,200),x=u(h.prompt!=null?h.prompt:_.prompt,4e3);if(!y&&!x)continue;const v={..._,name:y||`char${p+1}`,prompt:x||"girl"};"uc"in h&&(v.uc=u(h.uc,2e3));for(const S of["center_x","center_y"])if(S in h)try{v[S]=Number(h[S])}catch{}c.push(v)}}r.main_prompt=o,r.negative_prompt=s,r.characters_json=JSON.stringify(c);try{let d=Rt(r.meta_json||"{}",{});(!d||typeof d!="object"||Array.isArray(d))&&(d={});const m=d;m.setup=o,m.characters=c,r.meta_json=JSON.stringify(m)}catch{}await D("cards",r);const l=await dt(n,{}),f={id:n,main_prompt:o,negative_prompt:s,characters:c,paragraph:r.paragraph,shot_index:r.shot_index,y_percent:l.y_percent,message_index:l.message_index,content_hash:l.content_hash,image_url:Pe(n)};return await ot(f),{ok:!0,card:f}}async function _l(e,t="nai",n=null,r={}){const a=await z("cards",e);if(!a)return{ok:!1,error:{code:"not_found",message:"card not found"}};const i=a.session_id,o=Rt(a.meta_json||"{}",{}),s=o.location||{};if(!r.skipBusyCheck)try{const I=await dt(e,o),N=await mi({session_id:i,content_hash:u(I.content_hash||o.content_hash||"",128),message_index:O(I.message_index,-1)},i);if(N)return N}catch{}const c=u(s.character_id||o.character_id||"",200);let l=u(o.unified_session_id||s.unified_session_id||"",200);!l&&c&&(l=ru(c));let f=[];try{const I=await z("jobs",a.job_id);if(I?.request_json){const N=JSON.parse(I.request_json);Array.isArray(N.source_session_ids)&&(f=N.source_session_ids.map(R=>u(R,200)).filter(Boolean))}}catch{}const d=await Ea(i,l,c,f);if(t==="full"){const I=await z("jobs",a.job_id);if(!I)return{ok:!1,error:{code:"no_job",message:"original job missing"}};const N=JSON.parse(I.request_json);return N.force=!0,hl(N)}let m,p,g,h,_={};const y=n;if(y&&("main_prompt"in y||"negative_prompt"in y||"characters"in y))m="main_prompt"in y?u(y.main_prompt||""):u(a.main_prompt),p="negative_prompt"in y?u(y.negative_prompt||""):u(a.negative_prompt),h="characters"in y?y.characters:null,h==null&&(h=Rt(a.characters_json||"[]",[])),g=(h||[]).slice(0,rt(C().card||{})).map(I=>({prompt:ma(I.prompt||"girl")||"girl",uc:u(I.uc),center_x:Number(I.center_x??.5),center_y:Number(I.center_y??.5)}));else if(u(a.main_prompt||"")){const I=Rt(a.characters_json||"[]",[]),N=Array.isArray(I)?I:[],R=yl(o),H=N.map(pt=>{if(!pt||typeof pt!="object")return null;const Y=pt;if(Y.raw&&typeof Y.raw=="object"){const Re=Y.raw;return{...Re,name:u(Re.name||Y.name,200)}}return{name:u(Y.name,200),action:Y.action,expression:Y.expression,sex:Y.sex,label:Y.label,age:Y.age,original:Y.original||Y.original_tag}}).filter(Boolean),me=C().card||{},J=Ue(C().nai?.model||"nai-diffusion-4-5-full"),q=Object.values(Nn).filter(Boolean);Nn[J]&&q.unshift(Nn[J]);const G=Ku({setup:o.setup,main:a.main_prompt,person:o.person,stylePositives:Gu(me),qualitySuffixes:q}),qe={characters:R.length?R:H,paragraph:a.paragraph,camera:G.rebuildMain&&G.lockedSetup?"":u(o.camera||""),situation:G.rebuildMain&&G.lockedSetup?"":u(o.situation||o.scene||""),place:G.rebuildMain&&G.lockedSetup?"":u(o.place||""),action:G.rebuildMain&&G.lockedSetup?"":u(o.action||"")},oe=await Br({shot:qe,roster:d,lockedSetup:G.rebuildMain?G.lockedSetup:void 0});G.rebuildMain?(m=oe.main,p=oe.neg,g=oe.captions,h=oe.meta.characters,_={setup:oe.meta.setup,person:oe.meta.person,characters:h}):(m=u(a.main_prompt),p=u(a.negative_prompt)||oe.neg,g=oe.captions,h=oe.meta.characters,_={setup:m,person:oe.meta.person,characters:h})}else{const I=Rt(a.characters_json||"[]",[]),N=yl(o),R=(I||[]).map(q=>{if(!q||typeof q!="object")return null;const G=q;return G.raw||{name:G.name,action:G.action,expression:G.expression}}).filter(Boolean),H=u(o.setup||""),me={characters:N.length?N:R,paragraph:a.paragraph,camera:H?"":u(o.camera||""),situation:H?"":u(o.situation||o.scene||""),place:H?"":u(o.place||""),action:H?"":u(o.action||"")},J=await Br({shot:me,roster:d,lockedSetup:H});m=J.main,p=J.neg,g=J.captions,h=J.meta.characters,_={setup:J.meta.setup,person:J.meta.person,characters:h}}if(!g.length&&!(y&&"characters"in y)){const I={characters:(o.characters||[]).map(R=>R.raw||R),camera:o.setup},N=await Br({shot:I,roster:d});m=N.main,p=N.neg,g=N.captions,h=N.meta.characters,_={setup:N.meta.setup,person:N.meta.person,characters:h}}const{bytes:x,seed:v}=await nl({main:m,neg:p,captions:g},o.aspect??y?.aspect),S=bt(),$=Date.now()/1e3;let M=await Lt(a.id);Object.keys(M).length||(M=await dt(a.id,o));const T={version:1,image_id:S,session_id:i,unified_session_id:l,character_id:u(M.character_id||"",200),character_name:u(M.character_name||o.character_name||"",200),chat_id:u(M.chat_id||"",200),chat_name:u(M.chat_name||o.chat_name||"",200),char_index:O(M.char_index,-1),chat_index:O(M.chat_index,-1),message_index:O(M.message_index,-1),shot_index:O(M.shot_index,O(a.shot_index,0)),paragraph:O(M.paragraph,O(a.paragraph,0)),y_percent:or(M.y_percent),content_hash:u(M.content_hash||"",128),assistant_preview:u(M.assistant_preview||o.assistant_preview||"",Qe)};await ba(S,x,T);const B=si({...o,..._},T,x.byteLength);for(const I of["y_percent","anchor_percent","read_percent"])delete B[I];B.y_percent=T.y_percent,await D("cards",{id:S,job_id:a.job_id,session_id:i,shot_index:a.shot_index,paragraph:a.paragraph,main_prompt:m,negative_prompt:p,characters_json:JSON.stringify(h),seed:v,meta_json:JSON.stringify(B),created_at:$});try{await pi(e)}catch{}const k={id:S,image_url:Pe(S),main_prompt:m,negative_prompt:p,characters:h,seed:v,paragraph:T.paragraph,y_percent:T.y_percent,message_index:T.message_index,shot_index:T.shot_index,content_hash:T.content_hash,character_id:T.character_id,chat_id:T.chat_id,character_name:T.character_name,chat_name:T.chat_name,assistant_preview:T.assistant_preview,storage:"indexeddb",png_bytes:x.byteLength};return await ot(k),{ok:!0,replaced:e,card:k}}async function ag({session_id:e="",content_hash:t="",message_index:n=-1}={}){const r=u(e,200),a=u(t,128),i=O(n,-1);if(!r&&!a&&i<0)return{ok:!1,error:{code:"bad_request",message:"session_id or content_hash required"}};const o=await we("cards"),s=[];for(const h of o){if(r&&h.session_id!==r)continue;const _=Rt(h.meta_json||"{}",{}),y=await dt(h.id,_),x=u(y.content_hash||_.content_hash||"",128),v=O(y.message_index,-1);if(a){if(x!==a)continue}else if(i>=0){if(v!==i)continue}else continue;const S=y.y_percent??_.y_percent??_.anchor_percent??_.read_percent,$=Number(S);s.push({row:h,y:Number.isFinite($)?$:999,shot:O(h.shot_index,0),paragraph:O(h.paragraph??y.paragraph,0)})}if(s.sort((h,_)=>h.y-_.y||h.shot-_.shot||h.paragraph-_.paragraph),!s.length)return{ok:!1,error:{code:"not_found",message:"no cards for message"}};const c=r||u(s[0]?.row?.session_id||"",200),l={session_id:c,content_hash:a,message_index:i},f=await mi(l,c);if(f)return f;const d=Rr(l,c);ur.add(d);const m=[],p=[],g=[];try{for(const h of s){const _=h.row;try{const y=await _l(_.id,"nai",null,{skipBusyCheck:!0});if(y?.busy){g.push({id:_.id,error:y.error?.message||"busy"});break}y?.ok&&y.card?(m.push(y.card),y.replaced&&p.push(y.replaced)):g.push({id:_.id,error:u(y?.error?.message||"reroll failed",400)})}catch(y){g.push({id:_.id,error:u(y?.message||y,400)})}}return{ok:m.length>0,count:m.length,replaced:p,cards:m,failed:g}}finally{ur.delete(d)}}function yl(e){return Array.isArray(e.characters)?e.characters.map(t=>t&&typeof t=="object"?t.raw||t:null).filter(Boolean):[]}var ig=fn({HASH_REBIND_THRESHOLD:()=>ug,PREFIX_MATCH_MIN_CHARS:()=>24,PREFIX_MATCH_THRESHOLD:()=>lg,SIMILARITY_COMPARE_MAX:()=>800,VIEWER_THUMB_LAYOUT:()=>ne,activeSegmentIndex:()=>rh,bigramCounts:()=>yi,buildInlineChatHtml:()=>lh,claimStickyMarkerByCardId:()=>Tg,clampPinPercent:()=>Ke,clampReadingPercent:()=>nh,clampShotLine:()=>Bl,clampThumbScrollOffset:()=>Gg,composeDualProgressBarsHtml:()=>Qg,composeProgressToastHtml:()=>eh,composeSingleProgressBarHtml:()=>Tl,composeStickyThumbHtml:()=>Rg,createDebouncedSaveQueue:()=>Cg,createScrollSettleTracker:()=>Ig,createSessionChangeGuard:()=>Eg,describeDomApiCompare:()=>yg,diceBigramRatio:()=>vl,evenAnchorPercent:()=>vi,expandElementLineEntries:()=>Ul,findElementIndexForLine:()=>Jl,findElementIndexForLineWithFallback:()=>ch,findHashRebindCandidates:()=>fg,findPlainLineStartOffset:()=>Dl,fitBoxInside:()=>Bg,galleryFocusMessage:()=>mg,galleryForMessage:()=>pg,galleryIndexFromChildIndex:()=>Jg,gallerySelectedCount:()=>wi,galleryStripContentWidth:()=>Kg,galleryStripSplitAt:()=>zr,hasGenerationInfo:()=>Ml,htmlToPlainLn:()=>sh,injectInlineImagesIntoHtml:()=>Gl,isCharMessageRole:()=>hg,isMessageSelectionGesture:()=>Ag,isNearbyDomIndex:()=>Vg,lineTextOccurrence:()=>zl,linkCardsForMessage:()=>bi,longestCommonPrefixLength:()=>wl,markerBlockHtml:()=>Kl,mergeViewerPaintJob:()=>Xg,messageCompactKey:()=>ln,messageContextTriplet:()=>_g,messagesTextOverlapScore:()=>Nl,nearbyDomIndexWindow:()=>Yg,nearbyMessageImageIds:()=>Wg,normalizeInlineMatchText:()=>Hr,normalizeMatchText:()=>Fr,normalizeMessageRole:()=>_i,parseAutotagLookJson:()=>$l,pickMessageIndexNearPoint:()=>Sg,pinPercentToPx:()=>vg,pinPercentToPxFromBottom:()=>xg,pinPxToPercent:()=>Il,preferNearbyHostIndex:()=>Hl,prefixMatchRatio:()=>xl,rawMessageRole:()=>Dr,readingPercentInMessage:()=>jl,rebindGalleryMessageIndexes:()=>bg,resolveCardAnchorPercent:()=>th,resolveChatMessageMatch:()=>wg,resolveClickSelectionAction:()=>Mg,resolveIndexProgress:()=>Zg,resolveStickyThumbPct:()=>Og,resolveStoredPinPercent:()=>kg,roleFromGenerationInfo:()=>gg,scaleInlineThumbnail:()=>cg,shouldKeepStickyThumbHidden:()=>Pg,shouldRefreshGallery:()=>$g,shouldRewriteStickyThumb:()=>jg,shouldSelectMessageByTextDrag:()=>Ng,splitMessageLines:()=>Jr,stickyCornerEdgeBox:()=>El,stickyCornerImageBox:()=>Dg,stickyPinEdgeBox:()=>Ug,stickyPinOverImage:()=>zg,stickyThumbBoxFromPct:()=>Lg,stickyThumbNeedsHtmlPaint:()=>Fg,stripInlayInlineHtml:()=>Ll,thumbIndexAtStripX:()=>Hg,visibleGalleryImageIds:()=>qg}),og=528,sg=720;function b(e,t=0){const n=Number(e);return Number.isFinite(n)?n:t}function bl(e,t){const n={...e};for(const[r,a]of Object.entries(t||{})){const i=n[r];n[r]=a&&typeof a=="object"&&!Array.isArray(a)?bl(i&&typeof i=="object"?i:{},a):a}return n}function cg(e){const t=Math.max(1,b(e,100));return{width:Math.max(1,Math.round(og*t/100)),height:Math.max(1,Math.round(sg*t/100)),percent:t}}function Fr(e){return String(e||"").replace(/[^a-zA-Z0-9\uac00-\ud7a3\u3040-\u30ff\u3400-\u9fff\uff00-\uffef]/g,"").toLowerCase()}var lg=.6,ug=.6,Bh=24,Rh=800;function _i(e){const t=String(e||"").toLowerCase().trim();return t?t==="char"||t==="assistant"||t==="bot"?"char":t==="user"||t==="human"?"user":t:""}function wl(e,t){const n=String(e||""),r=String(t||""),a=Math.min(n.length,r.length);let i=0;for(;i<a&&n.charCodeAt(i)===r.charCodeAt(i);)i+=1;return i}function yi(e){const t=String(e||""),n=new Map;if(t.length<2)return n;for(let r=0;r<t.length-1;r+=1){const a=t.charCodeAt(r)+","+t.charCodeAt(r+1);n.set(a,(n.get(a)||0)+1)}return n}function vl(e,t,n=null){const r=String(e||""),a=String(t||"");if(!r||!a)return 0;if(r===a)return 1;if(r.length<2||a.length<2)return 0;const i=n||yi(r),o=yi(a);let s=0;for(const[f,d]of o){const m=i.get(f)||0;m&&(s+=Math.min(m,d))}const c=r.length-1,l=a.length-1;return 2*s/(c+l)}function xl(e,t){const n=Fr(e),r=Fr(t);if(!n||!r)return 0;const a=Math.max(n.length,r.length);if(a<24)return 0;if(n===r)return 1;const i=wl(n,r)/a;return i>=.6?i:vl(n.length>800?n.slice(0,800):n,r.length>800?r.slice(0,800):r)}function bi(e,t){if(!t)return[];const n=Array.isArray(e)?e:[],r=String(t.hash||"");return r?Al(n.filter(a=>a?.content_hash&&a.content_hash===r)):[]}function fg(e,t={}){const n=Array.isArray(e)?e:[],r=String(t.newHash||t.hash||""),a=String(t.text||""),i=String(t.characterId||""),o=String(t.chatId||""),s=String(t.sessionId||""),c=Number(t.messageIndex),l=_i(t.role||t.messageRole||"");if(!r||!a||!i||!l||!Number.isFinite(c)||c<0)return[];if(n.some(d=>d?.content_hash&&d.content_hash===r))return[];if(Fr(a).length<24)return[];const f=[];for(const d of n){const m=String(d?.content_hash||"");if(!m||m===r||String(d?.character_id||"")!==i)continue;const p=String(d?.chat_id||""),g=String(d?.session_id||"");if(o){if(p!==o)continue}else if(s){if(g&&g!==s)continue}else continue;if(Number(d?.message_index)!==c)continue;const h=_i(d?.message_role||d?.role||"");if(!h||h!==l)continue;const _=String(d?.assistant_preview||"");if(!_)continue;const y=xl(a,_);y<.6||f.push({card:d,score:y})}return f.sort((d,m)=>m.score-d.score||b(m.card?.created_at)-b(d.card?.created_at)),Al(f.map(d=>d.card))}function kl(e){const t=e?.y_percent??e?.anchor_percent??e?.read_percent,n=Number(t);return Number.isFinite(n)?n:Number.POSITIVE_INFINITY}function dg(e,t){return kl(e)-kl(t)||b(e?.paragraph)-b(t?.paragraph)||b(e?.shot_index)-b(t?.shot_index)||b(t?.created_at)-b(e?.created_at)}function Sl(e,t){return b(t?.created_at)-b(e?.created_at)||b(t?.message_index)-b(e?.message_index)||b(t?.paragraph)-b(e?.paragraph)||b(t?.shot_index)-b(e?.shot_index)}function Al(e){const t=new Map;for(const n of e||[]){const r=`${n?.paragraph??"?"}|${n?.shot_index??n?.id}`,a=t.get(r);(!a||b(n?.created_at)>=b(a?.created_at))&&t.set(r,n)}return[...t.values()]}function pg(e,t,n=8){const r=new Map;for(const f of e||[]){const d=String(f?.id||"");!d||r.has(d)||r.set(d,f)}const a=[...r.values()],i=Math.max(0,Math.min(64,b(n,8)||8));if(!t)return a.sort(Sl).slice(0,Math.max(1,i||8));const o=bi(a,t).sort(dg),s=new Set(o.map(f=>String(f.id))),c=String(t.sessionId||""),l=a.filter(f=>!s.has(String(f.id))).filter(f=>!c||!f?.session_id||f.session_id===c).sort(Sl).slice(0,i);return[...o,...l]}function wi(e,t){return t?bi(e,t).length:0}function mg(e,t,n){if(e&&wi(n,e)>0)return e;const r=String(e?.sessionId||e?.session_id||""),a=String(t?.sessionId||t?.session_id||"");return t&&wi(n,t)>0&&(!r||!a||r===a)?t:e||null}function Dr(e){const t=String(e?.role||e?.type||e?.speaker||e?.sender||e?.from||e?.name||"").trim().toLowerCase();return e?.isUser===!0||e?.fromUser===!0||/^(user|human|player|you|me)$/.test(t)||t.includes("user")?"user":e?.isAssistant===!0||e?.isBot===!0||e?.isChar===!0||/^(assistant|bot|char|character|model|ai)$/.test(t)||t.includes("assistant")||t.includes("bot")||t.includes("char")?"char":t==="system"||t==="developer"?"system":""}function Ml(e){const t=e?.generationInfo??e?.generation_info??e?.messageGenerationInfo;return t==null?!1:typeof t=="string"?t.trim().length>0:typeof t=="object"?Object.keys(t).length>0:!0}function gg(e){return Ml(e)?"char":"user"}function hg(e){if(e&&typeof e=="object")return Dr(e)==="char";const t=String(e||"").trim().toLowerCase();return t==="char"||t==="assistant"||t==="bot"}function ln(e,t=48){const n=Math.max(1,b(t,48));return String(e||"").replace(/\s+/g,"").slice(0,n)}function _g(e,t,n=48){const r=Array.isArray(e)?e:[],a=Math.floor(b(t,-1)),i=a>=0&&a<r.length?r[a]:null,o=a>0?r[a-1]:null,s=a>=0&&a<r.length-1?r[a+1]:null;return[ln(o?.text,n),ln(i?.text,n),ln(s?.text,n),Dr(i)||String(i?.role||"").trim().toLowerCase()].join("|")}function Nl(e,t){const n=String(e||"").replace(/\s+/g,""),r=String(t||"").replace(/\s+/g,"");if(!n||!r)return 0;if(n.includes(r))return r.length;if(r.includes(n))return n.length;let a=0;const i=(s,c,l=8,f=512)=>{let d=l,m=Math.min(s.length,f),p=0;for(;d<=m;){const g=d+m>>1;c.includes(s.slice(0,g))?(p=g,d=g+1):m=g-1}return p};a=Math.max(a,i(r,n),i(n,r));const o=Math.min(r.length,2400);for(let s=0;s+8<=o;s+=32){const c=Math.min(64,o-s);for(let l=c;l>=8;l-=8)if(n.includes(r.slice(s,s+l))){a=Math.max(a,l);break}}return a}function yg(e,t){const n=String(e||""),r=String(t||""),a=ln(n),i=ln(r),o=Nl(n,r),s=o>=8,c=a.length>0&&a.length<8&&a===i,l=i.slice(0,48),f=a.slice(0,48);return{domChars:n.length,apiChars:r.length,domCompactChars:a.length,apiCompactChars:i.length,shareScore:o,overlap:s,shortExact:c,apiInDom:!!(l&&l.length>=8&&a.includes(l)),domInApi:!!(f&&f.length>=8&&i.includes(f)),apiKey:l,domKey:f}}function bg(e,t,n){const r=Array.isArray(t)?t:[],a=typeof n=="function"?n:null,i=new Map;if(a)for(let s=0;s<r.length;s+=1){const c=String(r[s]?.text??r[s]?.data??r[s]?.content??""),l=String(a(c)||"");if(!l)continue;const f=Number.isFinite(Number(r[s]?.index))?Number(r[s].index):s,d=i.get(l);d?d.push(f):i.set(l,[f])}let o=0;return{cards:(Array.isArray(e)?e:[]).map(s=>{const c=String(s?.content_hash||""),l=c?i.get(c):void 0;return!c||!l?s:l.length!==1?Number(s?.message_index)!==-1?(o+=1,{...s,message_index:-1}):s:Number(s?.message_index)!==l[0]?(o+=1,{...s,message_index:l[0]}):s}),changed:o}}function wg(e,t,n,r,a){const i=Array.isArray(t)?t:[],o=Math.floor(b(n,-1)),s=(f,d,m,p=-1)=>({chatIndex:Number.isFinite(Number(f?.index))?Number(f?.index):p>=0?p:o,text:String(f?.text||e||""),role:Dr(f)||String(f?.role||"").trim().toLowerCase(),matchMethod:d,score:m});if(!i.length)return{chatIndex:Math.max(0,o),text:String(e||""),role:"",matchMethod:"fallback",score:0};const c=i.length-1-o;if(c>=0&&c<i.length)return s(i[c],"reverse",100,c);const l=Math.max(0,Math.min(i.length-1,c));return s(i[l],"reverse",50,l)}function Ke(e,t=0){const n=Number(e);return Number.isFinite(n)?Math.max(0,Math.min(100,Math.floor(n))):Math.max(0,Math.min(100,Math.floor(b(t,0))))}function vg(e,t,n=22,r=4){const a=Math.max(1,b(t,1)),i=Math.max(1,b(n,1)),o=Math.max(0,b(r,0)),s=Math.floor(a*Ke(e,0)/100);return Math.max(o,Math.min(a-i-o,s))}function xg(e,t,n=22,r=8){const a=Math.max(1,b(t,1)),i=Math.max(1,b(n,1)),o=Math.max(0,b(r,0)),s=a-Math.floor(a*Ke(e,0)/100)-i;return Math.max(o,Math.min(a-i-o,s))}function Il(e,t){const n=Math.max(1,b(t,1));return Ke(b(e,0)/n*100,0)}function kg(e,t,n,r=null){const a=r&&typeof r=="object"?r:{x:38,y:80},i=t==="y"||t==="Y",o=i?b(a.y,80):b(a.x,38),s=i?"overlay_y_pct":"overlay_x_pct",c=i?"overlay_y_offset":"overlay_x_offset",l=String(e?.overlay_pin_origin||""),f=l==="top"||l==="tl"||l==="top-left",d=e?.[s],m=d!=null&&Number.isFinite(Number(d)),p=Number(e?.[c]),g=Number.isFinite(p),h=_=>Ke(100-Number(_),o);return m?i&&f?h(d):Ke(d,o):g?i?Ke((b(n,1)-p)/Math.max(1,b(n,1))*100,o):Il(p,n):Ke(o,o)}function Sg(e,t,n=null,r=800){const a=Array.isArray(e)?e:[];if(!a.length)return-1;const i=Math.max(1,b(r,800)),o=Number.isFinite(Number(t))?Number(t):i*.5,s=Number.isFinite(Number(n))?Number(n):null;let c=-1,l=1/0,f=-1,d=1/0;for(let m=0;m<a.length;m+=1){const p=a[m];if(!p)continue;const g=b(p.top,0),h=b(p.bottom,g);if(h<=0||g>=i)continue;const _=(g+h)*.5,y=Number.isFinite(Number(p.left))&&Number.isFinite(Number(p.right))?(Number(p.left)+Number(p.right))*.5:null,x=y!=null&&s!=null?Math.hypot(y-s,_-o):Math.abs(_-o);x<d&&(d=x,f=m);const v=o>=g&&o<=h,S=s==null||!Number.isFinite(Number(p.left))||!Number.isFinite(Number(p.right))||s>=Number(p.left)&&s<=Number(p.right);if(v&&S){const $=Math.max(1,h-g);$<l&&(l=$,c=m)}}return c>=0?c:f}function Ag({gesture:e="single",detail:t=1,movement:n=0,textSelecting:r=!1,excludedTarget:a=!1}={}){const i=e==="double"?2:1;return!a&&!r&&b(n,1/0)<=8&&b(t)===i}function Mg({gesture:e="single",detail:t=1,pendingDomIndex:n=null,targetDomIndex:r=null}={}){const a=b(t,1);return e==="double"?a===1?{action:"provisional",pendingDomIndex:r}:a===2?{action:"confirm",clearPending:!0,matchedPending:n!=null&&Number(n)===Number(r)}:{action:"ignore"}:a===1?{action:"confirm",clearPending:!0}:{action:"ignore"}}function Ng({enabled:e=!0,movement:t=0,hasSelection:n=!1,excludedTarget:r=!1}={}){return!!e&&!r&&!!n&&b(t,0)>8}function Ig({delayMs:e=160,onSettle:t}={}){let n=null;const r=Math.max(0,b(e,160)),a=()=>{n=null,typeof t=="function"&&t()};return{bump(){n&&clearTimeout(n),n=setTimeout(a,r)},settleNow(){n&&clearTimeout(n),n=null,a()},cancel(){n&&clearTimeout(n),n=null},get pending(){return n!=null}}}function Eg(e=""){let t=String(e||""),n="",r=0;return{observe(a){const i=String(a||"");return!i||i===t?(n="",r=0,!1):i!==n?(n=i,r=1,!1):(r+=1,r<2?!1:(t=n,n="",r=0,!0))},current(){return t},reset(a=""){t=String(a||""),n="",r=0}}}function Cg(e,t=300){let n={},r=null,a=null;const i=async()=>a||(a=(async()=>{for(;Object.keys(n).length;){const o=n;n={},await e(o)}})().finally(()=>{a=null}),a);return{enqueue(o){n=bl(n,o),r&&clearTimeout(r),r=setTimeout(()=>{r=null,i().catch(()=>{})},Math.max(0,b(t,300)))},async flush(){r&&(clearTimeout(r),r=null);do await i();while(Object.keys(n).length||a)},hasPending(){return!!r||!!a||Object.keys(n).length>0}}}function $g(e,t){const n=Array.isArray(e)?e.map(String):[],r=Array.isArray(t)?t.map(String):[];if(n.length!==r.length)return!0;for(let a=0;a<n.length;a+=1)if(n[a]!==r[a])return!0;return!1}function Tg(e,t){const n=String(t||"");if(!n||!Array.isArray(e)||!e.length)return null;const r=e.findIndex(i=>String(i?.card?.id||"")===n);if(r<0)return null;const a=e[r];return a?.thumb?(e.splice(r,1),a):null}function jg(e,t){return String(e||"")!==String(t||"")}function Pg(e,t,n){if(!e)return!1;const r=String(t||""),a=String(n||"");return!!r&&!!a&&r===a}function Og(e){if(!e.alwaysOn||e.userCollapsed||e.editorOpen)return 0;const t=b(e.settingsPct,0);return t>0?t:0}function Lg(e,t,n){const r=Math.max(0,b(e,0)),a=Math.max(0,b(t,0)),i=Math.max(0,b(n,0));return{pct:r,w:Math.max(0,Math.round(a*r/100)),h:Math.max(0,Math.round(i*r/100))}}function Bg(e,t,n=0,r=0){const a=Math.max(0,b(e,0)),i=Math.max(0,b(t,0));if(a<=0||i<=0)return{w:0,h:0};const o=Math.max(0,b(n,0)),s=Math.max(0,b(r,0));if(o<=0||s<=0)return{w:a,h:i};const c=Math.min(a/o,i/s);return{w:Math.max(1,Math.round(o*c)),h:Math.max(1,Math.round(s*c))}}function Rg(e,t=""){const n=typeof e=="string"?e:"";return n?`<img src="${n}" style="width:100%;height:100%;object-fit:contain;display:block;background:transparent" />`:'<div style="width:100%;height:100%;background:transparent"></div>'}function Fg(e,t,n,r){const a=typeof t=="string"?t:"";return a?String(e||"")!==a||String(n||"")!==String(r||""):!1}function Dg(e,t,n,r=16){const a=Math.max(1,Math.round(b(t?.w,1))),i=Math.max(1,Math.round(b(t?.h,1))),o=Math.max(a,Math.round(b(n?.width,a))),s=Math.max(i,Math.round(b(n?.height,i))),c=Math.max(0,Math.round(b(r,16))),l=String(e||"bottom-right");return{left:l.includes("left")?c:Math.max(c,o-a-c),top:l.includes("top")?c:Math.max(c,s-i-c),w:a,h:i}}function El(e,t,n=16){const r=Math.max(1,Math.round(b(t?.w,1))),a=Math.max(1,Math.round(b(t?.h,1))),i=Math.max(0,Math.round(b(n,16))),o=String(e||"bottom-right");return{w:r,h:a,top:o.includes("top")?i:null,bottom:o.includes("bottom")?i:null,left:o.includes("left")?i:null,right:o.includes("right")?i:null}}function zg(e,t=28,n=6){const r=Math.round(b(e?.left,0)),a=Math.round(b(e?.top,0)),i=Math.max(1,Math.round(b(e?.w,1))),o=Math.max(1,Math.round(b(t,28))),s=Math.max(0,Math.round(b(n,6)));return{left:Math.round(r+(i-o)/2),top:a-s}}function Ug(e,t,n=28,r=6,a=16){const i=El(e,t,a),o=Math.max(1,Math.round(b(n,28))),s=Math.max(0,Math.round(b(r,6))),c=i.left!=null?i.left:i.right,l=Math.round(b(c,0)+(i.w-o)/2);return{size:o,top:i.top!=null?Math.round(i.top-s):null,bottom:i.bottom!=null?Math.round(i.bottom+i.h+s):null,left:i.left!=null?l:null,right:i.right!=null?l:null}}var ne=Object.freeze({width:64,height:88,gap:8,splitWidth:16,splitExtraMargin:4});function zr(e,t){const n=Math.max(0,Math.floor(b(t,0))),r=Math.max(0,Math.min(Math.floor(b(e,0)),n));return r>0&&r<n?r:0}function Jg(e,t,n){const r=Math.max(0,Math.floor(b(n,0))),a=Math.floor(b(e,-1));if(a<0||r<=0)return-1;const i=zr(t,r);if(i<=0)return a<r?a:-1;if(a===i)return-1;const o=a>i?a-1:a;return o>=0&&o<r?o:-1}function Hg(e,{count:t=0,selectedCount:n=0,thumbWidth:r=ne.width,gap:a=ne.gap,splitWidth:i=ne.splitWidth,splitExtraMargin:o=ne.splitExtraMargin}={}){const s=Math.max(0,Math.floor(b(t,0)));if(s<=0)return-1;const c=b(e,NaN);if(!Number.isFinite(c)||c<0)return-1;const l=Math.max(1,b(r,ne.width)),f=Math.max(0,b(a,ne.gap)),d=Math.max(0,b(i,ne.splitWidth)),m=Math.max(0,b(o,ne.splitExtraMargin)),p=zr(n,s),g=[];for(let _=0;_<s;_+=1)p>0&&_===p&&g.push({type:"split"}),g.push({type:"thumb",galIdx:_,marginLeft:p>0&&_===p?m:0});let h=0;for(let _=0;_<g.length;_+=1){_>0&&(h+=f);const y=g[_];if(y.type==="split"){if(c>=h&&c<h+d)return-1;h+=d;continue}if(h+=y.marginLeft,c>=h&&c<h+l)return y.galIdx;h+=l}return-1}function Kg({count:e=0,selectedCount:t=0,thumbWidth:n=ne.width,gap:r=ne.gap,splitWidth:a=ne.splitWidth,splitExtraMargin:i=ne.splitExtraMargin}={}){const o=Math.max(0,Math.floor(b(e,0)));if(o<=0)return 0;const s=Math.max(1,b(n,ne.width)),c=Math.max(0,b(r,ne.gap)),l=Math.max(0,b(a,ne.splitWidth)),f=Math.max(0,b(i,ne.splitExtraMargin)),d=zr(t,o),m=o+(d>0?1:0);let p=o*s+Math.max(0,m-1)*c;return d>0&&(p+=l+f),p}function Gg(e,t,n){const r=Math.max(0,b(t,0)-Math.max(0,b(n,0))),a=b(e,0);return Math.max(0,Math.min(r,a))}function qg(e,t=0,n=1,r=8,a=0){const i=Array.isArray(e)?e:[];if(!i.length)return[];const o=Math.max(0,Math.min(b(t,0),i.length-1)),s=Math.max(1,Math.min(i.length,b(r,8)||8)),c=Math.min(s,1+2*Math.max(0,b(n,1))),l=Math.max(c,s);let f=Math.max(0,o-Math.floor((l-1)/2));const d=Math.min(i.length-1,f+l-1);f=Math.max(0,d-l+1);const m=[],p=new Set,g=_=>{const y=String(i[_]?.id||"");!y||p.has(y)||(p.add(y),m.push(y))},h=Math.max(0,Math.min(i.length,b(a,0)));for(let _=0;_<h;_+=1)g(_);for(let _=f;_<=d&&(g(_),!(m.length>=s));_+=1);return m}function Wg(e,t,n=2,r=[]){const a=Array.isArray(e)?e:[],i=Math.max(0,Math.min(8,b(n,2))),o=b(t?.messageIndex??t?.message_index,NaN),s=String(t?.sessionId||t?.session_id||""),c=String(t?.chatId||t?.chat_id||""),l=String(t?.hash||""),f=new Set,d=[],m=p=>{const g=String(p||"");!g||f.has(g)||(f.add(g),d.push(g))};for(const p of r||[])m(p);for(const p of a){if(!p?.id||s&&p.session_id&&String(p.session_id)!==s||c&&p.chat_id&&String(p.chat_id)!==c)continue;const g=b(p.message_index,NaN);if(Number.isFinite(o)&&Number.isFinite(g)){Math.abs(g-o)<=i&&m(p.id);continue}l&&String(p.content_hash||"")===l&&m(p.id)}return d}function Vg(e,t,n=2){const r=b(e,NaN),a=b(t,NaN);if(!Number.isFinite(r)||!Number.isFinite(a))return!1;const i=Math.max(0,b(n,2));return Math.abs(r-a)<=i}function Yg(e,t,n=2){const r=Math.max(0,Math.floor(b(t,0))),a=Math.max(0,Math.min(Math.max(0,r-1),Math.floor(b(e,0)))),i=Math.max(0,Math.floor(b(n,2)));return r<=0?{lo:0,hi:-1,center:0}:{lo:Math.max(0,a-i),hi:Math.min(r-1,a+i),center:a}}var Cl={chrome:1,content:2,full:3};function Xg(e,t){const n=Cl[String(e??"")]||0,r=Cl[String(t??"")]||0;return!n&&!r?"full":r>=n?t||e||"full":e||t||"full"}function $l(e){const t=String(e||"").trim();if(!t)return{appearance:"",attire:"",accessories:"",text:"",gender:""};let n=null;const r=t.match(/```(?:json)?\s*([\s\S]*?)```/i),a=(r?r[1]:t).trim().match(/\{[\s\S]*\}/);if(a)try{const o=JSON.parse(a[0]);n=o&&typeof o=="object"?o:null}catch{n=null}if(n&&typeof n=="object"){const o=String(n.appearance??n.look??n.identity??"").trim(),s=String(n.attire??n.clothing??n.outfit??"").trim(),c=String(n.accessories??n.accessory??n.props??"").trim(),l=String(n.gender??n.sex??"").trim().toLowerCase(),f=["girl","female","f","woman"].includes(l)?"girl":["boy","male","m","man"].includes(l)?"boy":l==="other"?"other":"";return{appearance:o,attire:s,accessories:c,text:[o,s,c].filter(Boolean).join(", "),gender:f}}const i=t.replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/i,"").trim();return{appearance:i,attire:"",accessories:"",text:i,gender:""}}function Zg(e={}){const t=!!e.warmBusy,n=Math.max(0,Math.min(100,Math.round(b(e.warmPct,0)))),r=String(e.jobState||""),a=Math.max(0,Math.min(100,Math.round(b(e.jobPct,0))));return t?{pct:Math.max(t?6:0,n),busy:!0,label:"인덱싱"}:r==="tagging"?{pct:Math.max(6,a),busy:!0,label:"인덱싱"}:{pct:0,busy:!1,label:"인덱싱"}}function Qg(e={}){const t=!!e.jobBusy,n=!!e.indexBusy,r=Math.max(t?6:0,Math.min(100,Math.round(b(e.jobPct,0)))),a=Math.max(n?6:0,Math.min(100,Math.round(b(e.indexPct,0)))),i=e.error?"#f87171":"#7c6cff",o="#2dd4bf",s=(c,l)=>`<span style="display:block;height:4px;border-radius:2px;background:#1e2633;overflow:hidden"><span style="display:block;height:100%;width:${c}%;background:${l}"></span></span>`;return`<span style="flex:0 0 120px;display:flex;flex-direction:column;gap:3px;justify-content:center">${s(r,i)}${s(a,o)}</span>`}function Tl(e={}){const t=!!e.busy,n=Math.max(t?6:0,Math.min(100,Math.round(b(e.pct,0)))),r=String(e.tone||"");return`<span style="flex:0 0 120px;display:block;height:6px;border-radius:3px;background:#1e2633;overflow:hidden"><span style="display:block;height:100%;width:${n}%;background:${e.error?"#f87171":r==="index"?"#2dd4bf":"#7c6cff"}"></span></span>`}function eh(e={}){const t=!!e.jobBusy,n=!!e.indexBusy,r=!!e.busy||t||n;if(!r&&!e.error)return"";const a=typeof e.escapeHtml=="function"?e.escapeHtml:f=>String(f).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),i=String(e.tone||(!t&&n?"index":"job")),o=a(String(e.stage||(i==="index"?"인덱싱":"작업 중")).slice(0,80)),s=a(String(e.meta||"").slice(0,160));let c=e.pct;c==null&&(t?c=e.jobPct:n?c=e.indexPct:c=Math.max(b(e.jobPct,0),b(e.indexPct,0)));const l=Tl({pct:c,busy:r||!!e.error,error:e.error,tone:i});return`<div data-inlay-progress-toast="1" style="display:flex;align-items:center;gap:10px;min-width:220px;max-width:min(420px,92vw);padding:10px 14px;border-radius:8px;background:#121820;border:1px solid #2a3344;cursor:pointer;user-select:none"><span style="flex:0 0 auto;font-weight:700;color:${e.error?"#f87171":i==="index"?"#5eead4":"#c4b5fd"};font-size:11px;white-space:nowrap">${o}</span><span style="min-width:0;flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#8b97ab;font-size:11px">${s}</span>${l}</div>`}function Ur(e){return Math.max(0,Math.min(100,b(e,0)))}function vi(e,t){const n=Math.max(1,b(t,1));return Math.max(0,Math.min(b(e,0),n-1))/n*100}function th(e,t=0,n=1,r=null){if(r&&r.forceEven)return vi(t,n);const a=e?.y_percent??e?.anchor_percent??e?.read_percent,i=Number(a);return Number.isFinite(i)?Ur(i):vi(t,n)}function jl(e,t,n=.5){if(!e)return null;const r=Math.max(1,b(e.height,0)),a=Math.max(0,b(t,0))*(Number.isFinite(n)?n:.5);return a<b(e.top,0)||a>b(e.bottom,0)?null:Ur((a-b(e.top,0))/r*100)}function nh(e,t,n=.5){const r=jl(e,t,n);if(r!=null)return r;const a=Math.max(0,b(t,0))*(Number.isFinite(n)?n:.5);return b(e?.bottom,0)<a?100:0}function rh(e,t){const n=Array.isArray(e)?e:[];if(!n.length)return-1;const r=Ur(t),a=n.map(c=>Ur(c)),i=a.reduce((c,l,f)=>l<=20?c.concat(f):c,[]),o=i.length===1&&a[i[0]]>0?a.map((c,l)=>l===i[0]?1:c):a;if(o[0]>0&&r<o[0])return-1;let s=0;for(let c=0;c<o.length;c+=1)r>=o[c]&&(s=c);return s}var ah="data-inlay-inline-shot",Pl=/^<\/(?:p|div|li|blockquote|h[1-6]|tr)>$/i,Ol=/^<br\s*\/?>$/i;function Ll(e){const t=String(e||"");return t?t.replace(/<div[^>]*\bdata-inlay-inline-shot\b[^>]*>[\s\S]*?<\/div>/gi,"").replace(/<(?:span|p)[^>]*\bdata-inlay-inline-shot\b[^>]*>[\s\S]*?<\/(?:span|p)>/gi,""):""}function Jr(e){return String(e??"").replace(/\r\n/g,`
`).split(`
`).map(t=>t.trim()).filter(Boolean)}function Bl(e,t){const n=Math.floor(Number(e)),r=Math.max(0,Math.floor(Number(t)||0));return!r||!Number.isFinite(n)||n<1?null:Math.max(1,Math.min(r,n))}function Rl(e){return String(e||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function ih(e){const t=e.toLowerCase();if(t==="&nbsp;")return" ";if(t==="&amp;")return"&";if(t==="&lt;")return"<";if(t==="&gt;")return">";if(t==="&quot;")return'"';if(t==="&#39;"||t==="&apos;")return"'";const n=/^&#(\d+);$/.exec(e);if(n){const a=Number(n[1]);if(Number.isFinite(a)&&a>0&&a<1114112)return String.fromCodePoint(a)}const r=/^&#x([0-9a-f]+);$/i.exec(e);if(r){const a=Number.parseInt(r[1],16);if(Number.isFinite(a)&&a>0&&a<1114112)return String.fromCodePoint(a)}return null}function oh(e,t){let n=Math.max(0,Math.min(e.length,Math.floor(t)));for(;n>0;){let r=n;for(;r>0&&/[ \t\r\n]/.test(e[r-1]);)r-=1;if(r<=0||e[r-1]!==">")break;const a=e.lastIndexOf("<",r-1);if(a<0||a>=r-1)break;const i=e.slice(a,r);if(Ol.test(i)||Pl.test(i)||/^<\//.test(i)||!/^<[a-zA-Z][^>]*>$/.test(i))break;n=a}return n}function Fl(e){const t=[];let n=0;const r=String(e||"");for(;n<r.length;){if(r[n]==="<"){const s=r.indexOf(">",n);if(s<0)break;const c=r.slice(n,s+1);(Ol.test(c)||Pl.test(c))&&t.push({ch:`
`,htmlIndex:n}),n=s+1;continue}if(r[n]==="&"){const s=r.indexOf(";",n);if(s>n&&s-n<16){const c=ih(r.slice(n,s+1));if(c!=null){for(let l=0;l<c.length;l+=1)t.push({ch:c[l],htmlIndex:n});n=s+1;continue}}}t.push({ch:r[n],htmlIndex:n}),n+=1}const a=[];for(let s=0;s<t.length;s+=1){const c=t[s];if(c.ch!==`
...
`||o[o.length-1].ch===" ");)o.pop();return o}function sh(e){return Fl(String(e||"")).map(t=>t.ch).join("")}function Dl(e,t){const n=String(e??"").replace(/\r\n/g,`
`),r=Math.floor(Number(t));if(!Number.isFinite(r)||r<1)return null;let a=0,i=0;for(;i<=n.length;){const o=n.indexOf(`
`,i),s=o<0?n.length:o,c=n.slice(i,s);if(c.trim()&&(a+=1,a===r)){const l=/^\s*/.exec(c)?.[0].length||0;return i+l}if(o<0)break;i=o+1}return null}function Hr(e){return String(e??"").replace(/\s+/g," ").trim()}function zl(e,t){const n=Array.isArray(e)?e:[],r=Math.floor(Number(t))-1;if(!Number.isFinite(r)||r<0||r>=n.length)return null;const a=Hr(n[r]);if(!a)return null;let i=0;for(let o=0;o<=r;o+=1)Hr(n[o])===a&&(i+=1);return{text:a,occurrence:i}}function Ul(e){const t=[],n=Array.isArray(e)?e:[];for(let r=0;r<n.length;r+=1)for(const a of Jr(n[r])){const i=Hr(a);i&&t.push({text:i,elementIndex:r})}return t}function Jl(e,t,n){const r=zl(t,n);if(!r)return-1;const a=Ul(e);let i=0;for(const o of a)if(o.text===r.text&&(i+=1,i===r.occurrence))return o.elementIndex;return-1}function Hl(e,t,n=["P"]){const r=(Array.isArray(e)?e:[]).map(o=>String(o||"").toUpperCase()),a=Math.floor(Number(t));if(!Number.isFinite(a)||a<0||a>=r.length)return-1;const i=new Set((Array.isArray(n)&&n.length?n:["P"]).map(o=>String(o||"").toUpperCase()));if((r[a]||"")!=="DIV")return a;for(let o=1;o<r.length;o+=1){const s=a-o,c=a+o;if(s>=0&&i.has(r[s]))return s;if(c<r.length&&i.has(r[c]))return c}return-1}function ch(e,t,n,r,a=["P"]){const i=Array.isArray(n)?n:[],o=Math.floor(Number(r));if(!Number.isFinite(o)||o<1||!i.length)return null;const s=i.length;for(let c=Math.max(1,o);c<=s;c+=1){const l=Hl(t,Jl(e,i,c),a);if(l>=0)return{elementIndex:l,usedLine:c}}return null}function Kl(e,t){const n=Number.isFinite(Number(e.shotIndex))?String(Math.floor(Number(e.shotIndex))):"",r=Rl(String(e.cardId||n||e.line||"0"));return`<div ${ah}="${r}" x-inlay-inline-shot="${r}" contenteditable="false" style="display:block;margin:10px 0;text-align:center;line-height:0"><br><img src="${Rl(e.src)}" alt="" style="zoom:0.5;width:auto;height:auto;max-width:none;border-radius:8px;display:inline-block;vertical-align:top"><br></div>`}function Gl(e,t,n){const r=Ll(e),a=Array.isArray(t)?t:[];if(!r||!a.length)return r;const i=Fl(r),o=i.map(g=>g.ch).join(""),s=Jr(o).length;if(!s||!i.length)return r;const c=new Map,l=new Set;for(const g of a){const h=Bl(g?.line,s),_=String(g?.src||""),y=String(g?.cardId||"");!h||!/^data:image\//i.test(_)||y&&l.has(y)||c.has(h)||(y&&l.add(y),c.set(h,{...g,line:h,src:_}))}if(!c.size)return r;const f=n?.maxWidthPx,d=new Map;for(const[g,h]of c){const _=Dl(o,g);if(_==null||_<0||_>=i.length)continue;const y=oh(r,i[_].htmlIndex);d.has(y)||d.set(y,Kl(h,f))}if(!d.size)return r;const m=[...d.entries()].map(([g,h])=>({htmlIndex:g,html:h})).sort((g,h)=>h.htmlIndex-g.htmlIndex);let p=r;for(const g of m)p=p.slice(0,g.htmlIndex)+g.html+p.slice(g.htmlIndex);return p}function lh(e,t,n){const r=Jr(e);return r.length?Gl(r.map(a=>String(a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")).join("<br>"),t,n):""}te(),W(),mt(),ee(),vt(),F(),Da(),ai(),Nr(),ti(),_e(),ut();function xi(){const e=ge(),t=e.events;return{last_stage:e.last_stage,events:Array.isArray(t)?t.slice(-20):[]}}async function uh(e){if(e){const t=C(),n={...e};u(n.api_key)&&(t.llm.api_key=u(n.api_key)),delete n.api_key,delete n.api_key_configured,u(n.service_account_json)&&(t.llm.service_account_json=u(n.service_account_json)),delete n.service_account_json,delete n.service_account_configured,Object.keys(n).length&&(t.llm=tt(t.llm,n)),await Je()}try{const t=C().llm,n=Ir(t.source),r=Ot(t.provider);return n==="custom"&&!Tc(t)?{ok:!1,message:r==="vertex"?"Vertex AI: Model + Service Account JSON(또는 access token)이 필요합니다.":"태깅 LLM Model/API key가 비어 있습니다. NovelAI 키가 아니라 태깅용 LLM 키를 넣으세요."}:(n==="main"||n==="aux")&&!zt("runLLMModel")?{ok:!1,message:"RisuAI runLLMModel API를 사용할 수 없습니다."}:{ok:!0,message:`LLM ok (${n}): ${(await on(t,[{role:"user",content:"Reply with exactly: ok"}])).slice(0,120)}`}}catch(t){return{ok:!1,message:String(t?.message||t)}}}async function fh(){const e=C().nai;if(zn(e)==="comfy")try{if(!Ra(e))return{ok:!1,message:"ComfyUI 워크플로 JSON이 없습니다.",debug:ge()};const t=Vs({main:"test",neg:"test",captions:[{name:"",prompt:"char1"},{name:"",prompt:"char2"}],nai:e,seed:1}),n=Ys(e.comfy_workflow_json,t),r=Ws(e),{status:a,data:i}=await Fa(`${r}/system_stats`,{method:"GET"});if(a>=400)return{ok:!1,message:`ComfyUI 연결 실패 (HTTP ${a}) · ${r}`,debug:ge()};const o=i?.devices?.[0]?.name||i?.system?.os||"ok";return{ok:!0,message:`ComfyUI ok · ${r} · nodes=${Object.keys(n).length} · ${o}`,debug:xi()}}catch(t){return w("comfy.test",{message:String(t?.message||t)},"error"),{ok:!1,message:String(t?.message||t),debug:ge()}}if(!u(e.api_key))return{ok:!1,message:"NAI api_key missing",debug:ge()};try{const t=u(e.api_key);try{const n=ue("nai.test.anlas"),r=await jd(t);return n.end({message:"anlas ok"}),{ok:!0,message:`NAI token ok · Anlas=${JSON.stringify(r)}`,debug:xi()}}catch(n){const r=Ue(e.model||"nai-diffusion-4-5-full");return w("nai.test.anlas",{message:String(n?.message||n)},"warn"),{ok:!0,message:`NAI config present · model=${r} · anlas_skip=${n?.message||n}`,debug:xi()}}}catch(t){return w("nai.test",{message:String(t?.message||t)},"error"),{ok:!1,message:String(t?.message||t),debug:ge()}}}async function dh(){const e=C().nai,t=u(e.api_key);if(!t)return{ok:!1,message:"NAI api_key missing",debug:ge()};const n=String(ge().job_ctx??"");xn("probe-nai");const r=ue("nai.probe");try{w("nai.probe.start",{message:"tiny generate"});const a={prompt:"1girl, solo, simple background, best quality",negative_prompt:"lowres, bad quality",width:512,height:768,steps:10,cfg_scale:5,cfg_rescale:0,sampler:"k_euler_ancestral",scheduler:"karras",model:Ue(e.model||"nai-diffusion-4-5-full"),var_plus:!1,characters:[],seed:Math.floor(Math.random()*4294967295)||1},i=u(e.request_url)||"https://image.novelai.net/ai/generate-image",o=await Zs(t,a,i,{timeoutMs:9e4}),s=o.raw_bytes.byteLength,c=yt(Z(o.raw_bytes));return r.end({bytes:s,is_png:c,seed:o.seed}),{ok:!0,message:`probe ok · png=${c} · ${s}B · seed=${o.seed}`,bytes:s,is_png:c,seed:o.seed,debug:ge()}}catch(a){return r.fail(a),{ok:!1,message:String(a?.message||a),debug:ge()}}finally{xn(n)}}async function ph(e,t){if(!e?.byteLength)throw new Error("image is empty");const n=await Wu(e),r=n.bytes,a=n.mime||"image/png",i=n.filename||"image.png",o=`data:${a};base64,${await Ze(r)}`,s=sr(await se("autotag"))||["Tag ONE character reference image into Danbooru-style English prompts.",'Return ONE JSON object only: {"gender":"girl|boy","appearance":"...","attire":"...","accessories":"..."}.',"gender must be exactly girl or boy from visual evidence. appearance = identity/hair/eyes/body (no clothes). attire = clothing + permanent jewelry (earrings/glasses/watch/earbuds). accessories = weapons/bags/held props/ID only (not jewelry)."].join(`
`);w("autotag.start",{message:`llm-vision ${i} ${r.length}B`,bytes:r.length,focus:!0,source:Ir(C().llm.source)});const c=[{role:"system",content:s},{role:"user",content:[{type:"text",text:"Tag this character image. JSON only with gender, appearance, attire, accessories."},{type:"image_url",image_url:{url:o}}]}];let l="";try{l=await on(C().llm,c)}catch(x){throw w("autotag.llm.fail",{message:String(x?.message||x)},"error"),new Error(`오토태그 LLM 실패: ${String(x?.message||x).slice(0,240)}`)}const f=$l(l),d=u(f.appearance||"",4e3),m=u(f.attire||"",4e3),p=u(f.accessories||"",4e3),g=u(f.gender||"",20),h=u(f.text||[d,m,p].filter(Boolean).join(", "),8e3);if(!d&&!m&&!p)throw new Error("LLM이 외형/의상/악세사리 태그를 반환하지 않았습니다. 비전(이미지) 지원 모델인지 확인하세요.");const _=h.split(",").map(x=>x.trim()).filter(Boolean);w("autotag.done",{message:`gender=${g||"-"} app=${d.length} attire=${m.length} acc=${p.length}`,focus:!0});const y={ok:!0,appearance:d,attire:m,accessories:p,tags:_,text:h,count:_.length,threshold:Number(t||.2),engine:"llm-vision"};return g&&(y.gender=g),y}async function mh(e){if(!e?.byteLength)throw new Error("image is empty");const t=await Es(e);if(!t)throw new Error("이미지에서 NovelAI 메타데이터를 읽지 못했습니다. PNG/WebP NAI 원본인지 확인하세요.");const n=ws(t);if(!u(n))throw new Error("메타데이터에 프롬프트가 없습니다.");const r=Jf(t,n);if(!r.positive&&!r.negative)throw new Error("프리셋에 넣을 artist/품질 태그와 네거티브가 비어 있습니다.");return w("preset-from-image.done",{message:`pos=${r.positive.length} neg=${r.negative.length} cfg=${r.cfg_scale}`,focus:!0}),{ok:!0,positive:r.positive,negative:r.negative,cfg_scale:r.cfg_scale,cfg_rescale:r.cfg_rescale,name:"이미지 프리셋"}}F();function gh(e){const t=e.indexOf("?");if(t<0)return{pathname:e,query:{}};const n=e.slice(0,t),r=new URLSearchParams(e.slice(t+1)),a={};for(const[i,o]of r.entries()){const s=a[i];s===void 0?a[i]=o:Array.isArray(s)?s.push(o):a[i]=[s,o]}return{pathname:n,query:a}}function Qn(e,t,n=""){const r=e[t];return Array.isArray(r)?r[0]??n:r??n}function hh(e,t={}){const n=u(e.auth_token,4e3);if(!n)return!0;const r=String(t.Authorization||t.authorization||"");let a=r.toLowerCase().startsWith("bearer ")?r.slice(7):r;if(a=a||String(t["X-Inlay-Nexus-Token"]||t["x-inlay-nexus-token"]||""),typeof a!="string"||a.length!==n.length)return!1;let i=0;for(let o=0;o<n.length;o+=1)i|=a.charCodeAt(o)^n.charCodeAt(o);return i===0}W(),ee(),F(),te(),$o(),_e(),ut(),Vn();var P=(...e)=>t=>e.includes(t)?"":null,Ge=e=>t=>t.startsWith(e)?t.slice(e.length):null,ki=(e,t)=>n=>n.startsWith(e)&&n.endsWith(t)?n.slice(e.length,n.length-t.length):null,A=e=>({status:200,data:e}),Si=e=>({status:200,data:e,contentType:"image/png",raw:!0}),er=e=>{throw ht(404,{ok:!1,...ae(e,"not_found")},e)};function Kr(e){let t=e.image_b64||e.data||"";return typeof t=="string"&&t.startsWith("data:")&&(t=t.split(",",2)[1]??""),String(t||"")}var _h=[{match:P("/v1/settings/export"),handler:()=>A({ok:!0,json:Yp()})},{match:P("/v1/settings"),handler:()=>A({ok:!0,settings:Tr()})},{match:P("/v1/curation/status"),handler:async()=>A({ok:!0,status:await Wn()})},{match:P("/v1/curation/catalog"),handler:async()=>A({ok:!0,catalog:await ft()})},{match:P("/v1/prompts"),handler:async()=>A({ok:!0,prompts:await $r()})},{match:P("/v1/prompts/export"),handler:async()=>A({ok:!0,...await Gp()})},{match:Ge("/v1/prompts/"),handler:async({param:e})=>A({ok:!0,key:e,text:await se(e)})},{match:Ge("/v1/jobs/"),handler:async({param:e})=>A(await eg(e))},{match:Ge("/v1/gallery/explore"),handler:async({query:e})=>A(await Em(Number(Qn(e,"limit","400"))))},{match:P("/v1/gallery/favorites"),handler:async()=>A(await ol())},{match:Ge("/v1/gallery"),handler:async({query:e})=>A(await Cm(Qn(e,"session_id"),Number(Qn(e,"limit","40"))))},{match:P("/v1/nai/reference.png"),handler:async()=>Si(await Yc()??er("no reference"))},{match:P("/v1/nai/vibe.png"),handler:async()=>Si(await dm()??er("no vibe"))},{match:P("/v1/nai/reference"),handler:async()=>{const e=await cm();return A({ok:!0,configured:e,image_reference:C().nai?.image_reference||"none",preview_url:e?"/v1/nai/reference.png":""})}},{match:P("/v1/nai/vibe"),handler:async()=>{const e=await um();return A({ok:!0,configured:e,vibe_transfer:C().nai?.vibe_transfer||"none",preview_url:e?"/v1/nai/vibe.png":""})}},{match:Ge("/v1/characters"),handler:async({query:e})=>A(await Fn(Qn(e,"session_id"),Qn(e,"character_id")))},{match:Ge("/v1/appearance/"),handler:async({param:e})=>A(await Fn(e))},{match:Ge("/v1/images/"),handler:async({param:e})=>{let t=e.replace(/^\/+|\/+$/g,"");if(t.endsWith(".json")){const n=await Lt(t.slice(0,-5));return Object.keys(n).length||er("location missing"),A(n)}return t.endsWith(".png")&&(t=t.slice(0,-4)),Si(await sl(t)??er("image missing"))}}],yh=[{match:P("/v1/settings/reset"),handler:async()=>A(await Zp())},{match:P("/v1/settings/import"),handler:async({body:e})=>A(await Xp(String(e.json||e.text||"")))},{match:P("/v1/settings","/v1/settings/update"),handler:async({body:e})=>A(await em(e))},{match:P("/v1/curation/catalog"),handler:async({body:e})=>{const t=e.catalog??e;return A(await Rc(t))}},{match:P("/v1/curation/catalog/reset"),handler:async()=>A(await Fc())},{match:P("/v1/curation/embed"),handler:async()=>A(await Dc())},{match:P("/v1/curation/embed/test"),handler:async()=>A(await zc())},{match:P("/v1/curation/settings"),handler:async({body:e})=>A(await Wc(e))},{match:P("/v1/prompts/import"),handler:async({body:e})=>A(await qp(e?.json!=null?e.json:(e?.prompts!=null,e)))},{match:P("/v1/prompts/reset-defaults"),handler:async({body:e})=>A(await Wp({keep_author_note:e?.keep_author_note!==!1&&e?.keepAuthorNote!==!1}))},{match:ki("/v1/prompts/","/reset"),handler:async({param:e})=>A(await Oc(e,In(e)))},{match:Ge("/v1/prompts/"),handler:async({param:e,body:t})=>A(await Oc(e,String(t.text||"")))},{match:P("/v1/jobs/create","/v1/jobs"),handler:async({body:e})=>({status:202,data:await hl(e)})},{match:P("/v1/gallery/unlink","/v1/cards/unlink"),handler:async({body:e})=>A(await fi(String(e.session_id||""),String(e.content_hash||""),e.message_index))},{match:P("/v1/gallery/rebind-hash","/v1/cards/rebind-hash"),handler:async({body:e})=>A(await $m({session_id:String(e.session_id||e.sessionId||""),card_ids:e.card_ids||e.ids||[],to_hash:String(e.to_hash||e.content_hash||""),assistant_preview:String(e.assistant_preview||e.assistant_text||"")}))},{match:P("/v1/gallery/delete","/v1/cards/delete"),handler:async({body:e})=>{const t=u(e.folder_key||"",400),n=u(e.card_id||e.id||"",80),r=Array.isArray(e.card_ids)?e.card_ids:null;if(t)return A(await Lm(t));if(r?.length)return A(await Tm(r));if(n)return A(await pi(n));const a="card_id or folder_key required";throw ht(400,{ok:!1,...ae(a,"bad_request")},a)}},{match:P("/v1/gallery/export"),handler:async({body:e})=>A(await Pm(e))},{match:P("/v1/gallery/import"),handler:async({body:e})=>A(await Om(e))},{match:P("/v1/gallery/favorites"),handler:async({body:e})=>Array.isArray(e.ids)?A(await jm(e.ids)):A(await ol())},{match:ki("/v1/cards/","/tags"),handler:async({param:e,body:t})=>A(await rg(e,t))},{match:ki("/v1/cards/","/reroll"),handler:async({param:e,body:t})=>A(await _l(e,String(t.mode||"nai"),t.overrides))},{match:P("/v1/messages/reroll","/v1/gallery/reroll-message"),handler:async({body:e})=>A(await ag({session_id:String(e.session_id||e.sessionId||""),content_hash:String(e.content_hash||e.contentHash||""),message_index:Number(e.message_index??e.messageIndex??-1)}))},{match:P("/v1/characters/global-toggles","/v1/characters/global_toggles"),handler:async({body:e})=>A(await ed(String(e.character_id||""),e.disabled_globals||e.disabled||[]))},{match:P("/v1/characters/unify","/v1/characters/merge"),handler:async({body:e})=>A(await Bs(String(e.target_session_id||e.session_id||""),e.source_session_ids||e.session_ids||[],e.include_target!==!1))},{match:P("/v1/characters","/v1/characters/update"),handler:({body:e})=>bh(e)},{match:Ge("/v1/appearance/"),handler:async({param:e,body:t})=>t.characters!=null||t.global!=null?(t.characters!=null&&await Rn(e,t.characters||[],{prune:!0}),t.global!=null&&await Rn(Ve,t.global||[],{prune:!0}),A(await Fn(e))):A(await sd(e,t.appearance||{}))},{match:P("/v1/models/test"),handler:async({body:e})=>A(await uh(e.llm&&typeof e.llm=="object"?e.llm:null))},{match:P("/v1/nai/test"),handler:async()=>A(await fh())},{match:P("/v1/nai/probe","/v1/debug/probe-nai"),handler:async()=>A(await dh())},{match:P("/v1/debug/clear"),handler:()=>(Vr(),A({ok:!0,cleared:!0}))},{match:P("/v1/nai/reference","/v1/nai/reference/upload"),handler:async({body:e})=>{if(e.clear)return A(await Xc());const t=Kr(e);if(!u(t))throw new Error("image_b64 required");return A(await lm(Q($e(t))))}},{match:P("/v1/nai/reference/clear"),handler:async()=>A(await Xc())},{match:P("/v1/nai/vibe","/v1/nai/vibe/upload"),handler:async({body:e})=>{const t=u(e.preset_id||e.presetId||"",120);if(e.clear)return A(t?await el(t):await Zc());if(t&&e.copy_from){const a=u(e.copy_from,120),i=a?await hm(a,t):!1;return A({ok:!0,preset_id:t,configured:i,preview_url:i?ra(t):""})}const n=Kr(e);if(!u(n))throw new Error("image_b64 required");const r={model:e.model,information_extracted:e.information_extracted??e.vibe_transfer_information_extracted,strength:e.strength??e.vibe_transfer_strength};return A(t?await gm(t,Q($e(n)),r):await pm(Q($e(n)),r))}},{match:P("/v1/nai/vibe/clear"),handler:async({body:e})=>{const t=u(e?.preset_id||e?.presetId||"",120);return A(t?await el(t):await Zc())}},{match:P("/v1/autotag","/v1/autotag/evaluate"),handler:async({body:e})=>{const t=Kr(e).replace(/\s+/g,"");if(!t)throw new Error("image_b64 required");const n=$e(t);return A(await ph(Q(n),Number(e.threshold??.2)))}},{match:P("/v1/presets/from-image"),handler:async({body:e})=>{const t=Kr(e).replace(/\s+/g,"");if(!t)throw new Error("image_b64 required");const n=$e(t);return A(await mh(Q(n)))}}];async function bh(e){const t=u(e.session_id||"",200),n=u(e.character_id||"",200),r=(Array.isArray(e.root_session_ids)?e.root_session_ids:Array.isArray(e.cascade_session_ids)?e.cascade_session_ids:[]).map(i=>u(i,200)).filter(Boolean);if("characters"in e&&t&&await Rn(t,e.characters||[],{prune:!0,rootSessionIds:r}),"global"in e&&await Rn(Ve,e.global||[],{prune:!0}),"character"in e){const i=u(e.scope||t||"__global__",200);r.length&&i!=="__global__"?await Ls(r,[e.character||{}],""):await st(i,e.character||{})}const a=Array.isArray(e.root_delete)?e.root_delete:Array.isArray(e.cascade_delete)?e.cascade_delete:[];return a.length&&r.length&&await id(r,a,""),r.length&&t&&await Bs(t,r,!1),A(await Fn(t,n))}async function ql(e,t){for(const n of e){const r=n.match(t.pathname);if(r!==null)return n.handler({...t,param:r})}return er(t.pathname)}async function wh(e,t={}){const{pathname:n,query:r}=gh(e),a=String(t.method||"GET").toUpperCase();let i;if(typeof t.body=="string")try{i=JSON.parse(t.body)}catch{i={}}else i=t.body??{};if(["/v1/health","/healthz","/readyz"].includes(n))return A({ok:!0,health:Qp()});if(n==="/v1/debug"||n==="/v1/debug/log")return a==="DELETE"||a==="POST"&&i.clear?(Vr(),A({ok:!0,cleared:!0})):A(ge());if(!hh(C(),t.headers||{})){const s="invalid token";throw ht(401,{ok:!1,...ae(s,"unauthorized")},s)}const o={method:a,pathname:n,query:r,body:i};if(a==="GET")return ql(_h,o);if(["POST","PUT","PATCH"].includes(a))try{return await ql(yh,o)}catch(s){if(Ui(s))throw s;const c=String(s?.message||s);throw ht(500,{ok:!1,...ae(c,"internal")},c)}throw ht(405,{ok:!1,...ae(a,"method_not_allowed")},a)}te(),W(),mt(),_r(),Pc(),je(),_e(),ut();var Gr=null;async function vh(){w("boot.ready.start",{message:We}),await it();const e=await hr().catch(r=>{throw w("boot.storage",{message:String(r?.message||r)},"error"),r});w("boot.storage",{message:e.kind}),cr(await Hp()),await Kp(),await ld(),await ud();const t=await z("meta","reference_image");t?.png&&ta(Et(t.png));const n=await z("meta","vibe_transfer");n?.png&&na(Et(n.png)),await ym(),w("boot.ready.done",{message:We,has_nativeFetch:zt("nativeFetch"),has_idb:zt("getLocalPluginStorage")})}async function Ai(){return Gr||(Gr=vh().catch(e=>{throw Gr=null,e})),await Gr,!0}async function xh(e,t={},n=12e4){await Ai();try{const r=await wh(e,t);if(r.raw)return r.data;if(r.status>=400)throw ht(r.status,r.data);return r.data}catch(r){if(Ui(r))throw r;const a=String(r?.message||r);throw ht(500,{ok:!1,...ae(a,"internal")},a)}}async function kh(e){return await Ai(),Zt(e)}function Sh(){Ri(()=>({cards:jn("cards"),images:jn("images"),jobs:jn("jobs"),blob_urls:df()})),Reflect.set(globalThis,"__INLAY_NATIVE__",{VERSION:We,ready:Ai,fetch:xh,resolveImageUrl:Pe,refPreviewUrl:To,vibePreviewUrl:jo,ensureImageUrl:kh,warmImages:yf,pinImageUrls:_f,warmProgress:mf,onWarmProgress:gf,debug:ge,clearDebug:Vr})}var Ah=fn({applyExplorerClick:()=>Ih,clearSelection:()=>Ch,createSelectionState:()=>Mh,moveFocus:()=>$h,reorderByIds:()=>Ph,selectAll:()=>Eh,sortExplorerItems:()=>Th,thumbMinWidth:()=>jh,visibleIds:()=>Nh});function Mh(e=[]){return{selected:new Set((e||[]).map(String).filter(Boolean)),anchorId:"",focusId:""}}function Nh(e=[]){return(e||[]).map(t=>String(t?.id||"")).filter(Boolean)}function Ih(e,t,{index:n,ids:r,shift:a=!1,ctrl:i=!1}={}){const o={selected:new Set(e?.selected||[]),anchorId:String(e?.anchorId||""),focusId:String(e?.focusId||"")},s=String(t||"");if(!s)return o;const c=Array.isArray(r)?r.map(String):[],l=typeof n=="number"&&Number.isFinite(n)?n:c.indexOf(s);if(a&&o.anchorId&&c.length){const f=c.indexOf(o.anchorId),d=l>=0?l:c.indexOf(s);if(f>=0&&d>=0){const m=Math.min(f,d),p=Math.max(f,d);return o.selected=new Set(c.slice(m,p+1)),o.focusId=s,o}}return i?(o.selected.has(s)?o.selected.delete(s):o.selected.add(s),o.anchorId=s,o.focusId=s,o):(o.selected=new Set([s]),o.anchorId=s,o.focusId=s,o)}function Eh(e,t=[]){return{selected:new Set((t||[]).map(String).filter(Boolean)),anchorId:String(e?.anchorId||t?.[0]||""),focusId:String(e?.focusId||t?.[0]||"")}}function Ch(e=null){return{selected:new Set,anchorId:String(e?.anchorId||""),focusId:""}}function $h(e=[],t="",n=1){const r=(e||[]).map(String).filter(Boolean);if(!r.length)return"";const a=r.indexOf(String(t||""));return a<0?r[0]:r[Math.max(0,Math.min(r.length-1,a+n))]}function Th(e=[],t="newest"){const n=[...e||[]],r=(a,i=0)=>{const o=Number(a);return Number.isFinite(o)?o:i};return t==="oldest"?n.sort((a,i)=>r(a.created_at)-r(i.created_at)||String(a.id).localeCompare(String(i.id))):t==="message"?n.sort((a,i)=>r(a.message_index,1e9)-r(i.message_index,1e9)||r(a.shot_index)-r(i.shot_index)||r(i.created_at)-r(a.created_at)):t==="shot"?n.sort((a,i)=>r(a.shot_index)-r(i.shot_index)||r(a.message_index,1e9)-r(i.message_index,1e9)||r(i.created_at)-r(a.created_at)):n.sort((a,i)=>r(i.created_at)-r(a.created_at)||String(i.id).localeCompare(String(a.id)))}function jh(e="m"){return e==="s"?96:e==="l"?200:148}function Ph(e=[],t=[]){const n=i=>{const o=i&&typeof i=="object"?i.id:void 0;return String(o??i)},r=new Map((e||[]).map(i=>[n(i),i])),a=[];for(const i of t||[]){const o=String(i);r.has(o)&&(a.push(r.get(o)),r.delete(o))}for(const i of r.values())a.push(i);return a}Nr(),Ac();function Oh(){Reflect.set(globalThis,"__INLAY_VIEWER_CORE__",{...ig}),Reflect.set(globalThis,"__INLAY_LLM__",{...Op}),Reflect.set(globalThis,"__INLAY_EMBED__",{EMBEDDING_PROVIDERS:Qa,normalizeEmbeddingProvider:Ne,defaultEndpointForEmbedding:Ar,defaultModelForEmbedding:Kn,embeddingModelPlaceholder:Np,shouldAutoReplaceEmbeddingEndpoint:Ep,shouldAutoReplaceEmbeddingModel:Cp,embeddingProviderNeedsApiKey:$p}),Reflect.set(globalThis,"__INLAY_LORE_EXTRA__",{...Fm}),Reflect.set(globalThis,"__INLAY_EXPLORER__",{...Ah})}te(),W(),Oh(),Sh(),w("boot.loaded",{message:We})})();
