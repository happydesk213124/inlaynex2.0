...
//@display-name Inlay Nexus 2.1.5
//@api 3.0
//@version 2.1.5
...
`)}function Ec(){return["## Per-actor ids (strict catalog-id mode)",'Add `"characters": [ { "index": 0, "option_ids": ["id", "..."] }, ... ]` to EVERY shot.','Each shot input includes `cast`: `[{ "index": 0, "name": "...", "gender": "girl|boy|""" }, ...]`.',"`index` MUST match `cast[].index` / Pass-1 `characters` array order — use name+gender from `cast`, not chat mention order.","Example: if cast says index 0 = 보민 (boy) and index 1 = Serin (girl), put bound/tears on the boy's index, not the girl's.","Put an option on `characters[].option_ids` when it targets ONE specific actor (pose, contact, expression, gaze).","Put an option on the shot-level `curation_option_ids` instead when it is scene-wide (camera, place, framing) — do not duplicate it into every actor.","Every actor in `cast` needs an entry, even if `option_ids` is `[]`.","This mode has NO freeform camera/situation/action fallback — an unlisted concept is simply omitted, never paraphrased."]}function wm(e,t,n){const r=n?.strictIds===!0,a=new Set(t.map(s=>u(s,160)).filter(Boolean)),i=Ic(e).filter(s=>a.has(s.id)),o=(i.length?i:[]).map(s=>{const c=s.options.map(l=>`  - ${l.id}: ${l.description}`).join(`
`);return`### ${s.id} (${s.label})
${c}`});return["Curation two-stage ON (pass 2 — scene refine, BATCH).","You receive ALL shots in one request. Return ONE JSON object covering every shot.",r?'Schema: { "shots": [ { "shot_index": 0, "curation_option_ids": ["id", "..."], "characters": [ { "index": 0, "option_ids": ["id", "..."] } ] }, ... ] }':'Schema: { "shots": [ { "shot_index": 0, "curation_option_ids": ["id", "..."], "camera": "...", "situation": "...", "place": "...", "action": "..." }, ... ] }',r?'Example: { "shots": [ { "shot_index": 0, "curation_option_ids": ["bedroom"], "characters": [ { "index": 0, "option_ids": ["cowboy_shot", "smile"] }, { "index": 1, "option_ids": ["facing_each_other"] } ] } ] }':'Example: { "shots": [ { "shot_index": 0, "curation_option_ids": ["cowboy_shot", "facing_each_other", "smile", "bedroom"], "place": "bedroom" } ] }',"shot_index must match the input shots array (0-based). Return exactly one entry per input shot, same order.","","## CRITICAL: fill curation_option_ids","`curation_option_ids` is the MAIN output — host injects catalog tags from those ids.","For EVERY shot pick ALL listed options that match the chat at that y_percent (pose, contact, expression, place…).","Typical **2–8 ids**; `[]` ONLY if nothing fits. Do not skip obvious matches.",r?"Do NOT invent character appearance/attire. Do NOT invent option ids. Do NOT write freeform camera/situation/action text — ids only.":"Prefer ids over freeform camera/situation/action paraphrase.",...r?[]:["Do NOT invent character appearance/attire. Do NOT invent option ids."],...Tc(),...r?["",...Ec()]:[],"",o.length?"## Allowed options (union of all shots' groups)":`## Allowed options
(none — leave scene fields empty)`,...o].join(`
`)}function Tc(){return["## Chat context (separate user message, READ-ONLY)","A prior user message is the exact chat text from pass 1 (for LLM prompt cache).","Use it for continuity only (who the cast is, ongoing scene).","Do NOT rewrite, quote-edit, summarize into the JSON, or invent missing plot beats.","Do NOT change shot count, shot_index, composition_id, composition_variant, or y_percent.","","## Shot focus bands (PRIMARY evidence)","Each shot includes `y_percent`, `focus: { from_percent, to_percent }`, and `focus_hint` (a short quote from that band of the chat).","Pick options primarily from `focus_hint` / the `focus` percent range for THAT shot.","Do NOT pull actions from far-away bands into this shot. Full chat is background, not a license to copy the same ids onto every shot.","When focuses differ, prefer distinct `curation_option_ids` / per-actor ids across shots — avoid pasting one identical set on every shot_index.","Do NOT invent aftermath tags (`after_sex`, `afterglow`, cum_on_*, etc.) unless `focus_hint` explicitly shows that aftermath.","y_percent / focus are already final — never output new y_percent or focus fields."]}function vm(){return["Curation embed-snap mode: write DETAILED English Danbooru scene tags.","Fill camera, situation, place, action with concrete tags (framing, pose, contact, location, expression-as-scene).",'Use standard Danbooru spacing (underscores as spaces): e.g. "cowboy shot", "from side", "hand on own chest", "facing another".','Prefer specific pose/contact/framing over vague words like "intimate" or "dramatic".',"Character appearance/attire/accessories stay in character fields — do not move them into camera/situation.","`natural` still follows the Natural base mode system message."].join(`
`)}function xm(e){const t=e.prompt_order;if(!t?.length)return ei;const n=[],r=new Set;for(const a of t){const i=Ac(a)||(u(a,40).toLowerCase()==="char"?"char":null);i&&!r.has(i)&&(r.add(i),n.push(i))}for(const a of ei)r.has(a)||n.push(a);return n}function km(e,t){const n=jc(e,t);return C(...xm(e).map(r=>n[r]))}function jc(e,t){const n=Array.isArray(t)?t:u(t,800).split(/[,|]/),r={base:[],char:[],primary:[],secondary:[],female:[],male:[]},a=new Set;for(const i of n){const o=u(i,120);if(!o||a.has(o))continue;const s=on(e,o);if(!s)continue;a.add(o);const c=s.option.by_slot;if(c&&Object.keys(c).length){for(const l of Object.keys(c)){const f=u(c[l],800);f&&r[l].push(f)}continue}r[$t(s.group.id)].push(s.option.tags)}return{base:C(...r.base),char:C(...r.char),primary:C(...r.primary),secondary:C(...r.secondary),female:C(...r.female),male:C(...r.male)}}function Ot(e,t){t&&(e.action=C(u(e.action,600),t))}function Qa(e,t,n){const r={...Va(),...t};e.camera=u(r.base,800),e.situation=u(n?.situation,400)||"",n&&"place"in n&&(e.place=u(n.place,400));const a=(Array.isArray(e.characters)?e.characters:[]).filter(o=>o&&typeof o=="object"),i=!!(r.char||r.primary||r.secondary||r.female||r.male);if(!a.length){e.action=i?C(r.char,r.primary,r.secondary,r.female,r.male):"";return}i&&(e.action=""),Sm(a,r)}function $c(e){const t=Array.isArray(e?.characters)?e.characters:[],n=[];for(let r=0;r<t.length;r++){const a=t[r];if(!a||typeof a!="object"){n.push({index:r,name:"",gender:""});continue}const i=a,o=Tn(i);n.push({index:r,name:u(i.name,200),gender:o==="f"?"girl":o==="m"?"boy":""})}return n}function Pc(e){if(!Array.isArray(e))return[];const t=[];for(const n of e){if(!n||typeof n!="object")continue;const r=n,a=Number(r.index);!Number.isInteger(a)||a<0||t.push({index:a,option_ids:r.option_ids??r.ids??r.curation_option_ids})}return t}function Oc(e,t,n){if(!t.length)return;const r=Array.isArray(e.characters)?e.characters:[];for(const{index:a,option_ids:i}of t){const o=r[a];if(!o||typeof o!="object")continue;const s=km(n,i);s&&Ot(o,s)}}function Sm(e,t){t.primary&&Ot(e[0],t.primary),t.secondary&&e[1]&&Ot(e[1],t.secondary);for(const n of e){t.char&&Ot(n,t.char);const r=Tn(n);t.female&&r==="f"&&Ot(n,t.female),t.male&&r==="m"&&Ot(n,t.male)}if(!e.some(n=>Tn(n)!=null)){const n=C(t.female,t.male);if(n)for(const r of e)Ot(r,n)}}function Am(e){return e.groups.reduce((t,n)=>t+n.options.length,0)}var Mm,Nm,ei,ti=P((()=>{fm(),L(),it(),Mm=10,Nm=80,ei=["base","char","primary","secondary","female","male"]}));function Im(e){let t=u(e.composition_id,160)||u(e.preset_id,160)||u(e.leaf_id,160)||u(e.position_id,160)||u(e.curation_leaf,160)||u(e.composition,160);if(t&&/[|/]/.test(t)){const r=t.split(/[/|]/).map(a=>u(a,160)).filter(Boolean);r.length>=2&&((!Array.isArray(e.preset_path)||!e.preset_path.length)&&(e.preset_path=r),t=r[r.length-1])}const n=u(e.composition_variant,120)||u(e.selected_variant_id,120)||u(e.variant_id,120)||u(e.variant,120);return t&&(e.composition_id=t),n&&(e.composition_variant=n),Array.isArray(e.preset_path)&&e.preset_path.length?!0:!!t}function Lt(e){return e&&typeof e=="object"&&!Array.isArray(e)?e:null}function Cm(e){const t=[];for(const n of["children","variants"]){const r=e[n];if(Array.isArray(r))for(const a of r){const i=Lt(a);i&&t.push(i)}}return t}function Lc(e){return Array.isArray(e.variants)?e.variants.map(Lt).filter(Boolean):[]}function Em(e){const t=e.when_to_use;return typeof t=="string"?u(t,400):Array.isArray(t)?u(t.map(n=>u(n,200)).filter(Boolean).join(" "),400):u(e.description,400)}function Tm(e){const t=e.avoid_when;return typeof t=="string"?u(t,400):Array.isArray(t)?u(t.map(n=>u(n,200)).filter(Boolean).join(" "),400):""}function jm(e){if(!e||typeof e!="object"||Array.isArray(e))return null;const t=e,n=u(t.ref??t.id??t.group,160);if(!n)return null;const r=Array.isArray(t.include_options)?t.include_options.map(o=>u(o,120)).filter(Boolean):void 0,a=t.action&&typeof t.action=="object"&&!Array.isArray(t.action)?t.action:null,i=a?{source:u(a.source,40)||void 0,target:u(a.target,40)||void 0,include_options:Array.isArray(a.include_options)?a.include_options.map(o=>u(o,120)).filter(Boolean):void 0}:void 0;return{ref:n,include_options:r?.length?r:void 0,replace:t.replace===!0,order:Number.isFinite(Number(t.order))?Number(t.order):void 0,action:i}}function Bc(e){return Array.isArray(e.modifiers)?e.modifiers.map(jm).filter(Boolean):[]}function $m(e){if(!Hn(e))return[];const t=Lt(e.presets);if(!t)return[];const n=[],r=(a,i,o,s)=>{const c=u(a.id,160),l=u(a.type,40),f=c&&c!=="preset"?[...i,c]:i,d=!o&&c&&c!=="preset"?c:o,m=l==="category"&&c?c:s,p=Array.isArray(a.children)?a.children.map(Lt).filter(Boolean):[],g=Lc(a);if(c&&c!=="preset"&&(l==="position"||g.length>0&&p.length===0)){const h=Bc(a),_=[...new Set(h.map(y=>y.ref).filter(Boolean))];n.push({id:c,path:f,composition:d||c,category:m,when:Em(a),avoid:Tm(a),variants:g.map(y=>u(y.id,120)).filter(Boolean),description:u(a.description,300)||c,selectionModifiers:_})}for(const h of p)r(h,f,d,m)};return r(t,[],"",""),n}function Rc(e,t,n){const r=Lt(e.presets),a=u(t,160);if(!r||!a)return null;const i={chain:null},o=(d,m)=>{if(i.chain)return;const p=[...m,d];if(u(d.id,160)===a){i.chain=p;return}const g=Array.isArray(d.children)?d.children.map(Lt).filter(Boolean):[];for(const h of g)o(h,p)};o(r,[]);const s=i.chain;if(!s)return null;const c=u(n,120);if(!c)return s;const l=s[s.length-1],f=Lc(l).find(d=>u(d.id,120)===c);return f?[...s,f]:s}function ni(e,t){const n=t.preset_path;if(Array.isArray(n)&&n.length){const r=n.map(s=>u(s,160)).filter(Boolean),a=Lt(e.presets);if(!a||!r.length)return null;const i=[a];let o=a;for(const s of r){if(u(o.id,160)===s)continue;const c=Cm(o).find(l=>u(l.id,160)===s);if(!c)return Rc(e,r[r.length-1],void 0);i.push(c),o=c}return i}return Rc(e,t.composition_id??(typeof n=="string"?n:""),t.composition_variant)}function ri(e){const t=new Map;for(const n of e)for(const r of Bc(n)){const a=t.get(r.ref);if(r.replace||!a){t.set(r.ref,r);continue}const i=[...new Set([...a.include_options||[],...r.include_options||[]])];t.set(r.ref,{...a,...r,include_options:i.length?i:a.include_options,action:r.action||a.action})}return[...t.values()]}function Pm(e,t){const n=new Map,r=new Set;for(const i of t){for(const o of ri(i))n.set(o.ref,o);for(const o of Dc(i))r.add(o)}const a=[];for(const[i,o]of n){if(Xa(e,i))continue;const s=Cc(e,i);if(!s)continue;const c=o.include_options?.length?new Set(o.include_options):null;let l=c?s.options.filter(f=>c.has(f.id)):s.options;if(r.size){const f=zc(e,l.map(m=>m.id),r),d=new Set(f);l=l.filter(m=>d.has(m.id))}l.length&&a.push({id:s.id,label:s.label,options:l})}return a}function Om(e,t){return t?.length?ri(t).filter(n=>Xa(e,n.ref)):[]}function Fc(e){const t=u(e,40).toLowerCase();return t?["male","m","boy","man"].includes(t)?"male":["female","f","girl","woman"].includes(t)?"female":["primary","main"].includes(t)?"primary":["secondary","partner"].includes(t)?"secondary":["global","base","scene","camera"].includes(t)?"base":["char","both","all"].includes(t)?"char":null:null}function Lm(e){return an(Mc(e))}function Bm(e,t,n){const r=on(e,t);if(!r)return an({});const a=n.find(s=>s.ref!==r.group.id?!1:!s.include_options?.length&&!s.action?.include_options?.length?!0:new Set([...s.include_options||[],...s.action?.include_options||[]]).has(t))||n.find(s=>s.ref===r.group.id);if(r.option.by_slot&&Object.keys(r.option.by_slot).length)return an(r.option.by_slot);const i=r.option.tags||Kn(r.option.tags);if(!i)return an({});if(a?.action){const s=Fc(a.action.source),c=Fc(a.action.target),l={};if(s&&(l[s]=i),c&&(l[c]=C(l[c],i)),s||c)return an(l)}const o=$t(r.group.id);return an({[o]:i})}function Dc(e){const t=new Set;for(const n of e){const r=n.modifier_filter;if(!r||typeof r!="object"||Array.isArray(r))continue;const a=r.deny;if(!(!a||typeof a!="object"||Array.isArray(a))){for(const i of Object.values(a))if(Array.isArray(i))for(const o of i){const s=u(o,160).toLowerCase();s&&t.add(s)}}}return t}function zc(e,t,n){return n.size?t.filter(r=>{const a=r.toLowerCase();if(n.has(a))return!1;const i=on(e,r);if(!i)return!0;const o=u(i.option.tags,800).toLowerCase(),s=u(i.option.description,400).toLowerCase();for(const c of n){const l=c.replace(/_/g," ");if(o.includes(c)||o.includes(l)||s.includes(l))return!1}return!0}):t}function Rm(e,t,n){const r=new Map;for(const a of n)Number.isFinite(a.order)&&r.set(a.ref,a.order);return r.size?t.map((a,i)=>({id:a,i})).sort((a,i)=>{const o=on(e,a.id)?.group.id,s=on(e,i.id)?.group.id,c=o!=null&&r.has(o)?r.get(o):Number.MAX_SAFE_INTEGER,l=s!=null&&r.has(s)?r.get(s):Number.MAX_SAFE_INTEGER;return c!==l?c-l:a.i-i.i}).map(a=>a.id):t}function Fm(e,t,n){if(!Hn(e))return null;const r=ni(e,t);if(!r?.length)return null;const a=r.map(m=>Lm(m.prompt)),i=ri(r),o=new Set(i.map(m=>m.ref)),s=[...i,...(n?.bindings||[]).filter(m=>!o.has(m.ref))],c=Dc(r),l=[...kc(t.curation_option_ids),...n?.optionIds||[]];let f=Sc(e,l);f=zc(e,f,c),f=Rm(e,f,s);const d=f.map(m=>Bm(e,m,s));return _m(...a,...d)}function Dm(){return["# Preset selection (Asset Maid analyzer rules)","Use ONLY information in the chat/shot context. Do not invent unseen actors or actions.","","## Actors → composition first","Count actors that are required for the current moment's action/contact.","Pick a composition that matches actor count and genders (e.g. 1girl_solo vs 1girl_1boy).","Do NOT drop a visible partner to force solo. If a male hand/arm is visible contacting the girl from POV, that male is an actor → prefer 1girl_1boy, not 1girl_solo.","","## Walk the tree","Read the leaf list as composition → category → position.","Categories matter: `general` = non-sexual placement; `foreplay` / `sexual` / `insertion` = intimate contact. If the chat has kissing, licking, sexual touch, or sex, do NOT pick a `general` leaf just because posture when-text matches.","For each candidate use when / avoid / selection_modifiers. Prefer the leaf whose when matches AND whose selection_modifiers cover the stated actions.","selection_modifiers are modifier **group ids** this leaf unlocks for pass 2 (e.g. `interaction.partner.mouth_contact`). They are the main reason to prefer one leaf over another when posture is similar.","CRITICAL: do NOT pick a leaf that only matches vague posture words if it lacks the group needed for the chat action. Example: ear licking / kissing / licking neck need a leaf with `interaction.partner.mouth_contact` (usually under foreplay), not `general/facing_each_other` which only has `interaction.general`.","Pick exactly ONE `composition_id` (position leaf) per shot.","If the same leaf id appears under multiple compositions/categories (e.g. `free`), set `composition_id` to the full path string `composition / category / leaf` so the host can disambiguate.","","## Variant","If the leaf lists variants, pick `composition_variant` for the camera that fits (from_side, pov, default…).","If unsure and a default/from_side exists, prefer that over inventing camera tags.","","## What NOT to do in pass 1","Do NOT invent Danbooru camera/pose tags into camera/situation/action.","Do NOT return modifier option ids here (pass 2 does that — e.g. do not emit `licking_ear` in pass 1).","Do NOT set `curation_groups`."]}function zm(e,t=!1){const n=$m(e),r=new Map;for(const i of n){const o=r.get(i.composition)||[];o.push(i),r.set(i.composition,o)}const a=[];for(const[i,o]of r){a.push(`## composition: ${i}`);for(const s of o){const c=s.variants.length?s.variants.join("|"):"(none)",l=s.selectionModifiers.length?s.selectionModifiers.join(", "):"(none)",f=s.description&&s.description!==s.id&&s.description!==s.when?`description: ${s.description}`:"";a.push([`### ${s.id}`,`path: ${s.path.join(" / ")}`,s.category?`category: ${s.category}`:"",s.when?`when: ${s.when}`:"",s.avoid?`avoid: ${s.avoid}`:"",f,`selection_modifiers: ${l}`,`variants: ${c}`].filter(Boolean).join(`
`))}}return["Curation two-stage ON (pass 1 — Asset Maid presets).","CRITICAL: every shot MUST include `composition_id` (exact leaf id, or full path if id is ambiguous). This overrides the base format schema.","Also set `composition_variant` when the leaf lists variants.",...Dm(),"","On every shot set:","- `composition_id`: leaf id (e.g. `face_to_face_upright`) OR path `1girl_1boy / foreplay / face_to_face_upright` when ids collide","- `composition_variant`: one variant id for that leaf, or omit if none","Still fill characters (appearance/attire/accessories) as usual.","`place` may stay as a short location hint.",...t?["STRICT catalog-id mode is ON: leave `camera`, `situation`, `natural`, and every `characters[].action` / `characters[].expression` completely EMPTY.","Pass 2 assembles ALL scene and per-actor tags from catalog ids only, keyed off the `composition_id`/`composition_variant` you pick here."]:["Leave `situation` empty or non-person scene words only — NEVER put 1girl/1boy/solo there (host adds person-count tags).","`camera`/`action` must NOT include 1girl/1boy/solo either.","`natural` still follows the Natural base mode system message."],"","## Preset leaf catalog (do not invent ids)","selection_modifiers = group ids unlocked for pass 2 (not option ids). Use them to choose the leaf; pass 2 picks concrete options like licking_ear.",...a.length?a:["(empty presets tree)"]].join(`
`)}function Um(e){const t=e.modifier_lanes;if(!t||!Object.keys(t).length)return[];const n=["## Modifier lanes (hard caps — host also enforces)","Within each lane, keep at most max_active_groups distinct groups."];for(const[r,a]of Object.entries(t))n.push(`- lane \`${r}\`: max ${a.max_active_groups} group(s) among [${a.fallback_order.join(", ")}] (earlier pattern wins if over cap)`);return n.push("Example: do not pick both manual.arm_pose and manual.partner_contact options together."),n}function Jm(e,t,n){const r=n?.strictIds===!0,a=Pm(e,t).map(i=>{const o=i.options.map(s=>`  - ${s.id}: ${s.description}`).join(`
`);return`### ${i.id} (${i.label})
${o}`});return["Curation two-stage ON (pass 2 — preset modifiers, BATCH).","You receive ALL shots in one request. Return ONE JSON object covering every shot.",r?'Schema: { "shots": [ { "shot_index": 0, "curation_option_ids": ["id", "..."], "characters": [ { "index": 0, "option_ids": ["id", "..."] } ], "place": "..." }, ... ] }':'Schema: { "shots": [ { "shot_index": 0, "curation_option_ids": ["id", "..."], "place": "..." }, ... ] }',r?'Example: { "shots": [ { "shot_index": 0, "curation_option_ids": ["bedroom"], "characters": [ { "index": 0, "option_ids": ["hug", "from_side"] }, { "index": 1, "option_ids": ["blush"] } ] } ] }':'Example: { "shots": [ { "shot_index": 0, "curation_option_ids": ["hug", "from_side", "blush", "bedroom"], "place": "bedroom" } ] }',"shot_index must match the input shots array (0-based). Return exactly one entry per input shot, same order.","","## CRITICAL: fill curation_option_ids","`curation_option_ids` is the MAIN output. Host injects those catalog tags into the image — empty array = weak/empty pose.","For EVERY shot, pick ALL options that match the chat band at that shot's y_percent:","- contact / hug / hands / pose / expression / gaze / blush / location / lighting / clothing state when visible","Typical count: **2–8 ids** when the scene has clear action; 1 id only if truly minimal; `[]` ONLY if nothing in the list fits.","Do NOT leave `curation_option_ids` empty just to be safe — if hug/facing/hand contact is in the text and listed below, INCLUDE those ids.","Pick ONLY ids listed below. Never invent ids. Never invent character appearance/attire.","Do NOT put Danbooru tags in freeform fields instead of picking ids — prefer ids.","`place` may repeat a short location hint; optional.","Host assembles path prompts + these options into base vs char slots.",...r?["",...Ec()]:[],...Tc(),...Um(e),"",a.length?"## Allowed modifier options":`## Allowed modifier options
(none — return empty curation_option_ids)`,...a].join(`
`)}var Hm=P((()=>{L(),ti()}));function Km(e,t){if(!e.length||e.length!==t.length)return-1;let n=0,r=0,a=0;for(let i=0;i<e.length;i++){const o=e[i],s=t[i];n+=o*s,r+=o*o,a+=s*s}return r<=0||a<=0?-1:n/(Math.sqrt(r)*Math.sqrt(a))}function qm(e,t,n){const r=n?.minScore??.35,a=Ym(n?.groupId);let i=null,o=-1,s=null,c=-1;for(const l of t){const f=Km(e,l.vector);f>o&&(o=f,i=l),a&&l.groupId===a&&f>c&&(c=f,s=l)}return s&&c>=r?s:i&&o>=r?i:null}function Gm(e){return e.slot||$t(e.groupId)}function Wm(e,t,n,r=.4){const a=[],i=[],o=[],s=[],c=[],l=[],f=[];let d=0,m=0;const p=(g,h)=>{g==="base"?i.push(h):g==="primary"?s.push(h):g==="secondary"?c.push(h):g==="female"?l.push(h):g==="male"?f.push(h):o.push(h)};for(let g=0;g<e.length;g++){const h=e[g].trim();if(!h)continue;const _=t[g];if(!_||!_.length){a.push(h),i.push(h),m+=1;continue}const y=qm(_,n,{minScore:r});y?(a.push(y.tags),p(Gm(y),y.tags),d+=1):(a.push(h),i.push(h),m+=1)}return{tags:a,baseTags:i,charTags:o,primaryTags:s,secondaryTags:c,femaleTags:l,maleTags:f,snapped:d,kept:m}}function Vm(e){const t=String(e||"").trim();if(!t)return[];const n=[];let r="",a=0;for(let o=0;o<t.length;o++){const s=t[o];if(s===":"&&t[o+1]===":"){a+=r.includes("::")?-1:1,r+="::",o+=1;continue}if(s===","&&a<=0){const c=r.trim();c&&n.push(c),r="";continue}r+=s}const i=r.trim();return i&&n.push(i),n}function Ym(e){return typeof e=="string"?e.trim():""}var Xm=P((()=>{ti()}));function Ce(e){const t=u(e,40).toLowerCase().replace(/[ -]+/g,"_");return t==="openai_compatible"||t==="compat"?"openai_compat":Cr.includes(t)?t:"openai"}function Ir(e){switch(Ce(e)){case"openai":return"https://api.openai.com/v1/embeddings";case"voyage":return"https://api.voyageai.com/v1/embeddings";case"openrouter":return"https://openrouter.ai/api/v1/embeddings";case"lmstudio":return"http://127.0.0.1:1234/v1/embeddings";case"ollama":return"http://127.0.0.1:11434/api/embeddings";default:return"https://api.openai.com/v1/embeddings"}}function Gn(e){switch(Ce(e)){case"voyage":return"voyage-3-lite";case"ollama":return"nomic-embed-text";case"openrouter":return"openai/text-embedding-3-small";case"lmstudio":return"text-embedding-nomic-embed-text-v1.5";default:return"text-embedding-3-small"}}function Zm(e){return Gn(e)}function Qm(){const e=new Set;for(const t of Cr){const n=Ir(t).replace(/\/+$/,"");e.add(n),e.add(n.replace(/\/embeddings$/i,"")),e.add(n.replace(/\/api\/embeddings$/i,"")),e.add(n.replace(/\/v1$/i,""))}return e}function ep(e){const t=u(e,500);if(!t)return!0;const n=t.replace(/\/+$/,"");return Qm().has(n)}function tp(e){const t=u(e,200);return t?new Set(Cr.map(n=>Gn(n))).has(t):!0}function np(e){const t=Ce(e);return t!=="lmstudio"&&t!=="ollama"}function rp(e){const t=Ce(e.provider),n=u(e.endpoint,500)||Ir(t);return t==="ollama"?n.replace(/\/$/,""):/\/embeddings\/?$/i.test(n)?n.replace(/\/$/,""):/\/v1\/?$/i.test(n)?`${n.replace(/\/$/,"")}/embeddings`:n}function ap(e){const t=e&&typeof e=="object"?e:{},n=Array.isArray(t.data)?t.data:[],r=[];for(const a of n){const i=a&&typeof a=="object"?a.embedding:null;if(Array.isArray(i)&&i.every(o=>typeof o=="number"))r.push(i);else throw new Error("임베딩 응답에 숫자 벡터가 없습니다.")}return r}async function Uc(e){const t=e.ok!==!1&&(e.status===void 0||e.status>=200&&e.status<300);if(typeof e.json=="function")try{const n=await e.json();if(!t){const r=typeof n=="object"&&n?JSON.stringify(n).slice(0,400):String(n);throw new Error(`임베딩 HTTP ${e.status??"?"}: ${r}`)}return n}catch(n){if(n instanceof Error&&n.message.startsWith("임베딩 HTTP"))throw n}if(typeof e.text=="function"){const n=await e.text();if(!t)throw new Error(`임베딩 HTTP ${e.status??"?"}: ${n.slice(0,400)}`);return n?JSON.parse(n):{}}throw new Error("임베딩 응답을 읽을 수 없습니다.")}async function ai(e,t,n){if(!t.length)return{vectors:[],model:u(e.model)};const r=Ce(e.provider),a=u(e.model,200)||Gn(r),i=rp({...e,provider:r,model:a}),o=u(e.api_key,4e3);if(r==="ollama"){const l=[];for(const f of t){const d=(await Uc(await Fe(i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:a,prompt:f}),signal:n?.signal}))).embedding;if(!Array.isArray(d)||!d.every(m=>typeof m=="number"))throw new Error("Ollama 임베딩 응답이 올바르지 않습니다.");l.push(d)}return{vectors:l,model:a}}const s={"Content-Type":"application/json"};o&&(s.Authorization=`Bearer ${o}`);const c=ap(await Uc(await Fe(i,{method:"POST",headers:s,body:JSON.stringify(r==="voyage"?{model:a,input:t,input_type:"document"}:{model:a,input:t}),signal:n?.signal})));if(c.length!==t.length)throw new Error(`임베딩 개수 불일치: got ${c.length}, want ${t.length}`);return{vectors:c,model:a}}async function ip(e){const{vectors:t,model:n}=await ai(e,["curation embedding ping"]),r=t[0]?.length||0;if(!r)throw new Error("임베딩 차원이 0입니다.");return{ok:!0,dims:r,model:n}}var ii,Cr,Jc=P((()=>{L(),jt(),ii=Object.freeze([{value:"openai",label:"OpenAI"},{value:"voyage",label:"Voyage"},{value:"openrouter",label:"OpenRouter"},{value:"openai_compat",label:"OpenAI-compat"},{value:"lmstudio",label:"LM Studio (로컬)"},{value:"ollama",label:"Ollama (로컬)"},{value:"custom",label:"Custom endpoint"}]),Cr=ii.map(e=>e.value)})),op=fn({LLM_ENDPOINT_PRESETS:()=>ut,LLM_PROVIDERS:()=>Vc,REASONING_EFFORTS:()=>oi,applyReasoningToBody:()=>Wc,defaultEndpointForProvider:()=>Kc,endpointPresetKeyFor:()=>lp,ensureLlmRequestUrl:()=>Gc,knownLlmEndpoints:()=>qc,llmModelPlaceholder:()=>cp,normalizeLlmProvider:()=>Bt,normalizeReasoningEffort:()=>Hc,shouldAutoReplaceEndpoint:()=>sp});function Bt(e){const t=String(e||"").toLowerCase().replace(/[ -]+/g,"_").trim();return["openrouter"].includes(t)?"openrouter":["openai"].includes(t)?"openai":["google","google_ai","google_ai_studio","gemini"].includes(t)?"google_ai":["vertex","vertex_ai","google_vertex"].includes(t)?"vertex":["anthropic","anthropic_compat","anthropic_compatible","claude"].includes(t)?"anthropic_compatible":["lmstudio","lm_studio"].includes(t)?"lmstudio":["ollama"].includes(t)?"ollama":["custom"].includes(t)?"custom":["openai_compatible","openai_compat"].includes(t)?"openrouter":"custom"}function Hc(e){const t=String(e??"default").toLowerCase().trim();return!t||t==="auto"||t==="default"?"default":oi.includes(t)?t:"default"}function Kc(e,t={}){const n=Bt(e);return n==="vertex"?`https://${String(t.region||"us-central1").trim().replace(/[^a-z0-9-]/gi,"")||"us-central1"}-aiplatform.googleapis.com`:n==="anthropic_compatible"?ut.anthropic:n==="custom"?ut.openai:ut[n]||ut.openai}function qc(){const e=new Set;for(const t of Object.values(ut)){const n=String(t||"").replace(/\/+$/,"");n&&(e.add(n),e.add(n.replace(/\/chat\/completions$/i,"")),e.add(n.replace(/\/v1\/messages$/i,"")),e.add(n.replace(/\/v1$/i,"")))}return e.add("https://us-central1-aiplatform.googleapis.com"),e.add("https://europe-west1-aiplatform.googleapis.com"),e.add("https://asia-northeast1-aiplatform.googleapis.com"),e}function sp(e){const t=String(e||"").trim().replace(/\/+$/,"");return t?qc().has(t):!0}function Gc(e,t,n={}){const r=Bt(t);let a=String(e||"").trim().replace(/\/+$/,"");if(a||(a=Kc(r,n).replace(/\/+$/,"")),r==="anthropic_compatible")return/\/v1\/messages$/i.test(a)||/\/messages$/i.test(a)?a:/\/v1$/i.test(a)?`${a}/messages`:`${a}/v1/messages`;if(r==="vertex"){const i=String(n.region||"us-central1").trim().replace(/[^a-z0-9-]/gi,"")||"us-central1",o=String(n.projectId||"").trim();if(o)return`https://${i}-aiplatform.googleapis.com/v1/projects/${encodeURIComponent(o)}/locations/${encodeURIComponent(i)}/endpoints/openapi/chat/completions`;if(/aiplatform\.googleapis\.com/i.test(a)&&!/\/chat\/completions$/i.test(a))return a}return/\/chat\/completions$/i.test(a)?a:/\/openai$/i.test(a)?`${a}/chat/completions`:/\/v1beta$/i.test(a)?`${a}/openai/chat/completions`:/\/v1$/i.test(a)?`${a}/chat/completions`:a}function Wc(e,t){const n={};e&&typeof e=="object"&&Object.assign(n,e);const r=Hc(t);return r==="default"||(n.reasoning={effort:r}),n}function cp(e){const t=Bt(e),n={openrouter:"openai/gpt-4o-mini",openai:"gpt-4o-mini",google_ai:"gemini-2.5-flash",vertex:"gemini-2.0-flash-001",anthropic_compatible:"claude-sonnet-4",lmstudio:"local-model",ollama:"llama3.2",custom:"model-id"};return n[t]||n.custom}function lp(e){const t=String(e||"").trim().replace(/\/+$/,"");for(const[n,r]of Object.entries(ut)){const a=r.replace(/\/+$/,"");if(t===a||t===a.replace(/\/chat\/completions$/i,"")||t===a.replace(/\/v1\/messages$/i,""))return n}return"custom"}var ut,oi,Vc,Er=P((()=>{ut=Object.freeze({openai:"https://api.openai.com/v1/chat/completions",openrouter:"https://openrouter.ai/api/v1/chat/completions",google_ai:"https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",lmstudio:"http://127.0.0.1:1234/v1/chat/completions",ollama:"http://127.0.0.1:11434/v1/chat/completions",anthropic:"https://api.anthropic.com/v1/messages",vertex:"https://us-central1-aiplatform.googleapis.com"}),oi=Object.freeze(["default","none","minimal","low","medium","high","xhigh","max"]),Vc=Object.freeze([{value:"openrouter",label:"OpenRouter"},{value:"openai",label:"OpenAI"},{value:"google_ai",label:"Google AI Studio"},{value:"vertex",label:"Vertex AI (Google Cloud)"},{value:"anthropic_compatible",label:"Anthropic-compatible"},{value:"lmstudio",label:"LM Studio (로컬)"},{value:"ollama",label:"Ollama (로컬)"},{value:"custom",label:"Custom endpoint"}])}));function Tr(e){const t=String(e||"").trim().toLowerCase();return t==="main"||t==="risu_main"||t==="risu-main"?"main":t==="aux"||t==="otherax"||t==="other_ax"||t==="risu_aux"||t==="risu-aux"||t==="sub"||t==="secondary"?"aux":"custom"}function up(e){const t=Tr(e);return t==="main"||t==="aux"}function Yc(e){const t=e||{};return up(t.source)?!0:Bt(t.provider)==="vertex"?!!(u(t.model)&&(u(t.api_key)||u(t.service_account_json))):!!(u(t.model)&&u(t.api_key))}function fp(e){let t="";const n=[];for(const r of e||[]){const a=String(r?.role||""),i=r?.content;if(a==="system"){const o=typeof i=="string"?i:Array.isArray(i)?i.map(s=>typeof s=="object"?s.text||"":String(s)).join(""):String(i??"");t=t?`${t}
${o}`:o;continue}if(Array.isArray(i)){const o=[];for(const s of i){if(!s||typeof s!="object"){const d=String(s||"").trim();d&&o.push({type:"text",text:d});continue}const c=s.image_url,l=(typeof c=="object"&&c?c.url:"")||(typeof c=="string"?c:"");if(s.type==="image_url"||l){const d=String(l).match(/^data:([^;]+);base64,([\s\S]+)$/i);d&&o.push({type:"image",source:{type:"base64",media_type:d[1]||"image/png",data:d[2].replace(/\s+/g,"")}});continue}const f=String(s.text||"").trim();f&&o.push({type:"text",text:f})}n.push({role:a==="assistant"?"assistant":"user",content:o.length?o:[{type:"text",text:""}]});continue}n.push({role:a==="assistant"?"assistant":"user",content:String(i??"")})}return{system:t,messages:n}}function dp(e){const t=e?.choices||[];if(!t.length)throw new Error("LLM returned no choices.");let n=(t[0].message||{}).content;return Array.isArray(n)&&(n=n.map($r).join("")),u(n)}function mp(e){const t=e?.content,n=(Array.isArray(t)?t:[]).map($r).join(""),r=u(n||e?.completion||"");if(!r)throw new Error("Anthropic 응답이 비어 있습니다.");return r}async function jr(e){if(!Wn(e))return"";const t=e.getReader(),n=typeof TextDecoder<"u"?new TextDecoder:null;let r="",a="";for(;;){const{done:i,value:o}=await t.read();if(i)break;if(typeof o=="string")r=o,a+=o;else if(o instanceof Uint8Array)a+=n?n.decode(o,{stream:!0}):"";else if(o&&typeof o=="object"){const s=o,c=s[0];typeof c=="string"?r=c:typeof s.content=="string"?r=s.content:typeof s.text=="string"&&(r=s.text)}}return n&&(a+=n.decode()),r||a}async function pp(e){if(typeof e=="string")return e;if(Wn(e))return jr(e);if(e==null)return"";if(typeof e=="number"||typeof e=="boolean")return String(e);if(typeof e=="object"){const t=e,n=u(t.type,40).toLowerCase();if(n==="fail"||n==="error"){const a=u(t.result||t.message||t.error||"Risu LLM 실패",800);throw new Error(`Risu LLM 실패: ${a}`)}if(n==="streaming"||n==="stream"){const a=await jr(t.result??t.data??t.stream);if(a.trim())return a}if(n==="success"||n==="ok"){const a=t.result??t.data??t.content;if(typeof a=="string")return a;if(Wn(a))return jr(a)}const r=[t.choices?.[0]?.message?.content,t.choices?.[0]?.text,t.choices?.[0]?.delta?.content,t.message?.content,t.content,t.text,t.response,n?null:t.result,t.output];for(const a of r){if(typeof a=="string"&&a.trim())return a;if(Array.isArray(a)){const i=a.map($r).join("");if(i.trim())return i}if(Wn(a)){const i=await jr(a);if(i.trim())return i}}try{return JSON.stringify(e)}catch{return String(e)}}return String(e||"")}var Wn,$r,si=P((()=>{L(),Er(),Wn=e=>!!e&&typeof e.getReader=="function",$r=e=>typeof e=="object"?e?.text||"":String(e||"")}));async function gp(){try{const e=await Ne(Yr,$i);if(e==null||e==="")return ye(tt);const t=typeof e=="string"?JSON.parse(e):e;if(!t||typeof t!="object")return ye(tt);const n=Da(t),r=nt(tt,n);return JSON.stringify(t)!==JSON.stringify(n)&&await Xc(r),r}catch(e){console.warn("[Inlay Nexus] settings load failed",e?.message||e)}return ye(tt)}async function Xc(e){if(await Ie(Yr,ye(e)),await Ne("inx_native_settings")==null)throw new Error("설정 저장 실패: IndexedDB(getLocalPluginStorage)에 기록되지 않았습니다.")}var Zc=P((()=>{te(),xt(),Ht(),br()}));async function hp(){const e=await D("meta","prompt:__pack__"),t=!e||e.text!=="2026-08-06-asset-priority",n=Date.now()/1e3;for(const r of dn)await D("meta",`prompt:${r}`)&&!(t&&ji.includes(r))||await F("meta",{key:`prompt:${r}`,text:In(r),updated_at:n});await F("meta",{key:"prompt:__pack__",text:Ti,updated_at:n})}async function le(e){const t=await D("meta",`prompt:${e}`);return t?.text!=null?t.text:In(e)}async function Qc(e,t){const n=Date.now()/1e3;return await F("meta",{key:`prompt:${e}`,text:t,updated_at:n}),{ok:!0,key:e,updated_at:n}}async function Pr(){const e=await we("meta"),t=new Map(e.filter(r=>r.key?.startsWith("prompt:")&&!r.key.startsWith("prompt:__")).map(r=>[r.key.slice(7),{key:r.key.slice(7),text:r.text||"",updated_at:r.updated_at||0}])),n=[];for(const r of dn){const a=t.get(r);a?n.push(a):n.push({key:r,text:In(r),updated_at:0}),t.delete(r)}for(const r of t.values())n.push(r);return n}async function _p(){const e=await Pr(),t={};for(const n of e)t[n.key]=n.text;return{version:Ye,prompts:t}}async function yp(e){const t=wp(e);let n=0;const r=Date.now()/1e3;for(const a of dn)Object.prototype.hasOwnProperty.call(t,a)&&(await F("meta",{key:`prompt:${a}`,text:String(t[a]??""),updated_at:r}),n+=1);return{ok:!0,updated:n,prompts:await Pr()}}async function bp(e){const t=e?.keep_author_note!==!1,n=Date.now()/1e3;let r=0;for(const a of dn)t&&a==="author_note"||(await F("meta",{key:`prompt:${a}`,text:In(a),updated_at:n}),r+=1);return{ok:!0,updated:r,keep_author_note:t,prompts:await Pr()}}function wp(e){let t=e;if(typeof e=="string"){const i=e.trim();if(!i)return{};t=JSON.parse(i)}if(!t||typeof t!="object"||Array.isArray(t))return{};const n=t,r={},a=n.prompts;if(a&&typeof a=="object"&&!Array.isArray(a)){for(const[i,o]of Object.entries(a))typeof i=="string"&&(r[i]=String(o??""));return r}if(Array.isArray(a)){for(const i of a){if(!i||typeof i!="object")continue;const o=i,s=String(o.key||"");s&&(r[s]=String(o.text??""))}return r}if(typeof n.key=="string"&&"text"in n)return r[n.key]=String(n.text??""),r;for(const[i,o]of Object.entries(n))i==="version"||i==="ok"||typeof o=="string"&&(r[i]=o);return r}async function Ke(){const e=ye(j());await Ro.run(()=>Xc(e))}function vp(){return Bd(j())}async function xp(e){const t=Rd(String(e??"").slice(0,2000001)),n=ye(j());await F("meta",{key:"settings_backup",value:n,updated_at:Date.now()/1e3});const r=nt(tt,t);return r.llm={...r.llm,api_key:n.llm?.api_key||""},r.nai={...r.nai,api_key:n.nai?.api_key||""},r.auth_token=n.auth_token||"",ur(r),await Ke(),{ok:!0,settings:Or()}}async function kp(){const e=ye(j());await F("meta",{key:"settings_backup",value:e,updated_at:Date.now()/1e3});const t=ye(tt);t.llm={...t.llm,api_key:e.llm?.api_key||""},t.nai={...t.nai,api_key:e.nai?.api_key||""},t.auth_token=e.auth_token||"";const n=e.card;return t.card={...t.card,presets:Array.isArray(n.presets)?ye(n.presets):t.card.presets||[],active_preset_id:String(n.active_preset_id||t.card.active_preset_id||""),custom_pos:String(n.custom_pos??t.card.custom_pos??""),custom_neg:String(n.custom_neg??t.card.custom_neg??"")},ur(t),await Ke(),{ok:!0,settings:Or()}}function Sp(){const e=j(),t=e.nai,n=e.llm;let r=0,a=0,i=0,o=0;try{r=$n("cards"),a=$n("images"),i=bf();const s=new Set;for(const c of wf()){const l=u(c.character_id||"",200)||"unknown",f=u(c.chat_id||"",200)||"unknown";s.add(`${l}|${f}`)}o=s.size}catch{}return{ok:!0,version:Ye,pid:0,source_file:"native",nai_configured:Jn(t)==="comfy"?Ha(t):!!u(t.api_key),image_backend:Jn(t),llm_configured:Yc(n),port:0,image_mode:"data-url",storage:"indexeddb",storage_api:"getLocalPluginStorage",storage_scope:"device-local",cards:r,images:a,folders:o,png_bytes:i,last_stage:kn(),focus_stage:ht(),last_error:or(),debug_events:Hi()}}function Or(){const e=ye(j());e.nai?.api_key?(e.nai.api_key="",e.nai.api_key_configured=!0):e.nai={...e.nai,api_key_configured:!1},e.llm?.api_key?(e.llm.api_key="",e.llm.api_key_configured=!0):e.llm={...e.llm,api_key_configured:!1},e.llm?.service_account_json?(e.llm.service_account_json="",e.llm.service_account_configured=!0):e.llm={...e.llm,service_account_configured:!1};const t=e.nai;t.backend=Jn(t),t.image_backend=t.backend,t.comfy_configured=Ha(t);const n=Lo(),r=Bo(),a=!ci.includes(u(t.image_reference));t.image_reference_configured=!!(a&&n),t.image_reference||(t.image_reference=t.image_reference_configured?"file":"none"),t.image_reference_configured&&n&&(t.reference_preview_url=n);const i=!ci.includes(u(t.vibe_transfer));if(t.vibe_transfer_configured=!!(i&&r),t.vibe_transfer||(t.vibe_transfer=t.vibe_transfer_configured?"file":"none"),t.vibe_transfer_configured&&r&&(t.vibe_preview_url=r),Array.isArray(e.card?.presets))for(const c of e.card.presets){if(!c||typeof c!="object")continue;const l=c,f=u(l.id,120),d=f?oa(f):"";d?(l.vibe_configured=!0,l.vibe_preview_url=d):(delete l.vibe_configured,delete l.vibe_preview_url),delete l.vibe_transfer}e.card.character_max=at(e.card),e.database_path="indexeddb:getLocalPluginStorage",e.images_dir="indexeddb:inx_nximg_*",e.prompts_dir="embedded";const o=e.curation&&typeof e.curation=="object"?e.curation:{},s=o.embedding&&typeof o.embedding=="object"?{...o.embedding}:{};return s.api_key?(s.api_key="",s.api_key_configured=!0):s.api_key_configured=!1,o.embedding=s,e.curation=o,e.storage={...e.storage,backend:"indexeddb",api:"getLocalPluginStorage",image_encoding:"base64",scope:"device-local",image_mode:"data-url"},e}async function Ap(e){const t=e&&typeof e=="object"?e:{},n=j(),r={...t.nai||{}},a={...t.llm||{}},i={...t.card||{}};if("api_key"in r){const o=r.api_key;delete r.api_key,delete r.api_key_configured,u(o)&&(n.nai.api_key=o),(r.clearApiKey||t.clear_nai_key)&&(n.nai.api_key=""),delete r.clearApiKey}if("api_key"in a){const o=a.api_key;delete a.api_key,delete a.api_key_configured,u(o)&&(n.llm.api_key=o),(a.clearApiKey||t.clear_llm_key)&&(n.llm.api_key=""),delete a.clearApiKey}if("service_account_json"in a){const o=a.service_account_json;delete a.service_account_json,delete a.service_account_configured,u(o)&&(n.llm.service_account_json=o),(a.clearServiceAccount||t.clear_llm_service_account)&&(n.llm.service_account_json=""),delete a.clearServiceAccount}if(Object.keys(i).length){const o=nt(n.card,i);Array.isArray(i.presets)&&(o.presets=i.presets),"active_preset_id"in i&&(o.active_preset_id=i.active_preset_id),"custom_pos"in i&&(o.custom_pos=i.custom_pos),"custom_neg"in i&&(o.custom_neg=i.custom_neg),n.card=o,n.card.character_max=at(n.card)}if(Object.keys(r).length&&(n.nai=nt(n.nai,r)),Object.keys(a).length&&(n.llm=nt(n.llm,a)),t.curation&&typeof t.curation=="object"){const{updateCurationSettings:o}=await Promise.resolve().then(()=>(Xn(),Ep));await o(t.curation)}for(const o of["bind_host","port","auth_token"])o in t&&(n[o]=t[o]);return await Ke(),{ok:!0,settings:Or()}}var ci,ft=P((()=>{Ht(),Un(),te(),G(),xt(),L(),it(),si(),Zc(),Oe(),fe(),ci=["","none","off","false","0"]}));function el(e){const t=typeof e=="string"?e:JSON.stringify(e);return li(Ut(new TextEncoder().encode(t)))}function Mp(e){const t=ArrayBuffer.isView(e)?new Uint8Array(e.buffer,e.byteOffset,e.byteLength):X(e);return li(Ut(t))}async function Np(e){let t;try{t=typeof e=="string"?JSON.parse(e):e}catch{throw new Error("Vertex Service Account JSON 파싱 실패")}if(!t?.client_email||!t?.private_key)throw new Error("Service Account JSON에 client_email/private_key가 필요합니다.");if(!globalThis.crypto?.subtle)throw new Error("이 환경에서는 Vertex Service Account(JWT) 서명을 지원하지 않습니다. API key 칸에 OAuth access token을 넣거나 Google AI Studio를 쓰세요.");const n=Math.floor(Date.now()/1e3),r=`${el({alg:"RS256",typ:"JWT"})}.${el({iss:t.client_email,scope:"https://www.googleapis.com/auth/cloud-platform",aud:"https://oauth2.googleapis.com/token",iat:n,exp:n+3600})}`,a=String(t.private_key).replace(/\\n/g,`
`).replace(/-----BEGIN [^-]+-----/,"").replace(/-----END [^-]+-----/,"").replace(/\s+/g,""),i=Uint8Array.from(atob(a),f=>f.charCodeAt(0)),o=await crypto.subtle.importKey("pkcs8",i,{name:"RSASSA-PKCS1-v1_5",hash:"SHA-256"},!1,["sign"]),s=`${r}.${Mp(await crypto.subtle.sign("RSASSA-PKCS1-v1_5",o,new TextEncoder().encode(r)))}`,c=await Fe("https://oauth2.googleapis.com/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:`grant_type=${encodeURIComponent("urn:ietf:params:oauth:grant-type:jwt-bearer")}&assertion=${encodeURIComponent(s)}`});let l={};try{l=await c.json()}catch{l={}}if(!l?.access_token)throw new Error(`Vertex 토큰 발급 실패: ${JSON.stringify(l).slice(0,240)}`);return{accessToken:String(l.access_token),projectId:u(t.project_id||"")}}var li,Ip=P((()=>{Q(),L(),jt(),li=e=>e.replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/g,"")}));async function sn(e,t){const n=Tr(e.source);if(n==="main"||n==="aux")return Cp(e,t,n);const r=Bt(e.provider),a=u(e.model),i=u(e.vertex_region)||"us-central1";let o=u(e.api_key),s="";if(r==="vertex"&&u(e.service_account_json)){const p=await Np(e.service_account_json);o=p.accessToken,s=p.projectId}if(!a||!o)throw w("llm.config",{message:"missing model/api_key",provider:r},"error"),new Error(r==="vertex"?"Vertex AI: Model + Service Account JSON(또는 access token)이 필요합니다.":"태깅 LLM이 설정되지 않았습니다. 모델 설정에서 Provider·Model·API key를 넣으세요. (NovelAI 키와 별개)");const c=Gc(u(e.endpoint),r,{region:i,projectId:s});if(r==="vertex"&&!/\/chat\/completions$/i.test(c))throw new Error("Vertex AI: project_id가 있는 Service Account JSON이 필요합니다. (OpenAI-compatible endpoint 구성용)");const l=Number(e.timeout_seconds??180)*1e3,f=typeof AbortController<"u"?new AbortController:null,d=setTimeout(()=>{w("llm.abort",{message:`timeout ${l}ms`,model:a,provider:r},"warn"),f?.abort?.()},l),m=ue("llm.call");w("llm.call.start",{message:a,msgs:t?.length||0,timeout_ms:l,source:n,provider:r,reasoning:u(e.reasoning_effort)||"default"});try{let p;if(r==="anthropic_compatible"){const y=fp(t),x={model:a,max_tokens:Number(e.max_tokens??8e3),temperature:Math.min(1,Number(e.temperature??.4)),messages:y.messages};y.system&&(x.system=y.system),p=await Fe(c,{method:"POST",headers:{"Content-Type":"application/json","x-api-key":o,"anthropic-version":u(e.anthropic_version)||"2023-06-01"},body:JSON.stringify(x),signal:f?.signal})}else{const y=Wc({model:a,messages:t,temperature:Number(e.temperature??.4),max_tokens:Number(e.max_tokens??8e3)},e.reasoning_effort),x={"Content-Type":"application/json",Authorization:`Bearer ${o}`};r==="openrouter"&&(x["HTTP-Referer"]="https://risuai.xyz",x["X-Title"]="Inlay Nexus"),p=await Fe(c,{method:"POST",headers:x,body:JSON.stringify(y),signal:f?.signal})}const g=Number(p?.status||0);let h;try{h=await p.json()}catch{h={}}if(g>=400)throw m.fail(new Error(`HTTP ${g}`),{status:g,body:JSON.stringify(h).slice(0,200),provider:r}),new Error(`LLM HTTP ${g}: ${JSON.stringify(h).slice(0,500)}`);const _=r==="anthropic_compatible"?mp(h):dp(h);return m.end({message:a,status:g,bytes:_.length,provider:r}),_}catch(p){throw m.fail(p,{model:a,provider:r}),p}finally{clearTimeout(d)}}async function Cp(e,t,n,r=""){const a=Ae();if(!a||typeof a.runLLMModel!="function")throw new Error("RisuAI runLLMModel API를 사용할 수 없습니다. Risu를 최신으로 업데이트하세요.");const i=n==="main"?"model":"otherAx",o=Math.max(5e3,Number(e.timeout_seconds??180)*1e3),s=u(r)||"",c=ue("llm.call");w("llm.call.start",{message:`risu:${i}`,msgs:t?.length||0,timeout_ms:o,source:n,static_model:s});let l;const f=new Promise((d,m)=>{l=setTimeout(()=>m(new Error(`Risu LLM timeout ${o}ms (${i})`)),o)});try{const d=await Promise.race([a.runLLMModel({mode:i,...s?{staticModel:s}:{},allowPlugins:!0,messages:t}),f]),m=u(await pp(d));if(!m)throw new Error(`Risu LLM(${i}) 응답이 비어 있습니다.`);return c.end({message:`risu:${i}`,bytes:m.length}),m}catch(d){throw c.fail(d,{mode:i}),d}finally{l!==void 0&&clearTimeout(l)}}var ui=P((()=>{G(),gt(),L(),jt(),Ip(),Er(),si()})),Ep=fn({catalogSha:()=>qn,curationStatus:()=>Yn,curationTaggerSystemMessage:()=>sl,embedCurationCatalog:()=>al,getCurationMode:()=>Lr,getCurationStrictIds:()=>Br,getEmbedProgress:()=>tl,getEmbeddingSettingsFromConfig:()=>Vn,loadCurationCatalog:()=>dt,refineShotsWithCuration:()=>cl,resetCurationCatalog:()=>rl,saveCurationCatalog:()=>nl,snapShotsSceneTags:()=>fl,testCurationEmbedding:()=>il,updateCurationSettings:()=>dl});function tl(){return{...cn}}function Lr(){const e=j(),t=e.curation&&typeof e.curation=="object"?e.curation:{},n=La(t.mode);if(n!=="off")return n;const r=e.card;return r?.composition_curation===!0||r?.composition_curation==="true"||r?.composition_curation===1?"two_stage":"off"}function Br(){const e=j(),t=e.curation&&typeof e.curation=="object"?e.curation:{};return Ba(t.strict_ids)}function Vn(){const e=j(),t=e.curation&&typeof e.curation=="object"?e.curation:{},n=t.embedding&&typeof t.embedding=="object"?t.embedding:{},r=Ce(n.provider);return{provider:r,model:u(n.model,200)||Gn(r),endpoint:u(n.endpoint,500)||Ir(r),api_key:u(n.api_key,4e3)}}async function dt(){const e=await Ne(rr);if(e==null||e==="")return Za();try{const t=typeof e=="string"?JSON.parse(e):e;return Ya(t)}catch(t){return w("curation.catalog.load",{message:String(t?.message||t)},"warn"),Za()}}async function nl(e){const t=Ya(e);await Ie(rr,t);const n=await fi();return n&&(n.catalog_sha,t.sha),{ok:!0,status:await Yn()}}async function rl(){const e=Za();return await Ie(rr,e),await de(ar),{ok:!0,status:await Yn()}}async function fi(){const e=await Ne(ar);if(e==null||e==="")return null;try{const t=typeof e=="string"?JSON.parse(e):e;if(!t||typeof t!="object")return null;const n=t;return Array.isArray(n.items)?n:null}catch{return null}}async function Yn(){const e=await dt(),t=await fi(),n=Am(e);let r="missing";if(t?.items?.length){r=t.catalog_sha===e.sha?"ready":"stale";const a=Vn(),i=u(a.model);i&&t.model&&t.model!==i&&(r="stale")}return{mode:Lr(),catalog_name:e.name,catalog_sha:e.sha||qn(e),option_count:n,group_count:e.groups.length,has_presets:Hn(e),embed_status:r,embed_count:t?.items?.length||0,embed_model:t?.model||"",embed_updated_at:t?.updated_at||0,large_warning:n>ml,embed_progress:tl()}}async function al(e){const t=await dt(),n=ym(t),r=Vn();if(!n.length)throw new Error("임베딩할 카탈로그 항목이 없습니다.");if(Ce(r.provider)!=="ollama"&&Ce(r.provider)!=="lmstudio"&&!u(r.api_key))throw new Error("임베딩 API key가 없습니다. 큐레이팅 탭에서 키를 저장하세요.");const a=(c,l,f)=>{cn={running:c<l,done:c,total:l,message:f},e?.(c,l,f)},i=Ce(r.provider)==="ollama"?1:16,o=[];let s=r.model;a(0,n.length,"임베딩 시작…");try{for(let l=0;l<n.length;l+=i){const f=n.slice(l,l+i);a(l,n.length,`임베딩 중… ${l}/${n.length}`);const{vectors:d,model:m}=await ai(r,f.map(p=>p.text));s=m||s;for(let p=0;p<f.length;p++){const g=f[p],h=d[p];if(!h?.length)throw new Error(`임베딩 실패: ${g.key}`);o.push({key:g.key,groupId:g.groupId,optionId:g.optionId,tags:g.tags,slot:g.slot,vector:h})}}a(n.length,n.length,"저장 중…");const c={catalog_sha:t.sha||qn(t),model:s,provider:String(r.provider),updated_at:Date.now()/1e3,items:o};return await Ie(ar,c),a(n.length,n.length,"완료"),cn={running:!1,done:n.length,total:n.length,message:"완료"},{ok:!0,status:await Yn()}}catch(c){throw cn={running:!1,done:cn.done,total:n.length,message:`실패: ${c?.message||c}`},c}}async function il(){const e=Vn();return ip(e)}function ol(e){return!!(e.base||e.char||e.primary||e.secondary||e.female||e.male)}async function sl(){const e=Lr();if(e==="off")return null;if(e==="embed_snap")return u(await le("curation_embed_hint"),4e3)||vm();const t=await dt(),n=Br();return Hn(t)?zm(t,n):bm(t,n)}async function cl(e,t){if(!e.length)return;const n=u(t?.chatContext,1e5),r=await dt();if(Hn(r)){for(const a of e)Im(a);if(e.some(a=>!!ni(r,{preset_path:a.preset_path,composition_id:a.composition_id,composition_variant:a.composition_variant}))){await jp(r,e,n);return}w("curation.preset_refine",{message:"no composition_id on shots → falling back to group refine"},"warn")}await Tp(r,e,n)}function ll(e,t,n){const r=[{role:"system",content:e}];return t&&r.push({role:"user",content:t}),r.push({role:"user",content:JSON.stringify(n)}),r}async function Tp(e,t,n){const r=Br(),a=g=>{const h=g.curation_groups;return Array.isArray(h)?h.map(_=>u(_,160)).filter(Boolean):u(h,400).split(/[,|]/).map(_=>_.trim()).filter(Boolean)},i=t.map(a),o=[...new Set(i.flat())];if(!o.length){w("curation.group_refine",{message:"no curation_groups on shots — skip"},"warn");return}const s=wm(e,o,{strictIds:r}),c=`${u(await le("curation_refine"),6e3)||s}

${s}`,l=_c(t,n),f={shots:t.map((g,h)=>({shot_index:h,y_percent:g.y_percent??g.anchor_percent??null,...l[h],curation_groups:i[h],camera:g.camera||"",situation:g.situation||g.scene||"",place:g.place||"",action:g.action||"",natural:g.natural||"",character_count:Array.isArray(g.characters)?g.characters.length:0,cast:$c(g)}))},d=await sn(j().llm,ll(c,n,f)),m=na(d);if(!m||typeof m!="object")return;ul(t,m,(g,h)=>{if(!h||typeof h!="object")return;const _=h.curation_option_ids??h.option_ids,y=jc(e,_);ol(y)?(Qa(g,y,{place:h.place!=null?h.place:g.place,situation:""}),g.curation_option_ids=_):(h.place!=null&&(g.place=u(h.place,400)),r||(h.camera!=null&&(g.camera=u(h.camera,600)),h.situation!=null&&(g.situation=u(h.situation,600)),h.action!=null&&(g.action=u(h.action,600)))),r&&Array.isArray(h.characters)&&Oc(g,Pc(h.characters),e)})}async function jp(e,t,n){const r=Br(),a=t.map(d=>ni(e,{preset_path:d.preset_path,composition_id:d.composition_id,composition_variant:d.composition_variant}));if(!a.some(Boolean))return;const i=Jm(e,a.filter(Boolean),{strictIds:r}),o=_c(t,n),s={shots:t.map((d,m)=>({shot_index:m,y_percent:d.y_percent??d.anchor_percent??null,...o[m],composition_id:d.composition_id||"",composition_variant:d.composition_variant||"",place:d.place||"",natural:d.natural||"",character_count:Array.isArray(d.characters)?d.characters.length:0,cast:$c(d)}))};let c=null;try{const d=await sn(j().llm,ll(i,n,s));c=na(d)}catch(d){w("curation.preset_refine",{message:String(d?.message||d)},"warn")}const l=new Map,f=(d,m,p)=>{const g=m&&typeof m=="object"?m.curation_option_ids??m.option_ids:d.curation_option_ids,h=g!=null?Sc(e,g):[];for(const v of Om(e,a[p])){const k=[...v.include_options||[],...v.action?.include_options||[]];k.length&&l.set(v.ref,{ids:k,binding:v})}const _=[...l.values()].flatMap(v=>v.ids),y=[...l.values()].map(v=>v.binding),x=Fm(e,{preset_path:d.preset_path,composition_id:d.composition_id,composition_variant:d.composition_variant,curation_option_ids:h},{optionIds:_,bindings:y});if(x&&ol(x)&&(Qa(d,x,{place:m&&m.place!=null?m.place:d.place,situation:""}),g!=null&&(d.curation_option_ids=h),e.fixed_positive&&(d.curation_fixed_positive=e.fixed_positive)),r){const v=m&&typeof m=="object"?m.characters:void 0;Array.isArray(v)&&Oc(d,Pc(v),e)}};if(c&&typeof c=="object"){ul(t,c,f);return}t.forEach((d,m)=>f(d,null,m))}function ul(e,t,n){const r=Array.isArray(t.shots)?t.shots:Array.isArray(t)?t:null;if(r){for(let a=0;a<e.length;a++){const i=r.find(o=>o&&typeof o=="object"&&Number(o.shot_index)===a);n(e[a],i||r[a],a)}return}e.length===1&&n(e[0],t,0)}async function fl(e){if(!e.length)return{ok:!0,snapped:0};const t=await dt(),n=await fi(),r=t.sha||qn(t);if(!n?.items?.length)return{ok:!1,reason:"임베딩 없음 → 큐레이션 없이 생성"};if(n.catalog_sha!==r)return{ok:!1,reason:"임베딩 stale → 큐레이션 없이 생성"};const a=e.map(o=>{const s=C(u(o.camera,600),u(o.situation||o.scene,600),u(o.place,400),u(o.action,600));return Vm(s)}),i=a.flat();if(!i.length)return{ok:!0,snapped:0};try{const o=Vn(),{vectors:s}=await ai(o,i);let c=0,l=0;for(let f=0;f<e.length;f++){const d=a[f],m=s.slice(c,c+d.length);if(c+=d.length,!d.length)continue;const{baseTags:p,charTags:g,primaryTags:h,secondaryTags:_,femaleTags:y,maleTags:x,snapped:v}=Wm(d,m,n.items);l+=v,Qa(e[f],{base:C(...p),char:C(...g),primary:C(...h),secondary:C(..._),female:C(...y),male:C(...x)},{place:"",situation:""})}return{ok:!0,snapped:l}}catch(o){const s=`임베딩 API 실패 → 큐레이션 없이 생성 (${o?.message||o})`;return w("curation.snap",{message:s},"warn"),{ok:!1,reason:s}}}async function dl(e){const t=j(),n=t.curation&&typeof t.curation=="object"?{...t.curation}:{},r={...n,...e};if(e.mode!=null&&(r.mode=La(e.mode)),e.strict_ids!=null&&(r.strict_ids=Ba(e.strict_ids)),e.embedding&&typeof e.embedding=="object"){const a=n.embedding&&typeof n.embedding=="object"?{...n.embedding}:{},i=e.embedding,o={...a,...i};if("api_key"in i){const s=i.api_key;delete o.api_key,delete o.api_key_configured,u(s)&&(o.api_key=s),(i.clearApiKey||e.clear_embedding_key)&&(o.api_key=""),delete o.clearApiKey}o.provider!=null&&(o.provider=Ce(o.provider)),r.embedding=o}return t.curation=r,e.mode!=null&&t.card&&typeof t.card=="object"&&(t.card.composition_curation=!1),ur(t),await Ke(),{ok:!0}}var ml,cn,Xn=P((()=>{lm(),ti(),Hm(),Un(),Xm(),te(),G(),L(),Jc(),xt(),br(),fe(),ft(),ui(),ml=400,cn={running:!1,done:0,total:0,message:""}}));Xn(),te(),G(),yt(),Q(),jt();async function Rr(e,t,n,r=1,a=3){const i=Je(n),o=Math.max(0,Math.min(1,Number(r)||1)),s={image:await Qe(t),information_extracted:o,model:i};let c="";for(let l=1;l<=a;l++)try{w("nai.encode_vibe.start",{message:i,ie:o,attempt:l,focus:!0});const f=await Fe(Bi,{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify(s)}),d=Number(f?.status||0);if([502,503,504,520].includes(d)){c=`HTTP ${d}`,await Ze(2*l*1e3);continue}if(d<200||d>=300){let g="";try{g=typeof f?.text=="function"?await f.text():""}catch{}if(c=`HTTP ${d}: ${String(g).slice(0,200)}`,l<a){await Ze(1e3);continue}break}const m=await rn(f,{timeoutMs:6e4}),p=await Qe(m);return w("nai.encode_vibe.done",{message:i,bytes:m?.byteLength||0,focus:!0}),p}catch(f){if(c=String(f?.message||f),l<a){await Ze(1e3);continue}}throw new Error(`encode-vibe 실패 (${a}회): ${c}`)}te(),L(),Oe(),fe(),ft();var di=12582912,qe=32;async function $p(){const e=await D("meta","reference_image");return!!(e?.png&&e.png.byteLength>qe)}async function pl(){return(await D("meta","reference_image"))?.png||null}async function Pp(e){if(!e||e.byteLength<qe)throw new Error("참조 이미지가 비어 있습니다");if(e.byteLength>di)throw new Error("참조 이미지가 너무 큽니다 (최대 12MB)");return await F("meta",{key:"reference_image",png:e}),aa(Et(e)),j().nai.image_reference="file",await Ke(),{ok:!0,image_reference:"file",configured:!0,bytes:e.byteLength,preview_url:"/v1/nai/reference.png"}}async function gl(){return await Mt("meta","reference_image"),aa(""),j().nai.image_reference="none",await Ke(),{ok:!0,image_reference:"none",configured:!1}}async function Op(){const e=await D("meta","vibe_transfer");return!!(e?.encoded&&e?.png&&e.png.byteLength>qe)}async function Lp(){return await D("meta","vibe_transfer")||null}async function Bp(){return(await D("meta","vibe_transfer"))?.png||null}function Fr(e){let t=Number(e??1);return Number.isNaN(t)&&(t=1),Math.max(0,Math.min(1,t))}async function Rp(e,t={}){if(!e||e.byteLength<qe)throw new Error("Vibe 이미지가 비어 있습니다");if(e.byteLength>di)throw new Error("Vibe 이미지가 너무 큽니다 (최대 12MB)");const n=j(),r=u(n.nai.api_key);if(!r)throw new Error("NAI api_key가 설정되지 않았습니다. encode-vibe에 키가 필요합니다.");const a=He(t.model||n.nai.model||"nai-diffusion-4-5-full"),i=Fr(t.information_extracted??n.nai.vibe_transfer_information_extracted),o=await Rr(r,e,a,i);return await F("meta",{key:"vibe_transfer",png:e,encoded:o,model:Je(a),information_extracted:i}),ia(Et(e)),n.nai.vibe_transfer="file",t.strength!=null&&!Number.isNaN(Number(t.strength))&&(n.nai.vibe_transfer_strength=Math.max(0,Math.min(1,Number(t.strength)))),n.nai.vibe_transfer_information_extracted=i,await Ke(),{ok:!0,vibe_transfer:"file",configured:!0,bytes:e.byteLength,encoded_bytes:o.length,model:Je(a),information_extracted:i,preview_url:"/v1/nai/vibe.png"}}async function hl(){return await Mt("meta","vibe_transfer"),ia(""),j().nai.vibe_transfer="none",await Ke(),{ok:!0,vibe_transfer:"none",configured:!1}}async function Fp(){const e=await Lp();if(!e?.png||e.png.byteLength<qe)return null;const t=j(),n=u(t.nai.api_key);if(!n)throw new Error("NAI api_key가 설정되지 않았습니다.");const r=Je(He(t.nai.model||"nai-diffusion-4-5-full")),a=Fr(t.nai.vibe_transfer_information_extracted);if(!(!u(e.encoded)||u(e.model)!==r||Math.abs(Number(e.information_extracted??1)-a)>.001))return e;const i=await Rr(n,e.png,r,a),o={...e,key:"vibe_transfer",encoded:i,model:r,information_extracted:a};return await F("meta",o),o}async function _l(e){const t=u(e,120);return t&&await D("meta",Dt(t))||null}async function Dp(e,t,n={}){const r=u(e,120);if(!r)throw new Error("preset_id required");if(!t||t.byteLength<qe)throw new Error("Vibe 이미지가 비어 있습니다");if(t.byteLength>di)throw new Error("Vibe 이미지가 너무 큽니다 (최대 12MB)");const a=j(),i=u(a.nai.api_key);if(!i)throw new Error("NAI api_key가 설정되지 않았습니다. encode-vibe에 키가 필요합니다.");const o=He(n.model||a.nai.model||"nai-diffusion-4-5-full"),s=Fr(n.information_extracted??a.nai.vibe_transfer_information_extracted),c=await Rr(i,t,o,s),l=Dt(r);await F("meta",{key:l,png:t,encoded:c,model:Je(o),information_extracted:s});const f=Et(t);return fr(r,f),{ok:!0,preset_id:r,vibe_transfer:"file",configured:!0,bytes:t.byteLength,encoded_bytes:c.length,model:Je(o),information_extracted:s,preview_url:f}}async function yl(e){const t=u(e,120);if(!t)throw new Error("preset_id required");return await Mt("meta",Dt(t)),fr(t,""),{ok:!0,preset_id:t,vibe_transfer:"none",configured:!1}}async function zp(e,t){const n=await _l(e);if(!n?.png||n.png.byteLength<qe)return!1;const r=u(t,120);if(!r)return!1;await F("meta",{key:Dt(r),png:n.png,encoded:n.encoded||"",model:n.model||"",information_extracted:n.information_extracted??1});const a=oa(e)||(n.png?Et(n.png):"");return a&&fr(r,a),!0}async function Up(e){const t=await _l(e);if(!t?.png||t.png.byteLength<qe)return null;const n=j(),r=u(n.nai.api_key);if(!r)throw new Error("NAI api_key가 설정되지 않았습니다.");const a=Je(He(n.nai.model||"nai-diffusion-4-5-full")),i=Fr(n.nai.vibe_transfer_information_extracted);if(!(!u(t.encoded)||u(t.model)!==a||Math.abs(Number(t.information_extracted??1)-i)>.001))return t;const o=await Rr(r,t.png,a,i),s=Dt(u(e,120)),c={...t,key:s,encoded:o,model:a,information_extracted:i};return await F("meta",c),c}async function Jp(){for(const e of await we("meta")){const t=String(e?.key||"");if(!zt(t))continue;const n=e.png;!n||n.byteLength<qe||fr(yn(t),Et(n))}}Ht(),Un(),te(),G(),L(),Gt(),it(),qa(),Oe(),fe(),ft();function bl(e,t){const n=Number(e);return Math.max(1,Math.min(5e3,Number.isFinite(n)&&n>0?Math.round(n):t))}async function Dr(e){const{shot:t,roster:n}=e,r=j().card,a=j().nai,i=at(r),o=pa(t.characters||[],n,i).slice(0,i),s=Math.max(1,o.length),c=qo(r.person_tag_mode,r.auto_person_tags),l=Qu(ef(o,n,c),r.person_tag_weight),[f,d]=gu(await le("preset_1")),m=Array.isArray(r.presets)?r.presets:[],p=u(r.active_preset_id,120);let g=null;m.length&&(p&&(g=m.find(q=>typeof q=="object"&&u(q.id,120)===p)||null),!g&&typeof m[0]=="object"&&(g=m[0]));let h,_;g?(h=u(g.positive||g.pos||""),_=u(g.negative||g.neg||"")):(h=C(u(r.custom_pos),f),_=C(u(r.custom_neg),d));let y=t.situation||t.scene;const x=u(e.lockedSetup||"");let v;x?v=x:(v=C(t.camera,y,t.place,t.action),r.mode==="asset"&&(v=C(v,"white background","simple background","cowboy shot","looking at viewer","portrait")));const k=Ra(r.natural_base),E=k==="supplement"?600:k==="detailed"?480:400,I=k==="off"?"":u(t.natural||t.natural_base||t.nl||"",E);let T="",R="";if(u(t.composition_id,160)||u(t.curation_fixed_positive,200))try{const q=await dt();T=u(t.curation_fixed_positive,800)||u(q.fixed_positive,800),R=u(q.fixed_negative,800)}catch{T=u(t.curation_fixed_positive,800)}let S=C(h,I,v,T);c!=="off"&&(S=Ko(S),v=Ko(v));let A=l?S?`${l}, ${S}`:l:S;const N=He(a.model||"nai-diffusion-4-5-full");a.apply_quality_tags!==!1&&(A+=Nn[N]||"");const U=u(a.uc_preset)||"human_focus",ee=C(_,R,(Eo[N]||{})[U]||""),ge=[],J=[];for(let q=0;q<o.length;q++){const K=o[q],Ve=u(K.name,200),oe=Ve?be(Ve,n):null,pt=C(tf(oe,K)),V=u(K.negative),De=s===1?.5:Math.round((.1+.8*q/Math.max(1,s-1))*10)/10,re=.5;ge.push({prompt:pt||"girl",uc:V,center_x:De,center_y:re}),J.push({name:oe?.name||Ve,prompt:pt,uc:V,center_x:De,center_y:re,raw:K})}return{main:A,neg:ee,captions:ge,meta:{setup:v,person:l,characters:J,paragraph:t.paragraph}}}async function wl(e,t){const n=j().nai,r=!!j().card?.auto_aspect,a=e.width&&e.height?{width:bl(e.width,832),height:bl(e.height,1216),aspect:"settings"}:td(t,n,r),i=e.captions;if(Jn(n)==="comfy"){const v={...n,width:a.width,height:a.height},[k,E]=await em(v,e.main,e.neg,i);return{bytes:k,seed:E}}const o=u(n.api_key);if(!o)throw new Error("NAI api_key가 설정되지 않았습니다.");const s=[],c=u(n.image_reference||"none").toLowerCase();if(!["","none","off","false","0"].includes(c)){const v=await pl();if(v){let k=u(n.image_reference_type||"character&style")||"character&style";["character","style","character&style"].includes(k)||(k="character&style");let E=Number(n.image_reference_strength??.6),I=Number(n.image_reference_fidelity??1);Number.isNaN(E)&&(E=.6),Number.isNaN(I)&&(I=1),s.push({image:v,type:k,strength:Math.max(0,Math.min(1,E)),fidelity:Math.max(0,Math.min(1,I))})}}const l=j().card,f=Array.isArray(l.presets)?l.presets:[],d=u(l.active_preset_id,120);let m=null;f.length&&(d&&(m=f.find(v=>typeof v=="object"&&u(v.id,120)===d)||null),!m&&typeof f[0]=="object"&&(m=f[0]));const p=u(m?.id||d,120),g=Fd(n,m),h=[];let _=p?await Up(p):null;if(!_){const v=u(n.vibe_transfer||"none").toLowerCase();["","none","off","false","0"].includes(v)||(_=await Fp())}if(_?.encoded){let v=Number(n.vibe_transfer_strength??.6),k=Number(n.vibe_transfer_information_extracted??_.information_extracted??1);Number.isNaN(v)&&(v=.6),Number.isNaN(k)&&(k=1),h.push({encoded:_.encoded,strength:Math.max(0,Math.min(1,v)),information_extracted:Math.max(0,Math.min(1,k))})}const y={prompt:e.main,negative_prompt:e.neg,width:a.width,height:a.height,seed:Number(n.seed??0)||0,steps:Math.min(Number(n.steps??28)||28,28),cfg_scale:g.cfg_scale,cfg_rescale:g.cfg_rescale,sampler:u(n.sampler)||"k_euler_ancestral",scheduler:u(n.scheduler)||"karras",model:He(n.model||"nai-diffusion-4-5-full"),var_plus:!!n.variety_plus,characters:i,character_refs:s,vibes:h};w("nai.generate.dims",{message:`${y.width}x${y.height}`,aspect:a.aspect,auto_aspect:r,steps:y.steps,focus:!0}),y.seed||(y.seed=Math.floor(Math.random()*4294967295)||1);const x=u(n.request_url)||"https://image.novelai.net/ai/generate-image";return{bytes:(await hc(o,y,x,{timeoutMs:9e4})).raw_bytes,seed:y.seed||0}}function Hp({imageId:e,sessionId:t,request:n,shotIndex:r,paragraph:a,yPercent:i,line:o=null,contentHash:s=""}){const c=Math.floor(Number(o));return{version:1,image_id:u(e,80),session_id:u(t,200),unified_session_id:u(n.unified_session_id||"",200),character_id:u(n.character_id||"",200),character_name:u(n.character_name||"",200),chat_id:u(n.chat_id||"",200),chat_name:u(n.chat_name||"",200),char_index:O(n.char_index,-1),chat_index:O(n.chat_index,-1),message_index:O(n.message_index,-1),message_role:u(n.message_role||n.role||"",40).toLowerCase(),shot_index:O(r,0),paragraph:O(a,0),y_percent:i,line:Number.isFinite(c)&&c>=1?c:null,content_hash:u(s||n.content_hash||"",128),assistant_preview:u(n.assistant_text||"",et)}}async function Rt(e){return vf(e)}async function vl(e,t){const n=await D("images",e),r=n?{...n}:{id:e},a=t||{};r.location={...a,version:Number(a.version||1),image_id:u(e,80)},await F("images",r)}async function mt(e,t={}){const n=await Rt(e),r=typeof t=="object"&&t?t:{};let a=n.y_percent;a==null&&(a=r.y_percent??r.anchor_percent??r.read_percent);const i=Object.keys(n).length>0,o=Te(u(e,80));return{character_id:u(n.character_id||r.character_id||"",200),chat_id:u(n.chat_id||r.chat_id||"",200),character_name:u(n.character_name||r.character_name||"",200),chat_name:u(n.chat_name||r.chat_name||"",200),char_index:O(n.char_index??r.char_index,-1),chat_index:O(n.chat_index??r.chat_index,-1),message_index:O("message_index"in n?n.message_index:r.message_index,-1),message_role:u(n.message_role||r.message_role||"",40).toLowerCase(),shot_index:O(n.shot_index,-1),paragraph:O("paragraph"in n?n.paragraph:r.paragraph,0),y_percent:cr(a),line:(()=>{const s=Math.floor(Number(n.line??r.line));return Number.isFinite(s)&&s>=1?s:null})(),content_hash:u(n.content_hash||r.content_hash||"",128),assistant_preview:u(n.assistant_preview||r.assistant_preview||"",et),unified_session_id:u(n.unified_session_id||r.unified_session_id||"",200),location_file:i?`idb:${o}`:"",storage:"indexeddb",storage_key:o}}function mi(e,t,n=0){const r=typeof e=="object"&&e?{...e}:{},a=t||{};return{...r,character_id:u(a.character_id||r.character_id||"",200),chat_id:u(a.chat_id||r.chat_id||"",200),character_name:u(a.character_name||r.character_name||"",200),chat_name:u(a.chat_name||r.chat_name||"",200),char_index:O(a.char_index??r.char_index,-1),chat_index:O(a.chat_index??r.chat_index,-1),message_index:O(a.message_index??r.message_index,-1),message_role:u(a.message_role||r.message_role||"",40).toLowerCase(),content_hash:u(a.content_hash||r.content_hash||"",128),assistant_preview:u(a.assistant_preview||r.assistant_preview||"",et),unified_session_id:u(a.unified_session_id||r.unified_session_id||"",200),y_percent:cr(a.y_percent??r.y_percent),line:(()=>{const i=Math.floor(Number(a.line??r.line));return Number.isFinite(i)&&i>=1?i:null})(),storage:"indexeddb",storage_key:Te(u(a.image_id||"",80)),png_bytes:Number(n)||0}}Q();var Kp=(()=>{const e=new Uint32Array(256);for(let t=0;t<256;t+=1){let n=t;for(let r=0;r<8;r+=1)n=n&1?3988292384^n>>>1:n>>>1;e[t]=n>>>0}return e})();function pi(e){return Array.isArray(e)?new Uint8Array(e):X(e)}function qp(e){const t=pi(e);let n=4294967295;for(let r=0;r<t.length;r+=1)n=Kp[(n^t[r])&255]^n>>>8;return(n^4294967295)>>>0}function H(e){const t=new Uint8Array(2);return new DataView(t.buffer).setUint16(0,e>>>0,!0),t}function me(e){const t=new Uint8Array(4);return new DataView(t.buffer).setUint32(0,e>>>0,!0),t}function Zn(e){let t=0;for(const n of e)t+=n.length;return Sn(e,t)}function Gp(e){return new TextEncoder().encode(String(e??""))}function Wp(e=[]){const t=[],n=[];let r=0;for(const o of e){const s=Gp(o.name||"file"),c=pi(o.data),l=qp(c),f=Zn([me(67324752),H(20),H(0),H(0),H(0),H(0),me(l),me(c.length),me(c.length),H(s.length),H(0),s,c]),d=Zn([me(33639248),H(20),H(20),H(0),H(0),H(0),H(0),me(l),me(c.length),me(c.length),H(s.length),H(0),H(0),H(0),H(0),me(0),me(r),s]);t.push(f),n.push(d),r+=f.length}const a=Zn(n),i=Zn([me(101010256),H(0),H(0),H(e.length),H(e.length),me(a.length),me(r),H(0)]);return Zn([...t,a,i])}function gi(e,t){return e[t]|e[t+1]<<8}function xl(e,t){return(e[t]|e[t+1]<<8|e[t+2]<<16|e[t+3]<<24)>>>0}function Vp(e){const t=pi(e),n=new Map;let r=0;for(;r+30<=t.length;){const a=xl(t,r);if(a===33639248||a===101010256)break;if(a!==67324752){r+=1;continue}const i=gi(t,r+8),o=xl(t,r+18),s=gi(t,r+26),c=gi(t,r+28),l=r+30,f=new TextDecoder().decode(t.subarray(l,l+s)),d=l+s+c,m=d+o;if(m>t.length)break;i===0&&n.set(f.replace(/\\/g,"/"),t.subarray(d,m)),r=m}return n}function Yp(e=[],{exportedAt:t=Date.now()}={}){return{format:"inlay-nexus-gallery",version:1,exported_at:t,items:(e||[]).map(n=>({id:String(n.id||""),file:`images/${String(n.id||"unknown")}.png`,location:{character_id:n.character_id||"",chat_id:n.chat_id||"",character_name:n.character_name||"",chat_name:n.chat_name||"",char_index:n.char_index??-1,chat_index:n.chat_index??-1,message_index:n.message_index??-1,content_hash:n.content_hash||"",paragraph:n.paragraph??0,shot_index:n.shot_index??0,y_percent:n.y_percent??null,assistant_preview:n.assistant_preview||"",session_id:n.session_id||""},meta:{main_prompt:n.main_prompt||"",seed:n.seed??null,created_at:n.created_at??null,characters:n.characters||[]}}))}}function Xp(e,t=[]){const n=e?.location||{},r=String(n.content_hash||"").trim(),a=t||[];if(r){const l=a.find(f=>String(f.content_hash||"")===r);if(l)return{status:"exact",matchId:String(l.id),content_hash:r}}const i=String(n.character_id||""),o=String(n.chat_id||""),s=Number(n.message_index),c=a.filter(l=>i&&String(l.character_id||"")!==i||o&&String(l.chat_id||"")!==o||Number.isFinite(s)&&s>=0&&Number(l.message_index)!==s?!1:!!(i||o||Number.isFinite(s)&&s>=0)).slice(0,8);return c.length?{status:"candidate",candidates:c.map(l=>({id:l.id,content_hash:l.content_hash,message_index:l.message_index})),location:n}:{status:"orphan",location:n}}function Zp(e=[]){return Wp(e)}function Qp(e){const t=Vp(e);let n=null;const r=t.get("manifest.json");if(r)try{n=JSON.parse(new TextDecoder().decode(r))}catch{n=null}const a=new Map;for(const[i,o]of t.entries())i!=="manifest.json"&&/\.(png|webp|jpe?g)$/i.test(i)&&a.set(i.replace(/\\/g,"/"),o);return{manifest:n,images:a,files:t}}te(),G(),Q(),L(),Oe();function Qn(e){try{const t=JSON.parse(e.meta_json||"{}");return t&&typeof t=="object"?t:{}}catch{return{}}}async function kl(e,t){return(await yf(e))?.png_bytes||Number(t.png_bytes)||0}async function hi(e){const t=(await we("cards")).sort((i,o)=>(o.created_at||0)-(i.created_at||0)).slice(0,Math.max(1,Math.min(2e3,e))),n={},r=[];for(const i of t){const o=Qn(i),s=await mt(i.id,o),c=await Rt(i.id),l=u(s.character_name||c.character_name||o.character_name||"",200),f=u(s.chat_name||c.chat_name||o.chat_name||"",200),d=u(s.character_id||"",200)||"unknown",m=u(s.chat_id||"",200)||"unknown",p=`${d}|${m}`;let g=n[p];g||(g={key:p,character_id:d,chat_id:m,character_name:l||d.slice(0,12)||"Unknown",chat_name:f||`chat ${s.chat_index??"?"}`,char_index:s.char_index??-1,chat_index:s.chat_index??-1,count:0,storage:"indexeddb"},n[p]=g),l&&!g.character_name&&(g.character_name=l),f&&(g.chat_name=f),g.count+=1,r.push({id:i.id,job_id:i.job_id,session_id:i.session_id,folder_key:p,shot_index:s.shot_index>=0?s.shot_index:i.shot_index,paragraph:Object.keys(c).length?s.paragraph:i.paragraph,y_percent:s.y_percent,line:s.line,message_index:s.message_index??-1,message_role:s.message_role||"",content_hash:s.content_hash||"",character_id:d,character_name:g.character_name,chat_id:m,chat_name:g.chat_name,char_index:s.char_index??-1,chat_index:s.chat_index??-1,assistant_preview:u(s.assistant_preview||c.assistant_preview||o.assistant_preview||"",et),main_prompt:i.main_prompt,characters:JSON.parse(i.characters_json||"[]"),image_url:Le(i.id),seed:i.seed,created_at:i.created_at,storage:"indexeddb",storage_key:s.storage_key||Te(i.id),location_file:s.location_file||"",png_bytes:await kl(i.id,o)})}const a={ok:!0,folders:Object.values(n).sort((i,o)=>`${i.character_name||""}`.localeCompare(`${o.character_name||""}`,void 0,{sensitivity:"base"})||`${i.chat_name||""}`.localeCompare(`${o.chat_name||""}`,void 0,{sensitivity:"base"})),items:r,total:r.length,storage:"indexeddb",storage_api:"getLocalPluginStorage"};return await st(a,{cachedOnly:!0}),a}async function eg(e=400){return hi(e)}async function tg(e,t=40){const n=(await ds(e)).sort((i,o)=>(o.created_at||0)-(i.created_at||0)).slice(0,Number(t)),r=[];for(const i of n){const o=Qn(i),s=await mt(i.id,o),c=await Rt(i.id);r.push({id:i.id,job_id:i.job_id,shot_index:s.shot_index>=0?s.shot_index:i.shot_index,paragraph:Object.keys(c).length?s.paragraph:i.paragraph,y_percent:s.y_percent,line:s.line,message_index:s.message_index??-1,message_role:s.message_role||"",content_hash:s.content_hash||"",character_id:s.character_id||"",chat_id:s.chat_id||"",character_name:s.character_name||"",chat_name:s.chat_name||"",char_index:s.char_index??-1,chat_index:s.chat_index??-1,assistant_preview:s.assistant_preview||o.assistant_preview||"",main_prompt:i.main_prompt,negative_prompt:i.negative_prompt,characters:JSON.parse(i.characters_json||"[]"),image_url:Le(i.id),seed:i.seed,created_at:i.created_at,storage:"indexeddb",storage_key:s.storage_key||Te(i.id),png_bytes:await kl(i.id,o)})}const a={ok:!0,session_id:e,items:r,storage:"indexeddb"};return await st(a,{cachedOnly:!0}),a}async function ng(e={}){const t=u(e.session_id,200),n=u(e.to_hash,128),r=u(e.assistant_preview||"",et),a=(Array.isArray(e.card_ids)?e.card_ids:[]).map(o=>u(o,80)).filter(Boolean);if(!n||!a.length)return{ok:!1,...ae("to_hash and card_ids required","bad_request"),rebound:0,ids:[]};const i=[];for(const o of a){const s=await D("cards",o);if(!s||t&&u(s.session_id||"",200)!==t)continue;const c=Qn(s),l=await Rt(o),f={...l,image_id:o,content_hash:n,assistant_preview:r||l.assistant_preview||""};await vl(o,f),c.content_hash=n,r&&(c.assistant_preview=r),c.message_role=u(c.message_role||l.message_role||"",40).toLowerCase(),s.meta_json=JSON.stringify(c),await F("cards",s),i.push(o)}return w("gallery.rebind",{n:i.length,to:n.slice(0,8)}),{ok:!0,rebound:i.length,ids:i,content_hash:n}}async function _i(e,t="",n=null){const r=u(e,200),a=u(t);let i=null;try{n!=null&&String(n).trim()!==""&&(i=parseInt(String(n),10))}catch{i=null}if(!r||!a&&i==null)return{ok:!0,unlinked:0,ids:[]};const o=(await ds(r)).sort((c,l)=>(c.created_at||0)-(l.created_at||0)),s=[];for(const c of o){const l=Qn(c),f=await mt(c.id,l),d=u(f.content_hash||""),m=O(f.message_index,-1);let p=!1;if((a&&d&&d===a||i!=null&&i>=0&&m===i)&&(p=!0),!p)continue;const g={...await Rt(c.id),version:1,image_id:c.id,session_id:r,content_hash:"",message_index:-1,character_id:"",chat_id:"",char_index:-1,chat_index:-1,unlinked_at:Date.now()/1e3};await vl(c.id,g),(l.content_hash||l.message_index!=null||l.assistant_preview)&&(l.content_hash="",l.assistant_preview="",l.message_index=-1,l.unlinked_at=Date.now()/1e3,c.meta_json=JSON.stringify(l),await F("cards",c)),s.push(c.id)}return{ok:!0,unlinked:s.length,ids:s}}async function yi(e){const t=u(e,80);return t?await D("cards",t)?(await Mt("cards",t),await Mt("images",t),{ok:!0,deleted:1,ids:[t]}):{ok:!1,...ae("card not found","not_found")}:{ok:!1,...ae("card_id required","bad_request")}}async function bi(e){return yi(e)}async function rg(e=[]){const t=[...new Set((e||[]).map(r=>u(r,80)).filter(Boolean))],n=[];for(const r of t)(await yi(r)).ok&&n.push(r);return{ok:!0,deleted:n.length,ids:n}}async function Sl(){const e=(await D("meta","explorer_favorites"))?.ids;return{ok:!0,ids:Array.isArray(e)?e.map(t=>u(t,80)).filter(Boolean):[]}}async function ag(e=[]){const t=[...new Set((e||[]).map(n=>u(n,80)).filter(Boolean))].slice(0,5e3);return await F("meta",{key:"explorer_favorites",ids:t,updated_at:Date.now()/1e3}),{ok:!0,ids:t}}async function ig(e={}){let t=(await hi(2e3)).items;const n=u(e.folder_key||"",400);if(!e.all)if(n)t=t.filter(o=>o.folder_key===n);else if(Array.isArray(e.card_ids)&&e.card_ids.length){const o=new Set(e.card_ids.map(s=>u(s,80)));t=t.filter(s=>o.has(s.id))}else return{ok:!1,...ae("card_ids, folder_key, or all required","bad_request")};if(!t.length)return{ok:!1,...ae("no images to export","empty")};const r=Yp(t),a=[{name:"manifest.json",data:new TextEncoder().encode(JSON.stringify(r,null,2))}];for(const o of t){const s=await Al(o.id);s?.byteLength&&a.push({name:`images/${o.id}.png`,data:X(s)})}if(a.length<2)return{ok:!1,...ae("image bytes missing","empty")};const i=Zp(a);return{ok:!0,count:a.length-1,filename:`inlay-gallery-${Date.now()}.zip`,zip_base64:Ut(i),bytes:i.length}}async function og(e={}){const t=e.prefer_new_ids===void 0?!0:e.prefer_new_ids,n=String(e.zip_base64||"").replace(/^data:.*base64,/,"");if(!n)return{ok:!1,...ae("zip_base64 required","bad_request")};let r;try{r=$e(n)}catch(l){return{ok:!1,...ae(`invalid base64: ${l?.message||l}`,"bad_request")}}const{manifest:a,images:i}=Qp(r);if(!a?.items?.length)return{ok:!1,...ae("manifest.items missing","bad_request")};const o=(await hi(2e3)).items,s=[],c={exact:0,candidate:0,orphan:0,skipped:0};for(const l of a.items){const f=String(l.file||`images/${l.id}.png`).replace(/\\/g,"/"),d=i.get(f)||i.get(`images/${l.id}.png`);if(!d?.byteLength){c.skipped+=1;continue}const m=Xp(l,o);c[m.status]=(c[m.status]||0)+1;const p={...l.location||{}},g=t?wt():u(l.id||"",80)||wt(),h=u(p.session_id||"",200)||`import_${wt().replace(/-/g,"").slice(0,10)}`,_={version:1,image_id:g,session_id:h,character_id:u(p.character_id||"",200),character_name:u(p.character_name||"",200),chat_id:u(p.chat_id||"",200),chat_name:u(p.chat_name||"",200),char_index:O(p.char_index,-1),chat_index:O(p.chat_index,-1),message_index:O(p.message_index,-1),shot_index:O(p.shot_index,0),paragraph:O(p.paragraph,0),y_percent:cr(p.y_percent),content_hash:u(p.content_hash||"",128),assistant_preview:u(p.assistant_preview||"",et),imported_at:Date.now()/1e3,reattach:m.status},y=Z(d);await xa(g,y,_);const x=mi({...l.meta||{},assistant_preview:_.assistant_preview,imported_at:_.imported_at,reattach:m.status},_,y.byteLength||0);await F("cards",{id:g,job_id:`import_${g.slice(0,8)}`,session_id:h,shot_index:_.shot_index,paragraph:_.paragraph,main_prompt:u(l.meta?.main_prompt||"",8e3),negative_prompt:"",characters_json:JSON.stringify(l.meta?.characters||[]),seed:l.meta?.seed??0,meta_json:JSON.stringify(x),created_at:Number(l.meta?.created_at)||Date.now()/1e3}),s.push({id:g,reattach:m.status,content_hash:_.content_hash})}return{ok:!0,imported:s.length,items:s,report:c}}async function sg(e){const t=u(e,400);if(!t||!t.includes("|"))return{ok:!1,...ae("folder_key required","bad_request")};const[n,r]=t.split("|",2),a=u(n,200)||"unknown",i=u(r,200)||"unknown",o=await we("cards"),s=[];for(const c of o){const l=Qn(c),f=await mt(c.id,l),d=u(f.character_id||"",200)||"unknown",m=u(f.chat_id||"",200)||"unknown";d!==a||m!==i||(await yi(c.id)).ok&&s.push(c.id)}return{ok:!0,deleted:s.length,ids:s,folder_key:`${a}|${i}`}}async function Al(e){return Af(e)}L(),Oe(),fe();var cg=["queued","tagging","generating"];function zr(e={},t=""){const n=u(t||e.session_id||"",200)||"_",r=u(e.content_hash||"",128);if(r)return`${n}::h:${r}`;const a=O(e.message_index,-1);return a>=0?`${n}::m:${a}`:`${n}::all`}async function lg(e){if(dr.has(e))return"message-reroll";const t=Cn.get(e);if(!t?.jobId)return"";const n=kt.get(t.jobId);if(!n||n.cancelRequested)return"";const r=await D("jobs",t.jobId);return r&&cg.includes(String(r.state||""))?t.jobId:""}async function wi(e={},t=""){const n=await lg(zr(e,t));return n?{ok:!1,busy:!0,accepted:!1,job_id:n==="message-reroll"?"":n,session_id:u(t||e.session_id||"",200),job_state:"busy",error:{code:"busy",message:"같은 메시지 작업이 아직 진행 중입니다. 끝날 때까지 기다려 주세요."}}:null}G(),xt(),L(),Un(),Gt(),it(),fe(),Xn(),ft();function ug(e){for(let t=e.length-1;t>=0;t--){const n=e[t];if(!n||n.role!=="user")continue;const r=typeof n.content=="string"?n.content:"";if(r.trim()&&!r.startsWith("# Priority: Author"))return r}return""}async function fg(e){const t=nt(j().card,e.card||{}),n=u(e.session_id,200),r=[{role:"system",content:`${lr(await le("tagger"))}
...
## {{user}} Info
${u(e.persona_description,8e3)}`});const a=u(e.assistant_text,2e4),i=Array.isArray(e.source_session_ids)?e.source_session_ids.map(y=>u(y,200)).filter(Boolean):[],o=t.lorebook||t.char_appearance!==!1||t.asset_nai_tags?await Oa(n,u(e.unified_session_id||"",200),u(e.character_id||"",200),i):[],s=Rf(o);if(t.lorebook){const y=Array.isArray(e.lore_trigger_keys)?e.lore_trigger_keys:null,x=Cs(t.lore_extra),v=Ff(e.lorebook||[],a,s,5,1200,y,x),k=[],E=[];for(const T of v){const R=Tt(T)||T.always,S=u(T.content||"",R?5e4:1200);if(!S)continue;const A=u(T.comment||"",200),N=A?`### ${A}
...
`)}`})}const c=async y=>{if(!t.asset_nai_tags)return;const x=[...Array.isArray(e.lore_trigger_keys)?e.lore_trigger_keys:[],...Ca(e.lorebook||[],a)];try{const v=await xd(x);if(v?.block){let k=await le("asset_tags_inject");k=k.includes("{asset_tags_block}")?k.replace("{asset_tags_block}",v.block):`${k}

${v.block}`,r.push({role:"system",content:k}),w("asset-tags.inject",{reason:y,assets:v.packed.assets.map(E=>E.name),triggers:x.length})}else Ws(new Map),w("asset-tags.inject.skip",{reason:y,cause:"collect_empty",triggers:x.length})}catch(v){Ws(new Map),w("asset-tags.inject.fail",{reason:y,message:String(v?.message||v)},"warn")}};if(t.char_appearance!==!1){const y=o,x=y.filter(E=>Me(E)),v=y.filter(E=>u(E.name,200)&&!Me(E)),k=Du(a,y);if(x.length||v.length||k.length){const E=N=>{const U=u(N.name,200),ee=u(N.appearance||"",200),ge=u(N.attire||"",160),J=u(N.accessories||"",120),q=[ee?`appearance=${ee}`:"",ge?`attire=${ge}`:"",J?`accessories=${J}`:""].filter(Boolean);return q.length?`${U} ← ${q.join(" | ")}`:U},I=k.filter(N=>Me(N)),T=k.filter(N=>!Me(N)),R=I.length?I.map(E).filter(Boolean).join(`
...
${R}`,r.push({role:"system",content:A}),w("job.roster.split",{filled:x.map(N=>N.name),incomplete:v.map(N=>N.name),matched:k.map(N=>N.name),matched_with_looks:I.map(N=>N.name),session_id:n})}}await c("asset_nai_tags_on");const l=Ra(t.natural_base),f=at(t),d=Math.max(1,Number(t.image_min??1)||1),m=Math.max(d,Number(t.image_max??3)||3);r.push({role:"system",content:["Every shot MUST include `y_percent` (0–100): reading position top→bottom. Spread across the full range in order (shot0 < shot1 < …); ~even gaps. E.g. 2→~25/~75; 3→~20/~50/~80; 4→~15/~40/~65/~90. Forbidden: all under 40, duplicates, or gaps under ~15 unless 1 shot.",d===m?`SHOT COUNT: produce exactly ${m} shot(s) in scenes[].shots (across all scenes).`:`SHOT COUNT: produce between ${d} and ${m} shots in scenes[].shots (across all scenes). Prefer the count that fits the message; never fewer than ${d} or more than ${m}.`,mg(l),`CHARACTER CAP: at most ${f} characters per shot (char1..char${f}). If more are visible, keep the ${f} most important; fold extras into situation/place.`,t.auto_aspect?"ASPECT (required on every shot): set `aspect` to exactly one of `portrait` (832×1216 vertical), `square` (1024×1024), or `landscape` (1216×832 horizontal). Pick from the scene framing — tall full-body / standing → portrait; equal crop / face close-up square → square; wide group / side-by-side / scenic → landscape. Like Asset Maid size presets 1/5/2.":""].filter(Boolean).join(`
`)});const p=await sl();p&&r.push({role:"system",content:p});const g=[];if(Number(t.include_max||0)>0)for(const y of e.recent_messages||[]){const x=u(y?.role,40)||"char",v=u(y?.content||y?.data,12e3);v&&(a&&v===a&&["char","assistant","bot"].includes(x.toLowerCase())||g.push(`[${x}]
${v}`))}a&&g.push(a);const h=g.length?g.join(`

`):a;if(!h)throw new Error("태깅할 메시지 텍스트가 없습니다.");r.push({role:"user",content:h});const _=u(await le("author_note"),8e3);return _&&r.push({role:"user",content:`# Priority: Author's Note
${_}
> These are instructions explicitly given by the user. If in conflict with previous instructions, this section MUST take precedence.`}),r}function dg(e){const t=at(j().card),n=e,r=[];for(const a of n.scenes||[]){const i=u(a.place);for(const o of a.shots||[]){const s={...o,place:i,characters:pa(o.characters||[],[],t).slice(0,t)};r.push(s)}}return r}function mg(e){return e==="off"?"Natural base mode OFF. Omit the `natural` field (or leave it empty). Do not invent natural-language base phrases.":e==="detailed"?["Natural base mode DETAILED. Every shot MUST include `natural`: a compact English phrase for NovelAI base caption only (NOT Danbooru comma tags, NOT characters[].action).","Include framing/vantage when useful (side view, close-up, viewed through, reflected in…), then for each visible person in left-to-right (or front-to-back) order: hair color + age band + gender, facial expression, clothing, and pose/action as separate details.","Multi-person: NEVER use character names — use position (left girl, right boy) or hair. Add shared action and lighting/atmosphere briefly.","Telegraphic and objective. Example: `side view upper body, left red hair adult woman tense smile in coat pulling right blue hair teen boy startled arms pinned, warm cafe light`. Keep ~12–28 words, English only."].join(" "):e==="supplement"?["Natural base mode SUPPLEMENT. Every shot MUST include `natural`: telegraphic English sentences describing what tags cannot express (composition, framing, actions, atmosphere, lighting) for NovelAI base caption only (NOT Danbooru comma tags, NOT characters[].action).","Keep tags and natural language clearly separated. Multi-person: NEVER use names — identify by position (left girl, bottom boy) or appearance. For each person include facial expression and clothing as distinct details; also hair and pose unless already obvious.","Unusual framing welcome (viewed through, reflected in shards of a broken mirror, behind…). Concise, minimal, objective — no subjective interpretation.","Example: `Side view, upper body. Left: adult woman with red hair, tense smile, coat, pulling. Right: teen boy with blue hair, startled face, arms pinned. Warm cafe lighting.` Prefer ~2–5 short sentences, English only."].join(" "):["Natural base mode SHORT. Every shot MUST include `natural`: a short English natural-language phrase for NovelAI base caption only (NOT Danbooru comma tags, NOT characters[].action).","Include hair color + age band + gender for each visible person, plus the shared action. Example: `red hair adult woman forced hug blue hair boy`. Keep ~6–20 words, English only."].join(" ")}te(),G(),L(),xt(),jt(),ui(),it(),Gt(),Oe(),fe(),Xn(),ft();var Ml=5e3,pg=2500,gg=64;function hg(e,t,n){const r=zr(t,n),a=(Cn.get(r)?.epoch||0)+1;return Cn.set(r,{epoch:a,jobId:e}),kt.set(e,{key:r,epoch:a,cancelRequested:!1,publishedIds:[]}),{key:r,epoch:a}}function _g(e){const t=kt.get(e);if(!t||t.cancelRequested)return!1;const n=Cn.get(t.key);return!!(n&&n.jobId===e&&n.epoch===t.epoch)}async function yg(e){const t=kt.get(e),n=t?.publishedIds?[...t.publishedIds]:[];t&&(t.publishedIds=[]);for(const r of n)try{await bi(r)}catch{}return n.length}async function Ee(e,t="interrupted"){if(_g(e))return!1;const n=await yg(e);return await pe(e,"cancelled",{phase:"cancelled",progress:0,message:`${t}${n?` · discarded ${n}`:""}`,shot_count:0,shot_done:0},null),w("job.cancelled",{job_id:e,message:t,discarded:n,focus:!0}),!0}function er(e={}){return{shot_count:e.shot_count??0,shot_index:e.shot_index??0,shot_done:e.shot_done??0,progress:e.progress??0,phase:e.phase||"generating",message:e.message||"",cards_so_far:e.cards_so_far,debug_stage:ht()||kn(),debug_error:or()?.message||""}}function bg(e){return!e||typeof e!="object"?e:JSON.parse(JSON.stringify(e,(t,n)=>{if(t==="image_url"&&typeof n=="string"&&n.startsWith("data:"))return"";if(!(t==="tagged"||t==="appearance"||t==="debug_tail"))return n}))}async function pe(e,t,n=null,r=null){const a=await D("jobs",e);if(!a)return;const i=a.state,o={...a,state:t},s=bg(n);o.result_json=s!=null?JSON.stringify(s):null,o.error=r,o.updated_at=Date.now()/1e3;const c=t!=="generating"||i!=="generating"||!!r;await F("jobs",o,{persist:c}),(t==="done"||t==="error"||t==="cancelled")&&await us(),w("job.set",{message:`${t}${r?" ERR":""}${c?"":" (mem)"}`,job_id:e,state:t,persist:c,err:r?String(r).slice(0,160):"",background:t==="generating",focus:t!=="generating"})}async function Nl(e){if(!(j().card||{}).power&&!e.force)throw new Error("Power가 OFF 상태입니다.");const t=u(e.session_id,200)||`sess_${wt().replace(/-/g,"").slice(0,12)}`,n={...e,session_id:t},r=await wi(n,t);if(r)return w("job.busy",{message:r.error.message||"busy",key:zr(n,t),focus:!0},"warn"),r;const a=wt(),i=Date.now()/1e3;if(hg(a,n,t),n.force)try{await _i(t,u(n.content_hash||""),n.message_index)}catch{}return await F("jobs",{id:a,session_id:t,state:"queued",request_json:JSON.stringify(n),result_json:null,error:null,created_at:i,updated_at:i}),xg(a).catch(async o=>{console.error("[Inlay Nexus] job crashed",a,o);try{await pe(a,"error",null,String(o?.message||o).slice(0,1500))}catch{}}),{ok:!0,accepted:!0,job_id:a,session_id:t,job_state:"queued"}}async function wg(e){const t=await D("jobs",e);if(!t)return{ok:!1,error:{code:"not_found",message:"job not found"}};const n=t.result_json?JSON.parse(t.result_json):null,r={};if(n&&typeof n=="object")for(const a of["shot_count","shot_index","shot_done","progress","phase","message","cards_so_far","debug_stage","debug_error"])a in n&&(r[a]=n[a]);return n&&await st(n),{ok:!0,job_id:t.id,session_id:t.session_id,state:t.state,error:t.error,result:n,progress:r,debug:{last_stage:kn(),last_error:or(),events:ea(e,40)},created_at:t.created_at,updated_at:t.updated_at}}function vg(e){for(const t of["y_percent","anchor_percent","read_percent"]){const n=e[t];if(n!=null)try{return Math.max(0,Math.min(100,Number(n)))}catch{return null}}return null}async function xg(e){const t=await D("jobs",e);if(!t)return;const n=JSON.parse(t.request_json??"{}"),r=String(t.session_id??""),a=Ji();xn(e);const i=ue("job.run");w("job.start",{job_id:e,session_id:r,message_index:n.message_index,text_len:String(n.assistant_text||"").length});try{if(await Ee(e,"superseded before start")||(n.force&&await _i(r,u(n.content_hash||""),n.message_index),await Ee(e,"superseded before tagging")))return;await pe(e,"tagging",{phase:"tagging",progress:0,message:"장면 태깅 중…",shot_count:0,shot_done:0,debug_stage:"job.tagging"});const o=await fg(n),s=ug(o);if(w("job.tagger.messages",{msgs:o.length}),j().card?.preprocessing){const S=lr(await le("preprocess"));if(S){const A=[{role:"system",content:S},o[o.length-1]],N=await sn(j().llm,A);if(await Ee(e,"superseded during preprocess"))return;o.splice(o.length-1,0,{role:"system",content:`## Preprocess Summary
${N}`})}}const c=await sn(j().llm,o);if(await Ee(e,"superseded after tagging"))return;const l=na(c);let f=dg(l);if(w("job.tagger.done",{shots:f.length,raw_len:String(c||"").length}),!f.length)throw new Error("태거가 shot을 반환하지 않았습니다.");const d=j().card||{},m=Math.max(1,Number(d.image_min??1)),p=Math.max(m,Number(d.image_max??3));f=f.slice(0,p);const g=Lr();if(g==="two_stage"){if(await pe(e,"tagging",{phase:"tagging",progress:.35,message:"큐레이션 2단 씬 태그 보강 중…",shot_count:f.length,shot_done:0,debug_stage:"job.curation_refine"}),await Ee(e,"superseded during curation refine"))return;await cl(f,{chatContext:s})}else if(g==="embed_snap"){if(await pe(e,"tagging",{phase:"tagging",progress:.4,message:"씬 태그 임베딩 매칭 중…",shot_count:f.length,shot_done:0,debug_stage:"job.curation_snap"}),await Ee(e,"superseded during curation snap"))return;const S=await fl(f);S.ok||(w("job.curation_snap.fallback",{message:S.reason},"warn"),await pe(e,"tagging",{phase:"tagging",progress:.45,message:S.reason||"임베딩 없음 → 큐레이션 없이 생성",shot_count:f.length,shot_done:0,debug_stage:"job.curation_snap_fallback"}))}const h=f.flatMap(S=>S.characters||[]),_=u(n.unified_session_id||"",200),y=u(n.character_id||"",200),x=Array.isArray(n.source_session_ids)?n.source_session_ids.map(S=>u(S,200)).filter(Boolean):[],v=await $d({sessionId:r,tagged:l,shotChars:h,unifiedSessionId:_,characterId:y,sourceSessionIds:x});w("job.roster",{roster:v.length});const k=at(d);for(const S of f)S.characters=pa(S.characters||[],v,k);if(await Ee(e,"superseded before generate"))return;await pe(e,"generating",er({shot_count:f.length,shot_index:0,shot_done:0,progress:0,phase:"generating",message:`이미지 1/${f.length} 생성 준비`}));const E=[],I=!!d.llm_anchor_percent;for(let S=0;S<f.length;S+=1){if(await Ee(e,`superseded before shot ${S+1}`))return;const A=f[S],{main:N,neg:U,captions:ee,meta:ge}=await Dr({shot:A,roster:v}),J=wt(),q=Date.now()/1e3,K=u(n.content_hash||"");let Ve=vg(A);Ve==null&&I&&(Ve=Math.round(S/Math.max(1,f.length)*1e4)/100),w("job.shot.prepare",{shot:S,card_id:J,prompt_len:String(N||"").length,captions:(ee||[]).length}),await pe(e,"generating",er({shot_count:f.length,shot_index:S,shot_done:S,progress:Math.round(S/Math.max(1,f.length)*1e3)/10,phase:"generating",message:`NovelAI 요청 중 ${S+1}/${f.length}… [${ht()}]`}));let oe=0;const pt=setInterval(()=>{oe+=Ml/1e3;const un=uc();fc()&&Ja()>=gg&&un&&Date.now()-un>=pg&&Hd("heartbeat-idle");const ou=Ja()||lc();pe(e,"generating",er({shot_count:f.length,shot_index:S,shot_done:S,progress:Math.round(S/Math.max(1,f.length)*1e3)/10,phase:"generating",message:`NovelAI 대기 ${S+1}/${f.length} (${oe}s) · ${ht()}${ou?` ${Math.round(ou/1024)}KB`:""}`})).catch(()=>{})},Ml);let V,De;try{({bytes:V,seed:De}=await wl({main:N,neg:U,captions:ee},A.aspect))}finally{clearInterval(pt)}if(w("job.shot.nai_done",{shot:S,bytes:V?.byteLength||0,seed:De}),await Ee(e,`superseded after shot ${S+1} nai`))return;await pe(e,"generating",er({shot_count:f.length,shot_index:S,shot_done:S,progress:Math.round((S+.5)/Math.max(1,f.length)*1e3)/10,phase:"generating",message:`이미지 저장 중 ${S+1}/${f.length}… [${ht()}]`}));const re=Hp({imageId:J,sessionId:r,request:n,shotIndex:S,paragraph:A.paragraph,yPercent:Ve,line:(()=>{const un=Math.floor(Number(A.line));return Number.isFinite(un)&&un>=1?un:null})(),contentHash:K});await xa(J,V,re);const n_={...mi(ge,re,V?.byteLength||0),aspect:A.aspect||void 0};await F("cards",{id:J,job_id:e,session_id:r,shot_index:S,paragraph:Number(A.paragraph||0),main_prompt:N,negative_prompt:U,characters_json:JSON.stringify(ge.characters||[]),seed:De,meta_json:JSON.stringify(n_),created_at:q});const iu=kt.get(e);if(iu&&iu.publishedIds.push(J),await Ee(e,`superseded after shot ${S+1} save`))return;E.push({id:J,shot_index:S,paragraph:re.paragraph,y_percent:re.y_percent,line:re.line,message_index:re.message_index??-1,message_role:re.message_role||"",content_hash:re.content_hash||"",character_id:re.character_id||"",chat_id:re.chat_id||"",character_name:re.character_name||"",chat_name:re.chat_name||"",char_index:re.char_index??-1,chat_index:re.chat_index??-1,assistant_preview:u(n.assistant_text||"",Di),main_prompt:N,negative_prompt:U,characters:ge.characters||[],image_url:Le(J),seed:De,storage:"indexeddb",png_bytes:V?.byteLength||0}),w("job.shot.saved",{shot:S,card_id:J,has_url:!!Le(J)}),await pe(e,"generating",er({shot_count:f.length,shot_index:S,shot_done:S+1,progress:Math.round((S+1)/Math.max(1,f.length)*1e3)/10,phase:"generating",message:`이미지 ${S+1}/${f.length} 완료`,cards_so_far:E.length}))}if(await Ee(e,"superseded before done"))return;const T={cards:E,message_index:n.message_index!=null?Number(n.message_index):-1,shot_count:f.length,shot_done:f.length,progress:100,phase:"done",message:`이미지 ${f.length}/${f.length} 완료`};await st(T),await pe(e,"done",T);const R=kt.get(e);R&&(R.publishedIds=[]),i.end({message:"done",cards:E.length})}catch(o){i.fail(o);const s=o,c=`${s?.message||o}
${s?.stack||""}`.slice(-1500);await pe(e,"error",{phase:"error",message:String(s?.message||o).slice(0,240),debug_stage:kn(),debug_tail:ea(e,12)},c)}finally{xn(a)}}L(),it(),Ht(),Oe(),fe();function Ft(e,t){try{return JSON.parse(e)}catch{return t}}async function kg(e,t={}){const n=u(e,80),r=await D("cards",n);if(!r)return{ok:!1,error:{code:"not_found",message:"card not found"}};const a=Ft(r.characters_json||"[]",[]),i=Array.isArray(a)?a:[];let o=r.main_prompt;"main_prompt"in t&&(o=u(t.main_prompt,8e3));let s=r.negative_prompt;"negative_prompt"in t&&(s=u(t.negative_prompt,8e3));let c=i;if("characters"in t){const d=t.characters||[];if(!Array.isArray(d))return{ok:!1,error:{code:"bad_request",message:"characters must be a list"}};c=[];const m=at(j().card||{});for(let p=0;p<d.slice(0,m).length;p++){const g=d[p];if(typeof g!="object")continue;const h=g,_=p<i.length&&typeof i[p]=="object"?i[p]:{},y=u(h.name!=null?h.name:_.name,200),x=u(h.prompt!=null?h.prompt:_.prompt,4e3);if(!y&&!x)continue;const v={..._,name:y||`char${p+1}`,prompt:x||"girl"};"uc"in h&&(v.uc=u(h.uc,2e3));for(const k of["center_x","center_y"])if(k in h)try{v[k]=Number(h[k])}catch{}c.push(v)}}r.main_prompt=o,r.negative_prompt=s,r.characters_json=JSON.stringify(c);try{let d=Ft(r.meta_json||"{}",{});(!d||typeof d!="object"||Array.isArray(d))&&(d={});const m=d;m.setup=o,m.characters=c,r.meta_json=JSON.stringify(m)}catch{}await F("cards",r);const l=await mt(n,{}),f={id:n,main_prompt:o,negative_prompt:s,characters:c,paragraph:r.paragraph,shot_index:r.shot_index,y_percent:l.y_percent,message_index:l.message_index,content_hash:l.content_hash,image_url:Le(n)};return await st(f),{ok:!0,card:f}}async function Il(e,t="nai",n=null,r={}){const a=await D("cards",e);if(!a)return{ok:!1,error:{code:"not_found",message:"card not found"}};const i=a.session_id,o=Ft(a.meta_json||"{}",{}),s=o.location||{};if(!r.skipBusyCheck)try{const A=await mt(e,o),N=await wi({session_id:i,content_hash:u(A.content_hash||o.content_hash||"",128),message_index:O(A.message_index,-1)},i);if(N)return N}catch{}const c=u(s.character_id||o.character_id||"",200);let l=u(o.unified_session_id||s.unified_session_id||"",200);!l&&c&&(l=pu(c));let f=[];try{const A=await D("jobs",a.job_id);if(A?.request_json){const N=JSON.parse(A.request_json);Array.isArray(N.source_session_ids)&&(f=N.source_session_ids.map(U=>u(U,200)).filter(Boolean))}}catch{}const d=await Oa(i,l,c,f);if(t==="full"){const A=await D("jobs",a.job_id);if(!A)return{ok:!1,error:{code:"no_job",message:"original job missing"}};const N=JSON.parse(A.request_json);return N.force=!0,Nl(N)}let m,p,g,h,_={};const y=n;if(y&&("main_prompt"in y||"negative_prompt"in y||"characters"in y))m="main_prompt"in y?u(y.main_prompt||""):u(a.main_prompt),p="negative_prompt"in y?u(y.negative_prompt||""):u(a.negative_prompt),h="characters"in y?y.characters:null,h==null&&(h=Ft(a.characters_json||"[]",[])),g=(h||[]).slice(0,at(j().card||{})).map(A=>({prompt:_a(A.prompt||"girl")||"girl",uc:u(A.uc),center_x:Number(A.center_x??.5),center_y:Number(A.center_y??.5)}));else if(u(a.main_prompt||"")){const A=Ft(a.characters_json||"[]",[]),N=Array.isArray(A)?A:[],U=Cl(o),ee=N.map(pt=>{if(!pt||typeof pt!="object")return null;const V=pt;if(V.raw&&typeof V.raw=="object"){const De=V.raw;return{...De,name:u(De.name||V.name,200)}}return{name:u(V.name,200),action:V.action,expression:V.expression,sex:V.sex,label:V.label,age:V.age,original:V.original||V.original_tag}}).filter(Boolean),ge=j().card||{},J=He(j().nai?.model||"nai-diffusion-4-5-full"),q=Object.values(Nn).filter(Boolean);Nn[J]&&q.unshift(Nn[J]);const K=rf({setup:o.setup,main:a.main_prompt,person:o.person,stylePositives:af(ge),qualitySuffixes:q}),Ve={characters:U.length?U:ee,paragraph:a.paragraph,camera:K.rebuildMain&&K.lockedSetup?"":u(o.camera||""),situation:K.rebuildMain&&K.lockedSetup?"":u(o.situation||o.scene||""),place:K.rebuildMain&&K.lockedSetup?"":u(o.place||""),action:K.rebuildMain&&K.lockedSetup?"":u(o.action||"")},oe=await Dr({shot:Ve,roster:d,lockedSetup:K.rebuildMain?K.lockedSetup:void 0});K.rebuildMain?(m=oe.main,p=oe.neg,g=oe.captions,h=oe.meta.characters,_={setup:oe.meta.setup,person:oe.meta.person,characters:h}):(m=u(a.main_prompt),p=u(a.negative_prompt)||oe.neg,g=oe.captions,h=oe.meta.characters,_={setup:m,person:oe.meta.person,characters:h})}else{const A=Ft(a.characters_json||"[]",[]),N=Cl(o),U=(A||[]).map(q=>{if(!q||typeof q!="object")return null;const K=q;return K.raw||{name:K.name,action:K.action,expression:K.expression}}).filter(Boolean),ee=u(o.setup||""),ge={characters:N.length?N:U,paragraph:a.paragraph,camera:ee?"":u(o.camera||""),situation:ee?"":u(o.situation||o.scene||""),place:ee?"":u(o.place||""),action:ee?"":u(o.action||"")},J=await Dr({shot:ge,roster:d,lockedSetup:ee});m=J.main,p=J.neg,g=J.captions,h=J.meta.characters,_={setup:J.meta.setup,person:J.meta.person,characters:h}}if(!g.length&&!(y&&"characters"in y)){const A={characters:(o.characters||[]).map(U=>U.raw||U),camera:o.setup},N=await Dr({shot:A,roster:d});m=N.main,p=N.neg,g=N.captions,h=N.meta.characters,_={setup:N.meta.setup,person:N.meta.person,characters:h}}const{bytes:x,seed:v}=await wl({main:m,neg:p,captions:g},o.aspect??y?.aspect),k=wt(),E=Date.now()/1e3;let I=await Rt(a.id);Object.keys(I).length||(I=await mt(a.id,o));const T={version:1,image_id:k,session_id:i,unified_session_id:l,character_id:u(I.character_id||"",200),character_name:u(I.character_name||o.character_name||"",200),chat_id:u(I.chat_id||"",200),chat_name:u(I.chat_name||o.chat_name||"",200),char_index:O(I.char_index,-1),chat_index:O(I.chat_index,-1),message_index:O(I.message_index,-1),shot_index:O(I.shot_index,O(a.shot_index,0)),paragraph:O(I.paragraph,O(a.paragraph,0)),y_percent:cr(I.y_percent),content_hash:u(I.content_hash||"",128),assistant_preview:u(I.assistant_preview||o.assistant_preview||"",et)};await xa(k,x,T);const R=mi({...o,..._},T,x.byteLength);for(const A of["y_percent","anchor_percent","read_percent"])delete R[A];R.y_percent=T.y_percent,await F("cards",{id:k,job_id:a.job_id,session_id:i,shot_index:a.shot_index,paragraph:a.paragraph,main_prompt:m,negative_prompt:p,characters_json:JSON.stringify(h),seed:v,meta_json:JSON.stringify(R),created_at:E});try{await bi(e)}catch{}const S={id:k,image_url:Le(k),main_prompt:m,negative_prompt:p,characters:h,seed:v,paragraph:T.paragraph,y_percent:T.y_percent,message_index:T.message_index,shot_index:T.shot_index,content_hash:T.content_hash,character_id:T.character_id,chat_id:T.chat_id,character_name:T.character_name,chat_name:T.chat_name,assistant_preview:T.assistant_preview,storage:"indexeddb",png_bytes:x.byteLength};return await st(S),{ok:!0,replaced:e,card:S}}async function Sg({session_id:e="",content_hash:t="",message_index:n=-1}={}){const r=u(e,200),a=u(t,128),i=O(n,-1);if(!r&&!a&&i<0)return{ok:!1,error:{code:"bad_request",message:"session_id or content_hash required"}};const o=await we("cards"),s=[];for(const h of o){if(r&&h.session_id!==r)continue;const _=Ft(h.meta_json||"{}",{}),y=await mt(h.id,_),x=u(y.content_hash||_.content_hash||"",128),v=O(y.message_index,-1);if(a){if(x!==a)continue}else if(i>=0){if(v!==i)continue}else continue;const k=y.y_percent??_.y_percent??_.anchor_percent??_.read_percent,E=Number(k);s.push({row:h,y:Number.isFinite(E)?E:999,shot:O(h.shot_index,0),paragraph:O(h.paragraph??y.paragraph,0)})}if(s.sort((h,_)=>h.y-_.y||h.shot-_.shot||h.paragraph-_.paragraph),!s.length)return{ok:!1,error:{code:"not_found",message:"no cards for message"}};const c=r||u(s[0]?.row?.session_id||"",200),l={session_id:c,content_hash:a,message_index:i},f=await wi(l,c);if(f)return f;const d=zr(l,c);dr.add(d);const m=[],p=[],g=[];try{for(const h of s){const _=h.row;try{const y=await Il(_.id,"nai",null,{skipBusyCheck:!0});if(y?.busy){g.push({id:_.id,error:y.error?.message||"busy"});break}y?.ok&&y.card?(m.push(y.card),y.replaced&&p.push(y.replaced)):g.push({id:_.id,error:u(y?.error?.message||"reroll failed",400)})}catch(y){g.push({id:_.id,error:u(y?.message||y,400)})}}return{ok:m.length>0,count:m.length,replaced:p,cards:m,failed:g}}finally{dr.delete(d)}}function Cl(e){return Array.isArray(e.characters)?e.characters.map(t=>t&&typeof t=="object"?t.raw||t:null).filter(Boolean):[]}var Ag=fn({HASH_REBIND_THRESHOLD:()=>Eg,PREFIX_MATCH_MIN_CHARS:()=>24,PREFIX_MATCH_THRESHOLD:()=>Cg,SIMILARITY_COMPARE_MAX:()=>800,VIEWER_THUMB_LAYOUT:()=>ne,activeSegmentIndex:()=>kh,bigramCounts:()=>xi,buildInlineChatHtml:()=>Ch,claimStickyMarkerByCardId:()=>Zg,clampPinPercent:()=>Ge,clampReadingPercent:()=>xh,clampShotLine:()=>Wl,clampThumbScrollOffset:()=>dh,composeDualProgressBarsHtml:()=>bh,composeProgressToastHtml:()=>wh,composeSingleProgressBarHtml:()=>Jl,composeStickyThumbHtml:()=>ah,createDebouncedSaveQueue:()=>Yg,createScrollSettleTracker:()=>Wg,createSessionChangeGuard:()=>Vg,describeDomApiCompare:()=>Rg,diceBigramRatio:()=>jl,evenAnchorPercent:()=>Ai,expandElementLineEntries:()=>Ql,findElementIndexForLine:()=>eu,findElementIndexForLineWithFallback:()=>Ih,findHashRebindCandidates:()=>Tg,findPlainLineStartOffset:()=>Xl,fitBoxInside:()=>rh,galleryFocusMessage:()=>Pg,galleryForMessage:()=>$g,galleryIndexFromChildIndex:()=>lh,gallerySelectedCount:()=>Si,galleryStripContentWidth:()=>fh,galleryStripSplitAt:()=>Hr,hasGenerationInfo:()=>Bl,htmlToPlainLn:()=>Nh,injectInlineImagesIntoHtml:()=>ru,isCharMessageRole:()=>Lg,isMessageSelectionGesture:()=>Kg,isNearbyDomIndex:()=>gh,lineTextOccurrence:()=>Zl,linkCardsForMessage:()=>ki,longestCommonPrefixLength:()=>Tl,markerBlockHtml:()=>nu,mergeViewerPaintJob:()=>_h,messageCompactKey:()=>ln,messageContextTriplet:()=>Bg,messagesTextOverlapScore:()=>Rl,nearbyDomIndexWindow:()=>hh,nearbyMessageImageIds:()=>ph,normalizeInlineMatchText:()=>Gr,normalizeMatchText:()=>Ur,normalizeMessageRole:()=>vi,parseAutotagLookJson:()=>Ul,pickMessageIndexNearPoint:()=>Hg,pinPercentToPx:()=>zg,pinPercentToPxFromBottom:()=>Ug,pinPxToPercent:()=>Fl,preferNearbyHostIndex:()=>tu,prefixMatchRatio:()=>$l,rawMessageRole:()=>Jr,readingPercentInMessage:()=>Hl,rebindGalleryMessageIndexes:()=>Fg,resolveCardAnchorPercent:()=>vh,resolveChatMessageMatch:()=>Dg,resolveClickSelectionAction:()=>qg,resolveIndexProgress:()=>yh,resolveStickyThumbPct:()=>th,resolveStoredPinPercent:()=>Jg,roleFromGenerationInfo:()=>Og,scaleInlineThumbnail:()=>Ig,shouldKeepStickyThumbHidden:()=>eh,shouldRefreshGallery:()=>Xg,shouldRewriteStickyThumb:()=>Qg,shouldSelectMessageByTextDrag:()=>Gg,splitMessageLines:()=>qr,stickyCornerEdgeBox:()=>Dl,stickyCornerImageBox:()=>oh,stickyPinEdgeBox:()=>ch,stickyPinOverImage:()=>sh,stickyThumbBoxFromPct:()=>nh,stickyThumbNeedsHtmlPaint:()=>ih,stripInlayInlineHtml:()=>Gl,thumbIndexAtStripX:()=>uh,visibleGalleryImageIds:()=>mh}),Mg=528,Ng=720;function b(e,t=0){const n=Number(e);return Number.isFinite(n)?n:t}function El(e,t){const n={...e};for(const[r,a]of Object.entries(t||{})){const i=n[r];n[r]=a&&typeof a=="object"&&!Array.isArray(a)?El(i&&typeof i=="object"?i:{},a):a}return n}function Ig(e){const t=Math.max(1,b(e,100));return{width:Math.max(1,Math.round(Mg*t/100)),height:Math.max(1,Math.round(Ng*t/100)),percent:t}}function Ur(e){return String(e||"").replace(/[^a-zA-Z0-9\uac00-\ud7a3\u3040-\u30ff\u3400-\u9fff\uff00-\uffef]/g,"").toLowerCase()}var Cg=.6,Eg=.6,a_=24,i_=800;function vi(e){const t=String(e||"").toLowerCase().trim();return t?t==="char"||t==="assistant"||t==="bot"?"char":t==="user"||t==="human"?"user":t:""}function Tl(e,t){const n=String(e||""),r=String(t||""),a=Math.min(n.length,r.length);let i=0;for(;i<a&&n.charCodeAt(i)===r.charCodeAt(i);)i+=1;return i}function xi(e){const t=String(e||""),n=new Map;if(t.length<2)return n;for(let r=0;r<t.length-1;r+=1){const a=t.charCodeAt(r)+","+t.charCodeAt(r+1);n.set(a,(n.get(a)||0)+1)}return n}function jl(e,t,n=null){const r=String(e||""),a=String(t||"");if(!r||!a)return 0;if(r===a)return 1;if(r.length<2||a.length<2)return 0;const i=n||xi(r),o=xi(a);let s=0;for(const[f,d]of o){const m=i.get(f)||0;m&&(s+=Math.min(m,d))}const c=r.length-1,l=a.length-1;return 2*s/(c+l)}function $l(e,t){const n=Ur(e),r=Ur(t);if(!n||!r)return 0;const a=Math.max(n.length,r.length);if(a<24)return 0;if(n===r)return 1;const i=Tl(n,r)/a;return i>=.6?i:jl(n.length>800?n.slice(0,800):n,r.length>800?r.slice(0,800):r)}function ki(e,t){if(!t)return[];const n=Array.isArray(e)?e:[],r=String(t.hash||"");return r?Ll(n.filter(a=>a?.content_hash&&a.content_hash===r)):[]}function Tg(e,t={}){const n=Array.isArray(e)?e:[],r=String(t.newHash||t.hash||""),a=String(t.text||""),i=String(t.characterId||""),o=String(t.chatId||""),s=String(t.sessionId||""),c=Number(t.messageIndex),l=vi(t.role||t.messageRole||"");if(!r||!a||!i||!l||!Number.isFinite(c)||c<0)return[];if(n.some(d=>d?.content_hash&&d.content_hash===r))return[];if(Ur(a).length<24)return[];const f=[];for(const d of n){const m=String(d?.content_hash||"");if(!m||m===r||String(d?.character_id||"")!==i)continue;const p=String(d?.chat_id||""),g=String(d?.session_id||"");if(o){if(p!==o)continue}else if(s){if(g&&g!==s)continue}else continue;if(Number(d?.message_index)!==c)continue;const h=vi(d?.message_role||d?.role||"");if(!h||h!==l)continue;const _=String(d?.assistant_preview||"");if(!_)continue;const y=$l(a,_);y<.6||f.push({card:d,score:y})}return f.sort((d,m)=>m.score-d.score||b(m.card?.created_at)-b(d.card?.created_at)),Ll(f.map(d=>d.card))}function Pl(e){const t=e?.y_percent??e?.anchor_percent??e?.read_percent,n=Number(t);return Number.isFinite(n)?n:Number.POSITIVE_INFINITY}function jg(e,t){return Pl(e)-Pl(t)||b(e?.paragraph)-b(t?.paragraph)||b(e?.shot_index)-b(t?.shot_index)||b(t?.created_at)-b(e?.created_at)}function Ol(e,t){return b(t?.created_at)-b(e?.created_at)||b(t?.message_index)-b(e?.message_index)||b(t?.paragraph)-b(e?.paragraph)||b(t?.shot_index)-b(e?.shot_index)}function Ll(e){const t=new Map;for(const n of e||[]){const r=`${n?.paragraph??"?"}|${n?.shot_index??n?.id}`,a=t.get(r);(!a||b(n?.created_at)>=b(a?.created_at))&&t.set(r,n)}return[...t.values()]}function $g(e,t,n=8){const r=new Map;for(const f of e||[]){const d=String(f?.id||"");!d||r.has(d)||r.set(d,f)}const a=[...r.values()],i=Math.max(0,Math.min(64,b(n,8)||8));if(!t)return a.sort(Ol).slice(0,Math.max(1,i||8));const o=ki(a,t).sort(jg),s=new Set(o.map(f=>String(f.id))),c=String(t.sessionId||""),l=a.filter(f=>!s.has(String(f.id))).filter(f=>!c||!f?.session_id||f.session_id===c).sort(Ol).slice(0,i);return[...o,...l]}function Si(e,t){return t?ki(e,t).length:0}function Pg(e,t,n){if(e&&Si(n,e)>0)return e;const r=String(e?.sessionId||e?.session_id||""),a=String(t?.sessionId||t?.session_id||"");return t&&Si(n,t)>0&&(!r||!a||r===a)?t:e||null}function Jr(e){const t=String(e?.role||e?.type||e?.speaker||e?.sender||e?.from||e?.name||"").trim().toLowerCase();return e?.isUser===!0||e?.fromUser===!0||/^(user|human|player|you|me)$/.test(t)||t.includes("user")?"user":e?.isAssistant===!0||e?.isBot===!0||e?.isChar===!0||/^(assistant|bot|char|character|model|ai)$/.test(t)||t.includes("assistant")||t.includes("bot")||t.includes("char")?"char":t==="system"||t==="developer"?"system":""}function Bl(e){const t=e?.generationInfo??e?.generation_info??e?.messageGenerationInfo;return t==null?!1:typeof t=="string"?t.trim().length>0:typeof t=="object"?Object.keys(t).length>0:!0}function Og(e){return Bl(e)?"char":"user"}function Lg(e){if(e&&typeof e=="object")return Jr(e)==="char";const t=String(e||"").trim().toLowerCase();return t==="char"||t==="assistant"||t==="bot"}function ln(e,t=48){const n=Math.max(1,b(t,48));return String(e||"").replace(/\s+/g,"").slice(0,n)}function Bg(e,t,n=48){const r=Array.isArray(e)?e:[],a=Math.floor(b(t,-1)),i=a>=0&&a<r.length?r[a]:null,o=a>0?r[a-1]:null,s=a>=0&&a<r.length-1?r[a+1]:null;return[ln(o?.text,n),ln(i?.text,n),ln(s?.text,n),Jr(i)||String(i?.role||"").trim().toLowerCase()].join("|")}function Rl(e,t){const n=String(e||"").replace(/\s+/g,""),r=String(t||"").replace(/\s+/g,"");if(!n||!r)return 0;if(n.includes(r))return r.length;if(r.includes(n))return n.length;let a=0;const i=(s,c,l=8,f=512)=>{let d=l,m=Math.min(s.length,f),p=0;for(;d<=m;){const g=d+m>>1;c.includes(s.slice(0,g))?(p=g,d=g+1):m=g-1}return p};a=Math.max(a,i(r,n),i(n,r));const o=Math.min(r.length,2400);for(let s=0;s+8<=o;s+=32){const c=Math.min(64,o-s);for(let l=c;l>=8;l-=8)if(n.includes(r.slice(s,s+l))){a=Math.max(a,l);break}}return a}function Rg(e,t){const n=String(e||""),r=String(t||""),a=ln(n),i=ln(r),o=Rl(n,r),s=o>=8,c=a.length>0&&a.length<8&&a===i,l=i.slice(0,48),f=a.slice(0,48);return{domChars:n.length,apiChars:r.length,domCompactChars:a.length,apiCompactChars:i.length,shareScore:o,overlap:s,shortExact:c,apiInDom:!!(l&&l.length>=8&&a.includes(l)),domInApi:!!(f&&f.length>=8&&i.includes(f)),apiKey:l,domKey:f}}function Fg(e,t,n){const r=Array.isArray(t)?t:[],a=typeof n=="function"?n:null,i=new Map;if(a)for(let s=0;s<r.length;s+=1){const c=String(r[s]?.text??r[s]?.data??r[s]?.content??""),l=String(a(c)||"");if(!l)continue;const f=Number.isFinite(Number(r[s]?.index))?Number(r[s].index):s,d=i.get(l);d?d.push(f):i.set(l,[f])}let o=0;return{cards:(Array.isArray(e)?e:[]).map(s=>{const c=String(s?.content_hash||""),l=c?i.get(c):void 0;return!c||!l?s:l.length!==1?Number(s?.message_index)!==-1?(o+=1,{...s,message_index:-1}):s:Number(s?.message_index)!==l[0]?(o+=1,{...s,message_index:l[0]}):s}),changed:o}}function Dg(e,t,n,r,a){const i=Array.isArray(t)?t:[],o=Math.floor(b(n,-1)),s=(f,d,m,p=-1)=>({chatIndex:Number.isFinite(Number(f?.index))?Number(f?.index):p>=0?p:o,text:String(f?.text||e||""),role:Jr(f)||String(f?.role||"").trim().toLowerCase(),matchMethod:d,score:m});if(!i.length)return{chatIndex:Math.max(0,o),text:String(e||""),role:"",matchMethod:"fallback",score:0};const c=i.length-1-o;if(c>=0&&c<i.length)return s(i[c],"reverse",100,c);const l=Math.max(0,Math.min(i.length-1,c));return s(i[l],"reverse",50,l)}function Ge(e,t=0){const n=Number(e);return Number.isFinite(n)?Math.max(0,Math.min(100,Math.floor(n))):Math.max(0,Math.min(100,Math.floor(b(t,0))))}function zg(e,t,n=22,r=4){const a=Math.max(1,b(t,1)),i=Math.max(1,b(n,1)),o=Math.max(0,b(r,0)),s=Math.floor(a*Ge(e,0)/100);return Math.max(o,Math.min(a-i-o,s))}function Ug(e,t,n=22,r=8){const a=Math.max(1,b(t,1)),i=Math.max(1,b(n,1)),o=Math.max(0,b(r,0)),s=a-Math.floor(a*Ge(e,0)/100)-i;return Math.max(o,Math.min(a-i-o,s))}function Fl(e,t){const n=Math.max(1,b(t,1));return Ge(b(e,0)/n*100,0)}function Jg(e,t,n,r=null){const a=r&&typeof r=="object"?r:{x:38,y:80},i=t==="y"||t==="Y",o=i?b(a.y,80):b(a.x,38),s=i?"overlay_y_pct":"overlay_x_pct",c=i?"overlay_y_offset":"overlay_x_offset",l=String(e?.overlay_pin_origin||""),f=l==="top"||l==="tl"||l==="top-left",d=e?.[s],m=d!=null&&Number.isFinite(Number(d)),p=Number(e?.[c]),g=Number.isFinite(p),h=_=>Ge(100-Number(_),o);return m?i&&f?h(d):Ge(d,o):g?i?Ge((b(n,1)-p)/Math.max(1,b(n,1))*100,o):Fl(p,n):Ge(o,o)}function Hg(e,t,n=null,r=800){const a=Array.isArray(e)?e:[];if(!a.length)return-1;const i=Math.max(1,b(r,800)),o=Number.isFinite(Number(t))?Number(t):i*.5,s=Number.isFinite(Number(n))?Number(n):null;let c=-1,l=1/0,f=-1,d=1/0;for(let m=0;m<a.length;m+=1){const p=a[m];if(!p)continue;const g=b(p.top,0),h=b(p.bottom,g);if(h<=0||g>=i)continue;const _=(g+h)*.5,y=Number.isFinite(Number(p.left))&&Number.isFinite(Number(p.right))?(Number(p.left)+Number(p.right))*.5:null,x=y!=null&&s!=null?Math.hypot(y-s,_-o):Math.abs(_-o);x<d&&(d=x,f=m);const v=o>=g&&o<=h,k=s==null||!Number.isFinite(Number(p.left))||!Number.isFinite(Number(p.right))||s>=Number(p.left)&&s<=Number(p.right);if(v&&k){const E=Math.max(1,h-g);E<l&&(l=E,c=m)}}return c>=0?c:f}function Kg({gesture:e="single",detail:t=1,movement:n=0,textSelecting:r=!1,excludedTarget:a=!1}={}){const i=e==="double"?2:1;return!a&&!r&&b(n,1/0)<=8&&b(t)===i}function qg({gesture:e="single",detail:t=1,pendingDomIndex:n=null,targetDomIndex:r=null}={}){const a=b(t,1);return e==="double"?a===1?{action:"provisional",pendingDomIndex:r}:a===2?{action:"confirm",clearPending:!0,matchedPending:n!=null&&Number(n)===Number(r)}:{action:"ignore"}:a===1?{action:"confirm",clearPending:!0}:{action:"ignore"}}function Gg({enabled:e=!0,movement:t=0,hasSelection:n=!1,excludedTarget:r=!1}={}){return!!e&&!r&&!!n&&b(t,0)>8}function Wg({delayMs:e=160,onSettle:t}={}){let n=null;const r=Math.max(0,b(e,160)),a=()=>{n=null,typeof t=="function"&&t()};return{bump(){n&&clearTimeout(n),n=setTimeout(a,r)},settleNow(){n&&clearTimeout(n),n=null,a()},cancel(){n&&clearTimeout(n),n=null},get pending(){return n!=null}}}function Vg(e=""){let t=String(e||""),n="",r=0;return{observe(a){const i=String(a||"");return!i||i===t?(n="",r=0,!1):i!==n?(n=i,r=1,!1):(r+=1,r<2?!1:(t=n,n="",r=0,!0))},current(){return t},reset(a=""){t=String(a||""),n="",r=0}}}function Yg(e,t=300){let n={},r=null,a=null;const i=async()=>a||(a=(async()=>{for(;Object.keys(n).length;){const o=n;n={},await e(o)}})().finally(()=>{a=null}),a);return{enqueue(o){n=El(n,o),r&&clearTimeout(r),r=setTimeout(()=>{r=null,i().catch(()=>{})},Math.max(0,b(t,300)))},async flush(){r&&(clearTimeout(r),r=null);do await i();while(Object.keys(n).length||a)},hasPending(){return!!r||!!a||Object.keys(n).length>0}}}function Xg(e,t){const n=Array.isArray(e)?e.map(String):[],r=Array.isArray(t)?t.map(String):[];if(n.length!==r.length)return!0;for(let a=0;a<n.length;a+=1)if(n[a]!==r[a])return!0;return!1}function Zg(e,t){const n=String(t||"");if(!n||!Array.isArray(e)||!e.length)return null;const r=e.findIndex(i=>String(i?.card?.id||"")===n);if(r<0)return null;const a=e[r];return a?.thumb?(e.splice(r,1),a):null}function Qg(e,t){return String(e||"")!==String(t||"")}function eh(e,t,n){if(!e)return!1;const r=String(t||""),a=String(n||"");return!!r&&!!a&&r===a}function th(e){if(!e.alwaysOn||e.userCollapsed||e.editorOpen)return 0;const t=b(e.settingsPct,0);return t>0?t:0}function nh(e,t,n){const r=Math.max(0,b(e,0)),a=Math.max(0,b(t,0)),i=Math.max(0,b(n,0));return{pct:r,w:Math.max(0,Math.round(a*r/100)),h:Math.max(0,Math.round(i*r/100))}}function rh(e,t,n=0,r=0){const a=Math.max(0,b(e,0)),i=Math.max(0,b(t,0));if(a<=0||i<=0)return{w:0,h:0};const o=Math.max(0,b(n,0)),s=Math.max(0,b(r,0));if(o<=0||s<=0)return{w:a,h:i};const c=Math.min(a/o,i/s);return{w:Math.max(1,Math.round(o*c)),h:Math.max(1,Math.round(s*c))}}function ah(e,t=""){const n=typeof e=="string"?e:"";return n?`<img src="${n}" style="width:100%;height:100%;object-fit:contain;display:block;background:transparent" />`:'<div style="width:100%;height:100%;background:transparent"></div>'}function ih(e,t,n,r){const a=typeof t=="string"?t:"";return a?String(e||"")!==a||String(n||"")!==String(r||""):!1}function oh(e,t,n,r=16){const a=Math.max(1,Math.round(b(t?.w,1))),i=Math.max(1,Math.round(b(t?.h,1))),o=Math.max(a,Math.round(b(n?.width,a))),s=Math.max(i,Math.round(b(n?.height,i))),c=Math.max(0,Math.round(b(r,16))),l=String(e||"bottom-right");return{left:l.includes("left")?c:Math.max(c,o-a-c),top:l.includes("top")?c:Math.max(c,s-i-c),w:a,h:i}}function Dl(e,t,n=16){const r=Math.max(1,Math.round(b(t?.w,1))),a=Math.max(1,Math.round(b(t?.h,1))),i=Math.max(0,Math.round(b(n,16))),o=String(e||"bottom-right");return{w:r,h:a,top:o.includes("top")?i:null,bottom:o.includes("bottom")?i:null,left:o.includes("left")?i:null,right:o.includes("right")?i:null}}function sh(e,t=28,n=6){const r=Math.round(b(e?.left,0)),a=Math.round(b(e?.top,0)),i=Math.max(1,Math.round(b(e?.w,1))),o=Math.max(1,Math.round(b(t,28))),s=Math.max(0,Math.round(b(n,6)));return{left:Math.round(r+(i-o)/2),top:a-s}}function ch(e,t,n=28,r=6,a=16){const i=Dl(e,t,a),o=Math.max(1,Math.round(b(n,28))),s=Math.max(0,Math.round(b(r,6))),c=i.left!=null?i.left:i.right,l=Math.round(b(c,0)+(i.w-o)/2);return{size:o,top:i.top!=null?Math.round(i.top-s):null,bottom:i.bottom!=null?Math.round(i.bottom+i.h+s):null,left:i.left!=null?l:null,right:i.right!=null?l:null}}var ne=Object.freeze({width:64,height:88,gap:8,splitWidth:16,splitExtraMargin:4});function Hr(e,t){const n=Math.max(0,Math.floor(b(t,0))),r=Math.max(0,Math.min(Math.floor(b(e,0)),n));return r>0&&r<n?r:0}function lh(e,t,n){const r=Math.max(0,Math.floor(b(n,0))),a=Math.floor(b(e,-1));if(a<0||r<=0)return-1;const i=Hr(t,r);if(i<=0)return a<r?a:-1;if(a===i)return-1;const o=a>i?a-1:a;return o>=0&&o<r?o:-1}function uh(e,{count:t=0,selectedCount:n=0,thumbWidth:r=ne.width,gap:a=ne.gap,splitWidth:i=ne.splitWidth,splitExtraMargin:o=ne.splitExtraMargin}={}){const s=Math.max(0,Math.floor(b(t,0)));if(s<=0)return-1;const c=b(e,NaN);if(!Number.isFinite(c)||c<0)return-1;const l=Math.max(1,b(r,ne.width)),f=Math.max(0,b(a,ne.gap)),d=Math.max(0,b(i,ne.splitWidth)),m=Math.max(0,b(o,ne.splitExtraMargin)),p=Hr(n,s),g=[];for(let _=0;_<s;_+=1)p>0&&_===p&&g.push({type:"split"}),g.push({type:"thumb",galIdx:_,marginLeft:p>0&&_===p?m:0});let h=0;for(let _=0;_<g.length;_+=1){_>0&&(h+=f);const y=g[_];if(y.type==="split"){if(c>=h&&c<h+d)return-1;h+=d;continue}if(h+=y.marginLeft,c>=h&&c<h+l)return y.galIdx;h+=l}return-1}function fh({count:e=0,selectedCount:t=0,thumbWidth:n=ne.width,gap:r=ne.gap,splitWidth:a=ne.splitWidth,splitExtraMargin:i=ne.splitExtraMargin}={}){const o=Math.max(0,Math.floor(b(e,0)));if(o<=0)return 0;const s=Math.max(1,b(n,ne.width)),c=Math.max(0,b(r,ne.gap)),l=Math.max(0,b(a,ne.splitWidth)),f=Math.max(0,b(i,ne.splitExtraMargin)),d=Hr(t,o),m=o+(d>0?1:0);let p=o*s+Math.max(0,m-1)*c;return d>0&&(p+=l+f),p}function dh(e,t,n){const r=Math.max(0,b(t,0)-Math.max(0,b(n,0))),a=b(e,0);return Math.max(0,Math.min(r,a))}function mh(e,t=0,n=1,r=8,a=0){const i=Array.isArray(e)?e:[];if(!i.length)return[];const o=Math.max(0,Math.min(b(t,0),i.length-1)),s=Math.max(1,Math.min(i.length,b(r,8)||8)),c=Math.min(s,1+2*Math.max(0,b(n,1))),l=Math.max(c,s);let f=Math.max(0,o-Math.floor((l-1)/2));const d=Math.min(i.length-1,f+l-1);f=Math.max(0,d-l+1);const m=[],p=new Set,g=_=>{const y=String(i[_]?.id||"");!y||p.has(y)||(p.add(y),m.push(y))},h=Math.max(0,Math.min(i.length,b(a,0)));for(let _=0;_<h;_+=1)g(_);for(let _=f;_<=d&&(g(_),!(m.length>=s));_+=1);return m}function ph(e,t,n=2,r=[]){const a=Array.isArray(e)?e:[],i=Math.max(0,Math.min(8,b(n,2))),o=b(t?.messageIndex??t?.message_index,NaN),s=String(t?.sessionId||t?.session_id||""),c=String(t?.chatId||t?.chat_id||""),l=String(t?.hash||""),f=new Set,d=[],m=p=>{const g=String(p||"");!g||f.has(g)||(f.add(g),d.push(g))};for(const p of r||[])m(p);for(const p of a){if(!p?.id||s&&p.session_id&&String(p.session_id)!==s||c&&p.chat_id&&String(p.chat_id)!==c)continue;const g=b(p.message_index,NaN);if(Number.isFinite(o)&&Number.isFinite(g)){Math.abs(g-o)<=i&&m(p.id);continue}l&&String(p.content_hash||"")===l&&m(p.id)}return d}function gh(e,t,n=2){const r=b(e,NaN),a=b(t,NaN);if(!Number.isFinite(r)||!Number.isFinite(a))return!1;const i=Math.max(0,b(n,2));return Math.abs(r-a)<=i}function hh(e,t,n=2){const r=Math.max(0,Math.floor(b(t,0))),a=Math.max(0,Math.min(Math.max(0,r-1),Math.floor(b(e,0)))),i=Math.max(0,Math.floor(b(n,2)));return r<=0?{lo:0,hi:-1,center:0}:{lo:Math.max(0,a-i),hi:Math.min(r-1,a+i),center:a}}var zl={chrome:1,content:2,full:3};function _h(e,t){const n=zl[String(e??"")]||0,r=zl[String(t??"")]||0;return!n&&!r?"full":r>=n?t||e||"full":e||t||"full"}function Ul(e){const t=String(e||"").trim();if(!t)return{appearance:"",attire:"",accessories:"",text:"",gender:""};let n=null;const r=t.match(/```(?:json)?\s*([\s\S]*?)```/i),a=(r?r[1]:t).trim().match(/\{[\s\S]*\}/);if(a)try{const o=JSON.parse(a[0]);n=o&&typeof o=="object"?o:null}catch{n=null}if(n&&typeof n=="object"){const o=String(n.appearance??n.look??n.identity??"").trim(),s=String(n.attire??n.clothing??n.outfit??"").trim(),c=String(n.accessories??n.accessory??n.props??"").trim(),l=String(n.gender??n.sex??"").trim().toLowerCase(),f=["girl","female","f","woman"].includes(l)?"girl":["boy","male","m","man"].includes(l)?"boy":l==="other"?"other":"";return{appearance:o,attire:s,accessories:c,text:[o,s,c].filter(Boolean).join(", "),gender:f}}const i=t.replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/i,"").trim();return{appearance:i,attire:"",accessories:"",text:i,gender:""}}function yh(e={}){const t=!!e.warmBusy,n=Math.max(0,Math.min(100,Math.round(b(e.warmPct,0)))),r=String(e.jobState||""),a=Math.max(0,Math.min(100,Math.round(b(e.jobPct,0))));return t?{pct:Math.max(t?6:0,n),busy:!0,label:"인덱싱"}:r==="tagging"?{pct:Math.max(6,a),busy:!0,label:"인덱싱"}:{pct:0,busy:!1,label:"인덱싱"}}function bh(e={}){const t=!!e.jobBusy,n=!!e.indexBusy,r=Math.max(t?6:0,Math.min(100,Math.round(b(e.jobPct,0)))),a=Math.max(n?6:0,Math.min(100,Math.round(b(e.indexPct,0)))),i=e.error?"#f87171":"#7c6cff",o="#2dd4bf",s=(c,l)=>`<span style="display:block;height:4px;border-radius:2px;background:#1e2633;overflow:hidden"><span style="display:block;height:100%;width:${c}%;background:${l}"></span></span>`;return`<span style="flex:0 0 120px;display:flex;flex-direction:column;gap:3px;justify-content:center">${s(r,i)}${s(a,o)}</span>`}function Jl(e={}){const t=!!e.busy,n=Math.max(t?6:0,Math.min(100,Math.round(b(e.pct,0)))),r=String(e.tone||"");return`<span style="flex:0 0 120px;display:block;height:6px;border-radius:3px;background:#1e2633;overflow:hidden"><span style="display:block;height:100%;width:${n}%;background:${e.error?"#f87171":r==="index"?"#2dd4bf":"#7c6cff"}"></span></span>`}function wh(e={}){const t=!!e.jobBusy,n=!!e.indexBusy,r=!!e.busy||t||n;if(!r&&!e.error)return"";const a=typeof e.escapeHtml=="function"?e.escapeHtml:f=>String(f).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),i=String(e.tone||(!t&&n?"index":"job")),o=a(String(e.stage||(i==="index"?"인덱싱":"작업 중")).slice(0,80)),s=a(String(e.meta||"").slice(0,160));let c=e.pct;c==null&&(t?c=e.jobPct:n?c=e.indexPct:c=Math.max(b(e.jobPct,0),b(e.indexPct,0)));const l=Jl({pct:c,busy:r||!!e.error,error:e.error,tone:i});return`<div data-inlay-progress-toast="1" style="display:flex;align-items:center;gap:10px;min-width:220px;max-width:min(420px,92vw);padding:10px 14px;border-radius:8px;background:#121820;border:1px solid #2a3344;cursor:pointer;user-select:none"><span style="flex:0 0 auto;font-weight:700;color:${e.error?"#f87171":i==="index"?"#5eead4":"#c4b5fd"};font-size:11px;white-space:nowrap">${o}</span><span style="min-width:0;flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#8b97ab;font-size:11px">${s}</span>${l}</div>`}function Kr(e){return Math.max(0,Math.min(100,b(e,0)))}function Ai(e,t){const n=Math.max(1,b(t,1));return Math.max(0,Math.min(b(e,0),n-1))/n*100}function vh(e,t=0,n=1,r=null){if(r&&r.forceEven)return Ai(t,n);const a=e?.y_percent??e?.anchor_percent??e?.read_percent,i=Number(a);return Number.isFinite(i)?Kr(i):Ai(t,n)}function Hl(e,t,n=.5){if(!e)return null;const r=Math.max(1,b(e.height,0)),a=Math.max(0,b(t,0))*(Number.isFinite(n)?n:.5);return a<b(e.top,0)||a>b(e.bottom,0)?null:Kr((a-b(e.top,0))/r*100)}function xh(e,t,n=.5){const r=Hl(e,t,n);if(r!=null)return r;const a=Math.max(0,b(t,0))*(Number.isFinite(n)?n:.5);return b(e?.bottom,0)<a?100:0}function kh(e,t){const n=Array.isArray(e)?e:[];if(!n.length)return-1;const r=Kr(t),a=n.map(c=>Kr(c)),i=a.reduce((c,l,f)=>l<=20?c.concat(f):c,[]),o=i.length===1&&a[i[0]]>0?a.map((c,l)=>l===i[0]?1:c):a;if(o[0]>0&&r<o[0])return-1;let s=0;for(let c=0;c<o.length;c+=1)r>=o[c]&&(s=c);return s}var Sh="data-inlay-inline-shot",Kl=/^<\/(?:p|div|li|blockquote|h[1-6]|tr)>$/i,ql=/^<br\s*\/?>$/i;function Gl(e){const t=String(e||"");return t?t.replace(/<div[^>]*\bdata-inlay-inline-shot\b[^>]*>[\s\S]*?<\/div>/gi,"").replace(/<(?:span|p)[^>]*\bdata-inlay-inline-shot\b[^>]*>[\s\S]*?<\/(?:span|p)>/gi,""):""}function qr(e){return String(e??"").replace(/\r\n/g,`
`).split(`
`).map(t=>t.trim()).filter(Boolean)}function Wl(e,t){const n=Math.floor(Number(e)),r=Math.max(0,Math.floor(Number(t)||0));return!r||!Number.isFinite(n)||n<1?null:Math.max(1,Math.min(r,n))}function Vl(e){return String(e||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Ah(e){const t=e.toLowerCase();if(t==="&nbsp;")return" ";if(t==="&amp;")return"&";if(t==="&lt;")return"<";if(t==="&gt;")return">";if(t==="&quot;")return'"';if(t==="&#39;"||t==="&apos;")return"'";const n=/^&#(\d+);$/.exec(e);if(n){const a=Number(n[1]);if(Number.isFinite(a)&&a>0&&a<1114112)return String.fromCodePoint(a)}const r=/^&#x([0-9a-f]+);$/i.exec(e);if(r){const a=Number.parseInt(r[1],16);if(Number.isFinite(a)&&a>0&&a<1114112)return String.fromCodePoint(a)}return null}function Mh(e,t){let n=Math.max(0,Math.min(e.length,Math.floor(t)));for(;n>0;){let r=n;for(;r>0&&/[ \t\r\n]/.test(e[r-1]);)r-=1;if(r<=0||e[r-1]!==">")break;const a=e.lastIndexOf("<",r-1);if(a<0||a>=r-1)break;const i=e.slice(a,r);if(ql.test(i)||Kl.test(i)||/^<\//.test(i)||!/^<[a-zA-Z][^>]*>$/.test(i))break;n=a}return n}function Yl(e){const t=[];let n=0;const r=String(e||"");for(;n<r.length;){if(r[n]==="<"){const s=r.indexOf(">",n);if(s<0)break;const c=r.slice(n,s+1);(ql.test(c)||Kl.test(c))&&t.push({ch:`
`,htmlIndex:n}),n=s+1;continue}if(r[n]==="&"){const s=r.indexOf(";",n);if(s>n&&s-n<16){const c=Ah(r.slice(n,s+1));if(c!=null){for(let l=0;l<c.length;l+=1)t.push({ch:c[l],htmlIndex:n});n=s+1;continue}}}t.push({ch:r[n],htmlIndex:n}),n+=1}const a=[];for(let s=0;s<t.length;s+=1){const c=t[s];if(c.ch!==`
...
`||o[o.length-1].ch===" ");)o.pop();return o}function Nh(e){return Yl(String(e||"")).map(t=>t.ch).join("")}function Xl(e,t){const n=String(e??"").replace(/\r\n/g,`
`),r=Math.floor(Number(t));if(!Number.isFinite(r)||r<1)return null;let a=0,i=0;for(;i<=n.length;){const o=n.indexOf(`
`,i),s=o<0?n.length:o,c=n.slice(i,s);if(c.trim()&&(a+=1,a===r)){const l=/^\s*/.exec(c)?.[0].length||0;return i+l}if(o<0)break;i=o+1}return null}function Gr(e){return String(e??"").replace(/\s+/g," ").trim()}function Zl(e,t){const n=Array.isArray(e)?e:[],r=Math.floor(Number(t))-1;if(!Number.isFinite(r)||r<0||r>=n.length)return null;const a=Gr(n[r]);if(!a)return null;let i=0;for(let o=0;o<=r;o+=1)Gr(n[o])===a&&(i+=1);return{text:a,occurrence:i}}function Ql(e){const t=[],n=Array.isArray(e)?e:[];for(let r=0;r<n.length;r+=1)for(const a of qr(n[r])){const i=Gr(a);i&&t.push({text:i,elementIndex:r})}return t}function eu(e,t,n){const r=Zl(t,n);if(!r)return-1;const a=Ql(e);let i=0;for(const o of a)if(o.text===r.text&&(i+=1,i===r.occurrence))return o.elementIndex;return-1}function tu(e,t,n=["P"]){const r=(Array.isArray(e)?e:[]).map(o=>String(o||"").toUpperCase()),a=Math.floor(Number(t));if(!Number.isFinite(a)||a<0||a>=r.length)return-1;const i=new Set((Array.isArray(n)&&n.length?n:["P"]).map(o=>String(o||"").toUpperCase()));if((r[a]||"")!=="DIV")return a;for(let o=1;o<r.length;o+=1){const s=a-o,c=a+o;if(s>=0&&i.has(r[s]))return s;if(c<r.length&&i.has(r[c]))return c}return-1}function Ih(e,t,n,r,a=["P"]){const i=Array.isArray(n)?n:[],o=Math.floor(Number(r));if(!Number.isFinite(o)||o<1||!i.length)return null;const s=i.length;for(let c=Math.max(1,o);c<=s;c+=1){const l=tu(t,eu(e,i,c),a);if(l>=0)return{elementIndex:l,usedLine:c}}return null}function nu(e,t){const n=Number.isFinite(Number(e.shotIndex))?String(Math.floor(Number(e.shotIndex))):"",r=Vl(String(e.cardId||n||e.line||"0"));return`<div ${Sh}="${r}" x-inlay-inline-shot="${r}" contenteditable="false" style="display:block;margin:10px 0;text-align:center;line-height:0"><br><img src="${Vl(e.src)}" alt="" style="zoom:0.5;width:auto;height:auto;max-width:none;border-radius:8px;display:inline-block;vertical-align:top"><br></div>`}function ru(e,t,n){const r=Gl(e),a=Array.isArray(t)?t:[];if(!r||!a.length)return r;const i=Yl(r),o=i.map(g=>g.ch).join(""),s=qr(o).length;if(!s||!i.length)return r;const c=new Map,l=new Set;for(const g of a){const h=Wl(g?.line,s),_=String(g?.src||""),y=String(g?.cardId||"");!h||!/^data:image\//i.test(_)||y&&l.has(y)||c.has(h)||(y&&l.add(y),c.set(h,{...g,line:h,src:_}))}if(!c.size)return r;const f=n?.maxWidthPx,d=new Map;for(const[g,h]of c){const _=Xl(o,g);if(_==null||_<0||_>=i.length)continue;const y=Mh(r,i[_].htmlIndex);d.has(y)||d.set(y,nu(h,f))}if(!d.size)return r;const m=[...d.entries()].map(([g,h])=>({htmlIndex:g,html:h})).sort((g,h)=>h.htmlIndex-g.htmlIndex);let p=r;for(const g of m)p=p.slice(0,g.htmlIndex)+g.html+p.slice(g.htmlIndex);return p}function Ch(e,t,n){const r=qr(e);return r.length?ru(r.map(a=>String(a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")).join("<br>"),t,n):""}te(),G(),gt(),Q(),xt(),L(),qa(),ui(),Er(),si(),fe(),ft();function Mi(){const e=_e(),t=e.events;return{last_stage:e.last_stage,events:Array.isArray(t)?t.slice(-20):[]}}async function Eh(e){if(e){const t=j(),n={...e};u(n.api_key)&&(t.llm.api_key=u(n.api_key)),delete n.api_key,delete n.api_key_configured,u(n.service_account_json)&&(t.llm.service_account_json=u(n.service_account_json)),delete n.service_account_json,delete n.service_account_configured,Object.keys(n).length&&(t.llm=nt(t.llm,n)),await Ke()}try{const t=j().llm,n=Tr(t.source),r=Bt(t.provider);return n==="custom"&&!Yc(t)?{ok:!1,message:r==="vertex"?"Vertex AI: Model + Service Account JSON(또는 access token)이 필요합니다.":"태깅 LLM Model/API key가 비어 있습니다. NovelAI 키가 아니라 태깅용 LLM 키를 넣으세요."}:(n==="main"||n==="aux")&&!he("runLLMModel")?{ok:!1,message:"RisuAI runLLMModel API를 사용할 수 없습니다."}:{ok:!0,message:`LLM ok (${n}): ${(await sn(t,[{role:"user",content:"Reply with exactly: ok"}])).slice(0,120)}`}}catch(t){return{ok:!1,message:String(t?.message||t)}}}async function Th(){const e=j().nai;if(Jn(e)==="comfy")try{if(!Ha(e))return{ok:!1,message:"ComfyUI 워크플로 JSON이 없습니다.",debug:_e()};const t=mc({main:"test",neg:"test",captions:[{name:"",prompt:"char1"},{name:"",prompt:"char2"}],nai:e,seed:1}),n=pc(e.comfy_workflow_json,t),r=dc(e),{status:a,data:i}=await Ka(`${r}/system_stats`,{method:"GET"});if(a>=400)return{ok:!1,message:`ComfyUI 연결 실패 (HTTP ${a}) · ${r}`,debug:_e()};const o=i?.devices?.[0]?.name||i?.system?.os||"ok";return{ok:!0,message:`ComfyUI ok · ${r} · nodes=${Object.keys(n).length} · ${o}`,debug:Mi()}}catch(t){return w("comfy.test",{message:String(t?.message||t)},"error"),{ok:!1,message:String(t?.message||t),debug:_e()}}if(!u(e.api_key))return{ok:!1,message:"NAI api_key missing",debug:_e()};try{const t=u(e.api_key);try{const n=ue("nai.test.anlas"),r=await am(t);return n.end({message:"anlas ok"}),{ok:!0,message:`NAI token ok · Anlas=${JSON.stringify(r)}`,debug:Mi()}}catch(n){const r=He(e.model||"nai-diffusion-4-5-full");return w("nai.test.anlas",{message:String(n?.message||n)},"warn"),{ok:!0,message:`NAI config present · model=${r} · anlas_skip=${n?.message||n}`,debug:Mi()}}}catch(t){return w("nai.test",{message:String(t?.message||t)},"error"),{ok:!1,message:String(t?.message||t),debug:_e()}}}async function jh(){const e=j().nai,t=u(e.api_key);if(!t)return{ok:!1,message:"NAI api_key missing",debug:_e()};const n=String(_e().job_ctx??"");xn("probe-nai");const r=ue("nai.probe");try{w("nai.probe.start",{message:"tiny generate"});const a={prompt:"1girl, solo, simple background, best quality",negative_prompt:"lowres, bad quality",width:512,height:768,steps:10,cfg_scale:5,cfg_rescale:0,sampler:"k_euler_ancestral",scheduler:"karras",model:He(e.model||"nai-diffusion-4-5-full"),var_plus:!1,characters:[],seed:Math.floor(Math.random()*4294967295)||1},i=u(e.request_url)||"https://image.novelai.net/ai/generate-image",o=await hc(t,a,i,{timeoutMs:9e4}),s=o.raw_bytes.byteLength,c=bt(X(o.raw_bytes));return r.end({bytes:s,is_png:c,seed:o.seed}),{ok:!0,message:`probe ok · png=${c} · ${s}B · seed=${o.seed}`,bytes:s,is_png:c,seed:o.seed,debug:_e()}}catch(a){return r.fail(a),{ok:!1,message:String(a?.message||a),debug:_e()}}finally{xn(n)}}async function $h(e,t){if(!e?.byteLength)throw new Error("image is empty");const n=await sf(e),r=n.bytes,a=n.mime||"image/png",i=n.filename||"image.png",o=`data:${a};base64,${await Qe(r)}`,s=lr(await le("autotag"))||["Tag ONE character reference image into Danbooru-style English prompts.",'Return ONE JSON object only: {"gender":"girl|boy","appearance":"...","attire":"...","accessories":"..."}.',"gender must be exactly girl or boy from visual evidence. appearance = identity/hair/eyes/body (no clothes). attire = clothing + permanent jewelry (earrings/glasses/watch/earbuds). accessories = weapons/bags/held props/ID only (not jewelry)."].join(`
`);w("autotag.start",{message:`llm-vision ${i} ${r.length}B`,bytes:r.length,focus:!0,source:Tr(j().llm.source)});const c=[{role:"system",content:s},{role:"user",content:[{type:"text",text:"Tag this character image. JSON only with gender, appearance, attire, accessories."},{type:"image_url",image_url:{url:o}}]}];let l="";try{l=await sn(j().llm,c)}catch(x){throw w("autotag.llm.fail",{message:String(x?.message||x)},"error"),new Error(`오토태그 LLM 실패: ${String(x?.message||x).slice(0,240)}`)}const f=Ul(l),d=u(f.appearance||"",4e3),m=u(f.attire||"",4e3),p=u(f.accessories||"",4e3),g=u(f.gender||"",20),h=u(f.text||[d,m,p].filter(Boolean).join(", "),8e3);if(!d&&!m&&!p)throw new Error("LLM이 외형/의상/악세사리 태그를 반환하지 않았습니다. 비전(이미지) 지원 모델인지 확인하세요.");const _=h.split(",").map(x=>x.trim()).filter(Boolean);w("autotag.done",{message:`gender=${g||"-"} app=${d.length} attire=${m.length} acc=${p.length}`,focus:!0});const y={ok:!0,appearance:d,attire:m,accessories:p,tags:_,text:h,count:_.length,threshold:Number(t||.2),engine:"llm-vision"};return g&&(y.gender=g),y}async function Ph(e){if(!e?.byteLength)throw new Error("image is empty");const t=await zs(e);if(!t)throw new Error("이미지에서 NovelAI 메타데이터를 읽지 못했습니다. PNG/WebP NAI 원본인지 확인하세요.");const n=js(t);if(!u(n))throw new Error("메타데이터에 프롬프트가 없습니다.");const r=id(t,n);if(!r.positive&&!r.negative)throw new Error("프리셋에 넣을 artist/품질 태그와 네거티브가 비어 있습니다.");return w("preset-from-image.done",{message:`pos=${r.positive.length} neg=${r.negative.length} cfg=${r.cfg_scale}`,focus:!0}),{ok:!0,positive:r.positive,negative:r.negative,cfg_scale:r.cfg_scale,cfg_rescale:r.cfg_rescale,name:"이미지 프리셋"}}L();function Oh(e){const t=e.indexOf("?");if(t<0)return{pathname:e,query:{}};const n=e.slice(0,t),r=new URLSearchParams(e.slice(t+1)),a={};for(const[i,o]of r.entries()){const s=a[i];s===void 0?a[i]=o:Array.isArray(s)?s.push(o):a[i]=[s,o]}return{pathname:n,query:a}}function tr(e,t,n=""){const r=e[t];return Array.isArray(r)?r[0]??n:r??n}function Lh(e,t={}){const n=u(e.auth_token,4e3);if(!n)return!0;const r=String(t.Authorization||t.authorization||"");let a=r.toLowerCase().startsWith("bearer ")?r.slice(7):r;if(a=a||String(t["X-Inlay-Nexus-Token"]||t["x-inlay-nexus-token"]||""),typeof a!="string"||a.length!==n.length)return!1;let i=0;for(let o=0;o<n.length;o+=1)i|=a.charCodeAt(o)^n.charCodeAt(o);return i===0}G(),Q(),L(),te(),Oo(),fe(),ft(),Xn();var $=(...e)=>t=>e.includes(t)?"":null,We=e=>t=>t.startsWith(e)?t.slice(e.length):null,Ni=(e,t)=>n=>n.startsWith(e)&&n.endsWith(t)?n.slice(e.length,n.length-t.length):null,M=e=>({status:200,data:e}),Ii=e=>({status:200,data:e,contentType:"image/png",raw:!0}),nr=e=>{throw _t(404,{ok:!1,...ae(e,"not_found")},e)};function Wr(e){let t=e.image_b64||e.data||"";return typeof t=="string"&&t.startsWith("data:")&&(t=t.split(",",2)[1]??""),String(t||"")}var Bh=[{match:$("/v1/settings/export"),handler:()=>M({ok:!0,json:vp()})},{match:$("/v1/settings"),handler:()=>M({ok:!0,settings:Or()})},{match:$("/v1/curation/status"),handler:async()=>M({ok:!0,status:await Yn()})},{match:$("/v1/curation/catalog"),handler:async()=>M({ok:!0,catalog:await dt()})},{match:$("/v1/prompts"),handler:async()=>M({ok:!0,prompts:await Pr()})},{match:$("/v1/prompts/export"),handler:async()=>M({ok:!0,...await _p()})},{match:We("/v1/prompts/"),handler:async({param:e})=>M({ok:!0,key:e,text:await le(e)})},{match:We("/v1/jobs/"),handler:async({param:e})=>M(await wg(e))},{match:We("/v1/gallery/explore"),handler:async({query:e})=>M(await eg(Number(tr(e,"limit","400"))))},{match:$("/v1/gallery/favorites"),handler:async()=>M(await Sl())},{match:We("/v1/gallery"),handler:async({query:e})=>M(await tg(tr(e,"session_id"),Number(tr(e,"limit","40"))))},{match:$("/v1/nai/reference.png"),handler:async()=>Ii(await pl()??nr("no reference"))},{match:$("/v1/nai/vibe.png"),handler:async()=>Ii(await Bp()??nr("no vibe"))},{match:$("/v1/nai/reference"),handler:async()=>{const e=await $p();return M({ok:!0,configured:e,image_reference:j().nai?.image_reference||"none",preview_url:e?"/v1/nai/reference.png":""})}},{match:$("/v1/nai/vibe"),handler:async()=>{const e=await Op();return M({ok:!0,configured:e,vibe_transfer:j().nai?.vibe_transfer||"none",preview_url:e?"/v1/nai/vibe.png":""})}},{match:We("/v1/characters"),handler:async({query:e})=>M(await zn(tr(e,"session_id"),tr(e,"character_id")))},{match:We("/v1/appearance/"),handler:async({param:e})=>M(await zn(e))},{match:We("/v1/images/"),handler:async({param:e})=>{let t=e.replace(/^\/+|\/+$/g,"");if(t.endsWith(".json")){const n=await Rt(t.slice(0,-5));return Object.keys(n).length||nr("location missing"),M(n)}return t.endsWith(".png")&&(t=t.slice(0,-4)),Ii(await Al(t)??nr("image missing"))}}],Rh=[{match:$("/v1/settings/reset"),handler:async()=>M(await kp())},{match:$("/v1/settings/import"),handler:async({body:e})=>M(await xp(String(e.json||e.text||"")))},{match:$("/v1/settings","/v1/settings/update"),handler:async({body:e})=>M(await Ap(e))},{match:$("/v1/curation/catalog"),handler:async({body:e})=>{const t=e.catalog??e;return M(await nl(t))}},{match:$("/v1/curation/catalog/reset"),handler:async()=>M(await rl())},{match:$("/v1/curation/embed"),handler:async()=>M(await al())},{match:$("/v1/curation/embed/test"),handler:async()=>M(await il())},{match:$("/v1/curation/settings"),handler:async({body:e})=>M(await dl(e))},{match:$("/v1/prompts/import"),handler:async({body:e})=>M(await yp(e?.json!=null?e.json:(e?.prompts!=null,e)))},{match:$("/v1/prompts/reset-defaults"),handler:async({body:e})=>M(await bp({keep_author_note:e?.keep_author_note!==!1&&e?.keepAuthorNote!==!1}))},{match:Ni("/v1/prompts/","/reset"),handler:async({param:e})=>M(await Qc(e,In(e)))},{match:We("/v1/prompts/"),handler:async({param:e,body:t})=>M(await Qc(e,String(t.text||"")))},{match:$("/v1/jobs/create","/v1/jobs"),handler:async({body:e})=>({status:202,data:await Nl(e)})},{match:$("/v1/gallery/unlink","/v1/cards/unlink"),handler:async({body:e})=>M(await _i(String(e.session_id||""),String(e.content_hash||""),e.message_index))},{match:$("/v1/gallery/rebind-hash","/v1/cards/rebind-hash"),handler:async({body:e})=>M(await ng({session_id:String(e.session_id||e.sessionId||""),card_ids:e.card_ids||e.ids||[],to_hash:String(e.to_hash||e.content_hash||""),assistant_preview:String(e.assistant_preview||e.assistant_text||"")}))},{match:$("/v1/gallery/delete","/v1/cards/delete"),handler:async({body:e})=>{const t=u(e.folder_key||"",400),n=u(e.card_id||e.id||"",80),r=Array.isArray(e.card_ids)?e.card_ids:null;if(t)return M(await sg(t));if(r?.length)return M(await rg(r));if(n)return M(await bi(n));const a="card_id or folder_key required";throw _t(400,{ok:!1,...ae(a,"bad_request")},a)}},{match:$("/v1/gallery/export"),handler:async({body:e})=>M(await ig(e))},{match:$("/v1/gallery/import"),handler:async({body:e})=>M(await og(e))},{match:$("/v1/gallery/favorites"),handler:async({body:e})=>Array.isArray(e.ids)?M(await ag(e.ids)):M(await Sl())},{match:Ni("/v1/cards/","/tags"),handler:async({param:e,body:t})=>M(await kg(e,t))},{match:Ni("/v1/cards/","/reroll"),handler:async({param:e,body:t})=>M(await Il(e,String(t.mode||"nai"),t.overrides))},{match:$("/v1/messages/reroll","/v1/gallery/reroll-message"),handler:async({body:e})=>M(await Sg({session_id:String(e.session_id||e.sessionId||""),content_hash:String(e.content_hash||e.contentHash||""),message_index:Number(e.message_index??e.messageIndex??-1)}))},{match:$("/v1/characters/global-toggles","/v1/characters/global_toggles"),handler:async({body:e})=>M(await Ad(String(e.character_id||""),e.disabled_globals||e.disabled||[]))},{match:$("/v1/characters/unify","/v1/characters/merge"),handler:async({body:e})=>M(await tc(String(e.target_session_id||e.session_id||""),e.source_session_ids||e.session_ids||[],e.include_target!==!1))},{match:$("/v1/characters","/v1/characters/update"),handler:({body:e})=>Fh(e)},{match:We("/v1/appearance/"),handler:async({param:e,body:t})=>t.characters!=null||t.global!=null?(t.characters!=null&&await Dn(e,t.characters||[],{prune:!0}),t.global!=null&&await Dn(Xe,t.global||[],{prune:!0}),M(await zn(e))):M(await jd(e,t.appearance||{}))},{match:$("/v1/models/test"),handler:async({body:e})=>M(await Eh(e.llm&&typeof e.llm=="object"?e.llm:null))},{match:$("/v1/nai/test"),handler:async()=>M(await Th())},{match:$("/v1/nai/probe","/v1/debug/probe-nai"),handler:async()=>M(await jh())},{match:$("/v1/debug/clear"),handler:()=>(Zr(),M({ok:!0,cleared:!0}))},{match:$("/v1/debug/asset-tags"),handler:async({body:e})=>M(await kd(e))},{match:$("/v1/nai/reference","/v1/nai/reference/upload"),handler:async({body:e})=>{if(e.clear)return M(await gl());const t=Wr(e);if(!u(t))throw new Error("image_b64 required");return M(await Pp(Z($e(t))))}},{match:$("/v1/nai/reference/clear"),handler:async()=>M(await gl())},{match:$("/v1/nai/vibe","/v1/nai/vibe/upload"),handler:async({body:e})=>{const t=u(e.preset_id||e.presetId||"",120);if(e.clear)return M(t?await yl(t):await hl());if(t&&e.copy_from){const a=u(e.copy_from,120),i=a?await zp(a,t):!1;return M({ok:!0,preset_id:t,configured:i,preview_url:i?oa(t):""})}const n=Wr(e);if(!u(n))throw new Error("image_b64 required");const r={model:e.model,information_extracted:e.information_extracted??e.vibe_transfer_information_extracted,strength:e.strength??e.vibe_transfer_strength};return M(t?await Dp(t,Z($e(n)),r):await Rp(Z($e(n)),r))}},{match:$("/v1/nai/vibe/clear"),handler:async({body:e})=>{const t=u(e?.preset_id||e?.presetId||"",120);return M(t?await yl(t):await hl())}},{match:$("/v1/autotag","/v1/autotag/evaluate"),handler:async({body:e})=>{const t=Wr(e).replace(/\s+/g,"");if(!t)throw new Error("image_b64 required");const n=$e(t);return M(await $h(Z(n),Number(e.threshold??.2)))}},{match:$("/v1/presets/from-image"),handler:async({body:e})=>{const t=Wr(e).replace(/\s+/g,"");if(!t)throw new Error("image_b64 required");const n=$e(t);return M(await Ph(Z(n)))}}];async function Fh(e){const t=u(e.session_id||"",200),n=u(e.character_id||"",200),r=(Array.isArray(e.root_session_ids)?e.root_session_ids:Array.isArray(e.cascade_session_ids)?e.cascade_session_ids:[]).map(i=>u(i,200)).filter(Boolean);if("characters"in e&&t&&await Dn(t,e.characters||[],{prune:!0,rootSessionIds:r}),"global"in e&&await Dn(Xe,e.global||[],{prune:!0}),"character"in e){const i=u(e.scope||t||"__global__",200);r.length&&i!=="__global__"?await ec(r,[e.character||{}],""):await ct(i,e.character||{})}const a=Array.isArray(e.root_delete)?e.root_delete:Array.isArray(e.cascade_delete)?e.cascade_delete:[];return a.length&&r.length&&await Ed(r,a,""),r.length&&t&&await tc(t,r,!1),M(await zn(t,n))}async function au(e,t){for(const n of e){const r=n.match(t.pathname);if(r!==null)return n.handler({...t,param:r})}return nr(t.pathname)}async function Dh(e,t={}){const{pathname:n,query:r}=Oh(e),a=String(t.method||"GET").toUpperCase();let i;if(typeof t.body=="string")try{i=JSON.parse(t.body)}catch{i={}}else i=t.body??{};if(["/v1/health","/healthz","/readyz"].includes(n))return M({ok:!0,health:Sp()});if(n==="/v1/debug"||n==="/v1/debug/log")return a==="DELETE"||a==="POST"&&i.clear?(Zr(),M({ok:!0,cleared:!0})):M(_e());if(!Lh(j(),t.headers||{})){const s="invalid token";throw _t(401,{ok:!1,...ae(s,"unauthorized")},s)}const o={method:a,pathname:n,query:r,body:i};if(a==="GET")return au(Bh,o);if(["POST","PUT","PATCH"].includes(a))try{return await au(Rh,o)}catch(s){if(qi(s))throw s;const c=String(s?.message||s);throw _t(500,{ok:!1,...ae(c,"internal")},c)}throw _t(405,{ok:!1,...ae(a,"method_not_allowed")},a)}te(),G(),gt(),br(),Zc(),Oe(),fe(),ft();var Vr=null;async function zh(){w("boot.ready.start",{message:Ye}),await ot();const e=await yr().catch(r=>{throw w("boot.storage",{message:String(r?.message||r)},"error"),r});w("boot.storage",{message:e.kind}),ur(await gp()),await hp(),await Pd(),await Od();const t=await D("meta","reference_image");t?.png&&aa(Et(t.png));const n=await D("meta","vibe_transfer");n?.png&&ia(Et(n.png)),await Jp(),w("boot.ready.done",{message:Ye,has_nativeFetch:he("nativeFetch"),has_idb:he("getLocalPluginStorage")})}async function Ci(){return Vr||(Vr=zh().catch(e=>{throw Vr=null,e})),await Vr,!0}async function Uh(e,t={},n=12e4){await Ci();try{const r=await Dh(e,t);if(r.raw)return r.data;if(r.status>=400)throw _t(r.status,r.data);return r.data}catch(r){if(qi(r))throw r;const a=String(r?.message||r);throw _t(500,{ok:!1,...ae(a,"internal")},a)}}async function Jh(e){return await Ci(),Zt(e)}function Hh(){Ui(()=>({cards:$n("cards"),images:$n("images"),jobs:$n("jobs"),blob_urls:Sf()})),Reflect.set(globalThis,"__INLAY_NATIVE__",{VERSION:Ye,ready:Ci,fetch:Uh,resolveImageUrl:Le,refPreviewUrl:Lo,vibePreviewUrl:Bo,ensureImageUrl:Jh,warmImages:Ef,pinImageUrls:Cf,warmProgress:Mf,onWarmProgress:Nf,debug:_e,clearDebug:Zr})}var Kh=fn({applyExplorerClick:()=>Wh,clearSelection:()=>Yh,createSelectionState:()=>qh,moveFocus:()=>Xh,reorderByIds:()=>e_,selectAll:()=>Vh,sortExplorerItems:()=>Zh,thumbMinWidth:()=>Qh,visibleIds:()=>Gh});function qh(e=[]){return{selected:new Set((e||[]).map(String).filter(Boolean)),anchorId:"",focusId:""}}function Gh(e=[]){return(e||[]).map(t=>String(t?.id||"")).filter(Boolean)}function Wh(e,t,{index:n,ids:r,shift:a=!1,ctrl:i=!1}={}){const o={selected:new Set(e?.selected||[]),anchorId:String(e?.anchorId||""),focusId:String(e?.focusId||"")},s=String(t||"");if(!s)return o;const c=Array.isArray(r)?r.map(String):[],l=typeof n=="number"&&Number.isFinite(n)?n:c.indexOf(s);if(a&&o.anchorId&&c.length){const f=c.indexOf(o.anchorId),d=l>=0?l:c.indexOf(s);if(f>=0&&d>=0){const m=Math.min(f,d),p=Math.max(f,d);return o.selected=new Set(c.slice(m,p+1)),o.focusId=s,o}}return i?(o.selected.has(s)?o.selected.delete(s):o.selected.add(s),o.anchorId=s,o.focusId=s,o):(o.selected=new Set([s]),o.anchorId=s,o.focusId=s,o)}function Vh(e,t=[]){return{selected:new Set((t||[]).map(String).filter(Boolean)),anchorId:String(e?.anchorId||t?.[0]||""),focusId:String(e?.focusId||t?.[0]||"")}}function Yh(e=null){return{selected:new Set,anchorId:String(e?.anchorId||""),focusId:""}}function Xh(e=[],t="",n=1){const r=(e||[]).map(String).filter(Boolean);if(!r.length)return"";const a=r.indexOf(String(t||""));return a<0?r[0]:r[Math.max(0,Math.min(r.length-1,a+n))]}function Zh(e=[],t="newest"){const n=[...e||[]],r=(a,i=0)=>{const o=Number(a);return Number.isFinite(o)?o:i};return t==="oldest"?n.sort((a,i)=>r(a.created_at)-r(i.created_at)||String(a.id).localeCompare(String(i.id))):t==="message"?n.sort((a,i)=>r(a.message_index,1e9)-r(i.message_index,1e9)||r(a.shot_index)-r(i.shot_index)||r(i.created_at)-r(a.created_at)):t==="shot"?n.sort((a,i)=>r(a.shot_index)-r(i.shot_index)||r(a.message_index,1e9)-r(i.message_index,1e9)||r(i.created_at)-r(a.created_at)):n.sort((a,i)=>r(i.created_at)-r(a.created_at)||String(i.id).localeCompare(String(a.id)))}function Qh(e="m"){return e==="s"?96:e==="l"?200:148}function e_(e=[],t=[]){const n=i=>{const o=i&&typeof i=="object"?i.id:void 0;return String(o??i)},r=new Map((e||[]).map(i=>[n(i),i])),a=[];for(const i of t||[]){const o=String(i);r.has(o)&&(a.push(r.get(o)),r.delete(o))}for(const i of r.values())a.push(i);return a}Er(),Jc();function t_(){Reflect.set(globalThis,"__INLAY_VIEWER_CORE__",{...Ag}),Reflect.set(globalThis,"__INLAY_LLM__",{...op}),Reflect.set(globalThis,"__INLAY_EMBED__",{EMBEDDING_PROVIDERS:ii,normalizeEmbeddingProvider:Ce,defaultEndpointForEmbedding:Ir,defaultModelForEmbedding:Gn,embeddingModelPlaceholder:Zm,shouldAutoReplaceEmbeddingEndpoint:ep,shouldAutoReplaceEmbeddingModel:tp,embeddingProviderNeedsApiKey:np}),Reflect.set(globalThis,"__INLAY_LORE_EXTRA__",{...Lf}),Reflect.set(globalThis,"__INLAY_EXPLORER__",{...Kh})}te(),G(),t_(),Hh(),w("boot.loaded",{message:Ye})})();
...
var Zt = "inlay-nexus-native", ea = "Inlay Nexus", He = "2.1.5", rn = class extends Error {
  segment;
  constructor($) {
