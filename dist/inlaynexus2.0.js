...
`)}function Bs(){return["## Per-actor ids (strict catalog-id mode)",'Add `"characters": [ { "index": 0, "option_ids": ["id", "..."] }, ... ]` to EVERY shot.','Each shot input includes `cast`: `[{ "index": 0, "name": "...", "gender": "girl|boy|""" }, ...]`.',"`index` MUST match `cast[].index` / Pass-1 `characters` array order — use name+gender from `cast`, not chat mention order.","Example: if cast says index 0 = 보민 (boy) and index 1 = Serin (girl), put bound/tears on the boy's index, not the girl's.","Put an option on `characters[].option_ids` when it targets ONE specific actor (pose, contact, expression, gaze).","Put an option on the shot-level `curation_option_ids` instead when it is scene-wide (camera, place, framing) — do not duplicate it into every actor.","Every actor in `cast` needs an entry, even if `option_ids` is `[]`.","This mode has NO freeform camera/situation/action fallback — an unlisted concept is simply omitted, never paraphrased."]}function Dd(e,t,n){const r=n?.strictIds===!0,a=new Set(t.map(s=>l(s,160)).filter(Boolean)),i=Os(e).filter(s=>a.has(s.id)),o=(i.length?i:[]).map(s=>{const c=s.options.map(u=>`  - ${u.id}: ${u.description}`).join(`
`);return`### ${s.id} (${s.label})
${c}`});return["Curation two-stage ON (pass 2 — scene refine, BATCH).","You receive ALL shots in one request. Return ONE JSON object covering every shot.",r?'Schema: { "shots": [ { "shot_index": 0, "curation_option_ids": ["id", "..."], "characters": [ { "index": 0, "option_ids": ["id", "..."] } ] }, ... ] }':'Schema: { "shots": [ { "shot_index": 0, "curation_option_ids": ["id", "..."], "camera": "...", "situation": "...", "place": "...", "action": "..." }, ... ] }',r?'Example: { "shots": [ { "shot_index": 0, "curation_option_ids": ["bedroom"], "characters": [ { "index": 0, "option_ids": ["cowboy_shot", "smile"] }, { "index": 1, "option_ids": ["facing_each_other"] } ] } ] }':'Example: { "shots": [ { "shot_index": 0, "curation_option_ids": ["cowboy_shot", "facing_each_other", "smile", "bedroom"], "place": "bedroom" } ] }',"shot_index must match the input shots array (0-based). Return exactly one entry per input shot, same order.","","## CRITICAL: fill curation_option_ids","`curation_option_ids` is the MAIN output — host injects catalog tags from those ids.","For EVERY shot pick ALL listed options that match the chat at that y_percent (pose, contact, expression, place…).","Typical **2–8 ids**; `[]` ONLY if nothing fits. Do not skip obvious matches.",r?"Do NOT invent character appearance/attire. Do NOT invent option ids. Do NOT write freeform camera/situation/action text — ids only.":"Prefer ids over freeform camera/situation/action paraphrase.",...r?[]:["Do NOT invent character appearance/attire. Do NOT invent option ids."],...Rs(),...r?["",...Bs()]:[],"",o.length?"## Allowed options (union of all shots' groups)":`## Allowed options
(none — leave scene fields empty)`,...o].join(`
`)}function Rs(){return["## Chat context (separate user message, READ-ONLY)","A prior user message is the exact chat text from pass 1 (for LLM prompt cache).","Use it for continuity only (who the cast is, ongoing scene).","Do NOT rewrite, quote-edit, summarize into the JSON, or invent missing plot beats.","Do NOT change shot count, shot_index, composition_id, composition_variant, or y_percent.","","## Shot focus bands (PRIMARY evidence)","Each shot includes `y_percent`, `focus: { from_percent, to_percent }`, and `focus_hint` (a short quote from that band of the chat).","Pick options primarily from `focus_hint` / the `focus` percent range for THAT shot.","Do NOT pull actions from far-away bands into this shot. Full chat is background, not a license to copy the same ids onto every shot.","When focuses differ, prefer distinct `curation_option_ids` / per-actor ids across shots — avoid pasting one identical set on every shot_index.","Do NOT invent aftermath tags (`after_sex`, `afterglow`, cum_on_*, etc.) unless `focus_hint` explicitly shows that aftermath.","y_percent / focus are already final — never output new y_percent or focus fields."]}function zd(){return["Curation embed-snap mode: write DETAILED English Danbooru scene tags.","Fill camera, situation, place, action with concrete tags (framing, pose, contact, location, expression-as-scene).",'Use standard Danbooru spacing (underscores as spaces): e.g. "cowboy shot", "from side", "hand on own chest", "facing another".','Prefer specific pose/contact/framing over vague words like "intimate" or "dramatic".',"Character appearance/attire/accessories stay in character fields — do not move them into camera/situation.","`natural` still follows the Natural base mode system message."].join(`
`)}function Ud(e){const t=e.prompt_order;if(!t?.length)return za;const n=[],r=new Set;for(const a of t){const i=Cs(a)||(l(a,40).toLowerCase()==="char"?"char":null);i&&!r.has(i)&&(r.add(i),n.push(i))}for(const a of za)r.has(a)||n.push(a);return n}function Jd(e,t){const n=Fs(e,t);return I(...Ud(e).map(r=>n[r]))}function Fs(e,t){const n=Array.isArray(t)?t:l(t,800).split(/[,|]/),r={base:[],char:[],primary:[],secondary:[],female:[],male:[]},a=new Set;for(const i of n){const o=l(i,120);if(!o||a.has(o))continue;const s=tn(e,o);if(!s)continue;a.add(o);const c=s.option.by_slot;if(c&&Object.keys(c).length){for(const u of Object.keys(c)){const d=l(c[u],800);d&&r[u].push(d)}continue}r[It(s.group.id)].push(s.option.tags)}return{base:I(...r.base),char:I(...r.char),primary:I(...r.primary),secondary:I(...r.secondary),female:I(...r.female),male:I(...r.male)}}function jt(e,t){t&&(e.action=I(l(e.action,600),t))}function Da(e,t,n){const r={...La(),...t};e.camera=l(r.base,800),e.situation=l(n?.situation,400)||"",n&&"place"in n&&(e.place=l(n.place,400));const a=(Array.isArray(e.characters)?e.characters:[]).filter(o=>o&&typeof o=="object"),i=!!(r.char||r.primary||r.secondary||r.female||r.male);if(!a.length){e.action=i?I(r.char,r.primary,r.secondary,r.female,r.male):"";return}i&&(e.action=""),Hd(a,r)}function Ds(e){const t=Array.isArray(e?.characters)?e.characters:[],n=[];for(let r=0;r<t.length;r++){const a=t[r];if(!a||typeof a!="object"){n.push({index:r,name:"",gender:""});continue}const i=a,o=In(i);n.push({index:r,name:l(i.name,200),gender:o==="f"?"girl":o==="m"?"boy":""})}return n}function zs(e){if(!Array.isArray(e))return[];const t=[];for(const n of e){if(!n||typeof n!="object")continue;const r=n,a=Number(r.index);!Number.isInteger(a)||a<0||t.push({index:a,option_ids:r.option_ids??r.ids??r.curation_option_ids})}return t}function Us(e,t,n){if(!t.length)return;const r=Array.isArray(e.characters)?e.characters:[];for(const{index:a,option_ids:i}of t){const o=r[a];if(!o||typeof o!="object")continue;const s=Jd(n,i);s&&jt(o,s)}}function Hd(e,t){t.primary&&jt(e[0],t.primary),t.secondary&&e[1]&&jt(e[1],t.secondary);for(const n of e){t.char&&jt(n,t.char);const r=In(n);t.female&&r==="f"&&jt(n,t.female),t.male&&r==="m"&&jt(n,t.male)}if(!e.some(n=>In(n)!=null)){const n=I(t.female,t.male);if(n)for(const r of e)jt(r,n)}}function Kd(e){return e.groups.reduce((t,n)=>t+n.options.length,0)}var Gd,qd,za,Ua=$((()=>{$d(),D(),rt(),Gd=10,qd=80,za=["base","char","primary","secondary","female","male"]}));function Wd(e){let t=l(e.composition_id,160)||l(e.preset_id,160)||l(e.leaf_id,160)||l(e.position_id,160)||l(e.curation_leaf,160)||l(e.composition,160);if(t&&/[|/]/.test(t)){const r=t.split(/[/|]/).map(a=>l(a,160)).filter(Boolean);r.length>=2&&((!Array.isArray(e.preset_path)||!e.preset_path.length)&&(e.preset_path=r),t=r[r.length-1])}const n=l(e.composition_variant,120)||l(e.selected_variant_id,120)||l(e.variant_id,120)||l(e.variant,120);return t&&(e.composition_id=t),n&&(e.composition_variant=n),Array.isArray(e.preset_path)&&e.preset_path.length?!0:!!t}function $t(e){return e&&typeof e=="object"&&!Array.isArray(e)?e:null}function Vd(e){const t=[];for(const n of["children","variants"]){const r=e[n];if(Array.isArray(r))for(const a of r){const i=$t(a);i&&t.push(i)}}return t}function Js(e){return Array.isArray(e.variants)?e.variants.map($t).filter(Boolean):[]}function Yd(e){const t=e.when_to_use;return typeof t=="string"?l(t,400):Array.isArray(t)?l(t.map(n=>l(n,200)).filter(Boolean).join(" "),400):l(e.description,400)}function Zd(e){const t=e.avoid_when;return typeof t=="string"?l(t,400):Array.isArray(t)?l(t.map(n=>l(n,200)).filter(Boolean).join(" "),400):""}function Xd(e){if(!e||typeof e!="object"||Array.isArray(e))return null;const t=e,n=l(t.ref??t.id??t.group,160);if(!n)return null;const r=Array.isArray(t.include_options)?t.include_options.map(o=>l(o,120)).filter(Boolean):void 0,a=t.action&&typeof t.action=="object"&&!Array.isArray(t.action)?t.action:null,i=a?{source:l(a.source,40)||void 0,target:l(a.target,40)||void 0,include_options:Array.isArray(a.include_options)?a.include_options.map(o=>l(o,120)).filter(Boolean):void 0}:void 0;return{ref:n,include_options:r?.length?r:void 0,replace:t.replace===!0,order:Number.isFinite(Number(t.order))?Number(t.order):void 0,action:i}}function Hs(e){return Array.isArray(e.modifiers)?e.modifiers.map(Xd).filter(Boolean):[]}function Qd(e){if(!Rn(e))return[];const t=$t(e.presets);if(!t)return[];const n=[],r=(a,i,o,s)=>{const c=l(a.id,160),u=l(a.type,40),d=c&&c!=="preset"?[...i,c]:i,f=!o&&c&&c!=="preset"?c:o,p=u==="category"&&c?c:s,m=Array.isArray(a.children)?a.children.map($t).filter(Boolean):[],g=Js(a);if(c&&c!=="preset"&&(u==="position"||g.length>0&&m.length===0)){const h=Hs(a),_=[...new Set(h.map(y=>y.ref).filter(Boolean))];n.push({id:c,path:d,composition:f||c,category:p,when:Yd(a),avoid:Zd(a),variants:g.map(y=>l(y.id,120)).filter(Boolean),description:l(a.description,300)||c,selectionModifiers:_})}for(const h of m)r(h,d,f,p)};return r(t,[],"",""),n}function Ks(e,t,n){const r=$t(e.presets),a=l(t,160);if(!r||!a)return null;const i={chain:null},o=(f,p)=>{if(i.chain)return;const m=[...p,f];if(l(f.id,160)===a){i.chain=m;return}const g=Array.isArray(f.children)?f.children.map($t).filter(Boolean):[];for(const h of g)o(h,m)};o(r,[]);const s=i.chain;if(!s)return null;const c=l(n,120);if(!c)return s;const u=s[s.length-1],d=Js(u).find(f=>l(f.id,120)===c);return d?[...s,d]:s}function Ja(e,t){const n=t.preset_path;if(Array.isArray(n)&&n.length){const r=n.map(s=>l(s,160)).filter(Boolean),a=$t(e.presets);if(!a||!r.length)return null;const i=[a];let o=a;for(const s of r){if(l(o.id,160)===s)continue;const c=Vd(o).find(u=>l(u.id,160)===s);if(!c)return Ks(e,r[r.length-1],void 0);i.push(c),o=c}return i}return Ks(e,t.composition_id??(typeof n=="string"?n:""),t.composition_variant)}function Ha(e){const t=new Map;for(const n of e)for(const r of Hs(n)){const a=t.get(r.ref);if(r.replace||!a){t.set(r.ref,r);continue}const i=[...new Set([...a.include_options||[],...r.include_options||[]])];t.set(r.ref,{...a,...r,include_options:i.length?i:a.include_options,action:r.action||a.action})}return[...t.values()]}function ef(e,t){const n=new Map,r=new Set;for(const i of t){for(const o of Ha(i))n.set(o.ref,o);for(const o of qs(i))r.add(o)}const a=[];for(const[i,o]of n){if(Ra(e,i))continue;const s=Ls(e,i);if(!s)continue;const c=o.include_options?.length?new Set(o.include_options):null;let u=c?s.options.filter(d=>c.has(d.id)):s.options;if(r.size){const d=Ws(e,u.map(p=>p.id),r),f=new Set(d);u=u.filter(p=>f.has(p.id))}u.length&&a.push({id:s.id,label:s.label,options:u})}return a}function tf(e,t){return t?.length?Ha(t).filter(n=>Ra(e,n.ref)):[]}function Gs(e){const t=l(e,40).toLowerCase();return t?["male","m","boy","man"].includes(t)?"male":["female","f","girl","woman"].includes(t)?"female":["primary","main"].includes(t)?"primary":["secondary","partner"].includes(t)?"secondary":["global","base","scene","camera"].includes(t)?"base":["char","both","all"].includes(t)?"char":null:null}function nf(e){return en(Ts(e))}function rf(e,t,n){const r=tn(e,t);if(!r)return en({});const a=n.find(s=>s.ref!==r.group.id?!1:!s.include_options?.length&&!s.action?.include_options?.length?!0:new Set([...s.include_options||[],...s.action?.include_options||[]]).has(t))||n.find(s=>s.ref===r.group.id);if(r.option.by_slot&&Object.keys(r.option.by_slot).length)return en(r.option.by_slot);const i=r.option.tags||Fn(r.option.tags);if(!i)return en({});if(a?.action){const s=Gs(a.action.source),c=Gs(a.action.target),u={};if(s&&(u[s]=i),c&&(u[c]=I(u[c],i)),s||c)return en(u)}const o=It(r.group.id);return en({[o]:i})}function qs(e){const t=new Set;for(const n of e){const r=n.modifier_filter;if(!r||typeof r!="object"||Array.isArray(r))continue;const a=r.deny;if(!(!a||typeof a!="object"||Array.isArray(a))){for(const i of Object.values(a))if(Array.isArray(i))for(const o of i){const s=l(o,160).toLowerCase();s&&t.add(s)}}}return t}function Ws(e,t,n){return n.size?t.filter(r=>{const a=r.toLowerCase();if(n.has(a))return!1;const i=tn(e,r);if(!i)return!0;const o=l(i.option.tags,800).toLowerCase(),s=l(i.option.description,400).toLowerCase();for(const c of n){const u=c.replace(/_/g," ");if(o.includes(c)||o.includes(u)||s.includes(u))return!1}return!0}):t}function af(e,t,n){const r=new Map;for(const a of n)Number.isFinite(a.order)&&r.set(a.ref,a.order);return r.size?t.map((a,i)=>({id:a,i})).sort((a,i)=>{const o=tn(e,a.id)?.group.id,s=tn(e,i.id)?.group.id,c=o!=null&&r.has(o)?r.get(o):Number.MAX_SAFE_INTEGER,u=s!=null&&r.has(s)?r.get(s):Number.MAX_SAFE_INTEGER;return c!==u?c-u:a.i-i.i}).map(a=>a.id):t}function of(e,t,n){if(!Rn(e))return null;const r=Ja(e,t);if(!r?.length)return null;const a=r.map(p=>nf(p.prompt)),i=Ha(r),o=new Set(i.map(p=>p.ref)),s=[...i,...(n?.bindings||[]).filter(p=>!o.has(p.ref))],c=qs(r),u=[...js(t.curation_option_ids),...n?.optionIds||[]];let d=$s(e,u);d=Ws(e,d,c),d=af(e,d,s);const f=d.map(p=>rf(e,p,s));return Bd(...a,...f)}function sf(){return["# Preset selection (Asset Maid analyzer rules)","Use ONLY information in the chat/shot context. Do not invent unseen actors or actions.","","## Actors → composition first","Count actors that are required for the current moment's action/contact.","Pick a composition that matches actor count and genders (e.g. 1girl_solo vs 1girl_1boy).","Do NOT drop a visible partner to force solo. If a male hand/arm is visible contacting the girl from POV, that male is an actor → prefer 1girl_1boy, not 1girl_solo.","","## Walk the tree","Read the leaf list as composition → category → position.","Categories matter: `general` = non-sexual placement; `foreplay` / `sexual` / `insertion` = intimate contact. If the chat has kissing, licking, sexual touch, or sex, do NOT pick a `general` leaf just because posture when-text matches.","For each candidate use when / avoid / selection_modifiers. Prefer the leaf whose when matches AND whose selection_modifiers cover the stated actions.","selection_modifiers are modifier **group ids** this leaf unlocks for pass 2 (e.g. `interaction.partner.mouth_contact`). They are the main reason to prefer one leaf over another when posture is similar.","CRITICAL: do NOT pick a leaf that only matches vague posture words if it lacks the group needed for the chat action. Example: ear licking / kissing / licking neck need a leaf with `interaction.partner.mouth_contact` (usually under foreplay), not `general/facing_each_other` which only has `interaction.general`.","Pick exactly ONE `composition_id` (position leaf) per shot.","If the same leaf id appears under multiple compositions/categories (e.g. `free`), set `composition_id` to the full path string `composition / category / leaf` so the host can disambiguate.","","## Variant","If the leaf lists variants, pick `composition_variant` for the camera that fits (from_side, pov, default…).","If unsure and a default/from_side exists, prefer that over inventing camera tags.","","## What NOT to do in pass 1","Do NOT invent Danbooru camera/pose tags into camera/situation/action.","Do NOT return modifier option ids here (pass 2 does that — e.g. do not emit `licking_ear` in pass 1).","Do NOT set `curation_groups`."]}function cf(e,t=!1){const n=Qd(e),r=new Map;for(const i of n){const o=r.get(i.composition)||[];o.push(i),r.set(i.composition,o)}const a=[];for(const[i,o]of r){a.push(`## composition: ${i}`);for(const s of o){const c=s.variants.length?s.variants.join("|"):"(none)",u=s.selectionModifiers.length?s.selectionModifiers.join(", "):"(none)",d=s.description&&s.description!==s.id&&s.description!==s.when?`description: ${s.description}`:"";a.push([`### ${s.id}`,`path: ${s.path.join(" / ")}`,s.category?`category: ${s.category}`:"",s.when?`when: ${s.when}`:"",s.avoid?`avoid: ${s.avoid}`:"",d,`selection_modifiers: ${u}`,`variants: ${c}`].filter(Boolean).join(`
`))}}return["Curation two-stage ON (pass 1 — Asset Maid presets).","CRITICAL: every shot MUST include `composition_id` (exact leaf id, or full path if id is ambiguous). This overrides the base format schema.","Also set `composition_variant` when the leaf lists variants.",...sf(),"","On every shot set:","- `composition_id`: leaf id (e.g. `face_to_face_upright`) OR path `1girl_1boy / foreplay / face_to_face_upright` when ids collide","- `composition_variant`: one variant id for that leaf, or omit if none","Still fill characters (appearance/attire/accessories) as usual.","`place` may stay as a short location hint.",...t?["STRICT catalog-id mode is ON: leave `camera`, `situation`, `natural`, and every `characters[].action` / `characters[].expression` completely EMPTY.","Pass 2 assembles ALL scene and per-actor tags from catalog ids only, keyed off the `composition_id`/`composition_variant` you pick here."]:["Leave `situation` empty or non-person scene words only — NEVER put 1girl/1boy/solo there (host adds person-count tags).","`camera`/`action` must NOT include 1girl/1boy/solo either.","`natural` still follows the Natural base mode system message."],"","## Preset leaf catalog (do not invent ids)","selection_modifiers = group ids unlocked for pass 2 (not option ids). Use them to choose the leaf; pass 2 picks concrete options like licking_ear.",...a.length?a:["(empty presets tree)"]].join(`
`)}function lf(e){const t=e.modifier_lanes;if(!t||!Object.keys(t).length)return[];const n=["## Modifier lanes (hard caps — host also enforces)","Within each lane, keep at most max_active_groups distinct groups."];for(const[r,a]of Object.entries(t))n.push(`- lane \`${r}\`: max ${a.max_active_groups} group(s) among [${a.fallback_order.join(", ")}] (earlier pattern wins if over cap)`);return n.push("Example: do not pick both manual.arm_pose and manual.partner_contact options together."),n}function uf(e,t,n){const r=n?.strictIds===!0,a=ef(e,t).map(i=>{const o=i.options.map(s=>`  - ${s.id}: ${s.description}`).join(`
`);return`### ${i.id} (${i.label})
${o}`});return["Curation two-stage ON (pass 2 — preset modifiers, BATCH).","You receive ALL shots in one request. Return ONE JSON object covering every shot.",r?'Schema: { "shots": [ { "shot_index": 0, "curation_option_ids": ["id", "..."], "characters": [ { "index": 0, "option_ids": ["id", "..."] } ], "place": "..." }, ... ] }':'Schema: { "shots": [ { "shot_index": 0, "curation_option_ids": ["id", "..."], "place": "..." }, ... ] }',r?'Example: { "shots": [ { "shot_index": 0, "curation_option_ids": ["bedroom"], "characters": [ { "index": 0, "option_ids": ["hug", "from_side"] }, { "index": 1, "option_ids": ["blush"] } ] } ] }':'Example: { "shots": [ { "shot_index": 0, "curation_option_ids": ["hug", "from_side", "blush", "bedroom"], "place": "bedroom" } ] }',"shot_index must match the input shots array (0-based). Return exactly one entry per input shot, same order.","","## CRITICAL: fill curation_option_ids","`curation_option_ids` is the MAIN output. Host injects those catalog tags into the image — empty array = weak/empty pose.","For EVERY shot, pick ALL options that match the chat band at that shot's y_percent:","- contact / hug / hands / pose / expression / gaze / blush / location / lighting / clothing state when visible","Typical count: **2–8 ids** when the scene has clear action; 1 id only if truly minimal; `[]` ONLY if nothing in the list fits.","Do NOT leave `curation_option_ids` empty just to be safe — if hug/facing/hand contact is in the text and listed below, INCLUDE those ids.","Pick ONLY ids listed below. Never invent ids. Never invent character appearance/attire.","Do NOT put Danbooru tags in freeform fields instead of picking ids — prefer ids.","`place` may repeat a short location hint; optional.","Host assembles path prompts + these options into base vs char slots.",...r?["",...Bs()]:[],...Rs(),...lf(e),"",a.length?"## Allowed modifier options":`## Allowed modifier options
(none — return empty curation_option_ids)`,...a].join(`
`)}var df=$((()=>{D(),Ua()}));function ff(e,t){if(!e.length||e.length!==t.length)return-1;let n=0,r=0,a=0;for(let i=0;i<e.length;i++){const o=e[i],s=t[i];n+=o*s,r+=o*o,a+=s*s}return r<=0||a<=0?-1:n/(Math.sqrt(r)*Math.sqrt(a))}function mf(e,t,n){const r=n?.minScore??.35,a=_f(n?.groupId);let i=null,o=-1,s=null,c=-1;for(const u of t){const d=ff(e,u.vector);d>o&&(o=d,i=u),a&&u.groupId===a&&d>c&&(c=d,s=u)}return s&&c>=r?s:i&&o>=r?i:null}function pf(e){return e.slot||It(e.groupId)}function gf(e,t,n,r=.4){const a=[],i=[],o=[],s=[],c=[],u=[],d=[];let f=0,p=0;const m=(g,h)=>{g==="base"?i.push(h):g==="primary"?s.push(h):g==="secondary"?c.push(h):g==="female"?u.push(h):g==="male"?d.push(h):o.push(h)};for(let g=0;g<e.length;g++){const h=e[g].trim();if(!h)continue;const _=t[g];if(!_||!_.length){a.push(h),i.push(h),p+=1;continue}const y=mf(_,n,{minScore:r});y?(a.push(y.tags),m(pf(y),y.tags),f+=1):(a.push(h),i.push(h),p+=1)}return{tags:a,baseTags:i,charTags:o,primaryTags:s,secondaryTags:c,femaleTags:u,maleTags:d,snapped:f,kept:p}}function hf(e){const t=String(e||"").trim();if(!t)return[];const n=[];let r="",a=0;for(let o=0;o<t.length;o++){const s=t[o];if(s===":"&&t[o+1]===":"){a+=r.includes("::")?-1:1,r+="::",o+=1;continue}if(s===","&&a<=0){const c=r.trim();c&&n.push(c),r="";continue}r+=s}const i=r.trim();return i&&n.push(i),n}function _f(e){return typeof e=="string"?e.trim():""}var yf=$((()=>{Ua()}));function Ne(e){const t=l(e,40).toLowerCase().replace(/[ -]+/g,"_");return t==="openai_compatible"||t==="compat"?"openai_compat":kr.includes(t)?t:"openai"}function xr(e){switch(Ne(e)){case"openai":return"https://api.openai.com/v1/embeddings";case"voyage":return"https://api.voyageai.com/v1/embeddings";case"openrouter":return"https://openrouter.ai/api/v1/embeddings";case"lmstudio":return"http://127.0.0.1:1234/v1/embeddings";case"ollama":return"http://127.0.0.1:11434/api/embeddings";default:return"https://api.openai.com/v1/embeddings"}}function zn(e){switch(Ne(e)){case"voyage":return"voyage-3-lite";case"ollama":return"nomic-embed-text";case"openrouter":return"openai/text-embedding-3-small";case"lmstudio":return"text-embedding-nomic-embed-text-v1.5";default:return"text-embedding-3-small"}}function bf(e){return zn(e)}function wf(){const e=new Set;for(const t of kr){const n=xr(t).replace(/\/+$/,"");e.add(n),e.add(n.replace(/\/embeddings$/i,"")),e.add(n.replace(/\/api\/embeddings$/i,"")),e.add(n.replace(/\/v1$/i,""))}return e}function vf(e){const t=l(e,500);if(!t)return!0;const n=t.replace(/\/+$/,"");return wf().has(n)}function xf(e){const t=l(e,200);return t?new Set(kr.map(n=>zn(n))).has(t):!0}function kf(e){const t=Ne(e);return t!=="lmstudio"&&t!=="ollama"}function Sf(e){const t=Ne(e.provider),n=l(e.endpoint,500)||xr(t);return t==="ollama"?n.replace(/\/$/,""):/\/embeddings\/?$/i.test(n)?n.replace(/\/$/,""):/\/v1\/?$/i.test(n)?`${n.replace(/\/$/,"")}/embeddings`:n}function Af(e){const t=e&&typeof e=="object"?e:{},n=Array.isArray(t.data)?t.data:[],r=[];for(const a of n){const i=a&&typeof a=="object"?a.embedding:null;if(Array.isArray(i)&&i.every(o=>typeof o=="number"))r.push(i);else throw new Error("임베딩 응답에 숫자 벡터가 없습니다.")}return r}async function Vs(e){const t=e.ok!==!1&&(e.status===void 0||e.status>=200&&e.status<300);if(typeof e.json=="function")try{const n=await e.json();if(!t){const r=typeof n=="object"&&n?JSON.stringify(n).slice(0,400):String(n);throw new Error(`임베딩 HTTP ${e.status??"?"}: ${r}`)}return n}catch(n){if(n instanceof Error&&n.message.startsWith("임베딩 HTTP"))throw n}if(typeof e.text=="function"){const n=await e.text();if(!t)throw new Error(`임베딩 HTTP ${e.status??"?"}: ${n.slice(0,400)}`);return n?JSON.parse(n):{}}throw new Error("임베딩 응답을 읽을 수 없습니다.")}async function Ka(e,t,n){if(!t.length)return{vectors:[],model:l(e.model)};const r=Ne(e.provider),a=l(e.model,200)||zn(r),i=Sf({...e,provider:r,model:a}),o=l(e.api_key,4e3);if(r==="ollama"){const u=[];for(const d of t){const f=(await Vs(await Le(i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:a,prompt:d}),signal:n?.signal}))).embedding;if(!Array.isArray(f)||!f.every(p=>typeof p=="number"))throw new Error("Ollama 임베딩 응답이 올바르지 않습니다.");u.push(f)}return{vectors:u,model:a}}const s={"Content-Type":"application/json"};o&&(s.Authorization=`Bearer ${o}`);const c=Af(await Vs(await Le(i,{method:"POST",headers:s,body:JSON.stringify(r==="voyage"?{model:a,input:t,input_type:"document"}:{model:a,input:t}),signal:n?.signal})));if(c.length!==t.length)throw new Error(`임베딩 개수 불일치: got ${c.length}, want ${t.length}`);return{vectors:c,model:a}}async function Mf(e){const{vectors:t,model:n}=await Ka(e,["curation embedding ping"]),r=t[0]?.length||0;if(!r)throw new Error("임베딩 차원이 0입니다.");return{ok:!0,dims:r,model:n}}var Ga,kr,Ys=$((()=>{D(),Nt(),Ga=Object.freeze([{value:"openai",label:"OpenAI"},{value:"voyage",label:"Voyage"},{value:"openrouter",label:"OpenRouter"},{value:"openai_compat",label:"OpenAI-compat"},{value:"lmstudio",label:"LM Studio (로컬)"},{value:"ollama",label:"Ollama (로컬)"},{value:"custom",label:"Custom endpoint"}]),kr=Ga.map(e=>e.value)})),Nf=cn({LLM_ENDPOINT_PRESETS:()=>ct,LLM_PROVIDERS:()=>nc,REASONING_EFFORTS:()=>qa,applyReasoningToBody:()=>tc,defaultEndpointForProvider:()=>Xs,endpointPresetKeyFor:()=>jf,ensureLlmRequestUrl:()=>ec,knownLlmEndpoints:()=>Qs,llmModelPlaceholder:()=>Ef,normalizeLlmProvider:()=>Ct,normalizeReasoningEffort:()=>Zs,shouldAutoReplaceEndpoint:()=>If});function Ct(e){const t=String(e||"").toLowerCase().replace(/[ -]+/g,"_").trim();return["openrouter"].includes(t)?"openrouter":["openai"].includes(t)?"openai":["google","google_ai","google_ai_studio","gemini"].includes(t)?"google_ai":["vertex","vertex_ai","google_vertex"].includes(t)?"vertex":["anthropic","anthropic_compat","anthropic_compatible","claude"].includes(t)?"anthropic_compatible":["lmstudio","lm_studio"].includes(t)?"lmstudio":["ollama"].includes(t)?"ollama":["custom"].includes(t)?"custom":["openai_compatible","openai_compat"].includes(t)?"openrouter":"custom"}function Zs(e){const t=String(e??"default").toLowerCase().trim();return!t||t==="auto"||t==="default"?"default":qa.includes(t)?t:"default"}function Xs(e,t={}){const n=Ct(e);return n==="vertex"?`https://${String(t.region||"us-central1").trim().replace(/[^a-z0-9-]/gi,"")||"us-central1"}-aiplatform.googleapis.com`:n==="anthropic_compatible"?ct.anthropic:n==="custom"?ct.openai:ct[n]||ct.openai}function Qs(){const e=new Set;for(const t of Object.values(ct)){const n=String(t||"").replace(/\/+$/,"");n&&(e.add(n),e.add(n.replace(/\/chat\/completions$/i,"")),e.add(n.replace(/\/v1\/messages$/i,"")),e.add(n.replace(/\/v1$/i,"")))}return e.add("https://us-central1-aiplatform.googleapis.com"),e.add("https://europe-west1-aiplatform.googleapis.com"),e.add("https://asia-northeast1-aiplatform.googleapis.com"),e}function If(e){const t=String(e||"").trim().replace(/\/+$/,"");return t?Qs().has(t):!0}function ec(e,t,n={}){const r=Ct(t);let a=String(e||"").trim().replace(/\/+$/,"");if(a||(a=Xs(r,n).replace(/\/+$/,"")),r==="anthropic_compatible")return/\/v1\/messages$/i.test(a)||/\/messages$/i.test(a)?a:/\/v1$/i.test(a)?`${a}/messages`:`${a}/v1/messages`;if(r==="vertex"){const i=String(n.region||"us-central1").trim().replace(/[^a-z0-9-]/gi,"")||"us-central1",o=String(n.projectId||"").trim();if(o)return`https://${i}-aiplatform.googleapis.com/v1/projects/${encodeURIComponent(o)}/locations/${encodeURIComponent(i)}/endpoints/openapi/chat/completions`;if(/aiplatform\.googleapis\.com/i.test(a)&&!/\/chat\/completions$/i.test(a))return a}return/\/chat\/completions$/i.test(a)?a:/\/openai$/i.test(a)?`${a}/chat/completions`:/\/v1beta$/i.test(a)?`${a}/openai/chat/completions`:/\/v1$/i.test(a)?`${a}/chat/completions`:a}function tc(e,t){const n={};e&&typeof e=="object"&&Object.assign(n,e);const r=Zs(t);return r==="default"||(n.reasoning={effort:r}),n}function Ef(e){const t=Ct(e),n={openrouter:"openai/gpt-4o-mini",openai:"gpt-4o-mini",google_ai:"gemini-2.5-flash",vertex:"gemini-2.0-flash-001",anthropic_compatible:"claude-sonnet-4",lmstudio:"local-model",ollama:"llama3.2",custom:"model-id"};return n[t]||n.custom}function jf(e){const t=String(e||"").trim().replace(/\/+$/,"");for(const[n,r]of Object.entries(ct)){const a=r.replace(/\/+$/,"");if(t===a||t===a.replace(/\/chat\/completions$/i,"")||t===a.replace(/\/v1\/messages$/i,""))return n}return"custom"}var ct,qa,nc,Sr=$((()=>{ct=Object.freeze({openai:"https://api.openai.com/v1/chat/completions",openrouter:"https://openrouter.ai/api/v1/chat/completions",google_ai:"https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",lmstudio:"http://127.0.0.1:1234/v1/chat/completions",ollama:"http://127.0.0.1:11434/v1/chat/completions",anthropic:"https://api.anthropic.com/v1/messages",vertex:"https://us-central1-aiplatform.googleapis.com"}),qa=Object.freeze(["default","none","minimal","low","medium","high","xhigh","max"]),nc=Object.freeze([{value:"openrouter",label:"OpenRouter"},{value:"openai",label:"OpenAI"},{value:"google_ai",label:"Google AI Studio"},{value:"vertex",label:"Vertex AI (Google Cloud)"},{value:"anthropic_compatible",label:"Anthropic-compatible"},{value:"lmstudio",label:"LM Studio (로컬)"},{value:"ollama",label:"Ollama (로컬)"},{value:"custom",label:"Custom endpoint"}])}));function Ar(e){const t=String(e||"").trim().toLowerCase();return t==="main"||t==="risu_main"||t==="risu-main"?"main":t==="aux"||t==="otherax"||t==="other_ax"||t==="risu_aux"||t==="risu-aux"||t==="sub"||t==="secondary"?"aux":"custom"}function $f(e){const t=Ar(e);return t==="main"||t==="aux"}function rc(e){const t=e||{};return $f(t.source)?!0:Ct(t.provider)==="vertex"?!!(l(t.model)&&(l(t.api_key)||l(t.service_account_json))):!!(l(t.model)&&l(t.api_key))}function Cf(e){let t="";const n=[];for(const r of e||[]){const a=String(r?.role||""),i=r?.content;if(a==="system"){const o=typeof i=="string"?i:Array.isArray(i)?i.map(s=>typeof s=="object"?s.text||"":String(s)).join(""):String(i??"");t=t?`${t}
${o}`:o;continue}if(Array.isArray(i)){const o=[];for(const s of i){if(!s||typeof s!="object"){const f=String(s||"").trim();f&&o.push({type:"text",text:f});continue}const c=s.image_url,u=(typeof c=="object"&&c?c.url:"")||(typeof c=="string"?c:"");if(s.type==="image_url"||u){const f=String(u).match(/^data:([^;]+);base64,([\s\S]+)$/i);f&&o.push({type:"image",source:{type:"base64",media_type:f[1]||"image/png",data:f[2].replace(/\s+/g,"")}});continue}const d=String(s.text||"").trim();d&&o.push({type:"text",text:d})}n.push({role:a==="assistant"?"assistant":"user",content:o.length?o:[{type:"text",text:""}]});continue}n.push({role:a==="assistant"?"assistant":"user",content:String(i??"")})}return{system:t,messages:n}}function Tf(e){const t=e?.choices||[];if(!t.length)throw new Error("LLM returned no choices.");let n=(t[0].message||{}).content;return Array.isArray(n)&&(n=n.map(Nr).join("")),l(n)}function Pf(e){const t=e?.content,n=(Array.isArray(t)?t:[]).map(Nr).join(""),r=l(n||e?.completion||"");if(!r)throw new Error("Anthropic 응답이 비어 있습니다.");return r}async function Mr(e){if(!Un(e))return"";const t=e.getReader(),n=typeof TextDecoder<"u"?new TextDecoder:null;let r="",a="";for(;;){const{done:i,value:o}=await t.read();if(i)break;if(typeof o=="string")r=o,a+=o;else if(o instanceof Uint8Array)a+=n?n.decode(o,{stream:!0}):"";else if(o&&typeof o=="object"){const s=o,c=s[0];typeof c=="string"?r=c:typeof s.content=="string"?r=s.content:typeof s.text=="string"&&(r=s.text)}}return n&&(a+=n.decode()),r||a}async function Of(e){if(typeof e=="string")return e;if(Un(e))return Mr(e);if(e==null)return"";if(typeof e=="number"||typeof e=="boolean")return String(e);if(typeof e=="object"){const t=e,n=l(t.type,40).toLowerCase();if(n==="fail"||n==="error"){const a=l(t.result||t.message||t.error||"Risu LLM 실패",800);throw new Error(`Risu LLM 실패: ${a}`)}if(n==="streaming"||n==="stream"){const a=await Mr(t.result??t.data??t.stream);if(a.trim())return a}if(n==="success"||n==="ok"){const a=t.result??t.data??t.content;if(typeof a=="string")return a;if(Un(a))return Mr(a)}const r=[t.choices?.[0]?.message?.content,t.choices?.[0]?.text,t.choices?.[0]?.delta?.content,t.message?.content,t.content,t.text,t.response,n?null:t.result,t.output];for(const a of r){if(typeof a=="string"&&a.trim())return a;if(Array.isArray(a)){const i=a.map(Nr).join("");if(i.trim())return i}if(Un(a)){const i=await Mr(a);if(i.trim())return i}}try{return JSON.stringify(e)}catch{return String(e)}}return String(e||"")}var Un,Nr,Wa=$((()=>{D(),Sr(),Un=e=>!!e&&typeof e.getReader=="function",Nr=e=>typeof e=="object"?e?.text||"":String(e||"")}));async function Lf(){try{const e=await Ae(Jr,xi);if(e==null||e==="")return he(Qe);const t=typeof e=="string"?JSON.parse(e):e;if(!t||typeof t!="object")return he(Qe);const n=Na(t),r=et(Qe,n);return JSON.stringify(t)!==JSON.stringify(n)&&await ac(r),r}catch(e){console.warn("[Inlay Nexus] settings load failed",e?.message||e)}return he(Qe)}async function ac(e){if(await Me(Jr,he(e)),await Ae("inx_native_settings")==null)throw new Error("설정 저장 실패: IndexedDB(getLocalPluginStorage)에 기록되지 않았습니다.")}var ic=$((()=>{X(),yt(),Ut(),pr()}));async function Bf(){const e=await F("meta","prompt:__pack__"),t=!e||e.text!=="2026-08-06-inline-line",n=Date.now()/1e3;for(const r of ln)await F("meta",`prompt:${r}`)&&!(t&&vi.includes(r))||await B("meta",{key:`prompt:${r}`,text:An(r),updated_at:n});await B("meta",{key:"prompt:__pack__",text:wi,updated_at:n})}async function ue(e){const t=await F("meta",`prompt:${e}`);return t?.text!=null?t.text:An(e)}async function oc(e,t){const n=Date.now()/1e3;return await B("meta",{key:`prompt:${e}`,text:t,updated_at:n}),{ok:!0,key:e,updated_at:n}}async function Ir(){const e=await be("meta"),t=new Map(e.filter(r=>r.key?.startsWith("prompt:")&&!r.key.startsWith("prompt:__")).map(r=>[r.key.slice(7),{key:r.key.slice(7),text:r.text||"",updated_at:r.updated_at||0}])),n=[];for(const r of ln){const a=t.get(r);a?n.push(a):n.push({key:r,text:An(r),updated_at:0}),t.delete(r)}for(const r of t.values())n.push(r);return n}async function Rf(){const e=await Ir(),t={};for(const n of e)t[n.key]=n.text;return{version:We,prompts:t}}async function Ff(e){const t=zf(e);let n=0;const r=Date.now()/1e3;for(const a of ln)Object.prototype.hasOwnProperty.call(t,a)&&(await B("meta",{key:`prompt:${a}`,text:String(t[a]??""),updated_at:r}),n+=1);return{ok:!0,updated:n,prompts:await Ir()}}async function Df(e){const t=e?.keep_author_note!==!1,n=Date.now()/1e3;let r=0;for(const a of ln)t&&a==="author_note"||(await B("meta",{key:`prompt:${a}`,text:An(a),updated_at:n}),r+=1);return{ok:!0,updated:r,keep_author_note:t,prompts:await Ir()}}function zf(e){let t=e;if(typeof e=="string"){const i=e.trim();if(!i)return{};t=JSON.parse(i)}if(!t||typeof t!="object"||Array.isArray(t))return{};const n=t,r={},a=n.prompts;if(a&&typeof a=="object"&&!Array.isArray(a)){for(const[i,o]of Object.entries(a))typeof i=="string"&&(r[i]=String(o??""));return r}if(Array.isArray(a)){for(const i of a){if(!i||typeof i!="object")continue;const o=i,s=String(o.key||"");s&&(r[s]=String(o.text??""))}return r}if(typeof n.key=="string"&&"text"in n)return r[n.key]=String(n.text??""),r;for(const[i,o]of Object.entries(n))i==="version"||i==="ok"||typeof o=="string"&&(r[i]=o);return r}async function Je(){const e=he(E());await Mo.run(()=>ac(e))}function Uf(){return rd(E())}async function Jf(e){const t=ad(String(e??"").slice(0,2000001)),n=he(E());await B("meta",{key:"settings_backup",value:n,updated_at:Date.now()/1e3});const r=et(Qe,t);return r.llm={...r.llm,api_key:n.llm?.api_key||""},r.nai={...r.nai,api_key:n.nai?.api_key||""},r.auth_token=n.auth_token||"",ir(r),await Je(),{ok:!0,settings:Er()}}async function Hf(){const e=he(E());await B("meta",{key:"settings_backup",value:e,updated_at:Date.now()/1e3});const t=he(Qe);t.llm={...t.llm,api_key:e.llm?.api_key||""},t.nai={...t.nai,api_key:e.nai?.api_key||""},t.auth_token=e.auth_token||"";const n=e.card;return t.card={...t.card,presets:Array.isArray(n.presets)?he(n.presets):t.card.presets||[],active_preset_id:String(n.active_preset_id||t.card.active_preset_id||""),custom_pos:String(n.custom_pos??t.card.custom_pos??""),custom_neg:String(n.custom_neg??t.card.custom_neg??"")},ir(t),await Je(),{ok:!0,settings:Er()}}function Kf(){const e=E(),t=e.nai,n=e.llm;let r=0,a=0,i=0,o=0;try{r=jn("cards"),a=jn("images"),i=Cu();const s=new Set;for(const c of Tu()){const u=l(c.character_id||"",200)||"unknown",d=l(c.chat_id||"",200)||"unknown";s.add(`${u}|${d}`)}o=s.size}catch{}return{ok:!0,version:We,pid:0,source_file:"native",nai_configured:Bn(t)==="comfy"?$a(t):!!l(t.api_key),image_backend:Bn(t),llm_configured:rc(n),port:0,image_mode:"data-url",storage:"indexeddb",storage_api:"getLocalPluginStorage",storage_scope:"device-local",cards:r,images:a,folders:o,png_bytes:i,last_stage:wn(),focus_stage:mt(),last_error:tr(),debug_events:Ti()}}function Er(){const e=he(E());e.nai?.api_key?(e.nai.api_key="",e.nai.api_key_configured=!0):e.nai={...e.nai,api_key_configured:!1},e.llm?.api_key?(e.llm.api_key="",e.llm.api_key_configured=!0):e.llm={...e.llm,api_key_configured:!1},e.llm?.service_account_json?(e.llm.service_account_json="",e.llm.service_account_configured=!0):e.llm={...e.llm,service_account_configured:!1};const t=e.nai;t.backend=Bn(t),t.image_backend=t.backend,t.comfy_configured=$a(t);const n=So(),r=Ao(),a=!Va.includes(l(t.image_reference));t.image_reference_configured=!!(a&&n),t.image_reference||(t.image_reference=t.image_reference_configured?"file":"none"),t.image_reference_configured&&n&&(t.reference_preview_url=n);const i=!Va.includes(l(t.vibe_transfer));if(t.vibe_transfer_configured=!!(i&&r),t.vibe_transfer||(t.vibe_transfer=t.vibe_transfer_configured?"file":"none"),t.vibe_transfer_configured&&r&&(t.vibe_preview_url=r),Array.isArray(e.card?.presets))for(const c of e.card.presets){if(!c||typeof c!="object")continue;const u=c,d=l(u.id,120),f=d?ea(d):"";f?(u.vibe_configured=!0,u.vibe_preview_url=f):(delete u.vibe_configured,delete u.vibe_preview_url),delete u.vibe_transfer}e.card.character_max=nt(e.card),e.database_path="indexeddb:getLocalPluginStorage",e.images_dir="indexeddb:inx_nximg_*",e.prompts_dir="embedded";const o=e.curation&&typeof e.curation=="object"?e.curation:{},s=o.embedding&&typeof o.embedding=="object"?{...o.embedding}:{};return s.api_key?(s.api_key="",s.api_key_configured=!0):s.api_key_configured=!1,o.embedding=s,e.curation=o,e.storage={...e.storage,backend:"indexeddb",api:"getLocalPluginStorage",image_encoding:"base64",scope:"device-local",image_mode:"data-url"},e}async function Gf(e){const t=e&&typeof e=="object"?e:{},n=E(),r={...t.nai||{}},a={...t.llm||{}},i={...t.card||{}};if("api_key"in r){const o=r.api_key;delete r.api_key,delete r.api_key_configured,l(o)&&(n.nai.api_key=o),(r.clearApiKey||t.clear_nai_key)&&(n.nai.api_key=""),delete r.clearApiKey}if("api_key"in a){const o=a.api_key;delete a.api_key,delete a.api_key_configured,l(o)&&(n.llm.api_key=o),(a.clearApiKey||t.clear_llm_key)&&(n.llm.api_key=""),delete a.clearApiKey}if("service_account_json"in a){const o=a.service_account_json;delete a.service_account_json,delete a.service_account_configured,l(o)&&(n.llm.service_account_json=o),(a.clearServiceAccount||t.clear_llm_service_account)&&(n.llm.service_account_json=""),delete a.clearServiceAccount}if(Object.keys(i).length){const o=et(n.card,i);Array.isArray(i.presets)&&(o.presets=i.presets),"active_preset_id"in i&&(o.active_preset_id=i.active_preset_id),"custom_pos"in i&&(o.custom_pos=i.custom_pos),"custom_neg"in i&&(o.custom_neg=i.custom_neg),n.card=o,n.card.character_max=nt(n.card)}if(Object.keys(r).length&&(n.nai=et(n.nai,r)),Object.keys(a).length&&(n.llm=et(n.llm,a)),t.curation&&typeof t.curation=="object"){const{updateCurationSettings:o}=await Promise.resolve().then(()=>(Kn(),Zf));await o(t.curation)}for(const o of["bind_host","port","auth_token"])o in t&&(n[o]=t[o]);return await Je(),{ok:!0,settings:Er()}}var Va,lt=$((()=>{Ut(),Ln(),X(),W(),yt(),D(),rt(),Wa(),ic(),Ce(),_e(),Va=["","none","off","false","0"]}));function sc(e){const t=typeof e=="string"?e:JSON.stringify(e);return Ya(Dt(new TextEncoder().encode(t)))}function qf(e){const t=ArrayBuffer.isView(e)?new Uint8Array(e.buffer,e.byteOffset,e.byteLength):ge(e);return Ya(Dt(t))}async function Wf(e){let t;try{t=typeof e=="string"?JSON.parse(e):e}catch{throw new Error("Vertex Service Account JSON 파싱 실패")}if(!t?.client_email||!t?.private_key)throw new Error("Service Account JSON에 client_email/private_key가 필요합니다.");if(!globalThis.crypto?.subtle)throw new Error("이 환경에서는 Vertex Service Account(JWT) 서명을 지원하지 않습니다. API key 칸에 OAuth access token을 넣거나 Google AI Studio를 쓰세요.");const n=Math.floor(Date.now()/1e3),r=`${sc({alg:"RS256",typ:"JWT"})}.${sc({iss:t.client_email,scope:"https://www.googleapis.com/auth/cloud-platform",aud:"https://oauth2.googleapis.com/token",iat:n,exp:n+3600})}`,a=String(t.private_key).replace(/\\n/g,`
`).replace(/-----BEGIN [^-]+-----/,"").replace(/-----END [^-]+-----/,"").replace(/\s+/g,""),i=Uint8Array.from(atob(a),d=>d.charCodeAt(0)),o=await crypto.subtle.importKey("pkcs8",i,{name:"RSASSA-PKCS1-v1_5",hash:"SHA-256"},!1,["sign"]),s=`${r}.${qf(await crypto.subtle.sign("RSASSA-PKCS1-v1_5",o,new TextEncoder().encode(r)))}`,c=await Le("https://oauth2.googleapis.com/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:`grant_type=${encodeURIComponent("urn:ietf:params:oauth:grant-type:jwt-bearer")}&assertion=${encodeURIComponent(s)}`});let u={};try{u=await c.json()}catch{u={}}if(!u?.access_token)throw new Error(`Vertex 토큰 발급 실패: ${JSON.stringify(u).slice(0,240)}`);return{accessToken:String(u.access_token),projectId:l(t.project_id||"")}}var Ya,Vf=$((()=>{ce(),D(),Nt(),Ya=e=>e.replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/g,"")}));async function nn(e,t){const n=Ar(e.source);if(n==="main"||n==="aux")return Yf(e,t,n);const r=Ct(e.provider),a=l(e.model),i=l(e.vertex_region)||"us-central1";let o=l(e.api_key),s="";if(r==="vertex"&&l(e.service_account_json)){const m=await Wf(e.service_account_json);o=m.accessToken,s=m.projectId}if(!a||!o)throw w("llm.config",{message:"missing model/api_key",provider:r},"error"),new Error(r==="vertex"?"Vertex AI: Model + Service Account JSON(또는 access token)이 필요합니다.":"태깅 LLM이 설정되지 않았습니다. 모델 설정에서 Provider·Model·API key를 넣으세요. (NovelAI 키와 별개)");const c=ec(l(e.endpoint),r,{region:i,projectId:s});if(r==="vertex"&&!/\/chat\/completions$/i.test(c))throw new Error("Vertex AI: project_id가 있는 Service Account JSON이 필요합니다. (OpenAI-compatible endpoint 구성용)");const u=Number(e.timeout_seconds??180)*1e3,d=typeof AbortController<"u"?new AbortController:null,f=setTimeout(()=>{w("llm.abort",{message:`timeout ${u}ms`,model:a,provider:r},"warn"),d?.abort?.()},u),p=se("llm.call");w("llm.call.start",{message:a,msgs:t?.length||0,timeout_ms:u,source:n,provider:r,reasoning:l(e.reasoning_effort)||"default"});try{let m;if(r==="anthropic_compatible"){const y=Cf(t),v={model:a,max_tokens:Number(e.max_tokens??8e3),temperature:Math.min(1,Number(e.temperature??.4)),messages:y.messages};y.system&&(v.system=y.system),m=await Le(c,{method:"POST",headers:{"Content-Type":"application/json","x-api-key":o,"anthropic-version":l(e.anthropic_version)||"2023-06-01"},body:JSON.stringify(v),signal:d?.signal})}else{const y=tc({model:a,messages:t,temperature:Number(e.temperature??.4),max_tokens:Number(e.max_tokens??8e3)},e.reasoning_effort),v={"Content-Type":"application/json",Authorization:`Bearer ${o}`};r==="openrouter"&&(v["HTTP-Referer"]="https://risuai.xyz",v["X-Title"]="Inlay Nexus"),m=await Le(c,{method:"POST",headers:v,body:JSON.stringify(y),signal:d?.signal})}const g=Number(m?.status||0);let h;try{h=await m.json()}catch{h={}}if(g>=400)throw p.fail(new Error(`HTTP ${g}`),{status:g,body:JSON.stringify(h).slice(0,200),provider:r}),new Error(`LLM HTTP ${g}: ${JSON.stringify(h).slice(0,500)}`);const _=r==="anthropic_compatible"?Pf(h):Tf(h);return p.end({message:a,status:g,bytes:_.length,provider:r}),_}catch(m){throw p.fail(m,{model:a,provider:r}),m}finally{clearTimeout(f)}}async function Yf(e,t,n,r=""){const a=Rt();if(!a||typeof a.runLLMModel!="function")throw new Error("RisuAI runLLMModel API를 사용할 수 없습니다. Risu를 최신으로 업데이트하세요.");const i=n==="main"?"model":"otherAx",o=Math.max(5e3,Number(e.timeout_seconds??180)*1e3),s=l(r)||"",c=se("llm.call");w("llm.call.start",{message:`risu:${i}`,msgs:t?.length||0,timeout_ms:o,source:n,static_model:s});let u;const d=new Promise((f,p)=>{u=setTimeout(()=>p(new Error(`Risu LLM timeout ${o}ms (${i})`)),o)});try{const f=await Promise.race([a.runLLMModel({mode:i,...s?{staticModel:s}:{},allowPlugins:!0,messages:t}),d]),p=l(await Of(f));if(!p)throw new Error(`Risu LLM(${i}) 응답이 비어 있습니다.`);return c.end({message:`risu:${i}`,bytes:p.length}),p}catch(f){throw c.fail(f,{mode:i}),f}finally{u!==void 0&&clearTimeout(u)}}var Za=$((()=>{W(),Ft(),D(),Nt(),Vf(),Sr(),Wa()})),Zf=cn({catalogSha:()=>Dn,curationStatus:()=>Hn,curationTaggerSystemMessage:()=>pc,embedCurationCatalog:()=>dc,getCurationMode:()=>jr,getCurationStrictIds:()=>$r,getEmbedProgress:()=>cc,getEmbeddingSettingsFromConfig:()=>Jn,loadCurationCatalog:()=>ut,refineShotsWithCuration:()=>gc,resetCurationCatalog:()=>uc,saveCurationCatalog:()=>lc,snapShotsSceneTags:()=>yc,testCurationEmbedding:()=>fc,updateCurationSettings:()=>bc});function cc(){return{...rn}}function jr(){const e=E(),t=e.curation&&typeof e.curation=="object"?e.curation:{},n=ka(t.mode);if(n!=="off")return n;const r=e.card;return r?.composition_curation===!0||r?.composition_curation==="true"||r?.composition_curation===1?"two_stage":"off"}function $r(){const e=E(),t=e.curation&&typeof e.curation=="object"?e.curation:{};return Sa(t.strict_ids)}function Jn(){const e=E(),t=e.curation&&typeof e.curation=="object"?e.curation:{},n=t.embedding&&typeof t.embedding=="object"?t.embedding:{},r=Ne(n.provider);return{provider:r,model:l(n.model,200)||zn(r),endpoint:l(n.endpoint,500)||xr(r),api_key:l(n.api_key,4e3)}}async function ut(){const e=await Ae(Zn);if(e==null||e==="")return Fa();try{const t=typeof e=="string"?JSON.parse(e):e;return Ba(t)}catch(t){return w("curation.catalog.load",{message:String(t?.message||t)},"warn"),Fa()}}async function lc(e){const t=Ba(e);await Me(Zn,t);const n=await Xa();return n&&(n.catalog_sha,t.sha),{ok:!0,status:await Hn()}}async function uc(){const e=Fa();return await Me(Zn,e),await le(Xn),{ok:!0,status:await Hn()}}async function Xa(){const e=await Ae(Xn);if(e==null||e==="")return null;try{const t=typeof e=="string"?JSON.parse(e):e;if(!t||typeof t!="object")return null;const n=t;return Array.isArray(n.items)?n:null}catch{return null}}async function Hn(){const e=await ut(),t=await Xa(),n=Kd(e);let r="missing";if(t?.items?.length){r=t.catalog_sha===e.sha?"ready":"stale";const a=Jn(),i=l(a.model);i&&t.model&&t.model!==i&&(r="stale")}return{mode:jr(),catalog_name:e.name,catalog_sha:e.sha||Dn(e),option_count:n,group_count:e.groups.length,has_presets:Rn(e),embed_status:r,embed_count:t?.items?.length||0,embed_model:t?.model||"",embed_updated_at:t?.updated_at||0,large_warning:n>wc,embed_progress:cc()}}async function dc(e){const t=await ut(),n=Rd(t),r=Jn();if(!n.length)throw new Error("임베딩할 카탈로그 항목이 없습니다.");if(Ne(r.provider)!=="ollama"&&Ne(r.provider)!=="lmstudio"&&!l(r.api_key))throw new Error("임베딩 API key가 없습니다. 큐레이팅 탭에서 키를 저장하세요.");const a=(c,u,d)=>{rn={running:c<u,done:c,total:u,message:d},e?.(c,u,d)},i=Ne(r.provider)==="ollama"?1:16,o=[];let s=r.model;a(0,n.length,"임베딩 시작…");try{for(let u=0;u<n.length;u+=i){const d=n.slice(u,u+i);a(u,n.length,`임베딩 중… ${u}/${n.length}`);const{vectors:f,model:p}=await Ka(r,d.map(m=>m.text));s=p||s;for(let m=0;m<d.length;m++){const g=d[m],h=f[m];if(!h?.length)throw new Error(`임베딩 실패: ${g.key}`);o.push({key:g.key,groupId:g.groupId,optionId:g.optionId,tags:g.tags,slot:g.slot,vector:h})}}a(n.length,n.length,"저장 중…");const c={catalog_sha:t.sha||Dn(t),model:s,provider:String(r.provider),updated_at:Date.now()/1e3,items:o};return await Me(Xn,c),a(n.length,n.length,"완료"),rn={running:!1,done:n.length,total:n.length,message:"완료"},{ok:!0,status:await Hn()}}catch(c){throw rn={running:!1,done:rn.done,total:n.length,message:`실패: ${c?.message||c}`},c}}async function fc(){const e=Jn();return Mf(e)}function mc(e){return!!(e.base||e.char||e.primary||e.secondary||e.female||e.male)}async function pc(){const e=jr();if(e==="off")return null;if(e==="embed_snap")return l(await ue("curation_embed_hint"),4e3)||zd();const t=await ut(),n=$r();return Rn(t)?cf(t,n):Fd(t,n)}async function gc(e,t){if(!e.length)return;const n=l(t?.chatContext,1e5),r=await ut();if(Rn(r)){for(const a of e)Wd(a);if(e.some(a=>!!Ja(r,{preset_path:a.preset_path,composition_id:a.composition_id,composition_variant:a.composition_variant}))){await Qf(r,e,n);return}w("curation.preset_refine",{message:"no composition_id on shots → falling back to group refine"},"warn")}await Xf(r,e,n)}function hc(e,t,n){const r=[{role:"system",content:e}];return t&&r.push({role:"user",content:t}),r.push({role:"user",content:JSON.stringify(n)}),r}async function Xf(e,t,n){const r=$r(),a=g=>{const h=g.curation_groups;return Array.isArray(h)?h.map(_=>l(_,160)).filter(Boolean):l(h,400).split(/[,|]/).map(_=>_.trim()).filter(Boolean)},i=t.map(a),o=[...new Set(i.flat())];if(!o.length){w("curation.group_refine",{message:"no curation_groups on shots — skip"},"warn");return}const s=Dd(e,o,{strictIds:r}),c=`${l(await ue("curation_refine"),6e3)||s}

${s}`,u=Ss(t,n),d={shots:t.map((g,h)=>({shot_index:h,y_percent:g.y_percent??g.anchor_percent??null,...u[h],curation_groups:i[h],camera:g.camera||"",situation:g.situation||g.scene||"",place:g.place||"",action:g.action||"",natural:g.natural||"",character_count:Array.isArray(g.characters)?g.characters.length:0,cast:Ds(g)}))},f=await nn(E().llm,hc(c,n,d)),p=Yr(f);if(!p||typeof p!="object")return;_c(t,p,(g,h)=>{if(!h||typeof h!="object")return;const _=h.curation_option_ids??h.option_ids,y=Fs(e,_);mc(y)?(Da(g,y,{place:h.place!=null?h.place:g.place,situation:""}),g.curation_option_ids=_):(h.place!=null&&(g.place=l(h.place,400)),r||(h.camera!=null&&(g.camera=l(h.camera,600)),h.situation!=null&&(g.situation=l(h.situation,600)),h.action!=null&&(g.action=l(h.action,600)))),r&&Array.isArray(h.characters)&&Us(g,zs(h.characters),e)})}async function Qf(e,t,n){const r=$r(),a=t.map(f=>Ja(e,{preset_path:f.preset_path,composition_id:f.composition_id,composition_variant:f.composition_variant}));if(!a.some(Boolean))return;const i=uf(e,a.filter(Boolean),{strictIds:r}),o=Ss(t,n),s={shots:t.map((f,p)=>({shot_index:p,y_percent:f.y_percent??f.anchor_percent??null,...o[p],composition_id:f.composition_id||"",composition_variant:f.composition_variant||"",place:f.place||"",natural:f.natural||"",character_count:Array.isArray(f.characters)?f.characters.length:0,cast:Ds(f)}))};let c=null;try{const f=await nn(E().llm,hc(i,n,s));c=Yr(f)}catch(f){w("curation.preset_refine",{message:String(f?.message||f)},"warn")}const u=new Map,d=(f,p,m)=>{const g=p&&typeof p=="object"?p.curation_option_ids??p.option_ids:f.curation_option_ids,h=g!=null?$s(e,g):[];for(const x of tf(e,a[m])){const N=[...x.include_options||[],...x.action?.include_options||[]];N.length&&u.set(x.ref,{ids:N,binding:x})}const _=[...u.values()].flatMap(x=>x.ids),y=[...u.values()].map(x=>x.binding),v=of(e,{preset_path:f.preset_path,composition_id:f.composition_id,composition_variant:f.composition_variant,curation_option_ids:h},{optionIds:_,bindings:y});if(v&&mc(v)&&(Da(f,v,{place:p&&p.place!=null?p.place:f.place,situation:""}),g!=null&&(f.curation_option_ids=h),e.fixed_positive&&(f.curation_fixed_positive=e.fixed_positive)),r){const x=p&&typeof p=="object"?p.characters:void 0;Array.isArray(x)&&Us(f,zs(x),e)}};if(c&&typeof c=="object"){_c(t,c,d);return}t.forEach((f,p)=>d(f,null,p))}function _c(e,t,n){const r=Array.isArray(t.shots)?t.shots:Array.isArray(t)?t:null;if(r){for(let a=0;a<e.length;a++){const i=r.find(o=>o&&typeof o=="object"&&Number(o.shot_index)===a);n(e[a],i||r[a],a)}return}e.length===1&&n(e[0],t,0)}async function yc(e){if(!e.length)return{ok:!0,snapped:0};const t=await ut(),n=await Xa(),r=t.sha||Dn(t);if(!n?.items?.length)return{ok:!1,reason:"임베딩 없음 → 큐레이션 없이 생성"};if(n.catalog_sha!==r)return{ok:!1,reason:"임베딩 stale → 큐레이션 없이 생성"};const a=e.map(o=>{const s=I(l(o.camera,600),l(o.situation||o.scene,600),l(o.place,400),l(o.action,600));return hf(s)}),i=a.flat();if(!i.length)return{ok:!0,snapped:0};try{const o=Jn(),{vectors:s}=await Ka(o,i);let c=0,u=0;for(let d=0;d<e.length;d++){const f=a[d],p=s.slice(c,c+f.length);if(c+=f.length,!f.length)continue;const{baseTags:m,charTags:g,primaryTags:h,secondaryTags:_,femaleTags:y,maleTags:v,snapped:x}=gf(f,p,n.items);u+=x,Da(e[d],{base:I(...m),char:I(...g),primary:I(...h),secondary:I(..._),female:I(...y),male:I(...v)},{place:"",situation:""})}return{ok:!0,snapped:u}}catch(o){const s=`임베딩 API 실패 → 큐레이션 없이 생성 (${o?.message||o})`;return w("curation.snap",{message:s},"warn"),{ok:!1,reason:s}}}async function bc(e){const t=E(),n=t.curation&&typeof t.curation=="object"?{...t.curation}:{},r={...n,...e};if(e.mode!=null&&(r.mode=ka(e.mode)),e.strict_ids!=null&&(r.strict_ids=Sa(e.strict_ids)),e.embedding&&typeof e.embedding=="object"){const a=n.embedding&&typeof n.embedding=="object"?{...n.embedding}:{},i=e.embedding,o={...a,...i};if("api_key"in i){const s=i.api_key;delete o.api_key,delete o.api_key_configured,l(s)&&(o.api_key=s),(i.clearApiKey||e.clear_embedding_key)&&(o.api_key=""),delete o.clearApiKey}o.provider!=null&&(o.provider=Ne(o.provider)),r.embedding=o}return t.curation=r,e.mode!=null&&t.card&&typeof t.card=="object"&&(t.card.composition_curation=!1),ir(t),await Je(),{ok:!0}}var wc,rn,Kn=$((()=>{Ed(),Ua(),df(),Ln(),yf(),X(),W(),D(),Ys(),yt(),pr(),_e(),lt(),Za(),wc=400,rn={running:!1,done:0,total:0,message:""}}));Kn(),X(),W(),gt(),ce(),Nt();async function Cr(e,t,n,r=1,a=3){const i=ze(n),o=Math.max(0,Math.min(1,Number(r)||1)),s={image:await Ze(t),information_extracted:o,model:i};let c="";for(let u=1;u<=a;u++)try{w("nai.encode_vibe.start",{message:i,ie:o,attempt:u,focus:!0});const d=await Le(Mi,{method:"POST",headers:{Authorization:`Bearer ${e}`,"Content-Type":"application/json"},body:JSON.stringify(s)}),f=Number(d?.status||0);if([502,503,504,520].includes(f)){c=`HTTP ${f}`,await Ye(2*u*1e3);continue}if(f<200||f>=300){let g="";try{g=typeof d?.text=="function"?await d.text():""}catch{}if(c=`HTTP ${f}: ${String(g).slice(0,200)}`,u<a){await Ye(1e3);continue}break}const p=await Qt(d,{timeoutMs:6e4}),m=await Ze(p);return w("nai.encode_vibe.done",{message:i,bytes:p?.byteLength||0,focus:!0}),m}catch(d){if(c=String(d?.message||d),u<a){await Ye(1e3);continue}}throw new Error(`encode-vibe 실패 (${a}회): ${c}`)}X(),D(),Ce(),_e(),lt();var Qa=12582912,He=32;async function em(){const e=await F("meta","reference_image");return!!(e?.png&&e.png.byteLength>He)}async function vc(){return(await F("meta","reference_image"))?.png||null}async function tm(e){if(!e||e.byteLength<He)throw new Error("참조 이미지가 비어 있습니다");if(e.byteLength>Qa)throw new Error("참조 이미지가 너무 큽니다 (최대 12MB)");return await B("meta",{key:"reference_image",png:e}),Xr(Mt(e)),E().nai.image_reference="file",await Je(),{ok:!0,image_reference:"file",configured:!0,bytes:e.byteLength,preview_url:"/v1/nai/reference.png"}}async function xc(){return await xt("meta","reference_image"),Xr(""),E().nai.image_reference="none",await Je(),{ok:!0,image_reference:"none",configured:!1}}async function nm(){const e=await F("meta","vibe_transfer");return!!(e?.encoded&&e?.png&&e.png.byteLength>He)}async function rm(){return await F("meta","vibe_transfer")||null}async function am(){return(await F("meta","vibe_transfer"))?.png||null}function Tr(e){let t=Number(e??1);return Number.isNaN(t)&&(t=1),Math.max(0,Math.min(1,t))}async function im(e,t={}){if(!e||e.byteLength<He)throw new Error("Vibe 이미지가 비어 있습니다");if(e.byteLength>Qa)throw new Error("Vibe 이미지가 너무 큽니다 (최대 12MB)");const n=E(),r=l(n.nai.api_key);if(!r)throw new Error("NAI api_key가 설정되지 않았습니다. encode-vibe에 키가 필요합니다.");const a=Ue(t.model||n.nai.model||"nai-diffusion-4-5-full"),i=Tr(t.information_extracted??n.nai.vibe_transfer_information_extracted),o=await Cr(r,e,a,i);return await B("meta",{key:"vibe_transfer",png:e,encoded:o,model:ze(a),information_extracted:i}),Qr(Mt(e)),n.nai.vibe_transfer="file",t.strength!=null&&!Number.isNaN(Number(t.strength))&&(n.nai.vibe_transfer_strength=Math.max(0,Math.min(1,Number(t.strength)))),n.nai.vibe_transfer_information_extracted=i,await Je(),{ok:!0,vibe_transfer:"file",configured:!0,bytes:e.byteLength,encoded_bytes:o.length,model:ze(a),information_extracted:i,preview_url:"/v1/nai/vibe.png"}}async function kc(){return await xt("meta","vibe_transfer"),Qr(""),E().nai.vibe_transfer="none",await Je(),{ok:!0,vibe_transfer:"none",configured:!1}}async function om(){const e=await rm();if(!e?.png||e.png.byteLength<He)return null;const t=E(),n=l(t.nai.api_key);if(!n)throw new Error("NAI api_key가 설정되지 않았습니다.");const r=ze(Ue(t.nai.model||"nai-diffusion-4-5-full")),a=Tr(t.nai.vibe_transfer_information_extracted);if(!(!l(e.encoded)||l(e.model)!==r||Math.abs(Number(e.information_extracted??1)-a)>.001))return e;const i=await Cr(n,e.png,r,a),o={...e,key:"vibe_transfer",encoded:i,model:r,information_extracted:a};return await B("meta",o),o}async function Sc(e){const t=l(e,120);return t&&await F("meta",Lt(t))||null}async function sm(e,t,n={}){const r=l(e,120);if(!r)throw new Error("preset_id required");if(!t||t.byteLength<He)throw new Error("Vibe 이미지가 비어 있습니다");if(t.byteLength>Qa)throw new Error("Vibe 이미지가 너무 큽니다 (최대 12MB)");const a=E(),i=l(a.nai.api_key);if(!i)throw new Error("NAI api_key가 설정되지 않았습니다. encode-vibe에 키가 필요합니다.");const o=Ue(n.model||a.nai.model||"nai-diffusion-4-5-full"),s=Tr(n.information_extracted??a.nai.vibe_transfer_information_extracted),c=await Cr(i,t,o,s),u=Lt(r);await B("meta",{key:u,png:t,encoded:c,model:ze(o),information_extracted:s});const d=Mt(t);return or(r,d),{ok:!0,preset_id:r,vibe_transfer:"file",configured:!0,bytes:t.byteLength,encoded_bytes:c.length,model:ze(o),information_extracted:s,preview_url:d}}async function Ac(e){const t=l(e,120);if(!t)throw new Error("preset_id required");return await xt("meta",Lt(t)),or(t,""),{ok:!0,preset_id:t,vibe_transfer:"none",configured:!1}}async function cm(e,t){const n=await Sc(e);if(!n?.png||n.png.byteLength<He)return!1;const r=l(t,120);if(!r)return!1;await B("meta",{key:Lt(r),png:n.png,encoded:n.encoded||"",model:n.model||"",information_extracted:n.information_extracted??1});const a=ea(e)||(n.png?Mt(n.png):"");return a&&or(r,a),!0}async function lm(e){const t=await Sc(e);if(!t?.png||t.png.byteLength<He)return null;const n=E(),r=l(n.nai.api_key);if(!r)throw new Error("NAI api_key가 설정되지 않았습니다.");const a=ze(Ue(n.nai.model||"nai-diffusion-4-5-full")),i=Tr(n.nai.vibe_transfer_information_extracted);if(!(!l(t.encoded)||l(t.model)!==a||Math.abs(Number(t.information_extracted??1)-i)>.001))return t;const o=await Cr(r,t.png,a,i),s=Lt(l(e,120)),c={...t,key:s,encoded:o,model:a,information_extracted:i};return await B("meta",c),c}async function um(){for(const e of await be("meta")){const t=String(e?.key||"");if(!Bt(t))continue;const n=e.png;!n||n.byteLength<He||or(gn(t),Mt(n))}}Ut(),Ln(),X(),W(),D(),Kt(),rt(),Ta(),Ce(),_e(),lt();function Mc(e,t){const n=Number(e);return Math.max(1,Math.min(5e3,Number.isFinite(n)&&n>0?Math.round(n):t))}async function Pr(e){const{shot:t,roster:n}=e,r=E().card,a=E().nai,i=nt(r),o=ca(t.characters||[],n,i).slice(0,i),s=Math.max(1,o.length),c=Po(r.person_tag_mode,r.auto_person_tags),u=mu(pu(o,n,c),r.person_tag_weight),[d,f]=El(await ue("preset_1")),p=Array.isArray(r.presets)?r.presets:[],m=l(r.active_preset_id,120);let g=null;p.length&&(m&&(g=p.find(Y=>typeof Y=="object"&&l(Y.id,120)===m)||null),!g&&typeof p[0]=="object"&&(g=p[0]));let h,_;g?(h=l(g.positive||g.pos||""),_=l(g.negative||g.neg||"")):(h=I(l(r.custom_pos),d),_=I(l(r.custom_neg),f));let y=t.situation||t.scene;const v=l(e.lockedSetup||"");let x;v?x=v:(x=I(t.camera,y,t.place,t.action),r.mode==="asset"&&(x=I(x,"white background","simple background","cowboy shot","looking at viewer","portrait")));const N=Aa(r.natural_base),T=N==="supplement"?600:N==="detailed"?480:400,M=N==="off"?"":l(t.natural||t.natural_base||t.nl||"",T);let j="",R="";if(l(t.composition_id,160)||l(t.curation_fixed_positive,200))try{const Y=await ut();j=l(t.curation_fixed_positive,800)||l(Y.fixed_positive,800),R=l(Y.fixed_negative,800)}catch{j=l(t.curation_fixed_positive,800)}let k=I(h,M,x,j);c!=="off"&&(k=To(k),x=To(x));let S=u?k?`${u}, ${k}`:u:k;const P=Ue(a.model||"nai-diffusion-4-5-full");a.apply_quality_tags!==!1&&(S+=Sn[P]||"");const U=l(a.uc_preset)||"human_focus",Z=I(_,R,(yo[P]||{})[U]||""),me=[],J=[];for(let Y=0;Y<o.length;Y++){const K=o[Y],qe=l(K.name,200),ae=qe?ye(qe,n):null,ft=I(gu(ae,K)),q=l(K.negative),Be=s===1?.5:Math.round((.1+.8*Y/Math.max(1,s-1))*10)/10,te=.5;me.push({prompt:ft||"girl",uc:q,center_x:Be,center_y:te}),J.push({name:ae?.name||qe,prompt:ft,uc:q,center_x:Be,center_y:te,raw:K})}return{main:S,neg:Z,captions:me,meta:{setup:x,person:u,characters:J,paragraph:t.paragraph}}}async function Nc(e){const t=E().nai,n=e.captions;if(Bn(t)==="comfy"){const[_,y]=await wd(t,e.main,e.neg,n);return{bytes:_,seed:y}}const r=l(t.api_key);if(!r)throw new Error("NAI api_key가 설정되지 않았습니다.");const a=[],i=l(t.image_reference||"none").toLowerCase();if(!["","none","off","false","0"].includes(i)){const _=await vc();if(_){let y=l(t.image_reference_type||"character&style")||"character&style";["character","style","character&style"].includes(y)||(y="character&style");let v=Number(t.image_reference_strength??.6),x=Number(t.image_reference_fidelity??1);Number.isNaN(v)&&(v=.6),Number.isNaN(x)&&(x=1),a.push({image:_,type:y,strength:Math.max(0,Math.min(1,v)),fidelity:Math.max(0,Math.min(1,x))})}}const o=E().card,s=Array.isArray(o.presets)?o.presets:[],c=l(o.active_preset_id,120);let u=null;s.length&&(c&&(u=s.find(_=>typeof _=="object"&&l(_.id,120)===c)||null),!u&&typeof s[0]=="object"&&(u=s[0]));const d=l(u?.id||c,120),f=id(t,u),p=[];let m=d?await lm(d):null;if(!m){const _=l(t.vibe_transfer||"none").toLowerCase();["","none","off","false","0"].includes(_)||(m=await om())}if(m?.encoded){let _=Number(t.vibe_transfer_strength??.6),y=Number(t.vibe_transfer_information_extracted??m.information_extracted??1);Number.isNaN(_)&&(_=.6),Number.isNaN(y)&&(y=1),p.push({encoded:m.encoded,strength:Math.max(0,Math.min(1,_)),information_extracted:Math.max(0,Math.min(1,y))})}const g={prompt:e.main,negative_prompt:e.neg,width:Mc(t.width,832),height:Mc(t.height,1216),seed:Number(t.seed??0)||0,steps:Math.min(Number(t.steps??28)||28,28),cfg_scale:f.cfg_scale,cfg_rescale:f.cfg_rescale,sampler:l(t.sampler)||"k_euler_ancestral",scheduler:l(t.scheduler)||"karras",model:Ue(t.model||"nai-diffusion-4-5-full"),var_plus:!!t.variety_plus,characters:n,character_refs:a,vibes:p};w("nai.generate.dims",{message:`${g.width}x${g.height}`,steps:g.steps,focus:!0}),g.seed||(g.seed=Math.floor(Math.random()*4294967295)||1);const h=l(t.request_url)||"https://image.novelai.net/ai/generate-image";return{bytes:(await ks(r,g,h,{timeoutMs:9e4})).raw_bytes,seed:g.seed||0}}function dm({imageId:e,sessionId:t,request:n,shotIndex:r,paragraph:a,yPercent:i,line:o=null,contentHash:s=""}){const c=Math.floor(Number(o));return{version:1,image_id:l(e,80),session_id:l(t,200),unified_session_id:l(n.unified_session_id||"",200),character_id:l(n.character_id||"",200),character_name:l(n.character_name||"",200),chat_id:l(n.chat_id||"",200),chat_name:l(n.chat_name||"",200),char_index:O(n.char_index,-1),chat_index:O(n.chat_index,-1),message_index:O(n.message_index,-1),message_role:l(n.message_role||n.role||"",40).toLowerCase(),shot_index:O(r,0),paragraph:O(a,0),y_percent:i,line:Number.isFinite(c)&&c>=1?c:null,content_hash:l(s||n.content_hash||"",128),assistant_preview:l(n.assistant_text||"",Xe)}}async function Tt(e){return Pu(e)}async function Ic(e,t){const n=await F("images",e),r=n?{...n}:{id:e},a=t||{};r.location={...a,version:Number(a.version||1),image_id:l(e,80)},await B("images",r)}async function dt(e,t={}){const n=await Tt(e),r=typeof t=="object"&&t?t:{};let a=n.y_percent;a==null&&(a=r.y_percent??r.anchor_percent??r.read_percent);const i=Object.keys(n).length>0,o=Ee(l(e,80));return{character_id:l(n.character_id||r.character_id||"",200),chat_id:l(n.chat_id||r.chat_id||"",200),character_name:l(n.character_name||r.character_name||"",200),chat_name:l(n.chat_name||r.chat_name||"",200),char_index:O(n.char_index??r.char_index,-1),chat_index:O(n.chat_index??r.chat_index,-1),message_index:O("message_index"in n?n.message_index:r.message_index,-1),message_role:l(n.message_role||r.message_role||"",40).toLowerCase(),shot_index:O(n.shot_index,-1),paragraph:O("paragraph"in n?n.paragraph:r.paragraph,0),y_percent:rr(a),line:(()=>{const s=Math.floor(Number(n.line??r.line));return Number.isFinite(s)&&s>=1?s:null})(),content_hash:l(n.content_hash||r.content_hash||"",128),assistant_preview:l(n.assistant_preview||r.assistant_preview||"",Xe),unified_session_id:l(n.unified_session_id||r.unified_session_id||"",200),location_file:i?`idb:${o}`:"",storage:"indexeddb",storage_key:o}}function ei(e,t,n=0){const r=typeof e=="object"&&e?{...e}:{},a=t||{};return{...r,character_id:l(a.character_id||r.character_id||"",200),chat_id:l(a.chat_id||r.chat_id||"",200),character_name:l(a.character_name||r.character_name||"",200),chat_name:l(a.chat_name||r.chat_name||"",200),char_index:O(a.char_index??r.char_index,-1),chat_index:O(a.chat_index??r.chat_index,-1),message_index:O(a.message_index??r.message_index,-1),message_role:l(a.message_role||r.message_role||"",40).toLowerCase(),content_hash:l(a.content_hash||r.content_hash||"",128),assistant_preview:l(a.assistant_preview||r.assistant_preview||"",Xe),unified_session_id:l(a.unified_session_id||r.unified_session_id||"",200),y_percent:rr(a.y_percent??r.y_percent),line:(()=>{const i=Math.floor(Number(a.line??r.line));return Number.isFinite(i)&&i>=1?i:null})(),storage:"indexeddb",storage_key:Ee(l(a.image_id||"",80)),png_bytes:Number(n)||0}}ce();var fm=(()=>{const e=new Uint32Array(256);for(let t=0;t<256;t+=1){let n=t;for(let r=0;r<8;r+=1)n=n&1?3988292384^n>>>1:n>>>1;e[t]=n>>>0}return e})();function ti(e){return Array.isArray(e)?new Uint8Array(e):ge(e)}function mm(e){const t=ti(e);let n=4294967295;for(let r=0;r<t.length;r+=1)n=fm[(n^t[r])&255]^n>>>8;return(n^4294967295)>>>0}function H(e){const t=new Uint8Array(2);return new DataView(t.buffer).setUint16(0,e>>>0,!0),t}function de(e){const t=new Uint8Array(4);return new DataView(t.buffer).setUint32(0,e>>>0,!0),t}function Gn(e){let t=0;for(const n of e)t+=n.length;return vn(e,t)}function pm(e){return new TextEncoder().encode(String(e??""))}function gm(e=[]){const t=[],n=[];let r=0;for(const o of e){const s=pm(o.name||"file"),c=ti(o.data),u=mm(c),d=Gn([de(67324752),H(20),H(0),H(0),H(0),H(0),de(u),de(c.length),de(c.length),H(s.length),H(0),s,c]),f=Gn([de(33639248),H(20),H(20),H(0),H(0),H(0),H(0),de(u),de(c.length),de(c.length),H(s.length),H(0),H(0),H(0),H(0),de(0),de(r),s]);t.push(d),n.push(f),r+=d.length}const a=Gn(n),i=Gn([de(101010256),H(0),H(0),H(e.length),H(e.length),de(a.length),de(r),H(0)]);return Gn([...t,a,i])}function ni(e,t){return e[t]|e[t+1]<<8}function Ec(e,t){return(e[t]|e[t+1]<<8|e[t+2]<<16|e[t+3]<<24)>>>0}function hm(e){const t=ti(e),n=new Map;let r=0;for(;r+30<=t.length;){const a=Ec(t,r);if(a===33639248||a===101010256)break;if(a!==67324752){r+=1;continue}const i=ni(t,r+8),o=Ec(t,r+18),s=ni(t,r+26),c=ni(t,r+28),u=r+30,d=new TextDecoder().decode(t.subarray(u,u+s)),f=u+s+c,p=f+o;if(p>t.length)break;i===0&&n.set(d.replace(/\\/g,"/"),t.subarray(f,p)),r=p}return n}function _m(e=[],{exportedAt:t=Date.now()}={}){return{format:"inlay-nexus-gallery",version:1,exported_at:t,items:(e||[]).map(n=>({id:String(n.id||""),file:`images/${String(n.id||"unknown")}.png`,location:{character_id:n.character_id||"",chat_id:n.chat_id||"",character_name:n.character_name||"",chat_name:n.chat_name||"",char_index:n.char_index??-1,chat_index:n.chat_index??-1,message_index:n.message_index??-1,content_hash:n.content_hash||"",paragraph:n.paragraph??0,shot_index:n.shot_index??0,y_percent:n.y_percent??null,assistant_preview:n.assistant_preview||"",session_id:n.session_id||""},meta:{main_prompt:n.main_prompt||"",seed:n.seed??null,created_at:n.created_at??null,characters:n.characters||[]}}))}}function ym(e,t=[]){const n=e?.location||{},r=String(n.content_hash||"").trim(),a=t||[];if(r){const u=a.find(d=>String(d.content_hash||"")===r);if(u)return{status:"exact",matchId:String(u.id),content_hash:r}}const i=String(n.character_id||""),o=String(n.chat_id||""),s=Number(n.message_index),c=a.filter(u=>i&&String(u.character_id||"")!==i||o&&String(u.chat_id||"")!==o||Number.isFinite(s)&&s>=0&&Number(u.message_index)!==s?!1:!!(i||o||Number.isFinite(s)&&s>=0)).slice(0,8);return c.length?{status:"candidate",candidates:c.map(u=>({id:u.id,content_hash:u.content_hash,message_index:u.message_index})),location:n}:{status:"orphan",location:n}}function bm(e=[]){return gm(e)}function wm(e){const t=hm(e);let n=null;const r=t.get("manifest.json");if(r)try{n=JSON.parse(new TextDecoder().decode(r))}catch{n=null}const a=new Map;for(const[i,o]of t.entries())i!=="manifest.json"&&/\.(png|webp|jpe?g)$/i.test(i)&&a.set(i.replace(/\\/g,"/"),o);return{manifest:n,images:a,files:t}}X(),W(),ce(),D(),Ce();function qn(e){try{const t=JSON.parse(e.meta_json||"{}");return t&&typeof t=="object"?t:{}}catch{return{}}}async function jc(e,t){return(await $u(e))?.png_bytes||Number(t.png_bytes)||0}async function ri(e){const t=(await be("cards")).sort((i,o)=>(o.created_at||0)-(i.created_at||0)).slice(0,Math.max(1,Math.min(2e3,e))),n={},r=[];for(const i of t){const o=qn(i),s=await dt(i.id,o),c=await Tt(i.id),u=l(s.character_name||c.character_name||o.character_name||"",200),d=l(s.chat_name||c.chat_name||o.chat_name||"",200),f=l(s.character_id||"",200)||"unknown",p=l(s.chat_id||"",200)||"unknown",m=`${f}|${p}`;let g=n[m];g||(g={key:m,character_id:f,chat_id:p,character_name:u||f.slice(0,12)||"Unknown",chat_name:d||`chat ${s.chat_index??"?"}`,char_index:s.char_index??-1,chat_index:s.chat_index??-1,count:0,storage:"indexeddb"},n[m]=g),u&&!g.character_name&&(g.character_name=u),d&&(g.chat_name=d),g.count+=1,r.push({id:i.id,job_id:i.job_id,session_id:i.session_id,folder_key:m,shot_index:s.shot_index>=0?s.shot_index:i.shot_index,paragraph:Object.keys(c).length?s.paragraph:i.paragraph,y_percent:s.y_percent,line:s.line,message_index:s.message_index??-1,message_role:s.message_role||"",content_hash:s.content_hash||"",character_id:f,character_name:g.character_name,chat_id:p,chat_name:g.chat_name,char_index:s.char_index??-1,chat_index:s.chat_index??-1,assistant_preview:l(s.assistant_preview||c.assistant_preview||o.assistant_preview||"",Xe),main_prompt:i.main_prompt,characters:JSON.parse(i.characters_json||"[]"),image_url:Te(i.id),seed:i.seed,created_at:i.created_at,storage:"indexeddb",storage_key:s.storage_key||Ee(i.id),location_file:s.location_file||"",png_bytes:await jc(i.id,o)})}const a={ok:!0,folders:Object.values(n).sort((i,o)=>`${i.character_name||""}`.localeCompare(`${o.character_name||""}`,void 0,{sensitivity:"base"})||`${i.chat_name||""}`.localeCompare(`${o.chat_name||""}`,void 0,{sensitivity:"base"})),items:r,total:r.length,storage:"indexeddb",storage_api:"getLocalPluginStorage"};return await it(a,{cachedOnly:!0}),a}async function vm(e=400){return ri(e)}async function xm(e,t=40){const n=(await es(e)).sort((i,o)=>(o.created_at||0)-(i.created_at||0)).slice(0,Number(t)),r=[];for(const i of n){const o=qn(i),s=await dt(i.id,o),c=await Tt(i.id);r.push({id:i.id,job_id:i.job_id,shot_index:s.shot_index>=0?s.shot_index:i.shot_index,paragraph:Object.keys(c).length?s.paragraph:i.paragraph,y_percent:s.y_percent,line:s.line,message_index:s.message_index??-1,message_role:s.message_role||"",content_hash:s.content_hash||"",character_id:s.character_id||"",chat_id:s.chat_id||"",character_name:s.character_name||"",chat_name:s.chat_name||"",char_index:s.char_index??-1,chat_index:s.chat_index??-1,assistant_preview:s.assistant_preview||o.assistant_preview||"",main_prompt:i.main_prompt,negative_prompt:i.negative_prompt,characters:JSON.parse(i.characters_json||"[]"),image_url:Te(i.id),seed:i.seed,created_at:i.created_at,storage:"indexeddb",storage_key:s.storage_key||Ee(i.id),png_bytes:await jc(i.id,o)})}const a={ok:!0,session_id:e,items:r,storage:"indexeddb"};return await it(a,{cachedOnly:!0}),a}async function km(e={}){const t=l(e.session_id,200),n=l(e.to_hash,128),r=l(e.assistant_preview||"",Xe),a=(Array.isArray(e.card_ids)?e.card_ids:[]).map(o=>l(o,80)).filter(Boolean);if(!n||!a.length)return{ok:!1,...ne("to_hash and card_ids required","bad_request"),rebound:0,ids:[]};const i=[];for(const o of a){const s=await F("cards",o);if(!s||t&&l(s.session_id||"",200)!==t)continue;const c=qn(s),u=await Tt(o),d={...u,image_id:o,content_hash:n,assistant_preview:r||u.assistant_preview||""};await Ic(o,d),c.content_hash=n,r&&(c.assistant_preview=r),c.message_role=l(c.message_role||u.message_role||"",40).toLowerCase(),s.meta_json=JSON.stringify(c),await B("cards",s),i.push(o)}return w("gallery.rebind",{n:i.length,to:n.slice(0,8)}),{ok:!0,rebound:i.length,ids:i,content_hash:n}}async function ai(e,t="",n=null){const r=l(e,200),a=l(t);let i=null;try{n!=null&&String(n).trim()!==""&&(i=parseInt(String(n),10))}catch{i=null}if(!r||!a&&i==null)return{ok:!0,unlinked:0,ids:[]};const o=(await es(r)).sort((c,u)=>(c.created_at||0)-(u.created_at||0)),s=[];for(const c of o){const u=qn(c),d=await dt(c.id,u),f=l(d.content_hash||""),p=O(d.message_index,-1);let m=!1;if((a&&f&&f===a||i!=null&&i>=0&&p===i)&&(m=!0),!m)continue;const g={...await Tt(c.id),version:1,image_id:c.id,session_id:r,content_hash:"",message_index:-1,character_id:"",chat_id:"",char_index:-1,chat_index:-1,unlinked_at:Date.now()/1e3};await Ic(c.id,g),(u.content_hash||u.message_index!=null||u.assistant_preview)&&(u.content_hash="",u.assistant_preview="",u.message_index=-1,u.unlinked_at=Date.now()/1e3,c.meta_json=JSON.stringify(u),await B("cards",c)),s.push(c.id)}return{ok:!0,unlinked:s.length,ids:s}}async function ii(e){const t=l(e,80);return t?await F("cards",t)?(await xt("cards",t),await xt("images",t),{ok:!0,deleted:1,ids:[t]}):{ok:!1,...ne("card not found","not_found")}:{ok:!1,...ne("card_id required","bad_request")}}async function oi(e){return ii(e)}async function Sm(e=[]){const t=[...new Set((e||[]).map(r=>l(r,80)).filter(Boolean))],n=[];for(const r of t)(await ii(r)).ok&&n.push(r);return{ok:!0,deleted:n.length,ids:n}}async function $c(){const e=(await F("meta","explorer_favorites"))?.ids;return{ok:!0,ids:Array.isArray(e)?e.map(t=>l(t,80)).filter(Boolean):[]}}async function Am(e=[]){const t=[...new Set((e||[]).map(n=>l(n,80)).filter(Boolean))].slice(0,5e3);return await B("meta",{key:"explorer_favorites",ids:t,updated_at:Date.now()/1e3}),{ok:!0,ids:t}}async function Mm(e={}){let t=(await ri(2e3)).items;const n=l(e.folder_key||"",400);if(!e.all)if(n)t=t.filter(o=>o.folder_key===n);else if(Array.isArray(e.card_ids)&&e.card_ids.length){const o=new Set(e.card_ids.map(s=>l(s,80)));t=t.filter(s=>o.has(s.id))}else return{ok:!1,...ne("card_ids, folder_key, or all required","bad_request")};if(!t.length)return{ok:!1,...ne("no images to export","empty")};const r=_m(t),a=[{name:"manifest.json",data:new TextEncoder().encode(JSON.stringify(r,null,2))}];for(const o of t){const s=await Cc(o.id);s?.byteLength&&a.push({name:`images/${o.id}.png`,data:ge(s)})}if(a.length<2)return{ok:!1,...ne("image bytes missing","empty")};const i=bm(a);return{ok:!0,count:a.length-1,filename:`inlay-gallery-${Date.now()}.zip`,zip_base64:Dt(i),bytes:i.length}}async function Nm(e={}){const t=e.prefer_new_ids===void 0?!0:e.prefer_new_ids,n=String(e.zip_base64||"").replace(/^data:.*base64,/,"");if(!n)return{ok:!1,...ne("zip_base64 required","bad_request")};let r;try{r=Re(n)}catch(u){return{ok:!1,...ne(`invalid base64: ${u?.message||u}`,"bad_request")}}const{manifest:a,images:i}=wm(r);if(!a?.items?.length)return{ok:!1,...ne("manifest.items missing","bad_request")};const o=(await ri(2e3)).items,s=[],c={exact:0,candidate:0,orphan:0,skipped:0};for(const u of a.items){const d=String(u.file||`images/${u.id}.png`).replace(/\\/g,"/"),f=i.get(d)||i.get(`images/${u.id}.png`);if(!f?.byteLength){c.skipped+=1;continue}const p=ym(u,o);c[p.status]=(c[p.status]||0)+1;const m={...u.location||{}},g=t?ht():l(u.id||"",80)||ht(),h=l(m.session_id||"",200)||`import_${ht().replace(/-/g,"").slice(0,10)}`,_={version:1,image_id:g,session_id:h,character_id:l(m.character_id||"",200),character_name:l(m.character_name||"",200),chat_id:l(m.chat_id||"",200),chat_name:l(m.chat_name||"",200),char_index:O(m.char_index,-1),chat_index:O(m.chat_index,-1),message_index:O(m.message_index,-1),shot_index:O(m.shot_index,0),paragraph:O(m.paragraph,0),y_percent:rr(m.y_percent),content_hash:l(m.content_hash||"",128),assistant_preview:l(m.assistant_preview||"",Xe),imported_at:Date.now()/1e3,reattach:p.status},y=Q(f);await ha(g,y,_);const v=ei({...u.meta||{},assistant_preview:_.assistant_preview,imported_at:_.imported_at,reattach:p.status},_,y.byteLength||0);await B("cards",{id:g,job_id:`import_${g.slice(0,8)}`,session_id:h,shot_index:_.shot_index,paragraph:_.paragraph,main_prompt:l(u.meta?.main_prompt||"",8e3),negative_prompt:"",characters_json:JSON.stringify(u.meta?.characters||[]),seed:u.meta?.seed??0,meta_json:JSON.stringify(v),created_at:Number(u.meta?.created_at)||Date.now()/1e3}),s.push({id:g,reattach:p.status,content_hash:_.content_hash})}return{ok:!0,imported:s.length,items:s,report:c}}async function Im(e){const t=l(e,400);if(!t||!t.includes("|"))return{ok:!1,...ne("folder_key required","bad_request")};const[n,r]=t.split("|",2),a=l(n,200)||"unknown",i=l(r,200)||"unknown",o=await be("cards"),s=[];for(const c of o){const u=qn(c),d=await dt(c.id,u),f=l(d.character_id||"",200)||"unknown",p=l(d.chat_id||"",200)||"unknown";f!==a||p!==i||(await ii(c.id)).ok&&s.push(c.id)}return{ok:!0,deleted:s.length,ids:s,folder_key:`${a}|${i}`}}async function Cc(e){return Ru(e)}D(),Ce(),_e();var Em=["queued","tagging","generating"];function Or(e={},t=""){const n=l(t||e.session_id||"",200)||"_",r=l(e.content_hash||"",128);if(r)return`${n}::h:${r}`;const a=O(e.message_index,-1);return a>=0?`${n}::m:${a}`:`${n}::all`}async function jm(e){if(sr.has(e))return"message-reroll";const t=Mn.get(e);if(!t?.jobId)return"";const n=bt.get(t.jobId);if(!n||n.cancelRequested)return"";const r=await F("jobs",t.jobId);return r&&Em.includes(String(r.state||""))?t.jobId:""}async function si(e={},t=""){const n=await jm(Or(e,t));return n?{ok:!1,busy:!0,accepted:!1,job_id:n==="message-reroll"?"":n,session_id:l(t||e.session_id||"",200),job_state:"busy",error:{code:"busy",message:"같은 메시지 작업이 아직 진행 중입니다. 끝날 때까지 기다려 주세요."}}:null}var $m=cn({isCharacterImageExtraLore:()=>Pt,matchCharacterImageSectionTitles:()=>Oc,normalizeNameKey:()=>an,parseCharacterImageTagLore:()=>ci,trimCharacterImageTagLore:()=>Pc});function Pt(e){return String(e?.comment||e?.name||"").trim().toLowerCase()==="lb-xnai.lb.extra"}function ci(e){const t=String(e||"");if(!t.trim())return{header:"",sections:[]};const n=/^(#{2,3})\s+(.+?)\s*$/gm,r=[];let a;for(;(a=n.exec(t))!==null;)r.push({hashes:a[1],title:String(a[2]||"").trim(),start:a.index,titleEnd:a.index+a[0].length});if(!r.length)return{header:t.trimEnd(),sections:[]};let i=t.slice(0,r[0].start).trimEnd();const o=[];for(let s=0;s<r.length;s+=1){const c=r[s],u=s+1<r.length?r[s+1].start:t.length,d=t.slice(c.titleEnd,u).replace(/^\r?\n/,"").trimEnd();if(c.title.toLowerCase()==="character image tags"){const f=t.slice(c.start,u).trimEnd();i=i?`${i}

${f}`:f;continue}o.push({title:c.title,hashes:c.hashes,body:d})}return{header:i.trimEnd(),sections:o}}function an(e){return String(e||"").replace(/[^a-zA-Z0-9\uac00-\ud7a3\u3040-\u30ff\u3400-\u9fff\uff00-\uffef]/g,"").toLowerCase()}function Tc(e){return new Set((Array.isArray(e)?e:[]).map(t=>an(t)).filter(Boolean))}function Cm(e,t){if(!t.length)return"";const n=[];e&&n.push(e);for(const r of t){const a=`${r.hashes} ${r.title}`;n.push(r.body?`${a}
${r.body}`:a)}return n.join(`

`).trimEnd()}function Pc(e,t=[],n=[]){const r=ci(e);if(!r.sections.length)return"";const a=Tc(n);if(!a.size)return"";const i=Tc(t),o=r.sections.filter(s=>{const c=an(s.title);return a.has(c)&&!i.has(c)});return Cm(r.header,o)}function Oc(e,t,n=[]){const r=ci(e),a=an(t),i=new Set((Array.isArray(n)?n:[]).map(s=>an(s)).filter(s=>s.length>=2));if(!a&&!i.size)return[];const o=[];for(const s of r.sections){const c=an(s.title);c.length<2||(a&&a.includes(c)||i.has(c))&&o.push(s.title)}return o}D(),Kt(),rt();function Lc(e){const t=e.key||e.keys||e.trigger||"";let n;if(Array.isArray(t))n=t.map(o=>l(o,200));else{const o=l(t,2e3);n=o?o.split(/[,|\n]/):[]}const r=e.secondkey||e.second_key||"";r&&n.push(...V(r));const a=[],i=new Set;for(const o of n){const s=l(o,200),c=z(s),u=s?_t(s):"";!c||i.has(c)||s.startsWith("")||u.length<2||(i.add(c),a.push(s))}return a}function Bc(e,t,n){const r=z(e),a=_t(e);if(a.length<2)return 0;const i=(o,s)=>{if(!o||!s||s.length<2)return 0;let c=0,u=0;for(;u<=o.length-s.length;){const d=o.indexOf(s,u);if(d<0)break;c+=1,u=d+s.length}return c};return Math.max(i(t,r),i(n,a))}function Tm(e,t){const n=l(t).toLowerCase(),r=_t(t);if(!n)return[];const a=[],i=new Set;for(const o of e||[]){if(typeof o!="object"||Pt(o))continue;const s=l(o.mode||"",40).toLowerCase(),c=l(o.key||"",2e3);if(s==="folder"||c.startsWith("folder:"))continue;const u=Lc(o);if(!u.length)continue;let d=0;for(const f of u)d+=Bc(f,n,r);if(!(d<=0))for(const f of u){const p=z(f);!p||i.has(p)||(i.add(p),a.push(f))}}return a}function Rc(e,t=[]){const n=String(e||"");if(!/character\s*image\s*tags/i.test(n))return!1;const r=(n.match(/^#{2,3}\s+.+$/gm)||[]).filter(i=>!/character\s*image\s*tags/i.test(i)),a=Array.isArray(t)?t.filter(Boolean).length:0;return r.length>=3&&(a===0||r.length>a+1)}function Pm(e){const t=[],n=new Set;for(const r of e||[])if(Se(r))for(const a of ur(r)){const i=l(a,200),o=z(i);!i||!o||n.has(o)||(n.add(o),t.push(i))}return t}function Fc(e){return e===!1||e==="false"||e==="off"||e==="none"?"off":e==="full"?"full":"tags"}function Om(e,t,n=[],r=5,a=1200,i=null,o="tags"){const s=Array.isArray(e)?e:[],c=Fc(o),u=c==="off"?[]:s.filter(g=>Pt(g)),d=s.filter(g=>!Pt(g)),f=Lm(d,t,r,a),p=Array.isArray(i)&&i.length?i.map(g=>l(g,200)).filter(Boolean):Tm(d,t),m=[];for(const g of u){const h=l(g.content||g.data||"",5e4);if(h)if(c==="full")m.push({comment:l(g.comment||g.name||"lb-xnai.lb.extra",200),content:h,key:"full",always:!0,lore_extra_mode:"full"});else{const _=Oc(h,t,p),y=_.length?Pc(h,n,_):"";y&&!Rc(y,_)&&m.push({comment:l(g.comment||g.name||"lb-xnai.lb.extra",200),content:y,key:_.join(", "),always:!0,lore_extra_mode:"tags"})}}for(const g of f)Rc(l(g.content||g.data||"",a),[])||m.push(g);return m}function Lm(e,t,n=5,r=1200){const a=l(t).toLowerCase(),i=_t(t);if(!a)return[];const o=[];for(const s of e||[]){if(typeof s!="object"||Pt(s))continue;const c=l(s.mode||"",40).toLowerCase(),u=l(s.key||"",2e3);if(c==="folder"||u.startsWith("folder:"))continue;const d=l(s.content||s.data||"",r);if(!d)continue;const f=Lc(s);if(!f.length)continue;let p=0;for(const m of f)p+=Bc(m,a,i);p<=0||o.push({comment:l(s.comment||s.name||"",200),content:d,key:u,hits:p})}return o.sort((s,c)=>c.hits-s.hits||String(s.comment).localeCompare(String(c.comment))),o.slice(0,Math.max(1,n)).map(({hits:s,...c})=>c)}W(),yt(),D(),Ln(),Kt(),rt(),_e(),Kn(),lt();function Bm(e){for(let t=e.length-1;t>=0;t--){const n=e[t];if(!n||n.role!=="user")continue;const r=typeof n.content=="string"?n.content:"";if(r.trim()&&!r.startsWith("# Priority: Author"))return r}return""}async function Rm(e){const t=et(E().card,e.card||{}),n=l(e.session_id,200),r=[{role:"system",content:`${ar(await ue("tagger"))}

${ar(await ue("format"))}`.trim()}];t.char_info&&l(e.character_description)&&r.push({role:"system",content:`${await ue("char_inject")}
## {{char}} Info
${l(e.character_description,12e3)}`}),t.user_info&&l(e.persona_description)&&r.push({role:"system",content:`${await ue("char_inject")}
## {{user}} Info
${l(e.persona_description,8e3)}`});const a=l(e.assistant_text,2e4),i=Array.isArray(e.source_session_ids)?e.source_session_ids.map(_=>l(_,200)).filter(Boolean):[],o=t.lorebook||t.char_appearance!==!1?await xa(n,l(e.unified_session_id||"",200),l(e.character_id||"",200),i):[],s=Pm(o);if(t.lorebook){const _=Array.isArray(e.lore_trigger_keys)?e.lore_trigger_keys:null,y=Fc(t.lore_extra),v=Om(e.lorebook||[],a,s,5,1200,_,y),x=[],N=[];for(const M of v){const j=Pt(M)||M.always,R=l(M.content||"",j?5e4:1200);if(!R)continue;const k=l(M.comment||"",200),S=k?`### ${k}
...
`)),w("job.lore.extra",{trigger_keys:Array.isArray(_)?_.length:0,injected:x.length,reference:N.length,sections:v.filter(M=>Pt(M)||M.always).map(M=>M.key||"").filter(Boolean).join(" | ")}),T.length&&r.push({role:"system",content:`${await ue("lore_inject")}
${T.join(`

`)}`})}if(t.char_appearance!==!1){const _=o,y=_.filter(N=>Se(N)),v=_.filter(N=>l(N.name,200)&&!Se(N)),x=eu(a,_);if(y.length||v.length||x.length){const N=S=>{const P=l(S.name,200),U=l(S.appearance||"",200),Z=l(S.attire||"",160),me=l(S.accessories||"",120),J=[U?`appearance=${U}`:"",Z?`attire=${Z}`:"",me?`accessories=${me}`:""].filter(Boolean);return J.length?`${P} ← ${J.join(" | ")}`:P},T=x.filter(S=>Se(S)),M=x.filter(S=>!Se(S)),j=T.length?T.map(N).filter(Boolean).join(`
`):"(none)",R=M.length?M.map(S=>`- ${S.name} (aliases: ${ur(S).slice(0,8).join(", ")}) → empty appearance; full looks required in new_characters`).join(`
...
${j}`,r.push({role:"system",content:k}),w("job.roster.split",{filled:y.map(S=>S.name),incomplete:v.map(S=>S.name),matched:x.map(S=>S.name),matched_with_looks:T.map(S=>S.name),session_id:n})}}const c=Aa(t.natural_base),u=nt(t),d=Math.max(1,Number(t.image_min??1)||1),f=Math.max(d,Number(t.image_max??3)||3);r.push({role:"system",content:["Every shot MUST include `y_percent` (0–100): reading position top→bottom. Spread across the full range in order (shot0 < shot1 < …); ~even gaps. E.g. 2→~25/~75; 3→~20/~50/~80; 4→~15/~40/~65/~90. Forbidden: all under 40, duplicates, or gaps under ~15 unless 1 shot.",d===f?`SHOT COUNT: produce exactly ${f} shot(s) in scenes[].shots (across all scenes).`:`SHOT COUNT: produce between ${d} and ${f} shots in scenes[].shots (across all scenes). Prefer the count that fits the message; never fewer than ${d} or more than ${f}.`,Dm(c),`CHARACTER CAP: at most ${u} characters per shot (char1..char${u}). If more are visible, keep the ${u} most important; fold extras into situation/place.`].join(`
`)});const p=await pc();p&&r.push({role:"system",content:p});const m=[];if(Number(t.include_max||0)>0)for(const _ of e.recent_messages||[]){const y=l(_?.role,40)||"char",v=l(_?.content||_?.data,12e3);v&&(a&&v===a&&["char","assistant","bot"].includes(y.toLowerCase())||m.push(`[${y}]
...
${h}
> These are instructions explicitly given by the user. If in conflict with previous instructions, this section MUST take precedence.`}),r}function Fm(e){const t=nt(E().card),n=e,r=[];for(const a of n.scenes||[]){const i=l(a.place);for(const o of a.shots||[]){const s={...o,place:i,characters:ca(o.characters||[],[],t).slice(0,t)};r.push(s)}}return r}function Dm(e){return e==="off"?"Natural base mode OFF. Omit the `natural` field (or leave it empty). Do not invent natural-language base phrases.":e==="detailed"?["Natural base mode DETAILED. Every shot MUST include `natural`: a compact English phrase for NovelAI base caption only (NOT Danbooru comma tags, NOT characters[].action).","Include framing/vantage when useful (side view, close-up, viewed through, reflected in…), then for each visible person in left-to-right (or front-to-back) order: hair color + age band + gender, facial expression, clothing, and pose/action as separate details.","Multi-person: NEVER use character names — use position (left girl, right boy) or hair. Add shared action and lighting/atmosphere briefly.","Telegraphic and objective. Example: `side view upper body, left red hair adult woman tense smile in coat pulling right blue hair teen boy startled arms pinned, warm cafe light`. Keep ~12–28 words, English only."].join(" "):e==="supplement"?["Natural base mode SUPPLEMENT. Every shot MUST include `natural`: telegraphic English sentences describing what tags cannot express (composition, framing, actions, atmosphere, lighting) for NovelAI base caption only (NOT Danbooru comma tags, NOT characters[].action).","Keep tags and natural language clearly separated. Multi-person: NEVER use names — identify by position (left girl, bottom boy) or appearance. For each person include facial expression and clothing as distinct details; also hair and pose unless already obvious.","Unusual framing welcome (viewed through, reflected in shards of a broken mirror, behind…). Concise, minimal, objective — no subjective interpretation.","Example: `Side view, upper body. Left: adult woman with red hair, tense smile, coat, pulling. Right: teen boy with blue hair, startled face, arms pinned. Warm cafe lighting.` Prefer ~2–5 short sentences, English only."].join(" "):["Natural base mode SHORT. Every shot MUST include `natural`: a short English natural-language phrase for NovelAI base caption only (NOT Danbooru comma tags, NOT characters[].action).","Include hair color + age band + gender for each visible person, plus the shared action. Example: `red hair adult woman forced hug blue hair boy`. Keep ~6–20 words, English only."].join(" ")}X(),W(),D(),yt(),Nt(),Za(),rt(),Kt(),Ce(),_e(),Kn(),lt();var Dc=5e3,zm=2500,Um=64;function Jm(e,t,n){const r=Or(t,n),a=(Mn.get(r)?.epoch||0)+1;return Mn.set(r,{epoch:a,jobId:e}),bt.set(e,{key:r,epoch:a,cancelRequested:!1,publishedIds:[]}),{key:r,epoch:a}}function Hm(e){const t=bt.get(e);if(!t||t.cancelRequested)return!1;const n=Mn.get(t.key);return!!(n&&n.jobId===e&&n.epoch===t.epoch)}async function Km(e){const t=bt.get(e),n=t?.publishedIds?[...t.publishedIds]:[];t&&(t.publishedIds=[]);for(const r of n)try{await oi(r)}catch{}return n.length}async function Ie(e,t="interrupted"){if(Hm(e))return!1;const n=await Km(e);return await fe(e,"cancelled",{phase:"cancelled",progress:0,message:`${t}${n?` · discarded ${n}`:""}`,shot_count:0,shot_done:0},null),w("job.cancelled",{job_id:e,message:t,discarded:n,focus:!0}),!0}function Wn(e={}){return{shot_count:e.shot_count??0,shot_index:e.shot_index??0,shot_done:e.shot_done??0,progress:e.progress??0,phase:e.phase||"generating",message:e.message||"",cards_so_far:e.cards_so_far,debug_stage:mt()||wn(),debug_error:tr()?.message||""}}function Gm(e){return!e||typeof e!="object"?e:JSON.parse(JSON.stringify(e,(t,n)=>{if(t==="image_url"&&typeof n=="string"&&n.startsWith("data:"))return"";if(!(t==="tagged"||t==="appearance"||t==="debug_tail"))return n}))}async function fe(e,t,n=null,r=null){const a=await F("jobs",e);if(!a)return;const i=a.state,o={...a,state:t},s=Gm(n);o.result_json=s!=null?JSON.stringify(s):null,o.error=r,o.updated_at=Date.now()/1e3;const c=t!=="generating"||i!=="generating"||!!r;await B("jobs",o,{persist:c}),(t==="done"||t==="error"||t==="cancelled")&&await Xo(),w("job.set",{message:`${t}${r?" ERR":""}${c?"":" (mem)"}`,job_id:e,state:t,persist:c,err:r?String(r).slice(0,160):"",background:t==="generating",focus:t!=="generating"})}async function zc(e){if(!(E().card||{}).power&&!e.force)throw new Error("Power가 OFF 상태입니다.");const t=l(e.session_id,200)||`sess_${ht().replace(/-/g,"").slice(0,12)}`,n={...e,session_id:t},r=await si(n,t);if(r)return w("job.busy",{message:r.error.message||"busy",key:Or(n,t),focus:!0},"warn"),r;const a=ht(),i=Date.now()/1e3;if(Jm(a,n,t),n.force)try{await ai(t,l(n.content_hash||""),n.message_index)}catch{}return await B("jobs",{id:a,session_id:t,state:"queued",request_json:JSON.stringify(n),result_json:null,error:null,created_at:i,updated_at:i}),Vm(a).catch(async o=>{console.error("[Inlay Nexus] job crashed",a,o);try{await fe(a,"error",null,String(o?.message||o).slice(0,1500))}catch{}}),{ok:!0,accepted:!0,job_id:a,session_id:t,job_state:"queued"}}async function qm(e){const t=await F("jobs",e);if(!t)return{ok:!1,error:{code:"not_found",message:"job not found"}};const n=t.result_json?JSON.parse(t.result_json):null,r={};if(n&&typeof n=="object")for(const a of["shot_count","shot_index","shot_done","progress","phase","message","cards_so_far","debug_stage","debug_error"])a in n&&(r[a]=n[a]);return n&&await it(n),{ok:!0,job_id:t.id,session_id:t.session_id,state:t.state,error:t.error,result:n,progress:r,debug:{last_stage:wn(),last_error:tr(),events:qr(e,40)},created_at:t.created_at,updated_at:t.updated_at}}function Wm(e){for(const t of["y_percent","anchor_percent","read_percent"]){const n=e[t];if(n!=null)try{return Math.max(0,Math.min(100,Number(n)))}catch{return null}}return null}async function Vm(e){const t=await F("jobs",e);if(!t)return;const n=JSON.parse(t.request_json??"{}"),r=String(t.session_id??""),a=Ci();bn(e);const i=se("job.run");w("job.start",{job_id:e,session_id:r,message_index:n.message_index,text_len:String(n.assistant_text||"").length});try{if(await Ie(e,"superseded before start")||(n.force&&await ai(r,l(n.content_hash||""),n.message_index),await Ie(e,"superseded before tagging")))return;await fe(e,"tagging",{phase:"tagging",progress:0,message:"장면 태깅 중…",shot_count:0,shot_done:0,debug_stage:"job.tagging"});const o=await Rm(n),s=Bm(o);if(w("job.tagger.messages",{msgs:o.length}),E().card?.preprocessing){const k=ar(await ue("preprocess"));if(k){const S=[{role:"system",content:k},o[o.length-1]],P=await nn(E().llm,S);if(await Ie(e,"superseded during preprocess"))return;o.splice(o.length-1,0,{role:"system",content:`## Preprocess Summary
${P}`})}}const c=await nn(E().llm,o);if(await Ie(e,"superseded after tagging"))return;const u=Yr(c);let d=Fm(u);if(w("job.tagger.done",{shots:d.length,raw_len:String(c||"").length}),!d.length)throw new Error("태거가 shot을 반환하지 않았습니다.");const f=E().card||{},p=Math.max(1,Number(f.image_min??1)),m=Math.max(p,Number(f.image_max??3));d=d.slice(0,m);const g=jr();if(g==="two_stage"){if(await fe(e,"tagging",{phase:"tagging",progress:.35,message:"큐레이션 2단 씬 태그 보강 중…",shot_count:d.length,shot_done:0,debug_stage:"job.curation_refine"}),await Ie(e,"superseded during curation refine"))return;await gc(d,{chatContext:s})}else if(g==="embed_snap"){if(await fe(e,"tagging",{phase:"tagging",progress:.4,message:"씬 태그 임베딩 매칭 중…",shot_count:d.length,shot_done:0,debug_stage:"job.curation_snap"}),await Ie(e,"superseded during curation snap"))return;const k=await yc(d);k.ok||(w("job.curation_snap.fallback",{message:k.reason},"warn"),await fe(e,"tagging",{phase:"tagging",progress:.45,message:k.reason||"임베딩 없음 → 큐레이션 없이 생성",shot_count:d.length,shot_done:0,debug_stage:"job.curation_snap_fallback"}))}const h=d.flatMap(k=>k.characters||[]),_=l(n.unified_session_id||"",200),y=l(n.character_id||"",200),v=Array.isArray(n.source_session_ids)?n.source_session_ids.map(k=>l(k,200)).filter(Boolean):[],x=await Qu({sessionId:r,tagged:u,shotChars:h,unifiedSessionId:_,characterId:y,sourceSessionIds:v});w("job.roster",{roster:x.length});const N=nt(f);for(const k of d)k.characters=ca(k.characters||[],x,N);if(await Ie(e,"superseded before generate"))return;await fe(e,"generating",Wn({shot_count:d.length,shot_index:0,shot_done:0,progress:0,phase:"generating",message:`이미지 1/${d.length} 생성 준비`}));const T=[],M=!!f.llm_anchor_percent;for(let k=0;k<d.length;k+=1){if(await Ie(e,`superseded before shot ${k+1}`))return;const S=d[k],{main:P,neg:U,captions:Z,meta:me}=await Pr({shot:S,roster:x}),J=ht(),Y=Date.now()/1e3,K=l(n.content_hash||"");let qe=Wm(S);qe==null&&M&&(qe=Math.round(k/Math.max(1,d.length)*1e4)/100),w("job.shot.prepare",{shot:k,card_id:J,prompt_len:String(P||"").length,captions:(Z||[]).length}),await fe(e,"generating",Wn({shot_count:d.length,shot_index:k,shot_done:k,progress:Math.round(k/Math.max(1,d.length)*1e3)/10,phase:"generating",message:`NovelAI 요청 중 ${k+1}/${d.length}… [${mt()}]`}));let ae=0;const ft=setInterval(()=>{ae+=Dc/1e3;const sn=_s();ys()&&ja()>=Um&&sn&&Date.now()-sn>=zm&&ud("heartbeat-idle");const wl=ja()||hs();fe(e,"generating",Wn({shot_count:d.length,shot_index:k,shot_done:k,progress:Math.round(k/Math.max(1,d.length)*1e3)/10,phase:"generating",message:`NovelAI 대기 ${k+1}/${d.length} (${ae}s) · ${mt()}${wl?` ${Math.round(wl/1024)}KB`:""}`})).catch(()=>{})},Dc);let q,Be;try{({bytes:q,seed:Be}=await Nc({main:P,neg:U,captions:Z}))}finally{clearInterval(ft)}if(w("job.shot.nai_done",{shot:k,bytes:q?.byteLength||0,seed:Be}),await Ie(e,`superseded after shot ${k+1} nai`))return;await fe(e,"generating",Wn({shot_count:d.length,shot_index:k,shot_done:k,progress:Math.round((k+.5)/Math.max(1,d.length)*1e3)/10,phase:"generating",message:`이미지 저장 중 ${k+1}/${d.length}… [${mt()}]`}));const te=dm({imageId:J,sessionId:r,request:n,shotIndex:k,paragraph:S.paragraph,yPercent:qe,line:(()=>{const sn=Math.floor(Number(S.line));return Number.isFinite(sn)&&sn>=1?sn:null})(),contentHash:K});await ha(J,q,te);const Ig=ei(me,te,q?.byteLength||0);await B("cards",{id:J,job_id:e,session_id:r,shot_index:k,paragraph:Number(S.paragraph||0),main_prompt:P,negative_prompt:U,characters_json:JSON.stringify(me.characters||[]),seed:Be,meta_json:JSON.stringify(Ig),created_at:Y});const bl=bt.get(e);if(bl&&bl.publishedIds.push(J),await Ie(e,`superseded after shot ${k+1} save`))return;T.push({id:J,shot_index:k,paragraph:te.paragraph,y_percent:te.y_percent,line:te.line,message_index:te.message_index??-1,message_role:te.message_role||"",content_hash:te.content_hash||"",character_id:te.character_id||"",chat_id:te.chat_id||"",character_name:te.character_name||"",chat_name:te.chat_name||"",char_index:te.char_index??-1,chat_index:te.chat_index??-1,assistant_preview:l(n.assistant_text||"",Ei),main_prompt:P,negative_prompt:U,characters:me.characters||[],image_url:Te(J),seed:Be,storage:"indexeddb",png_bytes:q?.byteLength||0}),w("job.shot.saved",{shot:k,card_id:J,has_url:!!Te(J)}),await fe(e,"generating",Wn({shot_count:d.length,shot_index:k,shot_done:k+1,progress:Math.round((k+1)/Math.max(1,d.length)*1e3)/10,phase:"generating",message:`이미지 ${k+1}/${d.length} 완료`,cards_so_far:T.length}))}if(await Ie(e,"superseded before done"))return;const j={cards:T,message_index:n.message_index!=null?Number(n.message_index):-1,shot_count:d.length,shot_done:d.length,progress:100,phase:"done",message:`이미지 ${d.length}/${d.length} 완료`};await it(j),await fe(e,"done",j);const R=bt.get(e);R&&(R.publishedIds=[]),i.end({message:"done",cards:T.length})}catch(o){i.fail(o);const s=o,c=`${s?.message||o}
${s?.stack||""}`.slice(-1500);await fe(e,"error",{phase:"error",message:String(s?.message||o).slice(0,240),debug_stage:wn(),debug_tail:qr(e,12)},c)}finally{bn(a)}}D(),rt(),Ut(),Ce(),_e();function Ot(e,t){try{return JSON.parse(e)}catch{return t}}async function Ym(e,t={}){const n=l(e,80),r=await F("cards",n);if(!r)return{ok:!1,error:{code:"not_found",message:"card not found"}};const a=Ot(r.characters_json||"[]",[]),i=Array.isArray(a)?a:[];let o=r.main_prompt;"main_prompt"in t&&(o=l(t.main_prompt,8e3));let s=r.negative_prompt;"negative_prompt"in t&&(s=l(t.negative_prompt,8e3));let c=i;if("characters"in t){const f=t.characters||[];if(!Array.isArray(f))return{ok:!1,error:{code:"bad_request",message:"characters must be a list"}};c=[];const p=nt(E().card||{});for(let m=0;m<f.slice(0,p).length;m++){const g=f[m];if(typeof g!="object")continue;const h=g,_=m<i.length&&typeof i[m]=="object"?i[m]:{},y=l(h.name!=null?h.name:_.name,200),v=l(h.prompt!=null?h.prompt:_.prompt,4e3);if(!y&&!v)continue;const x={..._,name:y||`char${m+1}`,prompt:v||"girl"};"uc"in h&&(x.uc=l(h.uc,2e3));for(const N of["center_x","center_y"])if(N in h)try{x[N]=Number(h[N])}catch{}c.push(x)}}r.main_prompt=o,r.negative_prompt=s,r.characters_json=JSON.stringify(c);try{let f=Ot(r.meta_json||"{}",{});(!f||typeof f!="object"||Array.isArray(f))&&(f={});const p=f;p.setup=o,p.characters=c,r.meta_json=JSON.stringify(p)}catch{}await B("cards",r);const u=await dt(n,{}),d={id:n,main_prompt:o,negative_prompt:s,characters:c,paragraph:r.paragraph,shot_index:r.shot_index,y_percent:u.y_percent,message_index:u.message_index,content_hash:u.content_hash,image_url:Te(n)};return await it(d),{ok:!0,card:d}}async function Uc(e,t="nai",n=null,r={}){const a=await F("cards",e);if(!a)return{ok:!1,error:{code:"not_found",message:"card not found"}};const i=a.session_id,o=Ot(a.meta_json||"{}",{}),s=o.location||{};if(!r.skipBusyCheck)try{const S=await dt(e,o),P=await si({session_id:i,content_hash:l(S.content_hash||o.content_hash||"",128),message_index:O(S.message_index,-1)},i);if(P)return P}catch{}const c=l(s.character_id||o.character_id||"",200);let u=l(o.unified_session_id||s.unified_session_id||"",200);!u&&c&&(u=Il(c));let d=[];try{const S=await F("jobs",a.job_id);if(S?.request_json){const P=JSON.parse(S.request_json);Array.isArray(P.source_session_ids)&&(d=P.source_session_ids.map(U=>l(U,200)).filter(Boolean))}}catch{}const f=await xa(i,u,c,d);if(t==="full"){const S=await F("jobs",a.job_id);if(!S)return{ok:!1,error:{code:"no_job",message:"original job missing"}};const P=JSON.parse(S.request_json);return P.force=!0,zc(P)}let p,m,g,h,_={};const y=n;if(y&&("main_prompt"in y||"negative_prompt"in y||"characters"in y))p="main_prompt"in y?l(y.main_prompt||""):l(a.main_prompt),m="negative_prompt"in y?l(y.negative_prompt||""):l(a.negative_prompt),h="characters"in y?y.characters:null,h==null&&(h=Ot(a.characters_json||"[]",[])),g=(h||[]).slice(0,nt(E().card||{})).map(S=>({prompt:da(S.prompt||"girl")||"girl",uc:l(S.uc),center_x:Number(S.center_x??.5),center_y:Number(S.center_y??.5)}));else if(l(a.main_prompt||"")){const S=Ot(a.characters_json||"[]",[]),P=Array.isArray(S)?S:[],U=Jc(o),Z=P.map(ft=>{if(!ft||typeof ft!="object")return null;const q=ft;if(q.raw&&typeof q.raw=="object"){const Be=q.raw;return{...Be,name:l(Be.name||q.name,200)}}return{name:l(q.name,200),action:q.action,expression:q.expression,sex:q.sex,label:q.label,age:q.age,original:q.original||q.original_tag}}).filter(Boolean),me=E().card||{},J=Ue(E().nai?.model||"nai-diffusion-4-5-full"),Y=Object.values(Sn).filter(Boolean);Sn[J]&&Y.unshift(Sn[J]);const K=_u({setup:o.setup,main:a.main_prompt,person:o.person,stylePositives:yu(me),qualitySuffixes:Y}),qe={characters:U.length?U:Z,paragraph:a.paragraph,camera:K.rebuildMain&&K.lockedSetup?"":l(o.camera||""),situation:K.rebuildMain&&K.lockedSetup?"":l(o.situation||o.scene||""),place:K.rebuildMain&&K.lockedSetup?"":l(o.place||""),action:K.rebuildMain&&K.lockedSetup?"":l(o.action||"")},ae=await Pr({shot:qe,roster:f,lockedSetup:K.rebuildMain?K.lockedSetup:void 0});K.rebuildMain?(p=ae.main,m=ae.neg,g=ae.captions,h=ae.meta.characters,_={setup:ae.meta.setup,person:ae.meta.person,characters:h}):(p=l(a.main_prompt),m=l(a.negative_prompt)||ae.neg,g=ae.captions,h=ae.meta.characters,_={setup:p,person:ae.meta.person,characters:h})}else{const S=Ot(a.characters_json||"[]",[]),P=Jc(o),U=(S||[]).map(Y=>{if(!Y||typeof Y!="object")return null;const K=Y;return K.raw||{name:K.name,action:K.action,expression:K.expression}}).filter(Boolean),Z=l(o.setup||""),me={characters:P.length?P:U,paragraph:a.paragraph,camera:Z?"":l(o.camera||""),situation:Z?"":l(o.situation||o.scene||""),place:Z?"":l(o.place||""),action:Z?"":l(o.action||"")},J=await Pr({shot:me,roster:f,lockedSetup:Z});p=J.main,m=J.neg,g=J.captions,h=J.meta.characters,_={setup:J.meta.setup,person:J.meta.person,characters:h}}if(!g.length&&!(y&&"characters"in y)){const S={characters:(o.characters||[]).map(U=>U.raw||U),camera:o.setup},P=await Pr({shot:S,roster:f});p=P.main,m=P.neg,g=P.captions,h=P.meta.characters,_={setup:P.meta.setup,person:P.meta.person,characters:h}}const{bytes:v,seed:x}=await Nc({main:p,neg:m,captions:g}),N=ht(),T=Date.now()/1e3;let M=await Tt(a.id);Object.keys(M).length||(M=await dt(a.id,o));const j={version:1,image_id:N,session_id:i,unified_session_id:u,character_id:l(M.character_id||"",200),character_name:l(M.character_name||o.character_name||"",200),chat_id:l(M.chat_id||"",200),chat_name:l(M.chat_name||o.chat_name||"",200),char_index:O(M.char_index,-1),chat_index:O(M.chat_index,-1),message_index:O(M.message_index,-1),shot_index:O(M.shot_index,O(a.shot_index,0)),paragraph:O(M.paragraph,O(a.paragraph,0)),y_percent:rr(M.y_percent),content_hash:l(M.content_hash||"",128),assistant_preview:l(M.assistant_preview||o.assistant_preview||"",Xe)};await ha(N,v,j);const R=ei({...o,..._},j,v.byteLength);for(const S of["y_percent","anchor_percent","read_percent"])delete R[S];R.y_percent=j.y_percent,await B("cards",{id:N,job_id:a.job_id,session_id:i,shot_index:a.shot_index,paragraph:a.paragraph,main_prompt:p,negative_prompt:m,characters_json:JSON.stringify(h),seed:x,meta_json:JSON.stringify(R),created_at:T});try{await oi(e)}catch{}const k={id:N,image_url:Te(N),main_prompt:p,negative_prompt:m,characters:h,seed:x,paragraph:j.paragraph,y_percent:j.y_percent,message_index:j.message_index,shot_index:j.shot_index,content_hash:j.content_hash,character_id:j.character_id,chat_id:j.chat_id,character_name:j.character_name,chat_name:j.chat_name,assistant_preview:j.assistant_preview,storage:"indexeddb",png_bytes:v.byteLength};return await it(k),{ok:!0,replaced:e,card:k}}async function Zm({session_id:e="",content_hash:t="",message_index:n=-1}={}){const r=l(e,200),a=l(t,128),i=O(n,-1);if(!r&&!a&&i<0)return{ok:!1,error:{code:"bad_request",message:"session_id or content_hash required"}};const o=await be("cards"),s=[];for(const h of o){if(r&&h.session_id!==r)continue;const _=Ot(h.meta_json||"{}",{}),y=await dt(h.id,_),v=l(y.content_hash||_.content_hash||"",128),x=O(y.message_index,-1);if(a){if(v!==a)continue}else if(i>=0){if(x!==i)continue}else continue;const N=y.y_percent??_.y_percent??_.anchor_percent??_.read_percent,T=Number(N);s.push({row:h,y:Number.isFinite(T)?T:999,shot:O(h.shot_index,0),paragraph:O(h.paragraph??y.paragraph,0)})}if(s.sort((h,_)=>h.y-_.y||h.shot-_.shot||h.paragraph-_.paragraph),!s.length)return{ok:!1,error:{code:"not_found",message:"no cards for message"}};const c=r||l(s[0]?.row?.session_id||"",200),u={session_id:c,content_hash:a,message_index:i},d=await si(u,c);if(d)return d;const f=Or(u,c);sr.add(f);const p=[],m=[],g=[];try{for(const h of s){const _=h.row;try{const y=await Uc(_.id,"nai",null,{skipBusyCheck:!0});if(y?.busy){g.push({id:_.id,error:y.error?.message||"busy"});break}y?.ok&&y.card?(p.push(y.card),y.replaced&&m.push(y.replaced)):g.push({id:_.id,error:l(y?.error?.message||"reroll failed",400)})}catch(y){g.push({id:_.id,error:l(y?.message||y,400)})}}return{ok:p.length>0,count:p.length,replaced:m,cards:p,failed:g}}finally{sr.delete(f)}}function Jc(e){return Array.isArray(e.characters)?e.characters.map(t=>t&&typeof t=="object"?t.raw||t:null).filter(Boolean):[]}var Xm=cn({HASH_REBIND_THRESHOLD:()=>rp,PREFIX_MATCH_MIN_CHARS:()=>24,PREFIX_MATCH_THRESHOLD:()=>np,SIMILARITY_COMPARE_MAX:()=>800,VIEWER_THUMB_LAYOUT:()=>ee,activeSegmentIndex:()=>Yp,bigramCounts:()=>ui,buildInlineChatHtml:()=>ng,claimStickyMarkerByCardId:()=>Ap,clampPinPercent:()=>Ke,clampReadingPercent:()=>Vp,clampShotLine:()=>cl,clampThumbScrollOffset:()=>Fp,composeDualProgressBarsHtml:()=>Gp,composeProgressToastHtml:()=>qp,composeSingleProgressBarHtml:()=>rl,composeStickyThumbHtml:()=>$p,createDebouncedSaveQueue:()=>kp,createScrollSettleTracker:()=>vp,createSessionChangeGuard:()=>xp,describeDomApiCompare:()=>dp,diceBigramRatio:()=>Gc,evenAnchorPercent:()=>mi,expandElementLineEntries:()=>ml,findElementIndexForLine:()=>pl,findElementIndexForLineWithFallback:()=>tg,findHashRebindCandidates:()=>ap,findPlainLineStartOffset:()=>dl,fitBoxInside:()=>jp,galleryFocusMessage:()=>sp,galleryForMessage:()=>op,galleryIndexFromChildIndex:()=>Lp,gallerySelectedCount:()=>fi,galleryStripContentWidth:()=>Rp,galleryStripSplitAt:()=>Rr,hasGenerationInfo:()=>Zc,htmlToPlainLn:()=>eg,injectInlineImagesIntoHtml:()=>_l,isCharMessageRole:()=>lp,isMessageSelectionGesture:()=>yp,isNearbyDomIndex:()=>Up,lineTextOccurrence:()=>fl,linkCardsForMessage:()=>di,longestCommonPrefixLength:()=>Kc,markerBlockHtml:()=>hl,mergeViewerPaintJob:()=>Hp,messageCompactKey:()=>on,messageContextTriplet:()=>up,messagesTextOverlapScore:()=>Xc,nearbyDomIndexWindow:()=>Jp,nearbyMessageImageIds:()=>zp,normalizeInlineMatchText:()=>zr,normalizeMatchText:()=>Lr,normalizeMessageRole:()=>li,parseAutotagLookJson:()=>nl,pickMessageIndexNearPoint:()=>_p,pinPercentToPx:()=>pp,pinPercentToPxFromBottom:()=>gp,pinPxToPercent:()=>Qc,preferNearbyHostIndex:()=>gl,prefixMatchRatio:()=>qc,rawMessageRole:()=>Br,readingPercentInMessage:()=>al,rebindGalleryMessageIndexes:()=>fp,resolveCardAnchorPercent:()=>Wp,resolveChatMessageMatch:()=>mp,resolveClickSelectionAction:()=>bp,resolveIndexProgress:()=>Kp,resolveStickyThumbPct:()=>Ip,resolveStoredPinPercent:()=>hp,roleFromGenerationInfo:()=>cp,scaleInlineThumbnail:()=>tp,shouldKeepStickyThumbHidden:()=>Np,shouldRefreshGallery:()=>Sp,shouldRewriteStickyThumb:()=>Mp,shouldSelectMessageByTextDrag:()=>wp,splitMessageLines:()=>Dr,stickyCornerEdgeBox:()=>el,stickyCornerImageBox:()=>Tp,stickyPinEdgeBox:()=>Op,stickyPinOverImage:()=>Pp,stickyThumbBoxFromPct:()=>Ep,stickyThumbNeedsHtmlPaint:()=>Cp,stripInlayInlineHtml:()=>sl,thumbIndexAtStripX:()=>Bp,visibleGalleryImageIds:()=>Dp}),Qm=528,ep=720;function b(e,t=0){const n=Number(e);return Number.isFinite(n)?n:t}function Hc(e,t){const n={...e};for(const[r,a]of Object.entries(t||{})){const i=n[r];n[r]=a&&typeof a=="object"&&!Array.isArray(a)?Hc(i&&typeof i=="object"?i:{},a):a}return n}function tp(e){const t=Math.max(1,b(e,100));return{width:Math.max(1,Math.round(Qm*t/100)),height:Math.max(1,Math.round(ep*t/100)),percent:t}}function Lr(e){return String(e||"").replace(/[^a-zA-Z0-9\uac00-\ud7a3\u3040-\u30ff\u3400-\u9fff\uff00-\uffef]/g,"").toLowerCase()}var np=.6,rp=.6,Eg=24,jg=800;function li(e){const t=String(e||"").toLowerCase().trim();return t?t==="char"||t==="assistant"||t==="bot"?"char":t==="user"||t==="human"?"user":t:""}function Kc(e,t){const n=String(e||""),r=String(t||""),a=Math.min(n.length,r.length);let i=0;for(;i<a&&n.charCodeAt(i)===r.charCodeAt(i);)i+=1;return i}function ui(e){const t=String(e||""),n=new Map;if(t.length<2)return n;for(let r=0;r<t.length-1;r+=1){const a=t.charCodeAt(r)+","+t.charCodeAt(r+1);n.set(a,(n.get(a)||0)+1)}return n}function Gc(e,t,n=null){const r=String(e||""),a=String(t||"");if(!r||!a)return 0;if(r===a)return 1;if(r.length<2||a.length<2)return 0;const i=n||ui(r),o=ui(a);let s=0;for(const[d,f]of o){const p=i.get(d)||0;p&&(s+=Math.min(p,f))}const c=r.length-1,u=a.length-1;return 2*s/(c+u)}function qc(e,t){const n=Lr(e),r=Lr(t);if(!n||!r)return 0;const a=Math.max(n.length,r.length);if(a<24)return 0;if(n===r)return 1;const i=Kc(n,r)/a;return i>=.6?i:Gc(n.length>800?n.slice(0,800):n,r.length>800?r.slice(0,800):r)}function di(e,t){if(!t)return[];const n=Array.isArray(e)?e:[],r=String(t.hash||"");return r?Yc(n.filter(a=>a?.content_hash&&a.content_hash===r)):[]}function ap(e,t={}){const n=Array.isArray(e)?e:[],r=String(t.newHash||t.hash||""),a=String(t.text||""),i=String(t.characterId||""),o=String(t.chatId||""),s=String(t.sessionId||""),c=Number(t.messageIndex),u=li(t.role||t.messageRole||"");if(!r||!a||!i||!u||!Number.isFinite(c)||c<0)return[];if(n.some(f=>f?.content_hash&&f.content_hash===r))return[];if(Lr(a).length<24)return[];const d=[];for(const f of n){const p=String(f?.content_hash||"");if(!p||p===r||String(f?.character_id||"")!==i)continue;const m=String(f?.chat_id||""),g=String(f?.session_id||"");if(o){if(m!==o)continue}else if(s){if(g&&g!==s)continue}else continue;if(Number(f?.message_index)!==c)continue;const h=li(f?.message_role||f?.role||"");if(!h||h!==u)continue;const _=String(f?.assistant_preview||"");if(!_)continue;const y=qc(a,_);y<.6||d.push({card:f,score:y})}return d.sort((f,p)=>p.score-f.score||b(p.card?.created_at)-b(f.card?.created_at)),Yc(d.map(f=>f.card))}function Wc(e){const t=e?.y_percent??e?.anchor_percent??e?.read_percent,n=Number(t);return Number.isFinite(n)?n:Number.POSITIVE_INFINITY}function ip(e,t){return Wc(e)-Wc(t)||b(e?.paragraph)-b(t?.paragraph)||b(e?.shot_index)-b(t?.shot_index)||b(t?.created_at)-b(e?.created_at)}function Vc(e,t){return b(t?.created_at)-b(e?.created_at)||b(t?.message_index)-b(e?.message_index)||b(t?.paragraph)-b(e?.paragraph)||b(t?.shot_index)-b(e?.shot_index)}function Yc(e){const t=new Map;for(const n of e||[]){const r=`${n?.paragraph??"?"}|${n?.shot_index??n?.id}`,a=t.get(r);(!a||b(n?.created_at)>=b(a?.created_at))&&t.set(r,n)}return[...t.values()]}function op(e,t,n=8){const r=new Map;for(const d of e||[]){const f=String(d?.id||"");!f||r.has(f)||r.set(f,d)}const a=[...r.values()],i=Math.max(0,Math.min(64,b(n,8)||8));if(!t)return a.sort(Vc).slice(0,Math.max(1,i||8));const o=di(a,t).sort(ip),s=new Set(o.map(d=>String(d.id))),c=String(t.sessionId||""),u=a.filter(d=>!s.has(String(d.id))).filter(d=>!c||!d?.session_id||d.session_id===c).sort(Vc).slice(0,i);return[...o,...u]}function fi(e,t){return t?di(e,t).length:0}function sp(e,t,n){if(e&&fi(n,e)>0)return e;const r=String(e?.sessionId||e?.session_id||""),a=String(t?.sessionId||t?.session_id||"");return t&&fi(n,t)>0&&(!r||!a||r===a)?t:e||null}function Br(e){const t=String(e?.role||e?.type||e?.speaker||e?.sender||e?.from||e?.name||"").trim().toLowerCase();return e?.isUser===!0||e?.fromUser===!0||/^(user|human|player|you|me)$/.test(t)||t.includes("user")?"user":e?.isAssistant===!0||e?.isBot===!0||e?.isChar===!0||/^(assistant|bot|char|character|model|ai)$/.test(t)||t.includes("assistant")||t.includes("bot")||t.includes("char")?"char":t==="system"||t==="developer"?"system":""}function Zc(e){const t=e?.generationInfo??e?.generation_info??e?.messageGenerationInfo;return t==null?!1:typeof t=="string"?t.trim().length>0:typeof t=="object"?Object.keys(t).length>0:!0}function cp(e){return Zc(e)?"char":"user"}function lp(e){if(e&&typeof e=="object")return Br(e)==="char";const t=String(e||"").trim().toLowerCase();return t==="char"||t==="assistant"||t==="bot"}function on(e,t=48){const n=Math.max(1,b(t,48));return String(e||"").replace(/\s+/g,"").slice(0,n)}function up(e,t,n=48){const r=Array.isArray(e)?e:[],a=Math.floor(b(t,-1)),i=a>=0&&a<r.length?r[a]:null,o=a>0?r[a-1]:null,s=a>=0&&a<r.length-1?r[a+1]:null;return[on(o?.text,n),on(i?.text,n),on(s?.text,n),Br(i)||String(i?.role||"").trim().toLowerCase()].join("|")}function Xc(e,t){const n=String(e||"").replace(/\s+/g,""),r=String(t||"").replace(/\s+/g,"");if(!n||!r)return 0;if(n.includes(r))return r.length;if(r.includes(n))return n.length;let a=0;const i=(s,c,u=8,d=512)=>{let f=u,p=Math.min(s.length,d),m=0;for(;f<=p;){const g=f+p>>1;c.includes(s.slice(0,g))?(m=g,f=g+1):p=g-1}return m};a=Math.max(a,i(r,n),i(n,r));const o=Math.min(r.length,2400);for(let s=0;s+8<=o;s+=32){const c=Math.min(64,o-s);for(let u=c;u>=8;u-=8)if(n.includes(r.slice(s,s+u))){a=Math.max(a,u);break}}return a}function dp(e,t){const n=String(e||""),r=String(t||""),a=on(n),i=on(r),o=Xc(n,r),s=o>=8,c=a.length>0&&a.length<8&&a===i,u=i.slice(0,48),d=a.slice(0,48);return{domChars:n.length,apiChars:r.length,domCompactChars:a.length,apiCompactChars:i.length,shareScore:o,overlap:s,shortExact:c,apiInDom:!!(u&&u.length>=8&&a.includes(u)),domInApi:!!(d&&d.length>=8&&i.includes(d)),apiKey:u,domKey:d}}function fp(e,t,n){const r=Array.isArray(t)?t:[],a=typeof n=="function"?n:null,i=new Map;if(a)for(let s=0;s<r.length;s+=1){const c=String(r[s]?.text??r[s]?.data??r[s]?.content??""),u=String(a(c)||"");if(!u)continue;const d=Number.isFinite(Number(r[s]?.index))?Number(r[s].index):s,f=i.get(u);f?f.push(d):i.set(u,[d])}let o=0;return{cards:(Array.isArray(e)?e:[]).map(s=>{const c=String(s?.content_hash||""),u=c?i.get(c):void 0;return!c||!u?s:u.length!==1?Number(s?.message_index)!==-1?(o+=1,{...s,message_index:-1}):s:Number(s?.message_index)!==u[0]?(o+=1,{...s,message_index:u[0]}):s}),changed:o}}function mp(e,t,n,r,a){const i=Array.isArray(t)?t:[],o=Math.floor(b(n,-1)),s=(d,f,p,m=-1)=>({chatIndex:Number.isFinite(Number(d?.index))?Number(d?.index):m>=0?m:o,text:String(d?.text||e||""),role:Br(d)||String(d?.role||"").trim().toLowerCase(),matchMethod:f,score:p});if(!i.length)return{chatIndex:Math.max(0,o),text:String(e||""),role:"",matchMethod:"fallback",score:0};const c=i.length-1-o;if(c>=0&&c<i.length)return s(i[c],"reverse",100,c);const u=Math.max(0,Math.min(i.length-1,c));return s(i[u],"reverse",50,u)}function Ke(e,t=0){const n=Number(e);return Number.isFinite(n)?Math.max(0,Math.min(100,Math.floor(n))):Math.max(0,Math.min(100,Math.floor(b(t,0))))}function pp(e,t,n=22,r=4){const a=Math.max(1,b(t,1)),i=Math.max(1,b(n,1)),o=Math.max(0,b(r,0)),s=Math.floor(a*Ke(e,0)/100);return Math.max(o,Math.min(a-i-o,s))}function gp(e,t,n=22,r=8){const a=Math.max(1,b(t,1)),i=Math.max(1,b(n,1)),o=Math.max(0,b(r,0)),s=a-Math.floor(a*Ke(e,0)/100)-i;return Math.max(o,Math.min(a-i-o,s))}function Qc(e,t){const n=Math.max(1,b(t,1));return Ke(b(e,0)/n*100,0)}function hp(e,t,n,r=null){const a=r&&typeof r=="object"?r:{x:38,y:80},i=t==="y"||t==="Y",o=i?b(a.y,80):b(a.x,38),s=i?"overlay_y_pct":"overlay_x_pct",c=i?"overlay_y_offset":"overlay_x_offset",u=String(e?.overlay_pin_origin||""),d=u==="top"||u==="tl"||u==="top-left",f=e?.[s],p=f!=null&&Number.isFinite(Number(f)),m=Number(e?.[c]),g=Number.isFinite(m),h=_=>Ke(100-Number(_),o);return p?i&&d?h(f):Ke(f,o):g?i?Ke((b(n,1)-m)/Math.max(1,b(n,1))*100,o):Qc(m,n):Ke(o,o)}function _p(e,t,n=null,r=800){const a=Array.isArray(e)?e:[];if(!a.length)return-1;const i=Math.max(1,b(r,800)),o=Number.isFinite(Number(t))?Number(t):i*.5,s=Number.isFinite(Number(n))?Number(n):null;let c=-1,u=1/0,d=-1,f=1/0;for(let p=0;p<a.length;p+=1){const m=a[p];if(!m)continue;const g=b(m.top,0),h=b(m.bottom,g);if(h<=0||g>=i)continue;const _=(g+h)*.5,y=Number.isFinite(Number(m.left))&&Number.isFinite(Number(m.right))?(Number(m.left)+Number(m.right))*.5:null,v=y!=null&&s!=null?Math.hypot(y-s,_-o):Math.abs(_-o);v<f&&(f=v,d=p);const x=o>=g&&o<=h,N=s==null||!Number.isFinite(Number(m.left))||!Number.isFinite(Number(m.right))||s>=Number(m.left)&&s<=Number(m.right);if(x&&N){const T=Math.max(1,h-g);T<u&&(u=T,c=p)}}return c>=0?c:d}function yp({gesture:e="single",detail:t=1,movement:n=0,textSelecting:r=!1,excludedTarget:a=!1}={}){const i=e==="double"?2:1;return!a&&!r&&b(n,1/0)<=8&&b(t)===i}function bp({gesture:e="single",detail:t=1,pendingDomIndex:n=null,targetDomIndex:r=null}={}){const a=b(t,1);return e==="double"?a===1?{action:"provisional",pendingDomIndex:r}:a===2?{action:"confirm",clearPending:!0,matchedPending:n!=null&&Number(n)===Number(r)}:{action:"ignore"}:a===1?{action:"confirm",clearPending:!0}:{action:"ignore"}}function wp({enabled:e=!0,movement:t=0,hasSelection:n=!1,excludedTarget:r=!1}={}){return!!e&&!r&&!!n&&b(t,0)>8}function vp({delayMs:e=160,onSettle:t}={}){let n=null;const r=Math.max(0,b(e,160)),a=()=>{n=null,typeof t=="function"&&t()};return{bump(){n&&clearTimeout(n),n=setTimeout(a,r)},settleNow(){n&&clearTimeout(n),n=null,a()},cancel(){n&&clearTimeout(n),n=null},get pending(){return n!=null}}}function xp(e=""){let t=String(e||""),n="",r=0;return{observe(a){const i=String(a||"");return!i||i===t?(n="",r=0,!1):i!==n?(n=i,r=1,!1):(r+=1,r<2?!1:(t=n,n="",r=0,!0))},current(){return t},reset(a=""){t=String(a||""),n="",r=0}}}function kp(e,t=300){let n={},r=null,a=null;const i=async()=>a||(a=(async()=>{for(;Object.keys(n).length;){const o=n;n={},await e(o)}})().finally(()=>{a=null}),a);return{enqueue(o){n=Hc(n,o),r&&clearTimeout(r),r=setTimeout(()=>{r=null,i().catch(()=>{})},Math.max(0,b(t,300)))},async flush(){r&&(clearTimeout(r),r=null);do await i();while(Object.keys(n).length||a)},hasPending(){return!!r||!!a||Object.keys(n).length>0}}}function Sp(e,t){const n=Array.isArray(e)?e.map(String):[],r=Array.isArray(t)?t.map(String):[];if(n.length!==r.length)return!0;for(let a=0;a<n.length;a+=1)if(n[a]!==r[a])return!0;return!1}function Ap(e,t){const n=String(t||"");if(!n||!Array.isArray(e)||!e.length)return null;const r=e.findIndex(i=>String(i?.card?.id||"")===n);if(r<0)return null;const a=e[r];return a?.thumb?(e.splice(r,1),a):null}function Mp(e,t){return String(e||"")!==String(t||"")}function Np(e,t,n){if(!e)return!1;const r=String(t||""),a=String(n||"");return!!r&&!!a&&r===a}function Ip(e){if(!e.alwaysOn||e.userCollapsed||e.editorOpen)return 0;const t=b(e.settingsPct,0);return t>0?t:0}function Ep(e,t,n){const r=Math.max(0,b(e,0)),a=Math.max(0,b(t,0)),i=Math.max(0,b(n,0));return{pct:r,w:Math.max(0,Math.round(a*r/100)),h:Math.max(0,Math.round(i*r/100))}}function jp(e,t,n=0,r=0){const a=Math.max(0,b(e,0)),i=Math.max(0,b(t,0));if(a<=0||i<=0)return{w:0,h:0};const o=Math.max(0,b(n,0)),s=Math.max(0,b(r,0));if(o<=0||s<=0)return{w:a,h:i};const c=Math.min(a/o,i/s);return{w:Math.max(1,Math.round(o*c)),h:Math.max(1,Math.round(s*c))}}function $p(e,t=""){const n=typeof e=="string"?e:"";return n?`<img src="${n}" style="width:100%;height:100%;object-fit:contain;display:block;background:transparent" />`:'<div style="width:100%;height:100%;background:transparent"></div>'}function Cp(e,t,n,r){const a=typeof t=="string"?t:"";return a?String(e||"")!==a||String(n||"")!==String(r||""):!1}function Tp(e,t,n,r=16){const a=Math.max(1,Math.round(b(t?.w,1))),i=Math.max(1,Math.round(b(t?.h,1))),o=Math.max(a,Math.round(b(n?.width,a))),s=Math.max(i,Math.round(b(n?.height,i))),c=Math.max(0,Math.round(b(r,16))),u=String(e||"bottom-right");return{left:u.includes("left")?c:Math.max(c,o-a-c),top:u.includes("top")?c:Math.max(c,s-i-c),w:a,h:i}}function el(e,t,n=16){const r=Math.max(1,Math.round(b(t?.w,1))),a=Math.max(1,Math.round(b(t?.h,1))),i=Math.max(0,Math.round(b(n,16))),o=String(e||"bottom-right");return{w:r,h:a,top:o.includes("top")?i:null,bottom:o.includes("bottom")?i:null,left:o.includes("left")?i:null,right:o.includes("right")?i:null}}function Pp(e,t=28,n=6){const r=Math.round(b(e?.left,0)),a=Math.round(b(e?.top,0)),i=Math.max(1,Math.round(b(e?.w,1))),o=Math.max(1,Math.round(b(t,28))),s=Math.max(0,Math.round(b(n,6)));return{left:Math.round(r+(i-o)/2),top:a-s}}function Op(e,t,n=28,r=6,a=16){const i=el(e,t,a),o=Math.max(1,Math.round(b(n,28))),s=Math.max(0,Math.round(b(r,6))),c=i.left!=null?i.left:i.right,u=Math.round(b(c,0)+(i.w-o)/2);return{size:o,top:i.top!=null?Math.round(i.top-s):null,bottom:i.bottom!=null?Math.round(i.bottom+i.h+s):null,left:i.left!=null?u:null,right:i.right!=null?u:null}}var ee=Object.freeze({width:64,height:88,gap:8,splitWidth:16,splitExtraMargin:4});function Rr(e,t){const n=Math.max(0,Math.floor(b(t,0))),r=Math.max(0,Math.min(Math.floor(b(e,0)),n));return r>0&&r<n?r:0}function Lp(e,t,n){const r=Math.max(0,Math.floor(b(n,0))),a=Math.floor(b(e,-1));if(a<0||r<=0)return-1;const i=Rr(t,r);if(i<=0)return a<r?a:-1;if(a===i)return-1;const o=a>i?a-1:a;return o>=0&&o<r?o:-1}function Bp(e,{count:t=0,selectedCount:n=0,thumbWidth:r=ee.width,gap:a=ee.gap,splitWidth:i=ee.splitWidth,splitExtraMargin:o=ee.splitExtraMargin}={}){const s=Math.max(0,Math.floor(b(t,0)));if(s<=0)return-1;const c=b(e,NaN);if(!Number.isFinite(c)||c<0)return-1;const u=Math.max(1,b(r,ee.width)),d=Math.max(0,b(a,ee.gap)),f=Math.max(0,b(i,ee.splitWidth)),p=Math.max(0,b(o,ee.splitExtraMargin)),m=Rr(n,s),g=[];for(let _=0;_<s;_+=1)m>0&&_===m&&g.push({type:"split"}),g.push({type:"thumb",galIdx:_,marginLeft:m>0&&_===m?p:0});let h=0;for(let _=0;_<g.length;_+=1){_>0&&(h+=d);const y=g[_];if(y.type==="split"){if(c>=h&&c<h+f)return-1;h+=f;continue}if(h+=y.marginLeft,c>=h&&c<h+u)return y.galIdx;h+=u}return-1}function Rp({count:e=0,selectedCount:t=0,thumbWidth:n=ee.width,gap:r=ee.gap,splitWidth:a=ee.splitWidth,splitExtraMargin:i=ee.splitExtraMargin}={}){const o=Math.max(0,Math.floor(b(e,0)));if(o<=0)return 0;const s=Math.max(1,b(n,ee.width)),c=Math.max(0,b(r,ee.gap)),u=Math.max(0,b(a,ee.splitWidth)),d=Math.max(0,b(i,ee.splitExtraMargin)),f=Rr(t,o),p=o+(f>0?1:0);let m=o*s+Math.max(0,p-1)*c;return f>0&&(m+=u+d),m}function Fp(e,t,n){const r=Math.max(0,b(t,0)-Math.max(0,b(n,0))),a=b(e,0);return Math.max(0,Math.min(r,a))}function Dp(e,t=0,n=1,r=8,a=0){const i=Array.isArray(e)?e:[];if(!i.length)return[];const o=Math.max(0,Math.min(b(t,0),i.length-1)),s=Math.max(1,Math.min(i.length,b(r,8)||8)),c=Math.min(s,1+2*Math.max(0,b(n,1))),u=Math.max(c,s);let d=Math.max(0,o-Math.floor((u-1)/2));const f=Math.min(i.length-1,d+u-1);d=Math.max(0,f-u+1);const p=[],m=new Set,g=_=>{const y=String(i[_]?.id||"");!y||m.has(y)||(m.add(y),p.push(y))},h=Math.max(0,Math.min(i.length,b(a,0)));for(let _=0;_<h;_+=1)g(_);for(let _=d;_<=f&&(g(_),!(p.length>=s));_+=1);return p}function zp(e,t,n=2,r=[]){const a=Array.isArray(e)?e:[],i=Math.max(0,Math.min(8,b(n,2))),o=b(t?.messageIndex??t?.message_index,NaN),s=String(t?.sessionId||t?.session_id||""),c=String(t?.chatId||t?.chat_id||""),u=String(t?.hash||""),d=new Set,f=[],p=m=>{const g=String(m||"");!g||d.has(g)||(d.add(g),f.push(g))};for(const m of r||[])p(m);for(const m of a){if(!m?.id||s&&m.session_id&&String(m.session_id)!==s||c&&m.chat_id&&String(m.chat_id)!==c)continue;const g=b(m.message_index,NaN);if(Number.isFinite(o)&&Number.isFinite(g)){Math.abs(g-o)<=i&&p(m.id);continue}u&&String(m.content_hash||"")===u&&p(m.id)}return f}function Up(e,t,n=2){const r=b(e,NaN),a=b(t,NaN);if(!Number.isFinite(r)||!Number.isFinite(a))return!1;const i=Math.max(0,b(n,2));return Math.abs(r-a)<=i}function Jp(e,t,n=2){const r=Math.max(0,Math.floor(b(t,0))),a=Math.max(0,Math.min(Math.max(0,r-1),Math.floor(b(e,0)))),i=Math.max(0,Math.floor(b(n,2)));return r<=0?{lo:0,hi:-1,center:0}:{lo:Math.max(0,a-i),hi:Math.min(r-1,a+i),center:a}}var tl={chrome:1,content:2,full:3};function Hp(e,t){const n=tl[String(e??"")]||0,r=tl[String(t??"")]||0;return!n&&!r?"full":r>=n?t||e||"full":e||t||"full"}function nl(e){const t=String(e||"").trim();if(!t)return{appearance:"",attire:"",accessories:"",text:"",gender:""};let n=null;const r=t.match(/```(?:json)?\s*([\s\S]*?)```/i),a=(r?r[1]:t).trim().match(/\{[\s\S]*\}/);if(a)try{const o=JSON.parse(a[0]);n=o&&typeof o=="object"?o:null}catch{n=null}if(n&&typeof n=="object"){const o=String(n.appearance??n.look??n.identity??"").trim(),s=String(n.attire??n.clothing??n.outfit??"").trim(),c=String(n.accessories??n.accessory??n.props??"").trim(),u=String(n.gender??n.sex??"").trim().toLowerCase(),d=["girl","female","f","woman"].includes(u)?"girl":["boy","male","m","man"].includes(u)?"boy":u==="other"?"other":"";return{appearance:o,attire:s,accessories:c,text:[o,s,c].filter(Boolean).join(", "),gender:d}}const i=t.replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/i,"").trim();return{appearance:i,attire:"",accessories:"",text:i,gender:""}}function Kp(e={}){const t=!!e.warmBusy,n=Math.max(0,Math.min(100,Math.round(b(e.warmPct,0)))),r=String(e.jobState||""),a=Math.max(0,Math.min(100,Math.round(b(e.jobPct,0))));return t?{pct:Math.max(t?6:0,n),busy:!0,label:"인덱싱"}:r==="tagging"?{pct:Math.max(6,a),busy:!0,label:"인덱싱"}:{pct:0,busy:!1,label:"인덱싱"}}function Gp(e={}){const t=!!e.jobBusy,n=!!e.indexBusy,r=Math.max(t?6:0,Math.min(100,Math.round(b(e.jobPct,0)))),a=Math.max(n?6:0,Math.min(100,Math.round(b(e.indexPct,0)))),i=e.error?"#f87171":"#7c6cff",o="#2dd4bf",s=(c,u)=>`<span style="display:block;height:4px;border-radius:2px;background:#1e2633;overflow:hidden"><span style="display:block;height:100%;width:${c}%;background:${u}"></span></span>`;return`<span style="flex:0 0 120px;display:flex;flex-direction:column;gap:3px;justify-content:center">${s(r,i)}${s(a,o)}</span>`}function rl(e={}){const t=!!e.busy,n=Math.max(t?6:0,Math.min(100,Math.round(b(e.pct,0)))),r=String(e.tone||"");return`<span style="flex:0 0 120px;display:block;height:6px;border-radius:3px;background:#1e2633;overflow:hidden"><span style="display:block;height:100%;width:${n}%;background:${e.error?"#f87171":r==="index"?"#2dd4bf":"#7c6cff"}"></span></span>`}function qp(e={}){const t=!!e.jobBusy,n=!!e.indexBusy,r=!!e.busy||t||n;if(!r&&!e.error)return"";const a=typeof e.escapeHtml=="function"?e.escapeHtml:d=>String(d).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),i=String(e.tone||(!t&&n?"index":"job")),o=a(String(e.stage||(i==="index"?"인덱싱":"작업 중")).slice(0,80)),s=a(String(e.meta||"").slice(0,160));let c=e.pct;c==null&&(t?c=e.jobPct:n?c=e.indexPct:c=Math.max(b(e.jobPct,0),b(e.indexPct,0)));const u=rl({pct:c,busy:r||!!e.error,error:e.error,tone:i});return`<div data-inlay-progress-toast="1" style="display:flex;align-items:center;gap:10px;min-width:220px;max-width:min(420px,92vw);padding:10px 14px;border-radius:8px;background:#121820;border:1px solid #2a3344;cursor:pointer;user-select:none"><span style="flex:0 0 auto;font-weight:700;color:${e.error?"#f87171":i==="index"?"#5eead4":"#c4b5fd"};font-size:11px;white-space:nowrap">${o}</span><span style="min-width:0;flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#8b97ab;font-size:11px">${s}</span>${u}</div>`}function Fr(e){return Math.max(0,Math.min(100,b(e,0)))}function mi(e,t){const n=Math.max(1,b(t,1));return Math.max(0,Math.min(b(e,0),n-1))/n*100}function Wp(e,t=0,n=1,r=null){if(r&&r.forceEven)return mi(t,n);const a=e?.y_percent??e?.anchor_percent??e?.read_percent,i=Number(a);return Number.isFinite(i)?Fr(i):mi(t,n)}function al(e,t,n=.5){if(!e)return null;const r=Math.max(1,b(e.height,0)),a=Math.max(0,b(t,0))*(Number.isFinite(n)?n:.5);return a<b(e.top,0)||a>b(e.bottom,0)?null:Fr((a-b(e.top,0))/r*100)}function Vp(e,t,n=.5){const r=al(e,t,n);if(r!=null)return r;const a=Math.max(0,b(t,0))*(Number.isFinite(n)?n:.5);return b(e?.bottom,0)<a?100:0}function Yp(e,t){const n=Array.isArray(e)?e:[];if(!n.length)return-1;const r=Fr(t),a=n.map(c=>Fr(c)),i=a.reduce((c,u,d)=>u<=20?c.concat(d):c,[]),o=i.length===1&&a[i[0]]>0?a.map((c,u)=>u===i[0]?1:c):a;if(o[0]>0&&r<o[0])return-1;let s=0;for(let c=0;c<o.length;c+=1)r>=o[c]&&(s=c);return s}var Zp="data-inlay-inline-shot",il=/^<\/(?:p|div|li|blockquote|h[1-6]|tr)>$/i,ol=/^<br\s*\/?>$/i;function sl(e){const t=String(e||"");return t?t.replace(/<div[^>]*\bdata-inlay-inline-shot\b[^>]*>[\s\S]*?<\/div>/gi,"").replace(/<(?:span|p)[^>]*\bdata-inlay-inline-shot\b[^>]*>[\s\S]*?<\/(?:span|p)>/gi,""):""}function Dr(e){return String(e??"").replace(/\r\n/g,`
`).split(`
`).map(t=>t.trim()).filter(Boolean)}function cl(e,t){const n=Math.floor(Number(e)),r=Math.max(0,Math.floor(Number(t)||0));return!r||!Number.isFinite(n)||n<1?null:Math.max(1,Math.min(r,n))}function ll(e){return String(e||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Xp(e){const t=e.toLowerCase();if(t==="&nbsp;")return" ";if(t==="&amp;")return"&";if(t==="&lt;")return"<";if(t==="&gt;")return">";if(t==="&quot;")return'"';if(t==="&#39;"||t==="&apos;")return"'";const n=/^&#(\d+);$/.exec(e);if(n){const a=Number(n[1]);if(Number.isFinite(a)&&a>0&&a<1114112)return String.fromCodePoint(a)}const r=/^&#x([0-9a-f]+);$/i.exec(e);if(r){const a=Number.parseInt(r[1],16);if(Number.isFinite(a)&&a>0&&a<1114112)return String.fromCodePoint(a)}return null}function Qp(e,t){let n=Math.max(0,Math.min(e.length,Math.floor(t)));for(;n>0;){let r=n;for(;r>0&&/[ \t\r\n]/.test(e[r-1]);)r-=1;if(r<=0||e[r-1]!==">")break;const a=e.lastIndexOf("<",r-1);if(a<0||a>=r-1)break;const i=e.slice(a,r);if(ol.test(i)||il.test(i)||/^<\//.test(i)||!/^<[a-zA-Z][^>]*>$/.test(i))break;n=a}return n}function ul(e){const t=[];let n=0;const r=String(e||"");for(;n<r.length;){if(r[n]==="<"){const s=r.indexOf(">",n);if(s<0)break;const c=r.slice(n,s+1);(ol.test(c)||il.test(c))&&t.push({ch:`
`,htmlIndex:n}),n=s+1;continue}if(r[n]==="&"){const s=r.indexOf(";",n);if(s>n&&s-n<16){const c=Xp(r.slice(n,s+1));if(c!=null){for(let u=0;u<c.length;u+=1)t.push({ch:c[u],htmlIndex:n});n=s+1;continue}}}t.push({ch:r[n],htmlIndex:n}),n+=1}const a=[];for(let s=0;s<t.length;s+=1){const c=t[s];if(c.ch!==`
...
`||o[o.length-1].ch===" ");)o.pop();return o}function eg(e){return ul(String(e||"")).map(t=>t.ch).join("")}function dl(e,t){const n=String(e??"").replace(/\r\n/g,`
`),r=Math.floor(Number(t));if(!Number.isFinite(r)||r<1)return null;let a=0,i=0;for(;i<=n.length;){const o=n.indexOf(`
`,i),s=o<0?n.length:o,c=n.slice(i,s);if(c.trim()&&(a+=1,a===r)){const u=/^\s*/.exec(c)?.[0].length||0;return i+u}if(o<0)break;i=o+1}return null}function zr(e){return String(e??"").replace(/\s+/g," ").trim()}function fl(e,t){const n=Array.isArray(e)?e:[],r=Math.floor(Number(t))-1;if(!Number.isFinite(r)||r<0||r>=n.length)return null;const a=zr(n[r]);if(!a)return null;let i=0;for(let o=0;o<=r;o+=1)zr(n[o])===a&&(i+=1);return{text:a,occurrence:i}}function ml(e){const t=[],n=Array.isArray(e)?e:[];for(let r=0;r<n.length;r+=1)for(const a of Dr(n[r])){const i=zr(a);i&&t.push({text:i,elementIndex:r})}return t}function pl(e,t,n){const r=fl(t,n);if(!r)return-1;const a=ml(e);let i=0;for(const o of a)if(o.text===r.text&&(i+=1,i===r.occurrence))return o.elementIndex;return-1}function gl(e,t,n=["P"]){const r=(Array.isArray(e)?e:[]).map(o=>String(o||"").toUpperCase()),a=Math.floor(Number(t));if(!Number.isFinite(a)||a<0||a>=r.length)return-1;const i=new Set((Array.isArray(n)&&n.length?n:["P"]).map(o=>String(o||"").toUpperCase()));if((r[a]||"")!=="DIV")return a;for(let o=1;o<r.length;o+=1){const s=a-o,c=a+o;if(s>=0&&i.has(r[s]))return s;if(c<r.length&&i.has(r[c]))return c}return-1}function tg(e,t,n,r,a=["P"]){const i=Array.isArray(n)?n:[],o=Math.floor(Number(r));if(!Number.isFinite(o)||o<1||!i.length)return null;const s=i.length;for(let c=Math.max(1,o);c<=s;c+=1){const u=gl(t,pl(e,i,c),a);if(u>=0)return{elementIndex:u,usedLine:c}}return null}function hl(e,t){const n=Number.isFinite(Number(e.shotIndex))?String(Math.floor(Number(e.shotIndex))):"",r=ll(String(e.cardId||n||e.line||"0"));return`<div ${Zp}="${r}" x-inlay-inline-shot="${r}" contenteditable="false" style="display:block;margin:10px 0;text-align:center;line-height:0"><br><img src="${ll(e.src)}" alt="" style="zoom:0.5;width:auto;height:auto;max-width:none;border-radius:8px;display:inline-block;vertical-align:top"><br></div>`}function _l(e,t,n){const r=sl(e),a=Array.isArray(t)?t:[];if(!r||!a.length)return r;const i=ul(r),o=i.map(g=>g.ch).join(""),s=Dr(o).length;if(!s||!i.length)return r;const c=new Map,u=new Set;for(const g of a){const h=cl(g?.line,s),_=String(g?.src||""),y=String(g?.cardId||"");!h||!/^data:image\//i.test(_)||y&&u.has(y)||c.has(h)||(y&&u.add(y),c.set(h,{...g,line:h,src:_}))}if(!c.size)return r;const d=n?.maxWidthPx,f=new Map;for(const[g,h]of c){const _=dl(o,g);if(_==null||_<0||_>=i.length)continue;const y=Qp(r,i[_].htmlIndex);f.has(y)||f.set(y,hl(h,d))}if(!f.size)return r;const p=[...f.entries()].map(([g,h])=>({htmlIndex:g,html:h})).sort((g,h)=>h.htmlIndex-g.htmlIndex);let m=r;for(const g of p)m=m.slice(0,g.htmlIndex)+g.html+m.slice(g.htmlIndex);return m}function ng(e,t,n){const r=Dr(e);return r.length?_l(r.map(a=>String(a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")).join("<br>"),t,n):""}X(),W(),Ft(),ce(),yt(),D(),Ta(),Za(),Sr(),Wa(),_e(),lt();function pi(){const e=pe(),t=e.events;return{last_stage:e.last_stage,events:Array.isArray(t)?t.slice(-20):[]}}async function rg(e){if(e){const t=E(),n={...e};l(n.api_key)&&(t.llm.api_key=l(n.api_key)),delete n.api_key,delete n.api_key_configured,l(n.service_account_json)&&(t.llm.service_account_json=l(n.service_account_json)),delete n.service_account_json,delete n.service_account_configured,Object.keys(n).length&&(t.llm=et(t.llm,n)),await Je()}try{const t=E().llm,n=Ar(t.source),r=Ct(t.provider);return n==="custom"&&!rc(t)?{ok:!1,message:r==="vertex"?"Vertex AI: Model + Service Account JSON(또는 access token)이 필요합니다.":"태깅 LLM Model/API key가 비어 있습니다. NovelAI 키가 아니라 태깅용 LLM 키를 넣으세요."}:(n==="main"||n==="aux")&&!Qn("runLLMModel")?{ok:!1,message:"RisuAI runLLMModel API를 사용할 수 없습니다."}:{ok:!0,message:`LLM ok (${n}): ${(await nn(t,[{role:"user",content:"Reply with exactly: ok"}])).slice(0,120)}`}}catch(t){return{ok:!1,message:String(t?.message||t)}}}async function ag(){const e=E().nai;if(Bn(e)==="comfy")try{if(!$a(e))return{ok:!1,message:"ComfyUI 워크플로 JSON이 없습니다.",debug:pe()};const t=ws({main:"test",neg:"test",captions:[{name:"",prompt:"char1"},{name:"",prompt:"char2"}],nai:e,seed:1}),n=vs(e.comfy_workflow_json,t),r=bs(e),{status:a,data:i}=await Ca(`${r}/system_stats`,{method:"GET"});if(a>=400)return{ok:!1,message:`ComfyUI 연결 실패 (HTTP ${a}) · ${r}`,debug:pe()};const o=i?.devices?.[0]?.name||i?.system?.os||"ok";return{ok:!0,message:`ComfyUI ok · ${r} · nodes=${Object.keys(n).length} · ${o}`,debug:pi()}}catch(t){return w("comfy.test",{message:String(t?.message||t)},"error"),{ok:!1,message:String(t?.message||t),debug:pe()}}if(!l(e.api_key))return{ok:!1,message:"NAI api_key missing",debug:pe()};try{const t=l(e.api_key);try{const n=se("nai.test.anlas"),r=await Sd(t);return n.end({message:"anlas ok"}),{ok:!0,message:`NAI token ok · Anlas=${JSON.stringify(r)}`,debug:pi()}}catch(n){const r=Ue(e.model||"nai-diffusion-4-5-full");return w("nai.test.anlas",{message:String(n?.message||n)},"warn"),{ok:!0,message:`NAI config present · model=${r} · anlas_skip=${n?.message||n}`,debug:pi()}}}catch(t){return w("nai.test",{message:String(t?.message||t)},"error"),{ok:!1,message:String(t?.message||t),debug:pe()}}}async function ig(){const e=E().nai,t=l(e.api_key);if(!t)return{ok:!1,message:"NAI api_key missing",debug:pe()};const n=String(pe().job_ctx??"");bn("probe-nai");const r=se("nai.probe");try{w("nai.probe.start",{message:"tiny generate"});const a={prompt:"1girl, solo, simple background, best quality",negative_prompt:"lowres, bad quality",width:512,height:768,steps:10,cfg_scale:5,cfg_rescale:0,sampler:"k_euler_ancestral",scheduler:"karras",model:Ue(e.model||"nai-diffusion-4-5-full"),var_plus:!1,characters:[],seed:Math.floor(Math.random()*4294967295)||1},i=l(e.request_url)||"https://image.novelai.net/ai/generate-image",o=await ks(t,a,i,{timeoutMs:9e4}),s=o.raw_bytes.byteLength,c=xn(ge(o.raw_bytes));return r.end({bytes:s,is_png:c,seed:o.seed}),{ok:!0,message:`probe ok · png=${c} · ${s}B · seed=${o.seed}`,bytes:s,is_png:c,seed:o.seed,debug:pe()}}catch(a){return r.fail(a),{ok:!1,message:String(a?.message||a),debug:pe()}}finally{bn(n)}}async function og(e,t){if(!e?.byteLength)throw new Error("image is empty");const n=await wu(e),r=n.bytes,a=n.mime||"image/png",i=n.filename||"image.png",o=`data:${a};base64,${await Ze(r)}`,s=ar(await ue("autotag"))||["Tag ONE character reference image into Danbooru-style English prompts.",'Return ONE JSON object only: {"gender":"girl|boy","appearance":"...","attire":"...","accessories":"..."}.',"gender must be exactly girl or boy from visual evidence. appearance = identity/hair/eyes/body (no clothes). attire = clothing + permanent jewelry (earrings/glasses/watch/earbuds). accessories = weapons/bags/held props/ID only (not jewelry)."].join(`
`);w("autotag.start",{message:`llm-vision ${i} ${r.length}B`,bytes:r.length,focus:!0,source:Ar(E().llm.source)});const c=[{role:"system",content:s},{role:"user",content:[{type:"text",text:"Tag this character image. JSON only with gender, appearance, attire, accessories."},{type:"image_url",image_url:{url:o}}]}];let u="";try{u=await nn(E().llm,c)}catch(v){throw w("autotag.llm.fail",{message:String(v?.message||v)},"error"),new Error(`오토태그 LLM 실패: ${String(v?.message||v).slice(0,240)}`)}const d=nl(u),f=l(d.appearance||"",4e3),p=l(d.attire||"",4e3),m=l(d.accessories||"",4e3),g=l(d.gender||"",20),h=l(d.text||[f,p,m].filter(Boolean).join(", "),8e3);if(!f&&!p&&!m)throw new Error("LLM이 외형/의상/악세사리 태그를 반환하지 않았습니다. 비전(이미지) 지원 모델인지 확인하세요.");const _=h.split(",").map(v=>v.trim()).filter(Boolean);w("autotag.done",{message:`gender=${g||"-"} app=${f.length} attire=${p.length} acc=${m.length}`,focus:!0});const y={ok:!0,appearance:f,attire:p,accessories:m,tags:_,text:h,count:_.length,threshold:Number(t||.2),engine:"llm-vision"};return g&&(y.gender=g),y}D();function sg(e){const t=e.indexOf("?");if(t<0)return{pathname:e,query:{}};const n=e.slice(0,t),r=new URLSearchParams(e.slice(t+1)),a={};for(const[i,o]of r.entries()){const s=a[i];s===void 0?a[i]=o:Array.isArray(s)?s.push(o):a[i]=[s,o]}return{pathname:n,query:a}}function Vn(e,t,n=""){const r=e[t];return Array.isArray(r)?r[0]??n:r??n}function cg(e,t={}){const n=l(e.auth_token,4e3);if(!n)return!0;const r=String(t.Authorization||t.authorization||"");let a=r.toLowerCase().startsWith("bearer ")?r.slice(7):r;if(a=a||String(t["X-Inlay-Nexus-Token"]||t["x-inlay-nexus-token"]||""),typeof a!="string"||a.length!==n.length)return!1;let i=0;for(let o=0;o<n.length;o+=1)i|=a.charCodeAt(o)^n.charCodeAt(o);return i===0}W(),ce(),D(),X(),ko(),_e(),lt(),Kn();var C=(...e)=>t=>e.includes(t)?"":null,Ge=e=>t=>t.startsWith(e)?t.slice(e.length):null,gi=(e,t)=>n=>n.startsWith(e)&&n.endsWith(t)?n.slice(e.length,n.length-t.length):null,A=e=>({status:200,data:e}),hi=e=>({status:200,data:e,contentType:"image/png",raw:!0}),Yn=e=>{throw pt(404,{ok:!1,...ne(e,"not_found")},e)};function _i(e){let t=e.image_b64||e.data||"";return typeof t=="string"&&t.startsWith("data:")&&(t=t.split(",",2)[1]??""),String(t||"")}var lg=[{match:C("/v1/settings/export"),handler:()=>A({ok:!0,json:Uf()})},{match:C("/v1/settings"),handler:()=>A({ok:!0,settings:Er()})},{match:C("/v1/curation/status"),handler:async()=>A({ok:!0,status:await Hn()})},{match:C("/v1/curation/catalog"),handler:async()=>A({ok:!0,catalog:await ut()})},{match:C("/v1/prompts"),handler:async()=>A({ok:!0,prompts:await Ir()})},{match:C("/v1/prompts/export"),handler:async()=>A({ok:!0,...await Rf()})},{match:Ge("/v1/prompts/"),handler:async({param:e})=>A({ok:!0,key:e,text:await ue(e)})},{match:Ge("/v1/jobs/"),handler:async({param:e})=>A(await qm(e))},{match:Ge("/v1/gallery/explore"),handler:async({query:e})=>A(await vm(Number(Vn(e,"limit","400"))))},{match:C("/v1/gallery/favorites"),handler:async()=>A(await $c())},{match:Ge("/v1/gallery"),handler:async({query:e})=>A(await xm(Vn(e,"session_id"),Number(Vn(e,"limit","40"))))},{match:C("/v1/nai/reference.png"),handler:async()=>hi(await vc()??Yn("no reference"))},{match:C("/v1/nai/vibe.png"),handler:async()=>hi(await am()??Yn("no vibe"))},{match:C("/v1/nai/reference"),handler:async()=>{const e=await em();return A({ok:!0,configured:e,image_reference:E().nai?.image_reference||"none",preview_url:e?"/v1/nai/reference.png":""})}},{match:C("/v1/nai/vibe"),handler:async()=>{const e=await nm();return A({ok:!0,configured:e,vibe_transfer:E().nai?.vibe_transfer||"none",preview_url:e?"/v1/nai/vibe.png":""})}},{match:Ge("/v1/characters"),handler:async({query:e})=>A(await On(Vn(e,"session_id"),Vn(e,"character_id")))},{match:Ge("/v1/appearance/"),handler:async({param:e})=>A(await On(e))},{match:Ge("/v1/images/"),handler:async({param:e})=>{let t=e.replace(/^\/+|\/+$/g,"");if(t.endsWith(".json")){const n=await Tt(t.slice(0,-5));return Object.keys(n).length||Yn("location missing"),A(n)}return t.endsWith(".png")&&(t=t.slice(0,-4)),hi(await Cc(t)??Yn("image missing"))}}],ug=[{match:C("/v1/settings/reset"),handler:async()=>A(await Hf())},{match:C("/v1/settings/import"),handler:async({body:e})=>A(await Jf(String(e.json||e.text||"")))},{match:C("/v1/settings","/v1/settings/update"),handler:async({body:e})=>A(await Gf(e))},{match:C("/v1/curation/catalog"),handler:async({body:e})=>{const t=e.catalog??e;return A(await lc(t))}},{match:C("/v1/curation/catalog/reset"),handler:async()=>A(await uc())},{match:C("/v1/curation/embed"),handler:async()=>A(await dc())},{match:C("/v1/curation/embed/test"),handler:async()=>A(await fc())},{match:C("/v1/curation/settings"),handler:async({body:e})=>A(await bc(e))},{match:C("/v1/prompts/import"),handler:async({body:e})=>A(await Ff(e?.json!=null?e.json:(e?.prompts!=null,e)))},{match:C("/v1/prompts/reset-defaults"),handler:async({body:e})=>A(await Df({keep_author_note:e?.keep_author_note!==!1&&e?.keepAuthorNote!==!1}))},{match:gi("/v1/prompts/","/reset"),handler:async({param:e})=>A(await oc(e,An(e)))},{match:Ge("/v1/prompts/"),handler:async({param:e,body:t})=>A(await oc(e,String(t.text||"")))},{match:C("/v1/jobs/create","/v1/jobs"),handler:async({body:e})=>({status:202,data:await zc(e)})},{match:C("/v1/gallery/unlink","/v1/cards/unlink"),handler:async({body:e})=>A(await ai(String(e.session_id||""),String(e.content_hash||""),e.message_index))},{match:C("/v1/gallery/rebind-hash","/v1/cards/rebind-hash"),handler:async({body:e})=>A(await km({session_id:String(e.session_id||e.sessionId||""),card_ids:e.card_ids||e.ids||[],to_hash:String(e.to_hash||e.content_hash||""),assistant_preview:String(e.assistant_preview||e.assistant_text||"")}))},{match:C("/v1/gallery/delete","/v1/cards/delete"),handler:async({body:e})=>{const t=l(e.folder_key||"",400),n=l(e.card_id||e.id||"",80),r=Array.isArray(e.card_ids)?e.card_ids:null;if(t)return A(await Im(t));if(r?.length)return A(await Sm(r));if(n)return A(await oi(n));const a="card_id or folder_key required";throw pt(400,{ok:!1,...ne(a,"bad_request")},a)}},{match:C("/v1/gallery/export"),handler:async({body:e})=>A(await Mm(e))},{match:C("/v1/gallery/import"),handler:async({body:e})=>A(await Nm(e))},{match:C("/v1/gallery/favorites"),handler:async({body:e})=>Array.isArray(e.ids)?A(await Am(e.ids)):A(await $c())},{match:gi("/v1/cards/","/tags"),handler:async({param:e,body:t})=>A(await Ym(e,t))},{match:gi("/v1/cards/","/reroll"),handler:async({param:e,body:t})=>A(await Uc(e,String(t.mode||"nai"),t.overrides))},{match:C("/v1/messages/reroll","/v1/gallery/reroll-message"),handler:async({body:e})=>A(await Zm({session_id:String(e.session_id||e.sessionId||""),content_hash:String(e.content_hash||e.contentHash||""),message_index:Number(e.message_index??e.messageIndex??-1)}))},{match:C("/v1/characters/global-toggles","/v1/characters/global_toggles"),handler:async({body:e})=>A(await Ku(String(e.character_id||""),e.disabled_globals||e.disabled||[]))},{match:C("/v1/characters/unify","/v1/characters/merge"),handler:async({body:e})=>A(await cs(String(e.target_session_id||e.session_id||""),e.source_session_ids||e.session_ids||[],e.include_target!==!1))},{match:C("/v1/characters","/v1/characters/update"),handler:({body:e})=>dg(e)},{match:Ge("/v1/appearance/"),handler:async({param:e,body:t})=>t.characters!=null||t.global!=null?(t.characters!=null&&await Pn(e,t.characters||[],{prune:!0}),t.global!=null&&await Pn(Ve,t.global||[],{prune:!0}),A(await On(e))):A(await Xu(e,t.appearance||{}))},{match:C("/v1/models/test"),handler:async({body:e})=>A(await rg(e.llm&&typeof e.llm=="object"?e.llm:null))},{match:C("/v1/nai/test"),handler:async()=>A(await ag())},{match:C("/v1/nai/probe","/v1/debug/probe-nai"),handler:async()=>A(await ig())},{match:C("/v1/debug/clear"),handler:()=>(Kr(),A({ok:!0,cleared:!0}))},{match:C("/v1/nai/reference","/v1/nai/reference/upload"),handler:async({body:e})=>{if(e.clear)return A(await xc());const t=_i(e);if(!l(t))throw new Error("image_b64 required");return A(await tm(Q(Re(t))))}},{match:C("/v1/nai/reference/clear"),handler:async()=>A(await xc())},{match:C("/v1/nai/vibe","/v1/nai/vibe/upload"),handler:async({body:e})=>{const t=l(e.preset_id||e.presetId||"",120);if(e.clear)return A(t?await Ac(t):await kc());if(t&&e.copy_from){const a=l(e.copy_from,120),i=a?await cm(a,t):!1;return A({ok:!0,preset_id:t,configured:i,preview_url:i?ea(t):""})}const n=_i(e);if(!l(n))throw new Error("image_b64 required");const r={model:e.model,information_extracted:e.information_extracted??e.vibe_transfer_information_extracted,strength:e.strength??e.vibe_transfer_strength};return A(t?await sm(t,Q(Re(n)),r):await im(Q(Re(n)),r))}},{match:C("/v1/nai/vibe/clear"),handler:async({body:e})=>{const t=l(e?.preset_id||e?.presetId||"",120);return A(t?await Ac(t):await kc())}},{match:C("/v1/autotag","/v1/autotag/evaluate"),handler:async({body:e})=>{const t=_i(e).replace(/\s+/g,"");if(!t)throw new Error("image_b64 required");const n=Re(t);return A(await og(Q(n),Number(e.threshold??.2)))}}];async function dg(e){const t=l(e.session_id||"",200),n=l(e.character_id||"",200),r=(Array.isArray(e.root_session_ids)?e.root_session_ids:Array.isArray(e.cascade_session_ids)?e.cascade_session_ids:[]).map(i=>l(i,200)).filter(Boolean);if("characters"in e&&t&&await Pn(t,e.characters||[],{prune:!0,rootSessionIds:r}),"global"in e&&await Pn(Ve,e.global||[],{prune:!0}),"character"in e){const i=l(e.scope||t||"__global__",200);r.length&&i!=="__global__"?await ss(r,[e.character||{}],""):await ot(i,e.character||{})}const a=Array.isArray(e.root_delete)?e.root_delete:Array.isArray(e.cascade_delete)?e.cascade_delete:[];return a.length&&r.length&&await Yu(r,a,""),r.length&&t&&await cs(t,r,!1),A(await On(t,n))}async function yl(e,t){for(const n of e){const r=n.match(t.pathname);if(r!==null)return n.handler({...t,param:r})}return Yn(t.pathname)}async function fg(e,t={}){const{pathname:n,query:r}=sg(e),a=String(t.method||"GET").toUpperCase();let i;if(typeof t.body=="string")try{i=JSON.parse(t.body)}catch{i={}}else i=t.body??{};if(["/v1/health","/healthz","/readyz"].includes(n))return A({ok:!0,health:Kf()});if(n==="/v1/debug"||n==="/v1/debug/log")return a==="DELETE"||a==="POST"&&i.clear?(Kr(),A({ok:!0,cleared:!0})):A(pe());if(!cg(E(),t.headers||{})){const s="invalid token";throw pt(401,{ok:!1,...ne(s,"unauthorized")},s)}const o={method:a,pathname:n,query:r,body:i};if(a==="GET")return yl(lg,o);if(["POST","PUT","PATCH"].includes(a))try{return await yl(ug,o)}catch(s){if(Oi(s))throw s;const c=String(s?.message||s);throw pt(500,{ok:!1,...ne(c,"internal")},c)}throw pt(405,{ok:!1,...ne(a,"method_not_allowed")},a)}X(),W(),Ft(),pr(),ic(),Ce(),_e(),lt();var Ur=null;async function mg(){w("boot.ready.start",{message:We}),await at();const e=await mr().catch(r=>{throw w("boot.storage",{message:String(r?.message||r)},"error"),r});w("boot.storage",{message:e.kind}),ir(await Lf()),await Bf(),await ed(),await td();const t=await F("meta","reference_image");t?.png&&Xr(Mt(t.png));const n=await F("meta","vibe_transfer");n?.png&&Qr(Mt(n.png)),await um(),w("boot.ready.done",{message:We,has_nativeFetch:Qn("nativeFetch"),has_idb:Qn("getLocalPluginStorage")})}async function yi(){return Ur||(Ur=mg().catch(e=>{throw Ur=null,e})),await Ur,!0}async function pg(e,t={},n=12e4){await yi();try{const r=await fg(e,t);if(r.raw)return r.data;if(r.status>=400)throw pt(r.status,r.data);return r.data}catch(r){if(Oi(r))throw r;const a=String(r?.message||r);throw pt(500,{ok:!1,...ne(a,"internal")},a)}}async function gg(e){return await yi(),Yt(e)}function hg(){$i(()=>({cards:jn("cards"),images:jn("images"),jobs:jn("jobs"),blob_urls:Bu()})),Reflect.set(globalThis,"__INLAY_NATIVE__",{VERSION:We,ready:yi,fetch:pg,resolveImageUrl:Te,refPreviewUrl:So,vibePreviewUrl:Ao,ensureImageUrl:gg,warmImages:Ju,pinImageUrls:Uu,warmProgress:Fu,onWarmProgress:Du,debug:pe,clearDebug:Kr})}var _g=cn({applyExplorerClick:()=>wg,clearSelection:()=>xg,createSelectionState:()=>yg,moveFocus:()=>kg,reorderByIds:()=>Mg,selectAll:()=>vg,sortExplorerItems:()=>Sg,thumbMinWidth:()=>Ag,visibleIds:()=>bg});function yg(e=[]){return{selected:new Set((e||[]).map(String).filter(Boolean)),anchorId:"",focusId:""}}function bg(e=[]){return(e||[]).map(t=>String(t?.id||"")).filter(Boolean)}function wg(e,t,{index:n,ids:r,shift:a=!1,ctrl:i=!1}={}){const o={selected:new Set(e?.selected||[]),anchorId:String(e?.anchorId||""),focusId:String(e?.focusId||"")},s=String(t||"");if(!s)return o;const c=Array.isArray(r)?r.map(String):[],u=typeof n=="number"&&Number.isFinite(n)?n:c.indexOf(s);if(a&&o.anchorId&&c.length){const d=c.indexOf(o.anchorId),f=u>=0?u:c.indexOf(s);if(d>=0&&f>=0){const p=Math.min(d,f),m=Math.max(d,f);return o.selected=new Set(c.slice(p,m+1)),o.focusId=s,o}}return i?(o.selected.has(s)?o.selected.delete(s):o.selected.add(s),o.anchorId=s,o.focusId=s,o):(o.selected=new Set([s]),o.anchorId=s,o.focusId=s,o)}function vg(e,t=[]){return{selected:new Set((t||[]).map(String).filter(Boolean)),anchorId:String(e?.anchorId||t?.[0]||""),focusId:String(e?.focusId||t?.[0]||"")}}function xg(e=null){return{selected:new Set,anchorId:String(e?.anchorId||""),focusId:""}}function kg(e=[],t="",n=1){const r=(e||[]).map(String).filter(Boolean);if(!r.length)return"";const a=r.indexOf(String(t||""));return a<0?r[0]:r[Math.max(0,Math.min(r.length-1,a+n))]}function Sg(e=[],t="newest"){const n=[...e||[]],r=(a,i=0)=>{const o=Number(a);return Number.isFinite(o)?o:i};return t==="oldest"?n.sort((a,i)=>r(a.created_at)-r(i.created_at)||String(a.id).localeCompare(String(i.id))):t==="message"?n.sort((a,i)=>r(a.message_index,1e9)-r(i.message_index,1e9)||r(a.shot_index)-r(i.shot_index)||r(i.created_at)-r(a.created_at)):t==="shot"?n.sort((a,i)=>r(a.shot_index)-r(i.shot_index)||r(a.message_index,1e9)-r(i.message_index,1e9)||r(i.created_at)-r(a.created_at)):n.sort((a,i)=>r(i.created_at)-r(a.created_at)||String(i.id).localeCompare(String(a.id)))}function Ag(e="m"){return e==="s"?96:e==="l"?200:148}function Mg(e=[],t=[]){const n=i=>{const o=i&&typeof i=="object"?i.id:void 0;return String(o??i)},r=new Map((e||[]).map(i=>[n(i),i])),a=[];for(const i of t||[]){const o=String(i);r.has(o)&&(a.push(r.get(o)),r.delete(o))}for(const i of r.values())a.push(i);return a}Sr(),Ys();function Ng(){Reflect.set(globalThis,"__INLAY_VIEWER_CORE__",{...Xm}),Reflect.set(globalThis,"__INLAY_LLM__",{...Nf}),Reflect.set(globalThis,"__INLAY_EMBED__",{EMBEDDING_PROVIDERS:Ga,normalizeEmbeddingProvider:Ne,defaultEndpointForEmbedding:xr,defaultModelForEmbedding:zn,embeddingModelPlaceholder:bf,shouldAutoReplaceEmbeddingEndpoint:vf,shouldAutoReplaceEmbeddingModel:xf,embeddingProviderNeedsApiKey:kf}),Reflect.set(globalThis,"__INLAY_LORE_EXTRA__",{...$m}),Reflect.set(globalThis,"__INLAY_EXPLORER__",{..._g})}X(),W(),Ng(),hg(),w("boot.loaded",{message:We})})();
