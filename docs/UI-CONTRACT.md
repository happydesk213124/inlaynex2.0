...
| `POST /v1/gallery/export` | `{all\|folder_key\|card_ids}` → `{ok, zip_base64, filename, count}`. ZIP paths are `{folder}/000001_{old download name}.webp` newest-first **per explorer folder** (`.png`/`.jpg` only when the stored bytes are not WebP). Entry names are UTF-8 (bit 11 + Info-ZIP Unicode Path). Import still accepts legacy `images/{id}.png` and names without the 6-digit prefix. |
