...
export function getCurationMode(): CurationMode {
  ...
  const curation = cfg.curation && typeof cfg.curation === 'object'
    ...
    : {};
  const mode = normalizeCurationMode(curation.mode);
  if (mode !== 'off') return mode;
  // Legacy card toggle → treat as two_stage until migrated away.
  const card = cfg.card as { composition_curation?: unknown } | undefined;
  if (
