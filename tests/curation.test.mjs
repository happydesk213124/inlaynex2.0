...
import { CURATION_PIPELINE_LOCKED, normalizeCurationMode } from '../.test-build/settings-schema.mjs';
import {
...
test('normalizeCurationMode migrates legacy values', () => {
  ...
  if (CURATION_PIPELINE_LOCKED) {
    assert.equal(normalizeCurationMode('two_stage'), 'off');
    assert.equal(normalizeCurationMode('embed_snap'), 'off');
    assert.equal(normalizeCurationMode(true), 'off');
    assert.equal(normalizeCurationMode('full'), 'off');
  } else {
