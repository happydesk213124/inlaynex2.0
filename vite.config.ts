  ...
  `} else if (t.uiTab === "curation") {
      ...
      \`;
        ...
        <div class="card" style="margin-top:14px">
          ...
          <ul style="margin:10px 0 0;padding-left:18px;line-height:1.55;color:#c9d4e6;font-size:13px">
            <li>캐릭터 수정/생성 팝업: 오토태그·참고이미지 Ctrl+V가 설정 탭처럼 window paste로 동작</li>
          </ul>
        </div>
        <div class="card" style="margin-top:14px">
        ...
        <div class="card" style="margin-top:14px">
          ...
          <ul style="margin:10px 0 0;padding-left:18px;line-height:1.55;color:#c9d4e6;font-size:13px">
            <li>탐색기 탭에서만 상단 전체저장·설정 내보내기/불러오기·도움말 숨김(DOM :has 기반 — 다른 탭 복귀 시 항상 복구)</li>
          </ul>
        </div>
        <div class="card" style="margin-top:14px">
          <strong>2.2.12</strong>
          <ul style="margin:10px 0 0;padding-left:18px;line-height:1.55;color:#c9d4e6;font-size:13px">
            <li>탐색기: 폴더 ZIP 앞에 폴더 접기/펼치기 · ≤900px에서도 2열 유지(접을 때만 전체 너비)</li>
            <li>탐색기: 즐겨찾기만/삭제/선택해제·카드 ★ — 이미지에 가리지 않도록 z-index 보정</li>
          </ul>
        </div>
        <div class="card" style="margin-top:14px">
          <strong>2.2.11</strong>
          <ul style="margin:10px 0 0;padding-left:18px;line-height:1.55;color:#c9d4e6;font-size:13px">
            <li>카드 설정 탭 분리: 생성 옵션 / 스타일 프리셋 · 탭 순서 재배치(모델 설정 포함)</li>
          </ul>
        </div>
        <div class="card" style="margin-top:14px">
          <strong>2.2.10</strong>
          <ul style="margin:10px 0 0;padding-left:18px;line-height:1.55;color:#c9d4e6;font-size:13px">
            <li>고정 프롬프트: 저장 버튼(JSON 내보내기 왼쪽) · 8000자 상한으로 전체저장 setItem Error 완화</li>
          </ul>
        </div>
        <div class="card" style="margin-top:14px">
          <strong>2.2.9</strong>
          <ul style="margin:10px 0 0;padding-left:18px;line-height:1.55;color:#c9d4e6;font-size:13px">
            <li>재생성·태그 플로팅: 버튼 한 줄씩 크게(태그/재생성/중단/프리셋) · 접기→여백+펼치기만</li>
            <li>참고 Strength/Fidelity: parseInt 저장 버그 수정(소수점이 0·0.01로 깨지던 문제)</li>
            <li>캐릭터 참고 image: character / character&style / style 선택</li>
            <li>이미지 뷰어: 헤더 버튼·프리셋 목록·리롤 터치 크게</li>
            <li>뷰어 base/char 칩 키움 · 이미지 영역 예약 높이 조정(칩 잘림 방지)</li>
            <li>펼친 뷰어: 태그/재생성/중단 → 이미지 옵션바 왼쪽(리롤 맞은편)</li>
            <li>헤더 ◀▶상시·설정·접기 우측 고정 · 상시 라벨은 색만 변경</li>
            <li>이미지 옵션: 태그/재생성/중단 좌하단 · 리롤 포함 투명도 40%</li>
            <li>이미지 옵션 버튼 클릭: SafeDOM 좌표 hit-test로 복구</li>
            <li>캐릭터 참고이미지 「끄기」 저장이 이전 vibe/image로 되돌아가던 버그 수정</li>
            <li>캐릭터 참고이미지: 새로고침/업뎃 후 빈 미리보기+「설정됨」 — vibe처럼 전용 키로 저장·미리보기 복구</li>
            <li>image reference: Precise Reference와 vibe 동시 전송 금지(NAI 비호환) · director용 PNG 변환</li>
            <li>image reference: 원본 해상도 PNG로 페이로드 초과하던 문제 — NAI 캔버스에 맞춰 축소 후 전송</li>
            <li>image reference 400: vibe 필드/normalize 제거 · 정확한 NAI 캔버스 PNG만 전송(webp 폴백 제거)</li>
            <li>image reference: 비율→1024×1536/1536×1024/1472² 패딩 후 webp q0.5 전송(원본 크기 불일치가 400 원인)</li>
            <li>카드 생성옵션: 선행/후행 고정 프롬프트 2열 + JSON 내보내기·가져오기 · 생성 시 프리셋/장면에 항상 합침</li>
          </ul>
        </div>
        <div class="card" style="margin-top:14px">
          <strong>2.2.8</strong>
...
const VENDOR_VIEWER_HDR_CHROME_TOUCH_PATCH =
  `    const r = await H(e, "div", { style: Ft(startGeo, minimizedInit) }), i = await H(e, "div", { style: "min-height:52px;height:auto;display:flex;align-items:center;justify-content:flex-start;gap:8px;padding:8px 10px;background:rgba(255,255,255,.04);border-bottom:1px solid rgba(255,255,255,.06);cursor:move;user-select:none;flex-shrink:0;touch-action:none;flex-wrap:nowrap;" }), s = await H(e, "span", {
      ...
      // Risu SafeDOM blocks change/input events — use clickable control + pointer hit-test instead of <select>.
      style: "max-width:200px;min-width:120px;flex:0 1 200px;height:40px;border-radius:9px;border:1px solid rgba(255,255,255,.18);background:#0b0f18;color:#e8eef8;font-size:13px;font-weight:600;padding:0 12px;cursor:pointer;pointer-events:auto;display:inline-flex;align-items:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;box-sizing:border-box;",
      text: viewerPresetLabel
    }), viewerPresetMenu = await H(e, "div", {
      ...
      style: "display:flex;gap:8px;align-items:center;flex-shrink:0;flex-wrap:nowrap;justify-content:flex-end;margin-left:auto;",`;
  ...
  `    }, applyViewerChrome = async () => {
      ...
      try {
        if (actionsMin) {
          ...
          ].join("")), await c.setStyleAttribute(\`display:\${iconMin ? "none" : "flex"};gap:8px;align-items:center;flex-shrink:0;flex-wrap:nowrap;justify-content:flex-end;margin-left:auto;\`);
        }
      } catch {
      }`;
...
const VENDOR_VIEWER_STAGE_RESERVE_PATCH =
  `  function imageStageStyle(geo = {}) {
    const panelH = Math.max(280, Number(geo.h) || 560);
    // header(~52) + gaps + status + thumbs + chip row(~56) + padding — match enlarged chrome.
