...
const VENDOR_DEBUG_PANEL_PATCH = `        <div class="nx-seg" style="margin-bottom:12px">
        ...
        <div style="display:flex;align-items:flex-end;justify-content:space-between;gap:12px;margin-bottom:14px;flex-wrap:wrap">
          <div>
            ...
            <div class="muted" style="margin-top:4px;font-size:12px">구독 Anlas · Opus · V5 사용량(응답에 있으면)</div>
          </div>
      ...
      const paintQuota = (res) => {
        ...
        const html = !keys.length
          ...
          : \`<div style="display:grid;gap:12px">\${keys.map((k) => {
            ...
            const usage = k.v5_usage && Number(k.v5_usage.max) > 0 ? k.v5_usage : null;
            const extra = k.extra && typeof k.extra === "object" ? Object.entries(k.extra).slice(0, 8) : [];
            const bar = usage ? \`<div style="margin-top:12px"><div style="display:flex;justify-content:space-between;gap:8px;font-size:11px;color:#c4b5fd;margin-bottom:6px"><span>V5 사용량</span><span>\${esc(usage.label || (usage.remaining + " / " + usage.max))}</span></div><div style="height:8px;border-radius:999px;background:rgba(255,255,255,.08);overflow:hidden"><div style="height:100%;width:\${Math.max(0, Math.min(100, Number(usage.pct) || 0))}%;background:linear-gradient(90deg,#7c6cff,#c4b5fd);border-radius:999px"></div></div></div>\` : "";
            ...
            return \`<article style="border-radius:16px;border:1px solid rgba(255,255,255,.1);background:\${bg};padding:16px 16px 14px;box-shadow:0 12px 32px rgba(0,0,0,.28)"><div style="display:flex;justify-content:space-between;gap:8px;align-items:center"><div style="display:flex;gap:8px;align-items:center"><span style="display:inline-flex;align-items:center;justify-content:center;min-width:42px;height:22px;border-radius:999px;background:\${accent};color:#0b0f18;font:800 11px/1 Segoe UI,sans-serif">\${esc(fam)}</span><span style="color:#9aa6b8;font-size:12px">…\${esc(k.suffix || "")}</span></div>\${k.opus ? \`<span style="font:700 10px Segoe UI,sans-serif;color:#fde68a;background:rgba(251,191,36,.12);border:1px solid rgba(251,191,36,.35);border-radius:999px;padding:3px 8px">Opus</span>\` : ""}</div><div style="margin-top:14px;display:flex;align-items:baseline;gap:8px"><span style="font:800 28px/1.05 Segoe UI,sans-serif;letter-spacing:-.03em;color:#e8eef8">\${esc(k.total ?? 0)}</span><span style="font-size:12px;color:#9aa6b8">Anlas</span></div><div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:14px"><div style="padding:8px 10px;border-radius:10px;background:rgba(255,255,255,.04)"><div style="font-size:10px;color:#778398">고정</div><div style="margin-top:2px;font-weight:700">\${esc(k.fixed ?? 0)}</div></div><div style="padding:8px 10px;border-radius:10px;background:rgba(255,255,255,.04)"><div style="font-size:10px;color:#778398">구매</div><div style="margin-top:2px;font-weight:700">\${esc(k.purchased ?? 0)}</div></div><div style="padding:8px 10px;border-radius:10px;background:rgba(255,255,255,.04)"><div style="font-size:10px;color:#778398">합계</div><div style="margin-top:2px;font-weight:700">\${esc(k.total ?? 0)}</div></div></div>\${bar}\${chips}</article>\`;
          }).join("")}</div>\`;
...
const VENDOR_INLINE_HELP_PATCH =
  `    "nx-overlay": { title: "채팅 왼쪽 줄 오버레이", body: "채팅 왼쪽 핀·스티키 이미지를 보여 줍니다. 꺼도 내부 동기화는 유지하고, 상시 이미지 0% + 핀을 화면 밖으로 치워 가려 둡니다(꺼서 통째로 뜯으면 렉이 나서). 메시지 클릭·말풍선 삽화는 그대로입니다." },
    ...
    "nx-nai5-speech": { title: "NAI5 대사삽입", body: "V5 샷 메인 태그에 말풍선을 넣습니다. 프리셋의 spoken bubble 억제는 그 샷에서 빼입니다." },`;
    ...
    try {
      ...
      const inlineGoneFromSel = async () => {
        ...
        const unwrapGone = async (sel) => {
          try {
            const raw = await el.querySelectorAll(sel);
...
const loadVendorUi = (): string => {
  ...
  return (() => {
    ...
    if (!out.includes('wantActions') || !out.includes('prependBar') || out.includes('insertAdjacentHTML("afterbegin", barHtml)')) {
